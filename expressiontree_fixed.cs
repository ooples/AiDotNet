...
public class ExpressionTree<T, TInput, TOutput> : IFullModel<T, TInput, TOutput>
{
    ...
    /// <summary>
    /// Thread-local random number generator for all mutation and crossover operations.
    /// </summary>
    /// <remarks>
    /// Using a ThreadLocal Random instance ensures thread safety while maintaining randomness quality.
    /// Each thread gets its own Random instance, avoiding the issue where multiple instances created
    /// in quick succession get the same seed from the system clock.
    /// </remarks>
    ...
    /// modifying the current tree directly. This is different from UpdateCoefficients and WithParameters
    /// which create new trees with the updated values. Use this method when you want to modify the tree
    /// in place, such as during optimization iterations.
    /// <para>
    ...
    /// This value is calculated on-demand by traversing the tree.
    /// </remarks>
