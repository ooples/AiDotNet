...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        // Wrap log + plan + scope in a single try/finally so every exit
        // path (success, early-return on phase failure, unexpected
        // exception) disposes deterministically. The previous code
        // returned 1 from each Phase failure catch without flushing /
        // closing log and skipped plan.Dispose() entirely if any
        // post-compile phase faulted.
        var log = new System.IO.StreamWriter("perf-1305-compile-phases.log") { AutoFlush = true };
        ICompiledPlan<double>? plan = null;
