...
foreach ($pr in $prs) {
    ...
    # Get recursive tree
    $tree = gh api "repos/$owner/$repo/git/trees/$($sha2)?recursive=1" | ConvertFrom-Json
    $entry = $tree.tree | Where-Object { $_.path -eq $pr.f }

    if (!$entry) { Write-Host "  ERROR: File $($pr.f) not found"; continue }
