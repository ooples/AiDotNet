...
/// <summary>
/// Tests for <see cref="DatasetDownloader"/>'s MoveWithRetryAsync.
///
/// Regression: on Windows the freshly-written temp file can be briefly
/// locked by Windows Defender or the search indexer while the download
/// path calls File.Move; the original implementation had no retry, so a
/// single transient IOException would abort the entire download and wipe
/// the temp file from the <c>finally</c> block. The fix retries on
/// IOException / UnauthorizedAccessException with linear backoff.
/// </summary>
public class DatasetDownloaderMoveRetryTests
{
    private static readonly MethodInfo MoveWithRetry =
        typeof(DatasetDownloader).GetMethod(
            "MoveWithRetryAsync",
            BindingFlags.Static | BindingFlags.NonPublic)
        ?? throw new InvalidOperationException("MoveWithRetryAsync not found; fix may have been reverted.");

    private static Task InvokeMoveAsync(string src, string dst, int maxAttempts = 5, int delayMs = 1)
        => (Task)MoveWithRetry.Invoke(
            null,
