...
public class FederatedLearningAsyncIntegrationTests
{
    ...
    [InlineData("FedAsync")]
    [InlineData("FedBuff")]
    public async Task BuildAsync_WithAsyncFederatedLearning_CompletesAndReportsAsyncMode(string mode)
    {
        ...
        Assert.Equal(mode, metadata!.AsyncModeUsed);
