...
public class FederatedLearningServerOptimizerIntegrationTests
{
    ...
    [InlineData("FedAvgM")]
    [InlineData("FedAdam")]
    [InlineData("FedAdagrad")]
    [InlineData("FedYogi")]
    public async Task BuildAsync_WithServerOptimizer_CompletesAndReportsOptimizer(string optimizerName)
    {
        ...
        Assert.Equal(optimizerName, metadata!.ServerOptimizerUsed);
