...
public class FederatedLearningHomomorphicEncryptionIntegrationTests
{
    ...
    [InlineData("CKKS")]
    [InlineData("BFV")]
    public async Task BuildAsync_WithHomomorphicEncryption_HEOnly_CompletesAndReportsScheme(string scheme)
    {
        ...
        Assert.Equal(scheme, metadata.HomomorphicEncryptionSchemeUsed);
