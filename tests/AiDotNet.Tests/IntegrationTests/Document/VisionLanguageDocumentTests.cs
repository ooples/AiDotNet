...
public class VisionLanguageDocumentTests
{
    ...
    public async Task DocOwl_NativeConstruction_Succeeds()
    {
        ...
        using var model = new DocOwl<double>(arch, options: new DocOwlOptions { ImageSize = 64 });
    ...
    {
        var arch = CreateArchitecture();
        using var model = new InfographicVQA<double>(arch, options: new InfographicVQAOptions { ImageSize = 64 });
        Assert.NotNull(model);
    }
