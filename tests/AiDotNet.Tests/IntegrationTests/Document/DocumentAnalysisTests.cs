...
public class DocumentAnalysisTests
{
    ...
    public async Task DocBank_NativeConstruction_Succeeds()
    {
        ...
        using var model = new DocBank<double>(arch, options: new AiDotNet.Document.Options.DocBankOptions { ImageSize = 64 });
    ...
    {
        var arch = CreateArchitecture();
        using var model = new TableTransformer<double>(arch, options: new AiDotNet.Document.Options.TableTransformerOptions { ImageSize = 64 });
        Assert.NotNull(model);
    }
