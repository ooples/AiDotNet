...
public class ConvolutionalLayersIntegrationTests
{
    ...
    public void ConvolutionalLayer_WithReLU_ProducesNonNegativeOutput()
    {
        ...
        {
            Assert.True(output.Data[i] >= 0, $"ReLU output should be non-negative at index {i}");
        }
    }
    ...
    public void ConvolutionalLayer_WithTanh_ProducesOutputInRange()
    {
        ...
        for (int i = 0; i < output.Length; i++)
        {
            Assert.True(output.Data[i] >= -1 && output.Data[i] <= 1,
    ...
    private static void AssertNoNaNOrInf(Tensor<double> tensor)
    {
        ...
        {
            Assert.False(double.IsNaN(tensor.Data[i]), $"Tensor contains NaN at index {i}");
            Assert.False(double.IsInfinity(tensor.Data[i]), $"Tensor contains Infinity at index {i}");
        }
    }
