...
public class NormalizationLayersDeepMathIntegrationTests
{
    ...
    {
        // gamma initialized to 1.0 for each feature. Use the eager-init ctor (#1370): a NO-ARG BatchNorm can't
        // know its feature count, so it lazy-inits gamma/beta to length 0 until the first forward — passing the
        // feature count up front is the supported way to inspect init values without a forward pass.
    ...
    {
        // beta initialized to 0.0 for each feature (eager-init ctor #1370 — no-arg lazy-inits to length 0)
        var bn = new BatchNormalizationLayer<double>(3);
        var beta = bn.GetBeta();
    ...
    {
        var bn = new BatchNormalizationLayer<double>(3); // eager-init ctor #1370 (no-arg lazy-inits to length 0)
    ...
    {
        // ParameterCount = gamma.Length + beta.Length = 2 * numFeatures (eager-init ctor #1370; 5 features → 10)
    ...
    public async Task BatchNorm_CustomEpsilon_AffectsScale()
    {
        ...
        // eps=1.0: scale = 1 / sqrt(1 + 1) = 1/sqrt(2). The test must actually SET the custom epsilon (the no-arg
        // default is LargeEpsilon=1e-5); size is lazy-inferred from the [1,1] forward input.
    ...
    {
        // momentum=0.5: runningMean = 0.5*old + 0.5*batch. The test must actually SET the custom momentum (the
        // no-arg default is 0.9); size is lazy-inferred from the [2,1] forward input.
