...
public class TradingAgentLearningTests
{
    ...
    public void Dqn_epsilon_decays_from_start_toward_end()
    {
        ...
        var options = new FinancialDQNAgentOptions<double>
        {
            ...
            // TradingAgentOptions.WarmupSteps defaults to 1000 and these fixtures store tens of
            // transitions, so without this every Train() returns before applying a gradient and
            // neither the epsilon schedule nor the target-sync schedule would ever advance.
            WarmupSteps = 0,
            EpsilonStart = 1.0,
            EpsilonEnd = 0.05,
            EpsilonDecay = 0.9,
        ...
        Assert.Equal(1.0, agent.CurrentEpsilon, 9);

        TrainSteps(agent, 40);

        Assert.True(agent.CurrentEpsilon < 1.0, $"epsilon never moved from EpsilonStart (still {agent.CurrentEpsilon})");
        Assert.True(agent.CurrentEpsilon >= options.EpsilonEnd, $"epsilon fell below EpsilonEnd ({agent.CurrentEpsilon})");
    }
    ...
    public void Dqn_epsilon_decay_of_one_holds_the_rate_instead_of_annealing()
    {
        ...
        Assert.Equal(0.3, agent.CurrentEpsilon, 9);
    }
    ...
    public void Dqn_direct_gradients_share_the_training_and_target_schedule(int frequency)
    {
        ...
        Assert.Equal(options.EpsilonStart, agent.CurrentEpsilon);
        ...
        for (var step = 1; step <= frequency + 1; step++)
        {
            ...
            Assert.Equal(Math.Max(options.EpsilonEnd, options.EpsilonStart * Math.Pow(options.EpsilonDecay, step)), agent.CurrentEpsilon, 12);
            if (step % frequency == 0)
            {
    ...
    public void Dqn_publishes_the_exploration_rate_in_its_trading_metrics()
    {
        ...
        Assert.Equal(agent.CurrentEpsilon, Convert.ToDouble(metrics["Epsilon"]), 9);
        Assert.True(agent.CurrentEpsilon < 0.8, "epsilon did not decay, so the published value proves nothing");
    }
    ...
    public void Dqn_epsilon_is_monotonically_non_increasing()
    {
        ...
        using var agent = new FinancialDQNAgent<double>(Actor(), options);
        var curve = new List<double> { agent.CurrentEpsilon };
        for (var i = 0; i < 30; i++)
        {
            TrainSteps(agent, 1);
            curve.Add(agent.CurrentEpsilon);
        }
