    ...
    public class EnsembleFitDetectorTests
    {
        ...
        private ModelEvaluationData<double, Matrix<double>, Vector<double>> CreateMockEvaluationData()
        {
            return new ModelEvaluationData<double, Matrix<double>, Vector<double>>
            {
                ...
                {
                    ErrorStats = new ErrorStats<double> { MSE = 0.1 }
                },
                ValidationSet = new DataSetStats<double, Matrix<double>, Vector<double>>
                {
                    ErrorStats = new ErrorStats<double> { MSE = 0.12 }
                }
            };
        }
        ...
        public void DetectFit_IncludesGeneralRecommendationBasedOnFitType()
        {
            ...
            Assert.NotNull(result);
            Assert.NotEmpty(result.Recommendations);
            Assert.Contains(result.Recommendations, r => r.Contains("ensemble") || r.Contains("good fit"));
        }
