...
public class StableVideoDiffusionTests : DiffusionModelTestBase<float>
{
    protected override int[] InputShape => [1, 4, 16, 16];
    protected override int[] OutputShape => [1, 4, 16, 16];

    // Stable Video Diffusion defaults to a foundation-scale video UNet + temporal VAE that exceeds the
    // 120s model-family budget. Inject a tiny same-architecture VideoUNet + TemporalVAE — latentChannels
    // (4) and contextDim (1024, SD2.1/SVD) stay paper-correct; only base channels / level count shrink.
    protected override IDiffusionModel<float> CreateModel()
        => new StableVideoDiffusion<float>(
