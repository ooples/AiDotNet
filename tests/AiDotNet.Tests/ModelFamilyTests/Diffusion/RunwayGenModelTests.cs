...
public class RunwayGenModelTests : DiffusionModelTestBase<float>
{
    protected override int[] InputShape => [1, 4, 16, 16];
    protected override int[] OutputShape => [1, 4, 16, 16];

    // Runway Gen-2 defaults to a 320-base-channel video UNet ([1,2,4,4]) + temporal VAE: a single
    // forward exceeds the 120s model-family budget. Inject a tiny same-architecture VideoUNet +
    // TemporalVAE — latentChannels (4) and contextDim (1024, Gen-2) stay paper-correct; only base
    // channels / level count shrink.
    protected override IDiffusionModel<float> CreateModel()
        => new RunwayGenModel<float>(
