...
public class VideoP2PModelTests : DiffusionModelTestBase<float>
{
    ...
    // Video-P2P defaults to a 320-base-channel video UNet ([1,2,4,4] levels) + temporal VAE:
    // a single forward exceeds the 120s model-family budget. Inject a tiny same-architecture
    // VideoUNet + TemporalVAE — latentChannels (4) and contextDim (768) stay paper-correct; only
    // base channels / level count / res-blocks / VAE base channels shrink.
    protected override IDiffusionModel<float> CreateModel()
        => new VideoP2PModel<float>(
