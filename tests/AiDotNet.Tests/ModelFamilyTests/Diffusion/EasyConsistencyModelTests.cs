...
public class EasyConsistencyModelTests : DiffusionModelTestBase<float>
{
    ...
    // EasyConsistencyModel wraps a full SD1.5 UNet (320 base, [1,2,4,4]) + VAE: a single forward over
    // the 64x64 latent exceeds the 120s model-family budget. Inject a tiny same-architecture UNet + VAE —
    // latentChannels (4) and contextDim (768, SD1.5) stay paper-correct; only base channels / level
    // count / res-blocks shrink.
    protected override IDiffusionModel<float> CreateModel()
        => new EasyConsistencyModel<float>(
