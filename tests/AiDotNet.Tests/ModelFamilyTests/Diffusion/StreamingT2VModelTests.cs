...
public class StreamingT2VModelTests : DiffusionModelTestBase<float>
{
    ...
    // StreamingT2V defaults to a 320-base-channel video UNet + temporal VAE: a single forward exceeds
    // the 120s model-family budget. Inject a tiny same-architecture VideoUNet + TemporalVAE —
    // latentChannels (4) and contextDim (768) stay paper-correct; only base channels / level count shrink.
    protected override IDiffusionModel<float> CreateModel()
        => new StreamingT2VModel<float>(
