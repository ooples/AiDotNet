...
public class MergedPRBugFixTests
{
    ...
    // JitCompiler PR #770 tests removed — per-layer JIT compiler system has been deleted
    ...
    // The InferenceOptimization PR #768 bug-fix tests were removed alongside the
    // src/InferenceOptimization/ tree. That tree was orphaned JIT IR/passes/kernels
    // scaffolding with zero production callers. Compilation now flows through
    // AiDotNet.Tensors' AutoTracer → CompiledInferencePlan → CompiledTrainingPlan
    // pipeline (enabled by default; opt out via TensorCodecOptions).
    ...
    public async Task ParameterAnalyzer_CalculateDistributionStats_EmptyGroups_ReturnsEmptyDict()
    {
        ...
        // ACT
        var result = analyzer.CalculateDistributionStats(emptyGroups);
