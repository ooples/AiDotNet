...
internal sealed class TestModelRepository : IModelRepository
{
    ...
    {
        throw new NotSupportedException("Test repository does not support loading multimodal models.");
    }

    public IServableMultimodalModel<T>? GetMultimodalModel<T>(string name)
    {
        throw new NotSupportedException("Test repository does not support retrieving multimodal model instances.");
    }
}
