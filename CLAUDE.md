...
**CRITICAL**: When adding new features to AiDotNet, you MUST follow these architectural requirements:

### 1. Interface/Base Class Pattern

Every new component MUST follow one of these patterns:

**For Small, Single-Purpose Components** (bias detectors, fairness evaluators, fitness calculators):
- ✅ Create an interface (e.g., `IBiasDetector<T>`)
- ✅ Create an abstract base class (e.g., `BiasDetectorBase<T>`)
- ✅ Create multiple concrete implementations (e.g., `DisparateImpactBiasDetector<T>`, `DemographicParityBiasDetector<T>`)

**For Large Model Families** (regression, neural networks, time series):
- ✅ Create a base class (e.g., `RegressionBase<T>`)
- ✅ Create concrete implementations that extend the base

**Example - FitnessCalculator Pattern**:
```csharp
// 1. Interface
...
public abstract class FitnessCalculatorBase<T, TInput, TOutput> : IFitnessCalculator<T, TInput, TOutput>
{
    ...
    protected readonly bool _isHigherScoreBetter;

    protected FitnessCalculatorBase(bool isHigherScoreBetter) { ... }

    public abstract T GetFitnessScore(DataSetStats<T, TInput, TOutput> dataSet);
}

// 3. Concrete Implementations (27+ of them!)
...
### 2. Integration into PredictionModelBuilder

**ALL new features MUST be integrated into the PredictionModelBuilder facade:**

1. Add `Configure[FeatureName]()` method to `IPredictionModelBuilder<T, TInput, TOutput>` interface
2. Implement in `PredictionModelBuilder<T, TInput, TOutput>` class
3. Add private field for the interface
4. Follow the existing pattern of returning `this` for method chaining

**Example**:
```csharp
// IPredictionModelBuilder.cs
...
- ❌ **WRONG**: Copying the same helper methods into multiple classes
- ✅ **CORRECT**: Create shared static utility classes or helper methods

**Example**:
```csharp
// InterpretabilityMetricsHelper.cs - Shared utility class
...
// BiasDetector.cs and FairnessEvaluator.cs both use it
var groups = InterpretabilityMetricsHelper<T>.GetUniqueGroups(sensitiveFeature);
```

### 4. Workflow Integration

Determine WHERE in the workflow your feature fits:

**Pre-processing** (before training):
- Data cleaning, normalization, feature selection
- Configure in `DataPreprocessor`, `Normalizer`, `FeatureSelector`

**Training** (during model optimization):
- Model performance measurement, early stopping
- Configure in `Optimizer`, `FitnessCalculator`, `FitDetector`

**Post-training** (after model is built):
- Evaluation, analysis, ethical AI assessment
- Configure via `ConfigureBiasDetector()`, `ConfigureFairnessEvaluator()`
- Use after calling `Build()` method
...
builder
    ...
    .ConfigureNormalizer(new StandardNormalizer<double>())
    .ConfigureFitnessCalculator(new BinaryCrossEntropyLossFitnessCalculator<double>())
    .ConfigureBiasDetector(new BiasDetector<double>())  // Ethical AI
    .ConfigureFairnessEvaluator(new FairnessEvaluator<double>());  // Ethical AI

// Train model
var trainedModel = builder.Build(trainingData, labels);

// Post-training evaluation (user does this manually for now)
// Future: Could add builder.EvaluateEthics() method
```
...
- [ ] Interface created (for small components) OR base class created (for large families)
- [ ] Abstract base class created (following FitnessCalculatorBase pattern)
- [ ] At least one concrete implementation
- [ ] NO code duplication - shared utilities extracted
- [ ] Integration into `IPredictionModelBuilder` interface
- [ ] Integration into `PredictionModelBuilder` implementation
- [ ] Beginner-friendly XML documentation comments
