...
* Generic evolution engine types move to the standalone `AiDotNet.Evolution` assembly/namespace. Update imports and recompile; no binary type forwarding or automatic checkpoint migration is provided. External `IAiModelBuilder` implementations and binary callers must also adopt/recompile the migrated `ConfigureEvolution` signatures with `archiveFactory` and `winnerModelFactory`. See [package-integration migration](docs/evolution-package-integration.md).

* `IAiModelBuilder<T, TInput, TOutput>` now requires `ConfigureProgramCorrectness(IProgramFitnessEvaluator)`. External implementations must implement the member and recompile, forwarding or retaining the evaluator to gate program fitness; unsupported implementations should throw `NotSupportedException`, not silently bypass checks. Users of the supplied `AiModelBuilder` need no implementation changes. See [correctness-gate migration](docs/evolution-correctness-gate.md).
* Program identity now hashes exact source text and language rather than display-normalized text. Program-task v5, variation v5 and genome-codec v2 compatibility identities reject older checkpoints. Variation v5 includes bounded malformed-source retries, exact protected-region enforcement across edit formats, complete configured proposal-option hashing, and a closed-fence extraction path preserving source whitespace and line endings. Task v5 also fingerprints cost-unit semantics and separate marker identities. Start a new run or explicitly re-evaluate old sources; do not relabel cached scores. See [source-identity migration](docs/evolution-source-identity.md).
* Opt-in compiler-guided C# evolution: bounded syntax patches, deterministic Roslyn emit, compiler-feedback repair and write-once evidence through `ConfigureCSharpProgramEvolution`. The separate `AiDotNet.Evolution.CSharp` package supports .NET 8/10 and keeps Roslyn out of the main package. Compilation is not sandboxing, correctness or performance proof; see [contracts and limits](docs/evolution-csharp.md).
* Program evolution accepts caller-owned proposal loops and shared evaluation resource accounting. Failed/unknown proposal work retains its charge; automatic ledger/engine checkpoint resume is refused until coordinated persistence is available. Synchronous I/O evaluator v3 and asynchronous sandbox evaluator v2 count canceled dispatched calls, propagate fatal failures, own their versioned cases and withhold raw failure messages. Truncated asynchronous output cannot pass by matching an expected prefix. Correctness gate v2 and the program task reject missing receipts instead of manufacturing zero or partial actual costs. These evaluator identity changes require fresh evaluation rather than relabeling old cached scores.
* Repository C# execution worker supports offline Roslyn compilation, compile-only validation and real console execution on .NET 8/10 without `dotnet-script`. It is an interpreter, not a security sandbox; untrusted programs require an independently provisioned OS/container boundary. See [worker setup and limits](tools/AiDotNet.CSharp.Worker/README.md).
