...
* Program identity now hashes exact source text and language rather than display-normalized text. Program-task v4, variation v5 and genome-codec v2 compatibility identities reject older checkpoints. Variation v5 includes bounded malformed-source retries, exact protected-region enforcement across edit formats, complete configured proposal-option hashing, and a closed-fence extraction path preserving source whitespace and line endings. Start a new run or explicitly re-evaluate old sources; do not relabel cached scores. See [source-identity migration](docs/evolution-source-identity.md).
