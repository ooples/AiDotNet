...
* Program evolution accepts caller-owned proposal loops and shared evaluation resource accounting. Failed/unknown proposal work retains its charge; automatic ledger/engine checkpoint resume is refused until coordinated persistence is available. I/O evaluator v2 counts canceled dispatched calls and propagates fatal failures. Correctness gate v2 and the program task reject missing receipts instead of manufacturing zero or partial actual costs.
