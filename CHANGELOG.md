...
* Program identity now hashes exact source text and language rather than display-normalized text. Program-task v4, variation v4 and genome-codec v2 compatibility identities reject older checkpoints. Variation v4 additionally routes malformed source through bounded validation retries. Start a new run or explicitly re-evaluate old sources; do not relabel cached scores. See [source-identity migration](docs/evolution-source-identity.md).
