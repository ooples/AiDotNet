...
public class TorchSharpComparisonBenchmarks
{
    ...
    public void Setup()
    {
        ...
        _torchDevice = _torchUsesCuda ? torch.CUDA : torch.CPU;

        Console.WriteLine($"TorchSharp device: {(_torchUsesCuda ? "CUDA" : "CPU")}");
    ...
    private void ConsumeTorchResult(TorchTensor result)
    {
        if (_torchUsesCuda)
        {
            using var cpuResult = result.cpu();
            _consumer.Consume(cpuResult);
        }
        else
        {
