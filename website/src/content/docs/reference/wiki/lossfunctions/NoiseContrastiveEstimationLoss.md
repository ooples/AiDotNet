---
title: "NoiseContrastiveEstimationLoss<T>"
...
Noise Contrastive Estimation (NCE) is a loss function designed to efficiently train
...
| Constructor | Summary |
|:-----|:--------|
| `NoiseContrastiveEstimationLoss(Int32)` | Initializes a new instance of the NoiseContrastiveEstimationLoss class. |

## Methods

| Method | Summary |
|:-----|:--------|
| `Calculate(Vector<>,Matrix<>)` | Calculates the NCE loss between target and noise samples. |
| `CalculateDerivative(Vector<>,Matrix<>)` | Calculates the gradient of the NCE loss function. |
| `CalculateDerivative(Vector<>,Vector<>)` | This method is not used for NCE Loss as it requires specific input formats. |
| `CalculateLoss(Vector<>,Vector<>)` | This method is not used for NCE Loss as it requires specific input formats. |
| `ComputeTapeLoss(Tensor<>,Tensor<>)` |  |
| `Sigmoid()` | Applies the sigmoid function to a scalar value. |

## Fields

| Field | Summary |
|:-----|:--------|
| `_numNoiseSamples` | The number of noise samples to use per true sample. |
