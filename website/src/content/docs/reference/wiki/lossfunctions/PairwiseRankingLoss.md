...
| Constructor | Summary |
|:-----|:--------|
| `PairwiseRankingLoss(Double)` | Creates a new pairwise RankNet ranking loss. |

## Properties

| Property | Summary |
|:-----|:--------|
| `TailWeightPower` | Gets the tail-weighting power this loss was configured with. |

## Methods

| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Calculates the gradient of the (tail-weighted) pairwise RankNet loss with respect to each predicted score. |
| `CalculateLoss(Vector<>,Vector<>)` | Calculates the (tail-weighted) pairwise RankNet loss over all valid pairs in the group. |
| `ComputeExtremities(Vector<>)` | Computes a per-item extremity weight in [0, 1] for tail weighting: 0 at the median target, 1 at the most extreme top/bottom target. |
| `ComputeTapeLoss(Tensor<>,Tensor<>)` | Computes the loss as a tape-differentiable scalar tensor for automatic backpropagation. |
| `PairWeight(Double[],Int32,Int32)` | Weight of the pair (i, j) given the precomputed per-item extremities. |
| `Sigmoid(Double)` | Logistic sigmoid 1 / (1 + exp(-x)), computed in a numerically stable way. |
| `Softplus(Double)` | Numerically stable softplus: log(1 + exp(x)) = max(x, 0) + log(1 + exp(-\|x\|)). |
