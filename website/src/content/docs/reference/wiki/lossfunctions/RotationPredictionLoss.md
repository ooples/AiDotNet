---
title: "RotationPredictionLoss<T>"
...
This teaches the model to understand image structure without labels.
...
| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Gradient of categorical cross-entropy: -target / predicted. |
| `CalculateLoss(Vector<>,Vector<>)` | Categorical cross-entropy loss for rotation prediction (4-class). |
| `ComputeTapeLoss(Tensor<>,Tensor<>)` |  |
| `CopyImage(Tensor<>,Tensor<>,Int32,Int32,Int32,Int32,Int32,Func<Int32,Int32,ValueTuple<Int32,Int32>>)` | Copies an image with a coordinate transformation. |
| `CreateTask()` |  |
| `CreateTaskFromMatrix(Matrix<>)` | Creates a rotation prediction task from Matrix input (treats each row as a flattened image). |
| `RotateAndCopy(Tensor<>,Tensor<>,Int32,Int32,Int32,Int32,Int32,Int32)` | Rotates an image and copies it to the destination tensor. |
