...
Charbonnier loss is a differentiable approximation of the absolute error (L1 loss).
...
**Reference:** Charbonnier et al., "Two deterministic half-quadratic regularization algorithms
...
| Constructor | Summary |
|:-----|:--------|
| `CharbonnierLoss(Double)` | Initializes a new instance of the CharbonnierLoss class. |

## Methods

| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Calculates the derivative of the Charbonnier loss function. |
| `CalculateLoss(Vector<>,Vector<>)` | Calculates the Charbonnier loss between predicted and actual values. |
| `CalculateLossAndGradientGpu(Tensor<>,Tensor<>)` | Calculates both Charbonnier loss and gradient on GPU in a single efficient pass. |
...
| Field | Summary |
|:-----|:--------|
| `_epsilon` | Small constant to ensure differentiability at zero. |
| `_epsilonSquared` | The squared epsilon value, precomputed for efficiency. |
