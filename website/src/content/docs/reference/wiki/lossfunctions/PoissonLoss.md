...
| Constructor | Summary |
|:-----|:--------|
| `PoissonLoss` | Initializes a new instance of the PoissonLoss class. |

## Methods

| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Calculates the derivative of the Poisson loss function. |
| `CalculateLoss(Vector<>,Vector<>)` | Calculates the Poisson loss between predicted and actual values. |
| `CalculateLossAndGradientGpu(Tensor<>,Tensor<>)` | Calculates both Poisson loss and gradient on GPU in a single efficient pass. |
