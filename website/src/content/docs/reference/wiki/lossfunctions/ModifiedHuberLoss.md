...
| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Calculates the derivative of the Modified Huber Loss function. |
| `CalculateLoss(Vector<>,Vector<>)` | Calculates the Modified Huber Loss between predicted and actual values. |
| `CalculateLossAndGradientGpu(Tensor<>,Tensor<>)` | Calculates both Modified Huber loss and gradient on GPU in a single efficient pass. |
