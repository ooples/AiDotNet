...
| Method | Summary |
|:-----|:--------|
| `CalculateDerivative(Vector<>,Vector<>)` | Calculates the derivative of the Hinge loss function. |
| `CalculateLoss(Vector<>,Vector<>)` | Calculates the Hinge loss between predicted and actual values. |
| `CalculateLossAndGradientGpu(Tensor<>,Tensor<>)` | Calculates both Hinge loss and gradient on GPU in a single efficient pass. |
