...
DeepAR is like a weather forecaster that doesn't just say
...
| Constructor | Summary |
|:-----|:--------|
| `DeepARModel(DeepAROptions<>)` | Initializes a new instance of the DeepARModel class. |

## Methods

| Method | Summary |
|:-----|:--------|
| `ComputeGradients(Tensor<>,)` | Computes gradients using backpropagation through the LSTM and output layers. |
| `ForecastWithQuantiles(Vector<>,Double[])` | Generates probabilistic forecasts with quantile predictions. |
| `InitializeModel` | Initializes the model architecture with tensor-based parameters. |
| `PredictDistribution(Vector<>)` | Predicts distribution parameters (mean and scale) for a single input. |
| `TrainCore(Matrix<>,Vector<>)` | Trains the model using proper backpropagation through time (BPTT). |
| `ValidateDeepAROptions` | Validates DeepAR-specific options. |
