...
| Constructor | Summary |
|:-----|:--------|
| `AutoformerModel(AutoformerOptions<>)` | Initializes a new instance of the Autoformer model with the specified options. |

## Methods

| Method | Summary |
|:-----|:--------|
| `AutoCorrelation(Tensor<>,Tensor<>,Tensor<>,Int32)` | Performs auto-correlation based aggregation (O(L log L) via FFT-style computation). |
| `ComputeGradients(Matrix<>,Vector<>,ILossFunction<>)` | Uses the Autoformer's built-in autodiff gradient computation instead of SPSA. |
| `CreateInstance` |  |
| `DeserializeCore(BinaryReader)` |  |
| `GetGradientVector` | Extracts gradient accumulators as a flat vector. |
| `GetModelMetadata` |  |
| `Predict(Matrix<>)` | Predicts a single future value using the trained model. |
| `PredictMultiple(Vector<>)` | Predicts multiple time steps ahead using the Autoformer architecture. |
| `SerializeCore(BinaryWriter)` |  |
| `SeriesDecomposition(Tensor<>)` | Performs series decomposition using moving average. |
| `TrainCore(Matrix<>,Vector<>)` | Trains the model using backpropagation through the Autoformer architecture. |
