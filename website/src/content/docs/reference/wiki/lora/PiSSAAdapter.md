...
Think of PiSSA as "smart LoRA initialization".
...
| Constructor | Summary |
|:-----|:--------|
| `PiSSAAdapter(ILayer<>,Int32,Double,Boolean)` | Initializes a new PiSSA adapter wrapping an existing layer. |
...
| `ResidualWeights` | Gets the frozen residual weights matrix. |

## Methods

| Method | Summary |
|:-----|:--------|
| `Forward(Tensor<>)` | Performs the forward pass using residual weights plus trainable PiSSA adaptation. |
| `InitializeFromSVD(ILayer<>,Matrix<>,Int32,Double,Boolean,SvdAlgorithmType)` | Creates a PiSSA adapter initialized from SVD of pretrained weights. |
| `InitializeFromSVD(Matrix<>,SvdAlgorithmType)` | Initializes the adapter matrices from SVD of pretrained weights. |
| `MergeToOriginalLayer` | Merges the PiSSA adaptation into the original layer. |
...
| `_residualWeights` | The frozen residual weights after removing top-r principal components. |
