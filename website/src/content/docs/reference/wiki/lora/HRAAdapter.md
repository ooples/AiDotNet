...
HRA is like having two tools instead of one:
...
| Constructor | Summary |
|:-----|:--------|
| `HRAAdapter(ILayer<>,Int32,Double,Double,Boolean,Int32,Double,Boolean)` | Initializes a new HRA adapter with hybrid low-rank and sparse full-rank updates. |

## Properties

| Property | Summary |
|:-----|:--------|
| `ActiveSparseParams` | Gets the number of active sparse full-rank parameters. |
| `MaxSparseParams` | Gets the maximum allowed sparse parameters. |
| `ParameterCount` | Gets the total number of trainable parameters (low-rank + sparse full-rank). |
| `SparsityRatio` | Gets the current sparsity ratio. |

## Methods

| Method | Summary |
|:-----|:--------|
| `BackwardSparseFullRank(Tensor<>)` | Performs backward pass through the sparse full-rank component. |
| `Forward(Tensor<>)` | Performs the forward pass through the HRA adapter. |
| `ForwardSparseFullRank(Tensor<>)` | Performs forward pass through the sparse full-rank component. |
| `GetParameterImportance` | Gets a copy of the current parameter importance matrix. |
| `GetParameters` | Gets all parameters including base, LoRA, and sparse full-rank parameters. |
| `GetSparseUpdates` | Gets the positions and values of current sparse full-rank updates. |
| `MergeToOriginalLayer` | Merges the HRA adaptation into the base layer and returns the merged layer. |
| `ReallocateSparseParameters` | Reallocates sparse full-rank parameters to the most important locations. |
| `SetParameters(Vector<>)` | Sets all parameters including base, LoRA, and sparse full-rank parameters. |
| `UpdateImportanceScores(Tensor<>)` | Updates importance scores based on current gradient magnitudes. |
...
| Field | Summary |
|:-----|:--------|
| `_cachedInput` | Cached input from forward pass for computing sparse gradients in backward pass. |
| `_importanceEMA` | Exponential moving average factor for importance score updates. |
| `_importanceUpdateInterval` | Number of training steps between importance updates. |
| `_maxSparseParams` | Maximum number of sparse full-rank parameters to allocate. |
| `_parameterImportance` | Importance scores for each parameter in the weight matrix. |
| `_sparseFullRankUpdates` | Sparse full-rank update matrix storing only non-zero entries. |
| `_sparseGradients` | Gradient accumulator for the sparse full-rank component. |
| `_sparseScaling` | Scaling factor for the sparse full-rank component. |
| `_sparsityRatio` | Sparsity ratio for full-rank updates (0.0 to 1.0). |
| `_stepCount` | Current training step counter. |
| `_useDynamicAllocation` | Whether to use dynamic importance-based allocation. |
