...
Think of LoRA-XS as "ultra-compressed LoRA".
...
| Constructor | Summary |
|:-----|:--------|
| `LoRAXSAdapter(ILayer<>,Int32,Double,Boolean)` | Initializes a new LoRA-XS adapter wrapping an existing layer. |

## Properties

| Property | Summary |
|:-----|:--------|
| `FrozenSigma` | Gets the frozen singular values. |
| `FrozenU` | Gets the frozen U matrix (left singular vectors). |
| `FrozenVt` | Gets the frozen V^T matrix (right singular vectors transposed). |
| `InitializedFromSVD` | Gets whether this adapter was initialized from SVD. |
| `ParameterCount` | Gets the total number of trainable parameters (only r² for the R matrix). |
| `TrainableR` | Gets the trainable R matrix. |

## Methods

| Method | Summary |
|:-----|:--------|
| `ApplySigmaScaling(Vector<>,Tensor<>)` | Applies diagonal scaling by singular values: Σ * x |
| `ComputeMatrixGradient(Tensor<>,Tensor<>)` | Computes gradient matrix: grad = left * right^T |
| `Forward(Tensor<>)` | Performs the forward pass through the LoRA-XS adapter. |
| `GetParameters` | Gets the current parameters as a vector (only R matrix elements). |
| `InitializeFromSVD(Matrix<>,SvdAlgorithmType)` | Initializes the adapter from SVD of pretrained weights. |
| `MatrixVectorMultiply(Matrix<>,Tensor<>)` | Multiplies a matrix by a tensor (treating tensor as batch of vectors). |
| `MergeToOriginalLayer` | Merges the LoRA-XS adaptation into the base layer and returns the merged layer. |
| `ResetState` | Resets the internal state of the adapter. |
| `ScaleTensor(Tensor<>,)` | Scales all elements of a tensor by a scalar value. |
| `SetParameters(Vector<>)` | Sets the layer parameters from a vector (R matrix only). |
| `UpdateParameterGradientsFromR` | Updates the parameter gradients vector from R matrix gradient. |
| `UpdateParameters()` | Updates the trainable R matrix using the specified learning rate. |
| `UpdateParametersFromLayers` | Overrides base class parameter packing to prevent buffer overrun during base constructor. |
| `UpdateParametersFromR` | Updates the parameter vector from the R matrix. |
| `UpdateRFromParameters` | Updates the R matrix from the parameter vector. |

## Fields

| Field | Summary |
|:-----|:--------|
| `_cachedRVtInput` | Intermediate result from forward pass: R * (V_r^T * input) Cached for use in backward pass. |
| `_cachedSigmaRVtInput` | Intermediate result from forward pass: Σ_r * R * (V_r^T * input) Cached for use in backward pass. |
| `_cachedVtInput` | Intermediate result from forward pass: V_r^T * input Cached for use in backward pass. |
| `_frozenSigma` | Frozen singular values (diagonal of Σ_r) from SVD of pretrained weights. |
| `_frozenU` | Frozen left singular vectors (U_r) from SVD of pretrained weights. |
| `_frozenVt` | Frozen right singular vectors transposed (V_r^T) from SVD of pretrained weights. |
| `_initializedFromSVD` | Indicates whether the adapter was initialized from SVD of pretrained weights. |
| `_trainableR` | Trainable r×r mixing matrix R - the ONLY trainable parameters in LoRA-XS. |
| `_trainableRGradient` | Gradient of the trainable R matrix computed during backpropagation. |
