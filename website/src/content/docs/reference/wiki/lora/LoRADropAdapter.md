...
| Constructor | Summary |
|:-----|:--------|
| `LoRADropAdapter(ILayer<>,Int32,Double,Double,Boolean,Nullable<Int32>)` | Initializes a new LoRA-drop adapter with dropout regularization. |

## Properties

| Property | Summary |
|:-----|:--------|
| `DropoutRate` | Gets the dropout rate used for regularization. |
| `IsTraining` | Gets or sets whether the layer is in training mode. |

## Methods

| Method | Summary |
|:-----|:--------|
| `Forward(Tensor<>)` | Performs the forward pass with dropout applied to LoRA output. |
| `GenerateDropoutMask` | Generates a random dropout mask for the current forward pass. |
| `MergeToOriginalLayer` | Merges the LoRA adaptation into the base layer and returns the merged layer. |
| `ResetState` | Resets the internal state of both layers and clears the dropout mask. |
| `SetTraining(Boolean)` | Sets whether the layer is in training mode or inference mode. |
| `UpdateParameterGradientsFromLayers` | Updates parameter gradients from both layers (called by Backward). |

## Fields

| Field | Summary |
|:-----|:--------|
| `_dropoutMask` | Mask indicating which components to drop in the current forward pass. |
| `_dropoutRate` | Dropout rate (probability of dropping a component during training). |
| `_isTraining` | Indicates whether the layer is in training mode (dropout active) or inference mode (dropout inactive). |
| `_random` | Random number generator for dropout mask generation. |
