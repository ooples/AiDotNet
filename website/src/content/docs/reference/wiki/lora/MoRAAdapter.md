...
| Property | Summary |
|:-----|:--------|
| `SquareRank` | Gets the effective rank of the MoRA adaptation. |

## Methods

| Method | Summary |
|:-----|:--------|
| `CreateLoRALayer(Int32,Double)` | Creates a minimal placeholder LoRA layer to satisfy base class requirements. |
| `GetParameters` | Gets the current parameter values (base layer + MoRA matrix M). |
| `RebuildParameterSnapshot` | Reallocates and repopulates the Parameters and ParameterGradients vectors. |
| `SetParameters(Vector<>)` | Sets the parameter values (base layer + MoRA matrix M). |
| `UpdateParametersFromLayers` | Overrides the base parameter packing to use the MoRA matrix M instead of the placeholder LoRA layer. |

## Fields

| Field | Summary |
|:-----|:--------|
| `_compressionMatrix` | Compression matrix that reduces input dimension from d to r (non-trainable). |
| `_decompressionMatrix` | Decompression matrix that expands dimension from r back to d (non-trainable). |
| `_lastCompressed` | Cached compressed input from forward pass. |
| `_lastInput` | Stored input from the forward pass, needed for gradient computation. |
| `_matrixM` | Square matrix M for high-rank adaptation (r×r dimensions). |
| `_matrixMGradient` | Gradients for matrix M computed during backpropagation. |
| `_squareRank` | The dimension of the square matrix M. |
