...
/// var model = new PeakNetFP&lt;float&gt;(arch, "peaknet_fp.onnx");
...
public class PeakNetFP<T> : AudioNeuralNetworkBase<T>, IAudioFingerprinter<T>
{
    ...
    /// <summary>
    /// Creates a PeakNetFP model in ONNX inference mode.
    /// </summary>
    public PeakNetFP(NeuralNetworkArchitecture<T> architecture, string modelPath, PeakNetFPOptions? options = null)
        : base(architecture)
    {
        _options = options ?? new PeakNetFPOptions();
    ...
    /// <summary>
    /// Creates a PeakNetFP model in native training mode.
    /// </summary>
    public PeakNetFP(NeuralNetworkArchitecture<T> architecture, PeakNetFPOptions? options = null,
    ...
    internal static async Task<PeakNetFP<T>> CreateAsync(PeakNetFPOptions? options = null, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        options ??= new PeakNetFPOptions();
        string mp = options.ModelPath ?? string.Empty;
        if (string.IsNullOrEmpty(mp))
        {
            var dl = new OnnxModelDownloader();
            mp = await dl.DownloadAsync("peaknet_fp", "peaknet_fp.onnx", progress: progress, cancellationToken);
            options.ModelPath = mp;
        }
        var arch = new NeuralNetworkArchitecture<T>(inputFeatures: options.NumMels, outputSize: options.EmbeddingDim);
        return new PeakNetFP<T>(arch, mp, options);
    }
    ...
    protected override void InitializeLayers()
    {
        ...
        else Layers.AddRange(LayerHelper<T>.CreateDefaultPeakNetFPLayers(
            numMels: _options.NumMels, baseFilters: _options.BaseFilters,
            numEncoderBlocks: _options.NumEncoderBlocks, embeddingDim: _options.EmbeddingDim,
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        m.AdditionalInfo["PeaksPerFrame"] = _options.PeaksPerFrame.ToString();
        return m;
    }
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter w)
    {
        ...
        w.Write(_options.EmbeddingDim); w.Write(_options.NumEncoderBlocks);
        w.Write(_options.BaseFilters); w.Write(_options.PeaksPerFrame);
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader r)
    {
        ...
        _options.EmbeddingDim = r.ReadInt32(); _options.NumEncoderBlocks = r.ReadInt32();
        _options.BaseFilters = r.ReadInt32(); _options.PeaksPerFrame = r.ReadInt32();
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => new PeakNetFP<T>(Architecture, _options);

    #endregion

    #region Disposal

    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(PeakNetFP<T>)); }
