...
public class PANNs<T> : AudioClassifierBase<T>, IAudioEventDetector<T>
{
    ...
    protected override Tensor<T> PostprocessOutput(Tensor<T> o) { var r = new Tensor<T>(o.Shape._dims); for (int i = 0; i < o.Length; i++) r[i] = NumOps.FromDouble(1.0 / (1.0 + Math.Exp(-NumOps.ToDouble(o[i])))); return r; }
