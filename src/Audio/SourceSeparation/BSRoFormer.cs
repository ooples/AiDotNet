...
/// </para>
/// <para>
/// <b>For Beginners:</b> BS-RoFormer separates a mixed song into stems (vocals, drums, bass, other)
...
/// var model = new BSRoFormer&lt;float&gt;(arch, "bs_roformer.onnx");
...
[ModelPaper("Music Source Separation with Band-Split RNN", "https://doi.org/10.48550/arXiv.2309.02612", Year = 2023, Authors = "Wei-Tsung Lu, Ju-Chiang Wang, Qiuqiang Kong, Yun-Ning Hung")]
public class BSRoFormer<T> : AudioNeuralNetworkBase<T>, IMusicSourceSeparator<T>
{
    ...
    public BSRoFormer(NeuralNetworkArchitecture<T> architecture, string modelPath, BSRoFormerOptions? options = null)
        : base(architecture)
    {
        _options = options ?? new BSRoFormerOptions();
    ...
    public BSRoFormer(NeuralNetworkArchitecture<T> architecture, BSRoFormerOptions? options = null,
    ...
    internal static async Task<BSRoFormer<T>> CreateAsync(BSRoFormerOptions? options = null, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        options ??= new BSRoFormerOptions();
        string mp = options.ModelPath ?? string.Empty;
        if (string.IsNullOrEmpty(mp)) { var dl = new OnnxModelDownloader(); mp = await dl.DownloadAsync("bsroformer", "bs_roformer.onnx", progress: progress, cancellationToken); options.ModelPath = mp; }
        ...
        return new BSRoFormer<T>(arch, mp, options);
    }
    ...
    protected override void InitializeLayers()
    {
        ...
        else Layers.AddRange(LayerHelper<T>.CreateDefaultBSRoFormerLayers(
            numBands: _options.NumBands, bandEmbeddingDim: _options.BandEmbeddingDim,
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter w)
    {
        ...
        w.Write(_options.NumBands); w.Write(_options.TransformerDim); w.Write(_options.NumTransformerLayers);
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader r)
    {
        ...
        _options.NumBands = r.ReadInt32(); _options.TransformerDim = r.ReadInt32(); _options.NumTransformerLayers = r.ReadInt32();
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => new BSRoFormer<T>(Architecture, _options);
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(BSRoFormer<T>)); }
