...
[ModelPaper("Demucs: Deep Extractor for Music Sources with extra unlabeled data remixed", "https://doi.org/10.48550/arXiv.1909.01174", Year = 2019, Authors = "Alexandre Défossez, Nicolas Usunier, Léon Bottou, Francis Bach")]
public class MusicSourceSeparator<T> : AudioNeuralNetworkBase<T>, IMusicSourceSeparator<T>
{
    ...
    public Tensor<T> RemoveSource(Tensor<T> audio, string source)
    {
        ...
        foreach (var kvp in sources)
        {
            ...
            if (output is null)
            {
                output = new Tensor<T>(kvp.Value.Shape._dims);
        ...
        return output ?? new Tensor<T>(audio.Shape._dims);
    }
    ...
    public Tensor<T> Remix(SourceSeparationResult<T> separationResult, IReadOnlyDictionary<string, double> sourceVolumes)
    {
        ...
        return output ?? new Tensor<T>(separationResult.OriginalMix.Shape._dims);
    }
    ...
    private SourceSeparationResult<T> SeparateSpectral(Tensor<T> audio)
    {
        ...
        {
            vocals = harmonic;
            other = new Tensor<T>(audio.Shape._dims);
        }
