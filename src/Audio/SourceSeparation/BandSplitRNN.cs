...
/// </para>
/// <para>
/// <b>For Beginners:</b> BandSplitRNN separates a mixed song into individual instruments (vocals,
...
/// var model = new BandSplitRNN&lt;float&gt;(arch, "bandsplit_rnn.onnx");
...
public class BandSplitRNN<T> : AudioNeuralNetworkBase<T>, IMusicSourceSeparator<T>
{
    ...
    public BandSplitRNN(NeuralNetworkArchitecture<T> architecture, string modelPath, BandSplitRNNOptions? options = null)
        : base(architecture)
    {
    ...
    public BandSplitRNN(NeuralNetworkArchitecture<T> architecture, BandSplitRNNOptions? options = null,
    ...
    internal static async Task<BandSplitRNN<T>> CreateAsync(BandSplitRNNOptions? options = null, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        options ??= new BandSplitRNNOptions();
        string mp = options.ModelPath ?? string.Empty;
        if (string.IsNullOrEmpty(mp))
        {
            var dl = new OnnxModelDownloader();
            mp = await dl.DownloadAsync("bandsplit_rnn", "bandsplit_rnn.onnx", progress: progress, cancellationToken);
        ...
        return new BandSplitRNN<T>(arch, mp, options);
    }
    ...
    protected override void InitializeLayers()
    {
        ...
        else Layers.AddRange(LayerHelper<T>.CreateDefaultBandSplitRNNSeparationLayers(
            numBands: _options.NumBands, bandRnnHiddenSize: _options.BandRnnHiddenSize,
            numBandRnnLayers: _options.NumBandRnnLayers, sequenceRnnHiddenSize: _options.SequenceRnnHiddenSize,
            numSequenceRnnLayers: _options.NumSequenceRnnLayers, fusionDim: _options.FusionDim,
    ...
    public override void Train(Tensor<T> input, Tensor<T> expected)
    {
        ...
        SetTrainingMode(true); TrainWithTape(input, expected); SetTrainingMode(false);
    }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(BandSplitRNN<T>)); }
