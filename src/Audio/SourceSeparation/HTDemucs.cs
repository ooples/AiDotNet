...
/// </para>
/// <para>
/// <b>For Beginners:</b> HTDemucs is Meta's best music separator. It processes audio in two
...
/// <b>Usage:</b>
/// <code>
/// var arch = new NeuralNetworkArchitecture&lt;float&gt;(inputFeatures: 2049, outputSize: 2049);
/// var model = new HTDemucs&lt;float&gt;(arch, "htdemucs.onnx");
/// var result = model.Separate(mixedSong);
...
public class HTDemucs<T> : AudioNeuralNetworkBase<T>, IMusicSourceSeparator<T>
{
    ...
    public HTDemucs(NeuralNetworkArchitecture<T> architecture, string modelPath, HTDemucsOptions? options = null)
        : base(architecture)
    {
        _options = options ?? new HTDemucsOptions(); _useNativeMode = false;
    ...
    public HTDemucs(NeuralNetworkArchitecture<T> architecture, HTDemucsOptions? options = null,
    ...
    internal static async Task<HTDemucs<T>> CreateAsync(HTDemucsOptions? options = null, IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        options ??= new HTDemucsOptions(); string mp = options.ModelPath ?? string.Empty;
        if (string.IsNullOrEmpty(mp)) { var dl = new OnnxModelDownloader(); mp = await dl.DownloadAsync("htdemucs", "htdemucs.onnx", progress: progress, cancellationToken); options.ModelPath = mp; }
        ...
        return new HTDemucs<T>(arch, mp, options);
    }
    ...
    protected override void InitializeLayers()
    {
        ...
        else Layers.AddRange(LayerHelper<T>.CreateDefaultHTDemucsLayers(
            numFreqBins: _options.NumFreqBins, transformerDim: _options.TransformerDim,
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(HTDemucs<T>)); }
