...
public class HtmlDashboard : ITrainingDashboard
{
    ...
    {
        // Reuse the same snapshots for the summary, chart elements and scripts.
        // Logging/Clear may continue while the report is formatted.
        var scalars = GetScalarData();
        var histograms = GetHistogramData();
        ...
        if (histograms.Count > 0) sb.AppendLine("    <a href=\"#histograms\" class=\"nav-link\">Histograms</a>");
        ...
        sb.AppendLine(GenerateSummaryHtml(scalars));
        ...
        if (scalars.Count > 0)
        {
            ...
            sb.AppendLine(GenerateScalarsHtml(scalars));
            sb.AppendLine("</section>");
        }
        ...
        if (histograms.Count > 0)
        {
            ...
            sb.AppendLine(GenerateHistogramsHtml(histograms));
            sb.AppendLine("</section>");
        }
        ...
        sb.AppendLine(GenerateScripts(scalars, histograms));
        sb.AppendLine("</body>");
        sb.AppendLine("</html>");
    ...
    private string GenerateSummaryHtml(Dictionary<string, List<ScalarDataPoint>> scalars)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine($"    <div class=\"value\">{scalars.Count}</div>");
        sb.AppendLine("</div>");

        // Total data points
        var totalPoints = scalars.Values.Sum(s => s.Count);
        sb.AppendLine("<div class=\"summary-item\">");
        ...
        var lossKey = scalars.Keys.FirstOrDefault(k => k.Contains("loss", StringComparison.OrdinalIgnoreCase));
        if (lossKey is not null && scalars[lossKey].Count > 0)
        {
            var latestLoss = scalars[lossKey].Last().Value;
            sb.AppendLine("<div class=\"summary-item\">");
        ...
        var accKey = scalars.Keys.FirstOrDefault(k => k.Contains("accuracy", StringComparison.OrdinalIgnoreCase) || k.Contains("acc", StringComparison.OrdinalIgnoreCase));
        if (accKey is not null && scalars[accKey].Count > 0)
        {
            var latestAcc = scalars[accKey].Last().Value;
            sb.AppendLine("<div class=\"summary-item\">");
        ...
        // Training duration
        if (scalars.Values.Any(s => s.Count > 0))
        {
            var allTimes = scalars.Values.SelectMany(s => s.Select(p => p.WallTime)).ToList();
            if (allTimes.Count > 1)
            {
    ...
    private string GenerateScalarsHtml(Dictionary<string, List<ScalarDataPoint>> scalars)
    {
        var sb = new StringBuilder();
        sb.AppendLine("<div class=\"charts-grid\">");

        int chartIndex = 0;
        foreach (var kvp in scalars.OrderBy(k => k.Key))
        {
    ...
    private string GenerateHistogramsHtml(Dictionary<string, List<HistogramDataPoint>> histograms)
    {
        var sb = new StringBuilder();
        sb.AppendLine("<div class=\"charts-grid\">");

        int chartIndex = 0;
        foreach (var kvp in histograms.OrderBy(k => k.Key))
        {
    ...
    private string GenerateScripts(Dictionary<string, List<ScalarDataPoint>> scalars,
        Dictionary<string, List<HistogramDataPoint>> histograms)
    {
        var sb = new StringBuilder();
