...
/// <example>
/// <code>
/// // Create an OpenVoice model for versatile instant voice cloning
/// // with decoupled tone color conversion for flexible synthesis
...
/// var model = new OpenVoice&lt;double&gt;(architecture, "openvoice.onnx");
///
/// // Training mode with native layers
/// var trainModel = new OpenVoice&lt;double&gt;(architecture, new OpenVoiceOptions());
/// </code>
/// </example>
...
public class OpenVoice<T> : TtsModelBase<T>, IEndToEndTts<T>, IVoiceCloner<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "OpenVoice-Native" : "OpenVoice-ONNX", Description = "OpenVoice: Instant Voice Cloning with Tone Color Conversion (Qin et al., 2023)", FeatureCount = _options.HiddenDim }; m.AdditionalInfo["Architecture"] = "OpenVoice"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
