...
[ResearchPaper(
    "VALL-E X: Speak Foreign Languages with Your Own Voice via Cross-Lingual Neural Codec Language Modeling",
...
public class VALLEXClone<T> : TtsModelBase<T>, ICodecTts<T>, IVoiceCloner<T>
{
    ...
    public VALLEXClone(
        ...
        string modelPath,
        VALLEXCloneOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new VALLEXCloneOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // VALL-E X Clone: Cross-lingual voice cloning (Zhang et al. 2023)
        // Cross-lingual text encoder with speaker reference
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            h = Math.Tanh(e * 0.7 + h * 0.15 + spkRef * 0.18 + Math.Sin(f * 0.08) * 0.07);
            xlingEnc[f] = h;
        }
        // AR + NAR with speaker conditioning
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++)
            tokens[f] = Math.Tanh(xlingEnc[f] * 1.05);
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 191 / SampleRate) * 0.72
            );
        }
        return waveform;
    }
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(VALLEXClone<T>));
    }
