...
public class VALLEXClone<T> : TtsModelBase<T>, ICodecTts<T>, IVoiceCloner<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        double spkRef = 0.48;
        double[] xlingEnc = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.7 + h * 0.15 + spkRef * 0.18 + Math.Sin(f * 0.08) * 0.07); xlingEnc[f] = h; }
        // AR + NAR with speaker conditioning
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) tokens[f] = Math.Tanh(xlingEnc[f] * 1.05);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 191 / SampleRate) * 0.72); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "VALL-E-X-Clone-Native" : "VALL-E-X-Clone-ONNX", Description = "VALL-E X: Cross-Lingual Voice Cloning (Zhang et al., 2023)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
