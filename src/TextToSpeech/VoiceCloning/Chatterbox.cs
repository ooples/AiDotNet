...
public class Chatterbox<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Chatterbox(
        ...
        : base(architecture)
    {
        _options = options ?? new ChatterboxOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 195 / SampleRate) * 0.73
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Chatterbox-Native" : "Chatterbox-ONNX",
            Description =
                "Chatterbox: emotion-controllable zero-shot TTS with watermarking and exaggeration control.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "Chatterbox";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Chatterbox<T>));
    }
