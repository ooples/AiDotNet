...
[ResearchPaper("Chatterbox: Open-Source Voice Cloning", "https://github.com/resemble-ai/Chatterbox")]
public class Chatterbox<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "Chatterbox-Native" : "Chatterbox-ONNX", Description = "Chatterbox: emotion-controllable zero-shot TTS with watermarking and exaggeration control.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "Chatterbox"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
