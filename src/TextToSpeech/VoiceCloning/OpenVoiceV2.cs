...
[ResearchPaper(
    "OpenVoice V2: Improved Instant Voice Cloning",
...
public class OpenVoiceV2<T> : TtsModelBase<T>, IEndToEndTts<T>, IVoiceCloner<T>
{
    ...
    public OpenVoiceV2(
        ...
        string modelPath,
        OpenVoiceV2Options? options = null
    )
        : base(architecture)
    {
        ...
        : base(architecture)
    {
        _options = options ?? new OpenVoiceV2Options();
        _useNativeMode = true;
        // TtsModelOptions declares LearningRate (1e-4) and WeightDecay (0.01) and this model never
        // passed either, so training ran at AdamWOptimizerOptions' generic 1e-3 -- ten times the
        // documented rate -- and the option was unreachable no matter what a caller set. On the
        // memorization task that overshoots from the first step: the loss ROSE, 0.862768 at step 1
        // to 1.046356 at step 100.
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        int mF = textLen * 4;
        double[] baseMel = new double[mF];
        double p = 0;
        for (int f = 0; f < mF; f++)
        {
            int t = Math.Min(f * textLen / mF, textLen - 1);
            baseMel[f] = Math.Tanh(
                (text[t] % 128) / 128.0 * 0.55 + p * 0.3 + Math.Sin(f * 0.09) * 0.1
            );
            p = baseMel[f];
        }
        double[] conv = new double[mF];
        for (int f = 0; f < mF; f++)
            conv[f] = baseMel[f] * 0.9 + Math.Sin(f * 0.02) * 0.08;
        int waveLen = mF * _options.HopSize;
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++)
        {
            int fr = Math.Min(i / Math.Max(1, _options.HopSize), mF - 1);
            waveform[i] = NumOps.FromDouble(conv[fr] * Math.Sin(i * 0.007) * 0.87);
        }
        return waveform;
    }
    ...
    public Tensor<T> SynthesizeWithVoice(string text, Tensor<T> referenceAudio)
    {
        ...
        {
            double s = NumOps.ToDouble(baseAudio[i]);
            int d = i % embDim;
            double spk = NumOps.ToDouble(embedding[d]);
            double gain = 1.0 + spk * 0.3;
            double pitch = spk * 0.1;
            baseAudio[i] = NumOps.FromDouble(
                Math.Tanh(s * gain + Math.Sin(i * 0.001 + pitch) * 0.05)
            );
        }
        return baseAudio;
    }
    ...
    {
        int embDim = SpeakerEmbeddingDim;
        var emb = new Tensor<T>([embDim]);
        int hopLen = Math.Max(1, _options.HopSize);
        int numFrames = Math.Max(1, referenceAudio.Length / hopLen);
        for (int d = 0; d < embDim; d++)
        {
            double acc = 0;
            for (int f = 0; f < numFrames; f++)
            {
                int idx = Math.Min(f * hopLen + d % hopLen, referenceAudio.Length - 1);
                double val = NumOps.ToDouble(referenceAudio[idx]);
                acc += Math.Tanh(val * Math.Sin((d + 1.0) * (f + 1.0) * 0.001) * 0.5);
            }
            emb[d] = NumOps.FromDouble(acc / numFrames);
        }
        double norm = 0;
        for (int d = 0; d < embDim; d++)
        {
            double v = NumOps.ToDouble(emb[d]);
            norm += v * v;
        }
        norm = Math.Sqrt(norm);
        if (norm > 1e-8)
            for (int d = 0; d < embDim; d++)
                emb[d] = NumOps.FromDouble(NumOps.ToDouble(emb[d]) / norm);
        return emb;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "OpenVoiceV2-Native" : "OpenVoiceV2-ONNX",
            Description = "OpenVoiceV2 TTS",
            FeatureCount = _options.HiddenDim,
        };
        m.AdditionalInfo["Architecture"] = "OpenVoiceV2";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(OpenVoiceV2<T>));
    }
