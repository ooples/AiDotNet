...
public class OrpheusTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public OrpheusTTS(
        ...
        string modelPath,
        OrpheusTTSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new OrpheusTTSOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            double s12 = llmHidden[f];
            double s24 = Math.Sin(f * 0.15) * 0.12;
            double s48 = Math.Sin(f * 0.3) * 0.06;
            snac[f] = Math.Tanh(s12 + s24 + s48);
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                snac[fr] * Math.Sin(i * 2.0 * Math.PI * 192 / SampleRate) * 0.74
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "OrpheusTTS-Native" : "OrpheusTTS-ONNX",
            Description =
                "Orpheus: real-time emotion-controllable TTS with SNAC codec and LLaMA backbone.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "OrpheusTTS";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(OrpheusTTS<T>));
    }
