...
public class SoundStorm<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public SoundStorm(
        ...
        string modelPath,
        SoundStormOptions? options = null
    )
        : base(architecture)
    {
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SoundStorm-Native" : "SoundStorm-ONNX",
            Description = "SoundStorm: Parallel Audio Generation via MaskGIT (Borsos et al., 2023)",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "SoundStorm";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SoundStorm<T>));
    }
