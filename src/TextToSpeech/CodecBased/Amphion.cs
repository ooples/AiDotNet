...
public class Amphion<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Amphion(
        ...
        string modelPath,
        AmphionOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new AmphionOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            textEnc[f] = Math.Tanh((text[ci] % 128) / 128.0 * 1.5 - 0.75);
        }
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            double h = textEnc[f];
            for (int l = 0; l < 3; l++)
                h = Math.Tanh(h * 0.95 + Math.Sin(f * 0.1 * (l + 1)) * 0.15);
            codecOut[f] = h;
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                codecOut[fr] * Math.Sin(i * 2.0 * Math.PI * 180 / SampleRate) * 0.75
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Amphion-Native" : "Amphion-ONNX",
            Description =
                "Amphion: open-source audio toolkit supporting multiple TTS architectures.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "Amphion";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Amphion<T>(Architecture, mp, _options);
        return new Amphion<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Amphion<T>));
    }
