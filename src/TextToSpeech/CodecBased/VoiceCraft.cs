...
public class VoiceCraft<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        int textLen = Math.Min(text.Length, _options.MaxTextLength); int cF = textLen * 3; int nQ = _options.NumCodebooks;
        double[,] tok = new double[nQ, cF + nQ];
        for (int q = 0; q < nQ; q++) for (int f = 0; f < cF; f++) { int t = Math.Min(f * textLen / cF, textLen - 1); int dF = f + q; double p = (f > 0) ? tok[q, dF - 1] : 0; tok[q, dF] = Math.Tanh((text[t] % 128) / 128.0 * (0.7 - q * 0.06) + p * 0.25 + Math.Sin(f * 0.07 * (q + 1)) * 0.12); }
        int waveLen = cF * (SampleRate / _options.CodecFrameRate); var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, cF - 1); double s = 0; for (int q = 0; q < nQ; q++) s += tok[q, fr + q] * Math.Sin(i * (q + 1) * 0.005) / nQ; waveform[i] = NumOps.FromDouble(Math.Tanh(s)); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "VoiceCraft-Native" : "VoiceCraft-ONNX", Description = "VoiceCraft TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "VoiceCraft"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new VoiceCraft<T>(Architecture, mp, _options); return new VoiceCraft<T>(Architecture, _options); }
