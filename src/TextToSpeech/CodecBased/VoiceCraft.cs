...
public class VoiceCraft<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public VoiceCraft(
        ...
        string modelPath,
        VoiceCraftOptions? options = null
    )
        : base(architecture)
    {
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < cF; f++)
        {
            ...
            int dF = f + q;
            double p = (f > 0) ? tok[q, dF - 1] : 0;
            tok[q, dF] = Math.Tanh(
                (text[t] % 128) / 128.0 * (0.7 - q * 0.06)
                    + p * 0.25
                    + Math.Sin(f * 0.07 * (q + 1)) * 0.12
            );
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            double s = 0;
            for (int q = 0; q < nQ; q++)
                s += tok[q, fr + q] * Math.Sin(i * (q + 1) * 0.005) / nQ;
            waveform[i] = NumOps.FromDouble(Math.Tanh(s));
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description = "VoiceCraft TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "VoiceCraft";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(VoiceCraft<T>));
    }
