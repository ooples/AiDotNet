...
{
    private readonly ChatTTSOptions _options;
    ...
    public ChatTTS(
        ...
        string modelPath,
        ChatTTSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new ChatTTSOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "ChatTTS-Native" : "ChatTTS-ONNX",
            Description =
                "ChatTTS: conversational TTS with prosody control tokens for natural dialogue synthesis.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "ChatTTS";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new ChatTTS<T>(Architecture, mp, _options);
        return new ChatTTS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(ChatTTS<T>));
    }
