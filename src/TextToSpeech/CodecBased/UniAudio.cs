...
public class UniAudio<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public UniAudio(
        ...
        string modelPath,
        UniAudioOptions? options = null
    )
        : base(architecture, new CrossEntropyWithLogitsLoss<T>(classAxis: -1))
    {
        _options = options ?? new UniAudioOptions();
        ...
        : base(architecture, new CrossEntropyWithLogitsLoss<T>(classAxis: -1))
    {
        ...
        _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(
            ...
            {
                // UniAudio Appendix: peak pre-training LR 1e-4 with Noam decay.
                // The optimizer consumes the configured peak; scheduling remains
                // available through the framework's normal training surface.
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            unified[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0 + Math.Sin(f * 0.065) * 0.12);
        }
        // LLM backbone with task-specific adapters
        double[] adapted = new double[codecFrames];
        double h = 0;
        for (int f = 0; f < codecFrames; f++)
        {
            h = Math.Tanh(unified[f] * 0.8 + h * 0.15);
            adapted[f] = h;
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                adapted[fr] * Math.Sin(i * 2.0 * Math.PI * 197 / SampleRate) * 0.7
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "UniAudio-Native" : "UniAudio-ONNX",
            Description = "UniAudio TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "UniAudio";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(UniAudio<T>));
    }
