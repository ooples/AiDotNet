...
public class UniAudio<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // UniAudio: Unified codec-based audio generation (Yang et al. 2024)
        // Multi-task tokenizer: text -> unified token space
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] unified = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); unified[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0 + Math.Sin(f * 0.065) * 0.12); }
        // LLM backbone with task-specific adapters
        double[] adapted = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { h = Math.Tanh(unified[f] * 0.8 + h * 0.15); adapted[f] = h; }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(adapted[fr] * Math.Sin(i * 2.0 * Math.PI * 197 / SampleRate) * 0.7); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "UniAudio-Native" : "UniAudio-ONNX", Description = "UniAudio TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "UniAudio"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
