...
public class Bark<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "Bark-Native" : "Bark-ONNX", Description = "Bark: GPT-based text-to-audio model generating speech, music, and sound effects from text prompts.", FeatureCount = _options.LLMDim }; }
