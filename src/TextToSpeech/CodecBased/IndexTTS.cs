...
/// <remarks><para><b>References:</b><list type="bullet"><item>Project: "IndexTTS: An Industrial-Level Zero-Shot TTS System" (Index, 2024)</item></list></para></remarks>
public class IndexTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // IndexTTS: Pre-trained LM backbone with codec prediction (Index Team 2024)
        // LM-based text encoding with causal attention
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] lmOut = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.85 + h * 0.1 + Math.Sin(f * 0.065) * 0.12); lmOut[f] = h; }
        // Codec head: LM hidden states -> codec token logits
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) tokens[f] = Math.Tanh(lmOut[f] * 1.1 + Math.Cos(f * 0.13) * 0.08);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 205 / SampleRate) * 0.68); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "IndexTTS-Native" : "IndexTTS-ONNX", Description = "IndexTTS: LLM-based zero-shot TTS with reference audio indexing for voice cloning.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new IndexTTS<T>(Architecture, mp, _options); return new IndexTTS<T>(Architecture, _options); }
