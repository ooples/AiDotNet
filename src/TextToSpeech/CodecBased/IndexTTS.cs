...
public class IndexTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> EncodeToTokens(Tensor<T> audio) { ThrowIfDisposed(); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(audio); if (IsOnnxMode) throw new InvalidOperationException("ONNX model is not loaded. Cannot encode in ONNX mode without a model."); return Predict(audio); }
    ...
    public Tensor<T> DecodeFromTokens(Tensor<T> tokens) { ThrowIfDisposed(); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(tokens); if (IsOnnxMode) throw new InvalidOperationException("ONNX model is not loaded. Cannot decode in ONNX mode without a model."); return Predict(tokens); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "IndexTTS-Native" : "IndexTTS-ONNX", Description = "IndexTTS: LLM-based zero-shot TTS with reference audio indexing for voice cloning.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "IndexTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = _options.LLMDim; m.AdditionalInfo["SampleRate"] = _options.SampleRate; m.AdditionalInfo["MelChannels"] = _options.MelChannels; m.AdditionalInfo["HopSize"] = _options.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new IndexTTS<T>(Architecture, mp, _options); return new IndexTTS<T>(Architecture, _options, _optimizer); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(IndexTTS<T>)); }
