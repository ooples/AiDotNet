...
public class TortoiseTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public TortoiseTTS(
        ...
        string modelPath,
        TortoiseTTSOptions? options = null
    )
        : base(architecture)
    {
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "TortoiseTTS-Native" : "TortoiseTTS-ONNX",
            Description = "TortoiseTTS TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "TortoiseTTS";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new TortoiseTTS<T>(Architecture, mp, _options);
        return new TortoiseTTS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(TortoiseTTS<T>));
    }
