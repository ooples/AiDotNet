...
public class Zonos<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    /// <summary>Synthesizes speech. Zonos: text + speaker/emotion conditioning -> backbone transformer -> DAC decoder -> waveform.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // Zonos: Zonal TTS with code-level disentanglement (Zyphra 2025)
        // Disentangled zone encoding: content, speaker, prosody zones
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] content = new double[codecFrames]; double[] prosodyZone = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); content[f] = (text[ci] % 128) / 128.0; prosodyZone[f] = 0.5 + Math.Sin(f * 0.04) * 0.3; }
        // Zone fusion and codec generation
        double[] fused = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) fused[f] = Math.Tanh(content[f] * 0.7 + prosodyZone[f] * 0.3);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(fused[fr] * Math.Sin(i * 2.0 * Math.PI * 182 / SampleRate) * 0.74); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "Zonos-Native" : "Zonos-ONNX", Description = "Zonos: zero-shot TTS with conditioning for speaker, emotion, and quality.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "Zonos"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new Zonos<T>(Architecture, mp, _options); return new Zonos<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Zonos<T>)); }
