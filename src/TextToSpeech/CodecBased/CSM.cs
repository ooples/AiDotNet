...
{
    private readonly CSMOptions _options;
    ...
    public CSM(
        ...
        string modelPath,
        CSMOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new CSMOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            contextEnc[f] = Math.Tanh(e * 0.85 + Math.Sin(f * 0.055) * 0.15);
        }
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            hCtx = Math.Tanh(contextEnc[f] * 0.7 + hCtx * 0.25);
            tokens[f] = hCtx;
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 202 / SampleRate) * 0.71
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "CSM-Native" : "CSM-ONNX",
            Description =
                "CSM: Conversational Speech Model for context-sensitive multi-turn dialogue synthesis.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "CSM";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new CSM<T>(Architecture, mp, _options);
        return new CSM<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(CSM<T>));
    }
