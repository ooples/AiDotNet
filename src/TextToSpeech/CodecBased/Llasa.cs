...
public class Llasa<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "Llasa-Native" : "Llasa-ONNX", Description = "Llasa: LLaMA-based speech synthesis using XCodec2 for multi-level codec representation.", FeatureCount = _options.LLMDim }; }
