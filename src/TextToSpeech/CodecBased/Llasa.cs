...
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "Llasa: Scaling Compute for Llama-based Speech Synthesis" (Ye et al., 2025)</item></list></para></remarks>
public class Llasa<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // Llasa: LLM All-in-one Speech AR (Llasa Team 2024)
        // Unified LLM encoder with speech-text mixed tokens
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 4;
        double[] unified = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; double posEnc = Math.Sin(f * 0.02) * 0.1; h = Math.Tanh(e * 0.7 + h * 0.25 + posEnc); unified[f] = h; }
        // Codec prediction head with grouped quantization
        double[] quant = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) quant[f] = Math.Round(unified[f] * 4) / 4.0;
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(quant[fr] * Math.Sin(i * 2.0 * Math.PI * 215 / SampleRate) * 0.72); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "Llasa-Native" : "Llasa-ONNX", Description = "Llasa: LLaMA-based speech synthesis using XCodec2 for multi-level codec representation.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new Llasa<T>(Architecture, mp, _options); return new Llasa<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Llasa<T>)); }
