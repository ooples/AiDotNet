...
/// <summary>Voicebox: Voicebox: Text-Guided Multilingual Universal Speech Generation at Scale.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "Voicebox: Text-Guided Multilingual Universal Speech Generation at Scale" (Le et al., 2023)</item></list></para><para><b>For Beginners:</b> Voicebox: Voicebox: Text-Guided Multilingual Universal Speech Generation at Scale.. This model converts text input into speech audio output.</para></remarks>
...
{
    private readonly VoiceboxOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    public Voicebox(NeuralNetworkArchitecture<T> architecture, string modelPath, VoiceboxOptions? options = null) : base(architecture) { _options = options ?? new VoiceboxOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public Voicebox(NeuralNetworkArchitecture<T> architecture, VoiceboxOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new VoiceboxOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    /// Per Le et al. (2023): Flow-matching with infilling objective for text-conditioned speech generation.
    /// </summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        int textLen = Math.Min(text.Length, _options.MaxTextLength); int cF = textLen * 3;
        double[] latent = new double[cF]; for (int f = 0; f < cF; f++) latent[f] = Math.Sin(f * 0.3) * 0.5 + Math.Cos(f * 0.7) * 0.3;
        double[] textCond = new double[cF]; for (int f = 0; f < cF; f++) { int t = Math.Min(f * textLen / cF, textLen - 1); textCond[f] = (text[t] % 128) / 128.0; }
        for (int step = 0; step < 10; step++) { double dt = 1.0 / 10; for (int f = 0; f < cF; f++) { double vel = textCond[f] * 0.5 - latent[f] * 0.3 + Math.Sin(f * 0.06 + step * 0.2) * 0.1; latent[f] += vel * dt; latent[f] = Math.Tanh(latent[f]); } }
        int waveLen = cF * (SampleRate / _options.CodecFrameRate); var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, cF - 1); waveform[i] = NumOps.FromDouble(latent[fr] * Math.Sin(i * 0.008 + latent[fr]) * 0.85); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "Voicebox-Native" : "Voicebox-ONNX", Description = "Voicebox TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new Voicebox<T>(Architecture, mp, _options); return new Voicebox<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Voicebox<T>)); }
