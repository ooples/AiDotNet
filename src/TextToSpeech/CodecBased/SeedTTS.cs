...
public class SeedTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SeedTTS-Native" : "SeedTTS-ONNX",
            Description =
                "Seed-TTS: large-scale autoregressive TTS with self-distillation for in-context learning.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "SeedTTS";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SeedTTS<T>));
    }
