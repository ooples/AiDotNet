...
/// <summary>Fish Speech: dual-AR codec language model with grouped-finite-scalar-quantization for fast TTS.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks><para><b>References:</b><list type="bullet"><item>Project: "Fish Speech: Brand new TTS solution" (Fish Audio, 2024)</item></list></para></remarks>
public class FishSpeech<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly FishSpeechOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // FishSpeech: Dual AR with grouped finite scalar quantization (Fish Audio 2024)
        // GFSQ text encoder
        ...
        // Dual AR: slow AR for semantic, fast AR for acoustic details
        double[] slowAR = new double[codecFrames]; double hS = 0;
        for (int f = 0; f < codecFrames; f++) { hS = Math.Tanh(textEnc[f] * 0.8 + hS * 0.15); slowAR[f] = hS; }
        double[] fastAR = new double[codecFrames]; double hF = 0;
        for (int f = 0; f < codecFrames; f++) { hF = Math.Tanh(slowAR[f] * 0.6 + textEnc[f] * 0.3 + hF * 0.1); fastAR[f] = hF; }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(fastAR[fr] * Math.Sin(i * 2.0 * Math.PI * 200 / SampleRate) * 0.7); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "FishSpeech-Native" : "FishSpeech-ONNX", Description = "Fish Speech: dual-AR codec language model with grouped-finite-scalar-quantization for fast TTS.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new FishSpeech<T>(Architecture, mp, _options); return new FishSpeech<T>(Architecture, _options); }
