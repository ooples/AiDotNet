...
/// <summary>CosyVoice: multilingual TTS with supervised semantic tokens and conditional flow matching.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "CosyVoice: A Scalable Multilingual TTS Based on Supervised Semantic Tokens" (Du et al., 2024)</item></list></para></remarks>
public class CosyVoice<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly CosyVoiceOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    /// <summary>Synthesizes speech. CosyVoice: text -> LLM AR semantic tokens -> conditional flow matching -> mel -> HiFi-GAN.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // CosyVoice: LLM text encoder + conditional flow matching decoder (Alibaba 2024)
        // Text encoding via LLM backbone
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] textEmb = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); textEmb[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0); }
        // Conditional flow matching: ODE from noise to mel
        double[] latent = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) latent[f] = Math.Sin(f * 0.3) * 0.5;
        for (int step = 0; step < 8; step++) { double t = (step + 1.0) / 8; for (int f = 0; f < codecFrames; f++) { double vel = textEmb[f] - latent[f]; latent[f] += vel * (1.0 / 8); } }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(latent[fr] * Math.Sin(i * 2.0 * Math.PI * 190 / SampleRate) * 0.8); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "CosyVoice-Native" : "CosyVoice-ONNX", Description = "CosyVoice: multilingual TTS with supervised semantic tokens and conditional flow matching.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new CosyVoice<T>(Architecture, mp, _options); return new CosyVoice<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(CosyVoice<T>)); }
