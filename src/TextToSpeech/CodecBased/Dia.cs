...
/// <remarks><para><b>References:</b><list type="bullet"><item>Project: "Dia: Generate realistic dialogue" (Nari AI, 2025)</item></list></para></remarks>
public class Dia<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Dia(NeuralNetworkArchitecture<T> architecture, string modelPath, DiaOptions? options = null) : base(architecture) { _options = options ?? new DiaOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public Dia(NeuralNetworkArchitecture<T> architecture, DiaOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new DiaOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // Dia: Multi-stream token prediction (Nari Labs 2025)
        // Parallel stream text encoding
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] stream1 = new double[codecFrames]; double[] stream2 = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; stream1[f] = Math.Tanh(e * 1.1 + Math.Sin(f * 0.09) * 0.2); stream2[f] = Math.Tanh(e * 0.9 + Math.Cos(f * 0.07) * 0.25); }
        // Multi-stream fusion and codec generation
        double[] merged = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) merged[f] = Math.Tanh(stream1[f] * 0.6 + stream2[f] * 0.4);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(merged[fr] * Math.Sin(i * 2.0 * Math.PI * 210 / SampleRate) * 0.72); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "Dia-Native" : "Dia-ONNX", Description = "Dia: dialogue-oriented TTS generating multi-speaker conversations with expressive prosody.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new Dia<T>(Architecture, mp, _options); return new Dia<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Dia<T>)); }
