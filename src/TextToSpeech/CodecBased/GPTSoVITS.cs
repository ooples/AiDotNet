...
public class GPTSoVITS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public GPTSoVITS(
        ...
        string modelPath,
        GPTSoVITSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new GPTSoVITSOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "GPTSoVITS-Native" : "GPTSoVITS-ONNX",
            Description =
                "GPT-SoVITS: few-shot TTS combining GPT-style autoregressive with SoVITS decoder.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new GPTSoVITS<T>(Architecture, mp, _options);
        return new GPTSoVITS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(GPTSoVITS<T>));
    }
