...
/// <remarks><para><b>References:</b><list type="bullet"><item>Project: "GPT-SoVITS: 1 min voice data for good TTS" (RVC-Boss, 2024)</item></list></para></remarks>
public class GPTSoVITS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    /// <summary>Synthesizes speech. GPT-SoVITS: text + 5s reference -> GPT AR codec tokens -> SoVITS decoder -> waveform.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // GPT-SoVITS: GPT semantic prediction + SoVITS decoder (RVC-Boss 2024)
        // GPT stage: text -> semantic tokens
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int semFrames = textLen * 2;
        double[] semantic = new double[semFrames]; double hGPT = 0;
        for (int f = 0; f < semFrames; f++) { int ci = Math.Min(f / 2, textLen - 1); double e = (text[ci] % 128) / 128.0; hGPT = Math.Tanh(e * 0.75 + hGPT * 0.2 + Math.Sin(f * 0.09) * 0.08); semantic[f] = hGPT; }
        // SoVITS decoder: semantic tokens -> mel -> waveform via VITS-like flow
        int codecFrames = semFrames * 2;
        double[] mel = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int sf = Math.Min(f / 2, semFrames - 1); mel[f] = semantic[sf] + Math.Sin(f * 0.2) * 0.12 * (1.0 - Math.Abs(semantic[sf])); }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(mel[fr] * Math.Sin(i * 2.0 * Math.PI * 175 / SampleRate) * 0.73); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "GPTSoVITS-Native" : "GPTSoVITS-ONNX", Description = "GPT-SoVITS: few-shot TTS combining GPT-style autoregressive with SoVITS decoder.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new GPTSoVITS<T>(Architecture, mp, _options); return new GPTSoVITS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(GPTSoVITS<T>)); }
