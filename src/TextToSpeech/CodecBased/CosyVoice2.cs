...
public class CosyVoice2<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public CosyVoice2(NeuralNetworkArchitecture<T> architecture, string modelPath, CosyVoice2Options? options = null) : base(architecture) { _options = options ?? new CosyVoice2Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public CosyVoice2(NeuralNetworkArchitecture<T> architecture, CosyVoice2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new CosyVoice2Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // CosyVoice 2: Chunk-aware causal flow matching (Du et al. 2024)
        // Finite scalar quantization text encoder
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] textEnc = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); textEnc[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0 + Math.Sin(f * 0.08) * 0.1); }
        // Chunk-aware causal flow matching with streaming support
        int chunkSize = 16;
        double[] latent = new double[codecFrames];
        for (int c = 0; c < codecFrames; c += chunkSize) { int end = Math.Min(c + chunkSize, codecFrames); for (int step = 0; step < 6; step++) { double t = (step + 1.0) / 6; for (int f = c; f < end; f++) { double vel = textEnc[f] - latent[f]; latent[f] += vel * (1.0 / 6); } } }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(latent[fr] * Math.Sin(i * 2.0 * Math.PI * 195 / SampleRate) * 0.78); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "CosyVoice2-Native" : "CosyVoice2-ONNX", Description = "CosyVoice 2: improved streaming TTS with chunk-aware causal flow matching and FSQ.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "CosyVoice2"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new CosyVoice2<T>(Architecture, mp, _options); return new CosyVoice2<T>(Architecture, _options); }
