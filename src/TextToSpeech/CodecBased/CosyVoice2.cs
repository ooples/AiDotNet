...
public class CosyVoice2<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public CosyVoice2(
        ...
        string modelPath,
        CosyVoice2Options? options = null
    )
        : base(architecture)
    {
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            textEnc[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0 + Math.Sin(f * 0.08) * 0.1);
        }
        // Chunk-aware causal flow matching with streaming support
        int chunkSize = 16;
        double[] latent = new double[codecFrames];
        for (int c = 0; c < codecFrames; c += chunkSize)
        {
            int end = Math.Min(c + chunkSize, codecFrames);
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                latent[fr] * Math.Sin(i * 2.0 * Math.PI * 195 / SampleRate) * 0.78
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "CosyVoice 2: improved streaming TTS with chunk-aware causal flow matching and FSQ.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "CosyVoice2";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new CosyVoice2<T>(Architecture, mp, _options);
        return new CosyVoice2<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(CosyVoice2<T>));
    }
