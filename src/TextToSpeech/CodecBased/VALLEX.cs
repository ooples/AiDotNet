...
/// <summary>VALL-E X: cross-lingual zero-shot TTS extending VALL-E with language ID conditioning.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "VALL-E X: Speak Foreign Languages with Your Own Voice" (Zhang et al., 2023)</item></list></para><para><b>For Beginners:</b> VALL-E X: cross-lingual zero-shot TTS extending VALL-E with language ID conditioning.. This model converts text input into speech audio output.</para></remarks>
public class VALLEX<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly VALLEXOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    /// <summary>Synthesizes speech. VALL-E X: text + language ID + 3s prompt -> AR + NAR transformers -> EnCodec decoder -> waveform.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; double langEmb = Math.Sin(ci * 0.15) * 0.1; xlingEnc[f] = Math.Tanh(e * 0.8 + langEmb); }
        // AR + NAR with language-agnostic codec prediction
        double[] tokens = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { h = Math.Tanh(xlingEnc[f] * 0.75 + h * 0.2); tokens[f] = h; }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 193 / SampleRate) * 0.71); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "VALLEX-Native" : "VALLEX-ONNX", Description = "VALL-E X: cross-lingual zero-shot TTS extending VALL-E with language ID conditioning.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new VALLEX<T>(Architecture, mp, _options); return new VALLEX<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(VALLEX<T>)); }
