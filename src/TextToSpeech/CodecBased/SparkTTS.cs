...
public class SparkTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public SparkTTS(NeuralNetworkArchitecture<T> architecture, string modelPath, SparkTTSOptions? options = null) : base(architecture) { _options = options ?? new SparkTTSOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public SparkTTS(NeuralNetworkArchitecture<T> architecture, SparkTTSOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SparkTTSOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // SparkTTS: Multi-attribute controllable TTS (SparkAudio 2025)
        // Attribute-conditioned text encoder (pitch, speed, timbre)
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double pitch = 0.5; double speed = 1.0; double timbre = 0.5;
        double[] attrEnc = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; attrEnc[f] = Math.Tanh(e * 0.8 + pitch * 0.15 + timbre * 0.1 + Math.Sin(f * speed * 0.07) * 0.12); }
        // BiCodec decoder with attribute control
        double[] tokens = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { h = Math.Tanh(attrEnc[f] * 0.85 + h * 0.1); tokens[f] = h; }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 208 / SampleRate) * 0.69); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "SparkTTS-Native" : "SparkTTS-ONNX", Description = "Spark-TTS: LLM-based zero-shot TTS with BiCodec for controllable synthesis.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "SparkTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new SparkTTS<T>(Architecture, mp, _options); return new SparkTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SparkTTS<T>)); }
