...
public class SparkTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            attrEnc[f] = Math.Tanh(
                e * 0.8 + pitch * 0.15 + timbre * 0.1 + Math.Sin(f * speed * 0.07) * 0.12
            );
        }
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            h = Math.Tanh(attrEnc[f] * 0.85 + h * 0.1);
            tokens[f] = h;
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 208 / SampleRate) * 0.69
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SparkTTS-Native" : "SparkTTS-ONNX",
            Description =
                "Spark-TTS: LLM-based zero-shot TTS with BiCodec for controllable synthesis.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "SparkTTS";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new SparkTTS<T>(Architecture, mp, _options);
        return new SparkTTS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SparkTTS<T>));
    }
