...
public class FireRedTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public FireRedTTS(
        ...
        string modelPath,
        FireRedTTSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new FireRedTTSOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "FireRedTTS-Native" : "FireRedTTS-ONNX",
            Description =
                "FireRedTTS: foundation TTS model with large-scale training and multi-codebook generation.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "FireRedTTS";
    ...
    private static void ValidateOptions(FireRedTTSOptions opts)
    {
        ...
        if (opts.TextEncoderDim <= 0)
            throw new ArgumentOutOfRangeException(nameof(opts), "TextEncoderDim must be positive.");
        if (opts.NumHeads <= 0)
            throw new ArgumentOutOfRangeException(nameof(opts), "NumHeads must be positive.");
        if (opts.MelChannels <= 0)
            throw new ArgumentOutOfRangeException(nameof(opts), "MelChannels must be positive.");
        if (opts.HopSize <= 0)
            throw new ArgumentOutOfRangeException(nameof(opts), "HopSize must be positive.");
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(FireRedTTS<T>));
    }
