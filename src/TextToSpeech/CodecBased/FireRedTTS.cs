...
public class FireRedTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "FireRedTTS-Native" : "FireRedTTS-ONNX", Description = "FireRedTTS: foundation TTS model with large-scale training and multi-codebook generation.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "FireRedTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = _options.LLMDim; m.AdditionalInfo["SampleRate"] = _options.SampleRate; m.AdditionalInfo["MelChannels"] = _options.MelChannels; m.AdditionalInfo["HopSize"] = _options.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new FireRedTTS<T>(Architecture, mp, _options); return new FireRedTTS<T>(Architecture, _options, _optimizer); }
    private static void ValidateOptions(FireRedTTSOptions opts) { if (opts.SampleRate <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "SampleRate must be positive."); if (opts.MaxTextLength <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "MaxTextLength must be positive."); if (opts.NumCodebooks <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "NumCodebooks must be positive."); if (opts.CodebookSize <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "CodebookSize must be positive."); if (opts.LLMDim <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "LLMDim must be positive."); if (opts.TextEncoderDim <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "TextEncoderDim must be positive."); if (opts.NumHeads <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "NumHeads must be positive."); if (opts.MelChannels <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "MelChannels must be positive."); if (opts.HopSize <= 0) throw new ArgumentOutOfRangeException(nameof(opts), "HopSize must be positive."); }
