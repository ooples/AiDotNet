...
public class StyleTTS2<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int step = _options.NumStyleDiffusionSteps - 1; step >= 0; step--)
        {
            ...
            for (int s = 0; s < _options.StyleDim; s++)
                styleVec[s] = Math.Tanh(
                    Math.Sin(s * 0.3) * (1 - noiseScale) + styleVec[s] * noiseScale
                );
        }
        int totalFrames = 0;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description = "StyleTTS 2: Human-Level Style Diffusion TTS (Li et al., 2023)",
            FeatureCount = _options.HiddenDim,
        };
        m.AdditionalInfo["Architecture"] = "StyleTTS2";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new StyleTTS2<T>(Architecture, mp, _options);
        return new StyleTTS2<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(StyleTTS2<T>));
    }
