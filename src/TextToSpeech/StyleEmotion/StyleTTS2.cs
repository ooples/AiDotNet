...
[ResearchPaper("StyleTTS 2: Towards Human-Level Text-to-Speech through Style Diffusion and Adversarial Training with Large Speech Language Models", "https://arxiv.org/abs/2306.07691", Year = 2023, Authors = "Li et al.")]
public class StyleTTS2<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    private readonly StyleTTS2Options _options; public override ModelOptions GetOptions() => _options;
    ...
    public StyleTTS2(NeuralNetworkArchitecture<T> architecture, string modelPath, StyleTTS2Options? options = null) : base(architecture) { _options = options ?? new StyleTTS2Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public StyleTTS2(NeuralNetworkArchitecture<T> architecture, StyleTTS2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new StyleTTS2Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; InitializeLayers(); }
    ...
    /// Synthesizes speech using StyleTTS 2's enhanced style diffusion pipeline.
    /// Per the paper (Li et al., 2023):
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // Enhanced style diffusion prior
        double[] styleVec = new double[_options.StyleDim];
        for (int step = _options.NumStyleDiffusionSteps - 1; step >= 0; step--)
        {
            double noiseScale = (double)step / _options.NumStyleDiffusionSteps;
            for (int s = 0; s < _options.StyleDim; s++)
                styleVec[s] = Math.Tanh(Math.Sin(s * 0.3) * (1 - noiseScale) + styleVec[s] * noiseScale);
        }
        int totalFrames = 0;
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "StyleTTS2-Native" : "StyleTTS2-ONNX", Description = "StyleTTS 2: Human-Level Style Diffusion TTS (Li et al., 2023)", FeatureCount = _options.HiddenDim }; m.AdditionalInfo["Architecture"] = "StyleTTS2"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new StyleTTS2<T>(Architecture, mp, _options); return new StyleTTS2<T>(Architecture, _options); }
