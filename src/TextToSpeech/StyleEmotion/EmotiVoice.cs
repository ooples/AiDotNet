...
public class EmotiVoice<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public EmotiVoice(
        ...
        string modelPath,
        EmotiVoiceOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new EmotiVoiceOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int t = 0; t < textLen; t++)
        {
            durations[t] = Math.Max(1, (int)(3 + textHidden[t] * 2 + emotionBias));
        ...
        for (int r = 0; r < durations[t] && fIdx < totalFrames; r++, fIdx++)
            mel[fIdx] = Math.Tanh(
                textHidden[t] * 0.8 + Math.Sin(fIdx * 0.06) * 0.15 + emotionBias * 0.2
            );
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "EmotiVoice-Native" : "EmotiVoice-ONNX",
            Description = "EmotiVoice: Multi-Voice Prompt-Controlled TTS (NetEase, 2023)",
            FeatureCount = _options.HiddenDim,
            AdditionalInfo = new Dictionary<string, object>
            {
                ["NativeMode"] = _useNativeMode,
                ["SampleRate"] = _options.SampleRate,
                ["MelChannels"] = _options.MelChannels,
                ["HiddenDim"] = _options.HiddenDim,
                ["EmotionDim"] = _options.EmotionDim,
                ["NumEncoderLayers"] = _options.NumEncoderLayers,
                ["NumEmotionLayers"] = _options.NumEmotionLayers,
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(EmotiVoice<T>));
    }
