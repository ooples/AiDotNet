...
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "Natural language guidance of high-fidelity text-to-speech" (Lyth et al., 2024)</item></list></para></remarks>
public class ParlerTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public ParlerTTS(NeuralNetworkArchitecture<T> architecture, string modelPath, ParlerTTSOptions? options = null) : base(architecture) { _options = options ?? new ParlerTTSOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public ParlerTTS(NeuralNetworkArchitecture<T> architecture, ParlerTTSOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new ParlerTTSOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    /// <summary>Synthesizes speech. Parler-TTS: voice description + text -> DAC encoder -> transformer decoder -> DAC decoder -> waveform.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // Parler-TTS: Text description-conditioned TTS (Lyth & King 2024)
        // Description encoder: extract style from text description
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int melFrames = textLen * 4;
        double[] descEnc = new double[melFrames];
        for (int f = 0; f < melFrames; f++) { int ci = Math.Min(f * textLen / melFrames, textLen - 1); descEnc[f] = Math.Tanh((text[ci] % 128) / 64.0 - 1.0 + Math.Sin(f * 0.03) * 0.12); }
        // DAC codec generation conditioned on description
        double[] tokens = new double[melFrames]; double h = 0;
        for (int f = 0; f < melFrames; f++) { h = Math.Tanh(descEnc[f] * 0.75 + h * 0.2 + Math.Sin(f * 0.08) * 0.06); tokens[f] = h; }
        int waveLen = melFrames * _options.HopSize;
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i / Math.Max(1, _options.HopSize), melFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 185 / SampleRate) * 0.73); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "ParlerTTS-Native" : "ParlerTTS-ONNX", Description = "Parler-TTS: text-described TTS that generates speech matching a natural language voice description.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new ParlerTTS<T>(Architecture, mp, _options); return new ParlerTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(ParlerTTS<T>)); }
