...
public class SpiritLM<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SpiritLM-Native" : "SpiritLM-ONNX",
            Description = "SpiritLM TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "SpiritLM";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SpiritLM<T>));
    }
