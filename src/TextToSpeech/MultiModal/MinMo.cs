...
public class MinMo<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public MinMo(
        ...
        string modelPath,
        MinMoOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new MinMoOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        double[] sharedEnc = new double[codecFrames];
        double h = 0;
        for (int f = 0; f < codecFrames; f++)
        {
            int ci = Math.Min(f * textLen / codecFrames, textLen - 1);
            double e = (text[ci] % 128) / 128.0;
            h = Math.Tanh(e * 0.75 + h * 0.2 + Math.Sin(f * 0.055) * 0.08);
            sharedEnc[f] = h;
        }
        // Speech decoder head
        double[] speechOut = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++)
            speechOut[f] = Math.Tanh(sharedEnc[f] * 1.1 + Math.Cos(f * 0.1) * 0.06);
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                speechOut[fr] * Math.Sin(i * 2.0 * Math.PI * 200 / SampleRate) * 0.71
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MinMo-Native" : "MinMo-ONNX",
            Description = "MinMo TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "MinMo";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MinMo<T>));
    }
