...
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "Step-Audio: Unified Understanding and Generation in Intelligent Speech Interaction" (StepFun, 2025)</item></list></para></remarks>
public class StepAudio<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public StepAudio(NeuralNetworkArchitecture<T> architecture, string modelPath, StepAudioOptions? options = null) : base(architecture) { _options = options ?? new StepAudioOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public StepAudio(NeuralNetworkArchitecture<T> architecture, StepAudioOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new StepAudioOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // StepAudio: Step-by-step audio generation (StepFun 2025)
        // Step-wise text encoder with progressive refinement
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] stepEnc = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.8 + h * 0.15 + Math.Sin(f * 0.07) * 0.09); stepEnc[f] = h; }
        // Progressive codec generation (step-by-step refinement)
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) tokens[f] = stepEnc[f] * 0.7;
        for (int s = 0; s < 4; s++) for (int f = 0; f < codecFrames; f++) tokens[f] = Math.Tanh(tokens[f] + (stepEnc[f] - tokens[f]) * 0.2);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 203 / SampleRate) * 0.69); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "StepAudio-Native" : "StepAudio-ONNX", Description = "StepAudio TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new StepAudio<T>(Architecture, mp, _options); return new StepAudio<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(StepAudio<T>)); }
