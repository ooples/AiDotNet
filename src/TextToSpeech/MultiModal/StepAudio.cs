...
public class StepAudio<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "StepAudio-Native" : "StepAudio-ONNX", Description = "StepAudio TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "StepAudio"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = _options.LLMDim; m.AdditionalInfo["SampleRate"] = _options.SampleRate; m.AdditionalInfo["MelChannels"] = _options.MelChannels; m.AdditionalInfo["HopSize"] = _options.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new StepAudio<T>(Architecture, mp, _options); return new StepAudio<T>(Architecture, _options, _optimizer); }
