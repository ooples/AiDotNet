...
public class LlamaOmni<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "LlamaOmni-Native" : "LlamaOmni-ONNX",
            Description = "LlamaOmni TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "LlamaOmni";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new LlamaOmni<T>(Architecture, mp, _options);
        return new LlamaOmni<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(LlamaOmni<T>));
    }
