...
public class SpeechGPT<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "SpeechGPT-Native" : "SpeechGPT-ONNX", Description = "SpeechGPT TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "SpeechGPT"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SpeechGPT<T>(Architecture, mp, _options); return new SpeechGPT<T>(Architecture, _options, _optimizer); }
