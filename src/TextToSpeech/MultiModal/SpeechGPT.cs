...
public class SpeechGPT<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SpeechGPT-Native" : "SpeechGPT-ONNX",
            Description = "SpeechGPT TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "SpeechGPT";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SpeechGPT<T>));
    }
