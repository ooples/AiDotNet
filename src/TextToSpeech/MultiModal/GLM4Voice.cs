...
public class GLM4Voice<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public Tensor<T> SynthesizeNextChunk()
    {
        ...
        {
            var trimmed = new Tensor<T>([chunkSamples]);
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "GLM4Voice-Native" : "GLM4Voice-ONNX",
            Description = "GLM4Voice TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "GLM4Voice";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(GLM4Voice<T>));
    }
