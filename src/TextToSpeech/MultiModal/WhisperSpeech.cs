...
public class WhisperSpeech<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "WhisperSpeech-Native" : "WhisperSpeech-ONNX", Description = "WhisperSpeech: text-to-speech using Whisper encoder features as semantic tokens plus acoustic generation.", FeatureCount = _options.LLMDim }; }
