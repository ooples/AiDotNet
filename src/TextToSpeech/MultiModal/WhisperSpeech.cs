...
public class WhisperSpeech<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public WhisperSpeech(NeuralNetworkArchitecture<T> architecture, string modelPath, WhisperSpeechOptions? options = null) : base(architecture) { _options = options ?? new WhisperSpeechOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public WhisperSpeech(NeuralNetworkArchitecture<T> architecture, WhisperSpeechOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new WhisperSpeechOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // WhisperSpeech: Whisper-based pipeline (Collabora 2024)
        // T2S: Text-to-semantic using Whisper encoder features
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int semFrames = textLen * 2;
        double[] semantic = new double[semFrames]; double h = 0;
        for (int f = 0; f < semFrames; f++) { int ci = Math.Min(f / 2, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.85 + h * 0.1 + Math.Sin(f * 0.095) * 0.1); semantic[f] = h; }
        // S2A: Semantic-to-acoustic using EnCodec tokens
        int codecFrames = semFrames;
        double[] acoustic = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) acoustic[f] = Math.Tanh(semantic[f] * 1.1 + Math.Cos(f * 0.12) * 0.1);
        // Vocos decoder
        int waveLen = codecFrames * _options.HopSize;
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i / Math.Max(1, _options.HopSize), codecFrames - 1); waveform[i] = NumOps.FromDouble(acoustic[fr] * Math.Sin(i * 2.0 * Math.PI * 195 / SampleRate) * 0.75); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "WhisperSpeech-Native" : "WhisperSpeech-ONNX", Description = "WhisperSpeech: text-to-speech using Whisper encoder features as semantic tokens plus acoustic generation.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new WhisperSpeech<T>(Architecture, mp, _options); return new WhisperSpeech<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(WhisperSpeech<T>)); }
