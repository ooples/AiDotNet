...
[ResearchPaper("WhisperSpeech: Text-to-Speech via Whisper", "https://github.com/collabora/WhisperSpeech")]
public class WhisperSpeech<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "WhisperSpeech-Native" : "WhisperSpeech-ONNX", Description = "WhisperSpeech: text-to-speech using Whisper encoder features as semantic tokens plus acoustic generation.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "WhisperSpeech"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = _options.LLMDim; m.AdditionalInfo["SampleRate"] = _options.SampleRate; m.AdditionalInfo["MelChannels"] = _options.MelChannels; m.AdditionalInfo["HopSize"] = _options.HopSize; return m; }
