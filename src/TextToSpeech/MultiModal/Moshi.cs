...
public class Moshi<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            for (int cb = 0; cb < 8; cb++)
                mimi[f] += Math.Sin(f * 0.12 * (cb + 1)) * 0.04 / (cb + 1);
            mimi[f] = Math.Tanh(mimi[f]);
        }
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                mimi[fr] * Math.Sin(i * 2.0 * Math.PI * 187 / SampleRate) * 0.74
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Moshi-Native" : "Moshi-ONNX",
            Description = "Moshi TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "Moshi";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Moshi<T>(Architecture, mp, _options);
        return new Moshi<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Moshi<T>));
    }
