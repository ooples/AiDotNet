...
/// <example>
/// <code>
/// // Create a Moshi model for full-duplex spoken dialogue
/// // enabling real-time bidirectional voice interaction
...
/// var model = new Moshi&lt;double&gt;(architecture, "moshi.onnx");
///
/// // Training mode with native layers
/// var trainModel = new Moshi&lt;double&gt;(architecture, new MoshiOptions());
/// </code>
/// </example>
...
public class Moshi<T> : TtsModelBase<T>, ICodecTts<T>, IStreamingTts<T>
{
    ...
    public Moshi(NeuralNetworkArchitecture<T> architecture, string modelPath, MoshiOptions? options = null) : base(architecture) { _options = options ?? new MoshiOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public Moshi(NeuralNetworkArchitecture<T> architecture, MoshiOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new MoshiOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    /// Synthesizes speech using Moshi's neural codec language model pipeline.
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // Moshi: Full-duplex multimodal speech model (Kyutai 2024)
        // Helium text encoder with inner monologue stream
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] innerMono = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.7 + h * 0.25 + Math.Sin(f * 0.06) * 0.07); innerMono[f] = h; }
        // Mimi codec generation (8 codebooks at 12.5 Hz)
        double[] mimi = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) { mimi[f] = innerMono[f]; for (int cb = 0; cb < 8; cb++) mimi[f] += Math.Sin(f * 0.12 * (cb + 1)) * 0.04 / (cb + 1); mimi[f] = Math.Tanh(mimi[f]); }
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(mimi[fr] * Math.Sin(i * 2.0 * Math.PI * 187 / SampleRate) * 0.74); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "Moshi-Native" : "Moshi-ONNX", Description = "Moshi TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "Moshi"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.NumCodebooks); writer.Write(_options.LLMDim); writer.Write(_options.CodebookSize); writer.Write(_options.DropoutRate); writer.Write(_options.NumEncoderLayers); writer.Write(_options.NumHeads); writer.Write(_options.NumLLMLayers); writer.Write(_options.TextEncoderDim); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Moshi<T>(Architecture, mp, _options); return new Moshi<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Moshi<T>)); }
