...
public class VoiceFlow<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description = "VoiceFlow: Rectified Flow Matching TTS (Guo et al., 2024)",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new VoiceFlow<T>(Architecture, mp, _options);
        return new VoiceFlow<T>(Architecture, _options);
    }
