...
public class E3TTS<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        int totalFrames = textLen * 4;
        // Direct diffusion without duration predictor
        double[] mel = new double[totalFrames];
        for (int f = 0; f < totalFrames; f++) mel[f] = Math.Sin(f * 0.13) * 0.7;
        for (int step = _options.NumDiffusionSteps - 1; step >= 0; step--)
        {
            ...
            for (int f = 0; f < totalFrames; f++)
            {
                ...
                mel[f] = textHidden[srcT] * (1 - noiseScale) + mel[f] * noiseScale;
            }
        }
