...
public class F5TTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "F5TTS-Native" : "F5TTS-ONNX", Description = "F5-TTS: non-autoregressive flow-matching TTS that generates speech from text using DiT backbone.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "F5TTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
