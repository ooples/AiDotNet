...
[ResearchPaper(
    "CoMoSpeech: One-Step Speech Synthesis via Consistency Model",
...
public class CoMoSpeech<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        int mF = textLen * 4;
        double[] enc = new double[mF];
        for (int f = 0; f < mF; f++)
        {
            int t = Math.Min(f * textLen / mF, textLen - 1);
            enc[f] = (text[t] % 128) / 128.0;
        }
        double[] noise = new double[mF];
        for (int f = 0; f < mF; f++)
            noise[f] = Math.Sin(f * 0.3) * 0.5 + Math.Cos(f * 0.17) * 0.3;
        double[] mel = new double[mF];
        for (int f = 0; f < mF; f++)
            mel[f] = Math.Tanh(enc[f] * 0.7 + noise[f] * 0.15 + Math.Sin(f * 0.05) * 0.1);
