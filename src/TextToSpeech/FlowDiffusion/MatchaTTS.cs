...
public class MatchaTTS<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "Matcha-TTS-Native" : "Matcha-TTS-ONNX", Description = "Matcha-TTS: OT-CFM Fast TTS (Mehta et al., 2024)", FeatureCount = _options.HiddenDim }; m.AdditionalInfo["Architecture"] = "MatchaTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); writer.Write(_options.HiddenDim); writer.Write(_options.NumFlowSteps); writer.Write(_options.DropoutRate); writer.Write(_options.FlowDim); writer.Write(_options.NumEncoderLayers); writer.Write(_options.NumHeads); }
