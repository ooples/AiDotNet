...
public class MatchaTTS<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < totalFrames; f++)
            melFrames[f] = Math.Sin(f * 0.1) * 0.5;
        // ODE steps
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                Math.Tanh(melFrames[frame] * Math.Sin(i * 0.01 + melFrames[frame]) * 0.8)
            );
        }
        return waveform;
    }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MatchaTTS<T>(Architecture, mp, _options);
        return new MatchaTTS<T>(Architecture, _options);
    }
