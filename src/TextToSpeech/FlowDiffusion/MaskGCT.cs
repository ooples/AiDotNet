...
public class MaskGCT<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MaskGCT-Native" : "MaskGCT-ONNX",
            Description =
                "MaskGCT: non-autoregressive masked generative codec transformer for zero-shot TTS.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "MaskGCT";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MaskGCT<T>(Architecture, mp, _options);
        return new MaskGCT<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MaskGCT<T>));
    }
