...
{
    private readonly DiTToTTSOptions _options;
    ...
    public DiTToTTS(
        ...
        string modelPath,
        DiTToTTSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new DiTToTTSOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        int tF = 0;
        int[] dur = new int[textLen];
        for (int i = 0; i < textLen; i++)
        {
            dur[i] = Math.Max(2, (int)(3.0 + Math.Sin(i * 0.3) * 1.5));
            tF += dur[i];
        }
        tF = Math.Min(tF, 500);
        double[] tc = new double[tF];
        int fI = 0;
        for (int i = 0; i < textLen && fI < tF; i++)
        {
            double cv = (text[i] % 128) / 128.0;
            for (int d = 0; d < dur[i] && fI < tF; d++)
                tc[fI++] = cv;
        }
        double[] lat = new double[tF];
        for (int f = 0; f < tF; f++)
            lat[f] = Math.Sin(f * 0.25) * 0.6;
        for (int st = 0; st < 10; st++)
        {
            double t = 1.0 - (st + 1.0) / 10;
            for (int f = 0; f < tF; f++)
                lat[f] = Math.Tanh(
                    tc[f] * (1.0 - t) + lat[f] * t + Math.Sin(f * 0.1 + st * 0.5) * t * 0.2
                );
        }
        int waveLen = tF * _options.HopSize;
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++)
        {
            int fr = Math.Min(i / Math.Max(1, _options.HopSize), tF - 1);
            waveform[i] = NumOps.FromDouble(lat[fr] * Math.Sin(i * 0.006) * 0.85);
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "DiTToTTS-Native" : "DiTToTTS-ONNX",
            Description = "DiTToTTS TTS",
            FeatureCount = _options.HiddenDim,
        };
        m.AdditionalInfo["Architecture"] = "DiTToTTS";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new DiTToTTS<T>(Architecture, mp, _options);
        return new DiTToTTS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(DiTToTTS<T>));
    }
