...
public class KaniTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public KaniTTS(NeuralNetworkArchitecture<T> architecture, string modelPath, KaniTTSOptions? options = null) : base(architecture) { _options = options ?? new KaniTTSOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public KaniTTS(NeuralNetworkArchitecture<T> architecture, KaniTTSOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new KaniTTSOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        double[] tok = new double[cF]; double p = 0;
        for (int f = 0; f < cF; f++) { int t = Math.Min(f * textLen / cF, textLen - 1); tok[f] = Math.Tanh((text[t] % 128) / 128.0 * 0.7 + p * 0.2 + Math.Sin(f * 0.1) * 0.08); p = tok[f]; }
        int waveLen = cF * (SampleRate / _options.CodecFrameRate); var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, cF - 1); waveform[i] = NumOps.FromDouble(Math.Tanh(tok[fr] * Math.Sin(i * 0.007) * 0.85)); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "KaniTTS-Native" : "KaniTTS-ONNX", Description = "KaniTTS TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "KaniTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new KaniTTS<T>(Architecture, mp, _options); return new KaniTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(KaniTTS<T>)); }
