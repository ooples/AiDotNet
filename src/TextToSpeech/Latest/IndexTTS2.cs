...
[ResearchPaper("IndexTTS 2: Improved Zero-Shot Text-to-Speech", "https://github.com/indexteam/IndexTTS")]
public class IndexTTS2<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly IndexTTS2Options _options; public override ModelOptions GetOptions() => _options;
    ...
    public IndexTTS2(NeuralNetworkArchitecture<T> architecture, string modelPath, IndexTTS2Options? options = null) : base(architecture) { _options = options ?? new IndexTTS2Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public IndexTTS2(NeuralNetworkArchitecture<T> architecture, IndexTTS2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new IndexTTS2Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "IndexTTS2-Native" : "IndexTTS2-ONNX", Description = "IndexTTS2 TTS", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "IndexTTS2"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
