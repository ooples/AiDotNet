...
[ResearchPaper(
    "IndexTTS 2: Improved Zero-Shot Text-to-Speech",
...
public class IndexTTS2<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public IndexTTS2(
        ...
        string modelPath,
        IndexTTS2Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new IndexTTS2Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "IndexTTS2-Native" : "IndexTTS2-ONNX",
            Description = "IndexTTS2 TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "IndexTTS2";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new IndexTTS2<T>(Architecture, mp, _options);
        return new IndexTTS2<T>(Architecture, _options, _optimizer);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(IndexTTS2<T>));
    }
