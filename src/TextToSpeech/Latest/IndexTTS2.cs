...
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "IndexTTS-2: Duration and Emotion Control for Zero-Shot TTS" (Bilibili Team, 2026)</item></list></para></remarks>
public class IndexTTS2<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < cF; f++) { int t = Math.Min(f * textLen / cF, textLen - 1); tok[f] = Math.Tanh((text[t] % 128) / 128.0 * 0.55 + p * 0.28 + 0.05 + Math.Sin(f * 0.09) * 0.08); p = tok[f]; }
        int waveLen = cF * (SampleRate / _options.CodecFrameRate); var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, cF - 1); waveform[i] = NumOps.FromDouble(Math.Tanh(tok[fr] * Math.Sin(i * 0.006 + tok[fr]) * 0.85)); }
        return waveform;
    }
