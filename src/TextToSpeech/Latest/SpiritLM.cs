...
public class SpiritLM<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    /// Synthesizes speech using SpiritLM's neural codec language model pipeline.
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // Spirit-LM: Interleaved text-speech language model (Meta 2024)
        // Interleaved token generation: text and speech tokens in one sequence
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] interleaved = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; bool isSpeech = f % 3 != 0; h = Math.Tanh(e * (isSpeech ? 0.6 : 0.9) + h * (isSpeech ? 0.3 : 0.05) + Math.Sin(f * 0.065) * 0.08); interleaved[f] = h; }
        // Extract speech tokens and decode
        double[] speechOnly = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) speechOnly[f] = interleaved[f];
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(speechOnly[fr] * Math.Sin(i * 2.0 * Math.PI * 189 / SampleRate) * 0.7); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "SpiritLM-Native" : "SpiritLM-ONNX", Description = "SpiritLM TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SpiritLM<T>(Architecture, mp, _options); return new SpiritLM<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SpiritLM<T>)); }
