...
public class KaniTTS2<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public KaniTTS2(
        ...
        string modelPath,
        KaniTTS2Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new KaniTTS2Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "KaniTTS2-Native" : "KaniTTS2-ONNX",
            Description = "KaniTTS2 TTS",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "KaniTTS2";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new KaniTTS2<T>(Architecture, mp, _options);
        return new KaniTTS2<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(KaniTTS2<T>));
    }
