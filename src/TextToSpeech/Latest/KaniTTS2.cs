...
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "Kani-TTS-2: Improved Lightweight Codec TTS" (-, 2026)</item></list></para></remarks>
public class KaniTTS2<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly KaniTTS2Options _options; public override ModelOptions GetOptions() => _options;
    ...
    public KaniTTS2(NeuralNetworkArchitecture<T> architecture, string modelPath, KaniTTS2Options? options = null) : base(architecture) { _options = options ?? new KaniTTS2Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public KaniTTS2(NeuralNetworkArchitecture<T> architecture, KaniTTS2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new KaniTTS2Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        double[] enc = new double[cF]; for (int f = 0; f < cF; f++) { int t = Math.Min(f * textLen / cF, textLen - 1); enc[f] = (text[t] % 128) / 128.0; }
        double[,] codes = new double[2, cF]; double p = 0;
        for (int f = 0; f < cF; f++) { codes[0, f] = Math.Tanh(enc[f] * 0.65 + p * 0.25 + Math.Sin(f * 0.09) * 0.08); p = codes[0, f]; }
        for (int f = 0; f < cF; f++) codes[1, f] = codes[0, f] * 0.8 + Math.Sin(f * 0.15) * 0.12;
        int waveLen = cF * (SampleRate / _options.CodecFrameRate); var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, cF - 1); waveform[i] = NumOps.FromDouble(Math.Tanh((codes[0, fr] + codes[1, fr]) * 0.5 * Math.Sin(i * 0.006) * 0.88)); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "KaniTTS2-Native" : "KaniTTS2-ONNX", Description = "KaniTTS2 TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new KaniTTS2<T>(Architecture, mp, _options); return new KaniTTS2<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(KaniTTS2<T>)); }
