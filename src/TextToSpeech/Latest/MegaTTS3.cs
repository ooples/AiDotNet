...
public class MegaTTS3<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MegaTTS3-Native" : "MegaTTS3-ONNX",
            Description =
                "MegaTTS 3: sparse diffusion transformer TTS using DiT backbone for efficient generation.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "MegaTTS3";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MegaTTS3<T>));
    }
