...
/// <summary>MARS5: two-stage AR+diffusion model for zero-shot speech synthesis with coarse-to-fine generation.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks><para><b>References:</b><list type="bullet"><item>Paper: "MARS5-TTS: A Two-Stage Approach to Zero-Shot Speech Synthesis" (CAMB.AI, 2024)</item></list></para></remarks>
public class MARS5<T> : TtsModelBase<T>, ICodecTts<T>
{
    private readonly MARS5Options _options; public override ModelOptions GetOptions() => _options;
    ...
    public MARS5(NeuralNetworkArchitecture<T> architecture, string modelPath, MARS5Options? options = null) : base(architecture) { _options = options ?? new MARS5Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public MARS5(NeuralNetworkArchitecture<T> architecture, MARS5Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new MARS5Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.LLMDim; InitializeLayers(); }
    ...
    /// Synthesizes speech using MARS5's neural codec language model pipeline.
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // MARS5: Two-stage multi-speaker AR model (Camb.ai 2024)
        // Deep AR text processing with speaker prompt
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] arOut = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.85 + h * 0.1 + Math.Sin(f * 0.075) * 0.1); arOut[f] = h; }
        // NAR refinement pass
        double[] refined = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) refined[f] = Math.Tanh(arOut[f] * 1.05 + Math.Sin(f * 0.18) * 0.08);
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(refined[fr] * Math.Sin(i * 2.0 * Math.PI * 192 / SampleRate) * 0.73); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "MARS5-Native" : "MARS5-ONNX", Description = "MARS5 TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new MARS5<T>(Architecture, mp, _options); return new MARS5<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(MARS5<T>)); }
