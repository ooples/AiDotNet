...
public class OuteTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public OuteTTS(
        ...
        string modelPath,
        OuteTTSOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new OuteTTSOptions();
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int f = 0; f < codecFrames; f++)
        {
            ...
            h = Math.Tanh(e * 0.8 + h * 0.15 + Math.Sin(f * 0.065) * 0.1);
            textEnc[f] = h;
        }
        // LLM-guided token generation
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++)
            tokens[f] = Math.Round(textEnc[f] * 6) / 6.0;
        ...
        for (int i = 0; i < waveLen; i++)
        {
            ...
            waveform[i] = NumOps.FromDouble(
                tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 205 / SampleRate) * 0.7
            );
        }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "OuteTTS-Native" : "OuteTTS-ONNX",
            Description = "OuteTTS: text-to-speech using pure language modeling on audio tokens.",
            FeatureCount = _options.LLMDim,
        };
        m.AdditionalInfo["Architecture"] = "OuteTTS";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new OuteTTS<T>(Architecture, mp, _options);
        return new OuteTTS<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(OuteTTS<T>));
    }
