...
public class OuteTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    /// <summary>Synthesizes speech. OuteTTS: text + voice tokens -> LLM AR prediction -> WavTokenizer decoder -> waveform.</summary>
    public Tensor<T> Synthesize(string text)
    {
        ThrowIfDisposed(); var input = PreprocessText(text); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input);
        // OuteTTS: Efficient TTS with WavTokenizer (OuteAI 2024)
        // WavTokenizer encoding: single-codebook 75Hz tokens
        int textLen = Math.Min(text.Length, _options.MaxTextLength);
        int codecFrames = textLen * 3;
        double[] textEnc = new double[codecFrames]; double h = 0;
        for (int f = 0; f < codecFrames; f++) { int ci = Math.Min(f * textLen / codecFrames, textLen - 1); double e = (text[ci] % 128) / 128.0; h = Math.Tanh(e * 0.8 + h * 0.15 + Math.Sin(f * 0.065) * 0.1); textEnc[f] = h; }
        // LLM-guided token generation
        double[] tokens = new double[codecFrames];
        for (int f = 0; f < codecFrames; f++) tokens[f] = Math.Round(textEnc[f] * 6) / 6.0;
        int waveLen = codecFrames * (SampleRate / _options.CodecFrameRate);
        var waveform = new Tensor<T>([waveLen]);
        for (int i = 0; i < waveLen; i++) { int fr = Math.Min(i * _options.CodecFrameRate / SampleRate, codecFrames - 1); waveform[i] = NumOps.FromDouble(tokens[fr] * Math.Sin(i * 2.0 * Math.PI * 205 / SampleRate) * 0.7); }
        return waveform;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "OuteTTS-Native" : "OuteTTS-ONNX", Description = "OuteTTS: text-to-speech using pure language modeling on audio tokens.", FeatureCount = _options.LLMDim }; m.AdditionalInfo["Architecture"] = "OuteTTS"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = base.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is {} mp && !string.IsNullOrEmpty(mp)) return new OuteTTS<T>(Architecture, mp, _options); return new OuteTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(OuteTTS<T>)); }
