...
public class OuteTTS<T> : TtsModelBase<T>, ICodecTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "OuteTTS-Native" : "OuteTTS-ONNX", Description = "OuteTTS: text-to-speech using pure language modeling on audio tokens.", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.LLMDim }; }
