...
public class ParallelWaveGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    public ParallelWaveGAN(
        ...
        string modelPath,
        ParallelWaveGANOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new ParallelWaveGANOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "ParallelWaveGAN-Native" : "ParallelWaveGAN-ONNX",
            Description = "Parallel WaveGAN (Yamamoto et al., 2020)",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new ParallelWaveGAN<T>(Architecture, mp, _options);
        return new ParallelWaveGAN<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(ParallelWaveGAN<T>));
    }
