...
public class ParallelWaveGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    /// Per the paper (Yamamoto et al., 2020):
    /// (1) Input: white noise z concatenated with upsampled mel conditioning,
    /// (2) 30-layer non-causal dilated residual conv blocks (no autoregression),
    ...
    public Tensor<T> MelToWaveform(Tensor<T> melSpectrogram)
    {
        ...
        for (int b = 0; b < Math.Min(_options.NumResBlocks, 10); b++)
        {
            int dilation = 1 << (b % 10);
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) Layers.AddRange(Architecture.Layers); else Layers.AddRange(LayerHelper<T>.CreateDefaultVocoderLayers(_options.MelChannels, _options.ResChannels, 1, 4, _options.NumResBlocks, _options.DropoutRate)); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "ParallelWaveGAN-Native" : "ParallelWaveGAN-ONNX", Description = "Parallel WaveGAN (Yamamoto et al., 2020)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.MelChannels, Complexity = _options.NumResBlocks }; m.AdditionalInfo["Architecture"] = "ParallelWaveGAN"; return m; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); writer.Write(_options.NumResBlocks); }
    protected override void DeserializeNetworkSpecificData(BinaryReader reader) { _useNativeMode = reader.ReadBoolean(); string mp = reader.ReadString(); if (!string.IsNullOrEmpty(mp)) _options.ModelPath = mp; _options.SampleRate = reader.ReadInt32(); _options.MelChannels = reader.ReadInt32(); _options.HopSize = reader.ReadInt32(); _options.NumResBlocks = reader.ReadInt32(); if (!_useNativeMode && _options.ModelPath is { } p && !string.IsNullOrEmpty(p)) OnnxModel = new OnnxModel<T>(p, _options.OnnxOptions); }
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new ParallelWaveGAN<T>(Architecture, mp, _options); return new ParallelWaveGAN<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(ParallelWaveGAN<T>)); }
