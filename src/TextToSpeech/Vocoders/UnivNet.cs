...
public class UnivNet<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) { Layers.AddRange(Architecture.Layers); return; } var d = new UnivNetOptions(); if (_options.NumLMBlocks != d.NumLMBlocks || _options.DropoutRate > double.Epsilon) throw new InvalidOperationException("UnivNetOptions.NumLMBlocks/DropoutRate are configured but not applied by the paper-faithful HiFi-GAN generator default; supply explicit Architecture.Layers for a custom LVCNet configuration."); Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, 512, 1)); }
    ...
    public override void UpdateParameters(Vector<T> parameters) { ThrowIfDisposed(); if (!_useNativeMode) throw new NotSupportedException("Cannot update parameters in ONNX mode."); int idx = 0; foreach (var l in Layers) { int c = (int)l.ParameterCount; l.UpdateParameters(parameters.Slice(idx, c)); idx += c; } }
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "UnivNet-Native" : "UnivNet-ONNX", Description = "UnivNet: Universal Neural Vocoder (Jang et al., 2021)", FeatureCount = _options.MelChannels, AdditionalInfo = new Dictionary<string, object> { ["MelChannels"] = _options.MelChannels, ["Mode"] = _useNativeMode ? "Native" : "ONNX" } }; }
