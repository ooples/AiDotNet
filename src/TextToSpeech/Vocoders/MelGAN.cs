...
/// <example>
/// <code>
/// // Create a MelGAN vocoder for lightweight non-autoregressive synthesis
/// // with feature matching loss for fast real-time inference
...
/// var model = new MelGAN&lt;double&gt;(architecture, "melgan.onnx");
///
/// // Training mode with native layers
/// var trainModel = new MelGAN&lt;double&gt;(architecture, new MelGANOptions());
/// </code>
/// </example>
...
public class MelGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers()
    {
        ...
        var d = new MelGANOptions();
        if (_options.NumResStacks != d.NumResStacks || _options.DropoutRate > double.Epsilon)
            throw new InvalidOperationException(
                "MelGANOptions.NumResStacks/DropoutRate are configured but not applied by the paper-faithful HiFi-GAN generator default; supply explicit Architecture.Layers for a custom residual-stack configuration."
            );
        Layers.AddRange(
            LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, _options.NgfBase, 1)
        );
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MelGAN-Native" : "MelGAN-ONNX",
            Description =
            ...
            Complexity = _options.NumResStacks * 4,
        };
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.NgfBase);
        writer.Write(_options.NumResStacks);
    }
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.NgfBase = reader.ReadInt32();
        _options.NumResStacks = reader.ReadInt32();
        base.SampleRate = _options.SampleRate;
        base.MelChannels = _options.MelChannels;
        base.HopSize = _options.HopSize;
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MelGAN<T>(Architecture, mp, _options);
        return new MelGAN<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MelGAN<T>));
    }
