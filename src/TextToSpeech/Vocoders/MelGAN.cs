...
public class MelGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) { Layers.AddRange(Architecture.Layers); return; } var d = new MelGANOptions(); if (_options.NumResStacks != d.NumResStacks || _options.DropoutRate > double.Epsilon) throw new InvalidOperationException("MelGANOptions.NumResStacks/DropoutRate are configured but not applied by the paper-faithful HiFi-GAN generator default; supply explicit Architecture.Layers for a custom residual-stack configuration."); Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, _options.NgfBase, 1)); }
