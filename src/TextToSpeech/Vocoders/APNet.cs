...
public class APNet<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) { Layers.AddRange(Architecture.Layers); return; } if (_options.DropoutRate > double.Epsilon) throw new InvalidOperationException("APNetOptions.DropoutRate is configured but the paper-faithful HiFi-GAN generator (Kong 2020) applies no dropout; leave DropoutRate at 0 for native mode or supply explicit Architecture.Layers."); Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, 512, _options.FftSize / 2 + 1)); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "APNet-Native" : "APNet-ONNX", Description = "APNet: Amplitude-Phase Network Vocoder (Ai et al., 2023)", FeatureCount = _options.MelChannels, AdditionalInfo = new Dictionary<string, object> { ["MelChannels"] = _options.MelChannels, ["Mode"] = _useNativeMode ? "Native" : "ONNX" } }; }
