...
[ResearchPaper("APNet 2: High-Quality and High-Efficiency Neural Vocoder with Direct Prediction of Amplitude and Phase Spectra", "https://arxiv.org/abs/2311.11545", Year = 2023, Authors = "Du et al.")]
public class APNet2<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) { Layers.AddRange(Architecture.Layers); return; } if (_options.DropoutRate > double.Epsilon) throw new InvalidOperationException("APNet2Options.DropoutRate is configured but the paper-faithful HiFi-GAN generator (Kong 2020) applies no dropout; leave DropoutRate at 0 for native mode or supply explicit Architecture.Layers."); Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, 512, _options.FftSize / 2 + 1)); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "APNet2-Native" : "APNet2-ONNX", Description = "APNet 2: Improved Amplitude-Phase Network (Du et al., 2023)", FeatureCount = _options.MelChannels, AdditionalInfo = new Dictionary<string, object> { ["MelChannels"] = _options.MelChannels, ["Mode"] = _useNativeMode ? "Native" : "ONNX" } }; }
