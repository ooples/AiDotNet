...
public class APNet2<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) Layers.AddRange(Architecture.Layers); else Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, 512, _options.FftSize / 2 + 1, 4, 3, _options.DropoutRate)); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "APNet2-Native" : "APNet2-ONNX", Description = "APNet 2: Improved Amplitude-Phase Network (Du et al., 2023)", FeatureCount = _options.MelChannels }; }
