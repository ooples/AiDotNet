...
public class HiFiGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    public int UpsampleFactor
    {
        ...
        {
            int f = 1;
            foreach (var r in _options.UpsampleRates)
                f *= r;
            return f;
        }
    }
    ...
    protected override void InitializeLayers()
    {
        ...
        if (_options.DropoutRate > double.Epsilon)
            throw new InvalidOperationException(
                "HiFiGANOptions.DropoutRate is configured but the paper-faithful HiFi-GAN generator (Kong 2020) applies no dropout; leave DropoutRate at 0 for native mode or supply explicit Architecture.Layers."
            );
        Layers.AddRange(
            LayerHelper<T>.CreateDefaultHiFiGANLayers(
                _options.MelChannels,
                _options.UpsampleInitialChannels,
                1,
                _options.UpsampleRates,
                _options.ResblockKernelSizes,
                null
            )
        );
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "HiFiGAN-Native" : "HiFiGAN-ONNX",
            Description =
            ...
            Complexity = _options.UpsampleRates.Length + _options.ResblockKernelSizes.Length,
        };
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.UpsampleRates.Length);
        foreach (var r in _options.UpsampleRates)
            writer.Write(r);
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.UpsampleRates = new int[n];
        for (int i = 0; i < n; i++)
            _options.UpsampleRates[i] = reader.ReadInt32();
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new HiFiGAN<T>(Architecture, mp, _options);
        return new HiFiGAN<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(HiFiGAN<T>));
    }
