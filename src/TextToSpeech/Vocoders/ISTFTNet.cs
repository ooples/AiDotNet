...
public class ISTFTNet<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) Layers.AddRange(Architecture.Layers); else Layers.AddRange(LayerHelper<T>.CreateDefaultHiFiGANLayers(_options.MelChannels, 512, _options.StftWindow / 2 + 1)); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "iSTFTNet-Native" : "iSTFTNet-ONNX", Description = "iSTFTNet: Fast Mel-Spectrogram Vocoder with Inverse STFT (Kaneko et al., 2022)", FeatureCount = _options.MelChannels }; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); writer.Write(_options.DropoutRate); writer.Write(_options.NumUpsampleLayers); writer.Write(_options.StftWindow); }
