...
public class BigVGAN<T> : TtsModelBase<T>, IVocoder<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "BigVGAN-Native" : "BigVGAN-ONNX", Description = "BigVGAN: Universal Neural Vocoder with AMP + Snake (Lee et al., 2023)", FeatureCount = _options.MelChannels }; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); writer.Write(_options.DropoutRate); writer.Write(_options.HiddenChannels); writer.Write(_options.NumUpsampleLayers); }
