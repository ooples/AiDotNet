...
public class Tacotron2<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HopSize); writer.Write(_options.HiddenDim); writer.Write(_options.EncoderDim); writer.Write(_options.DecoderDim); writer.Write(_options.NumEncoderLayers); writer.Write(_options.NumDecoderLayers); writer.Write(_options.PrenetDim); writer.Write(_options.AttentionRnnDim); writer.Write(_options.MaxTextLength); writer.Write(_options.NumHeads); writer.Write(_options.DropoutRate); writer.Write(_options.VocabSize); }
