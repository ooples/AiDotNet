...
/// <summary>
/// SpeedySpeech: lightweight non-autoregressive TTS with convolutional residual blocks and teacher-student duration distillation.
/// </summary>
...
public class SpeedySpeech<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    /// <summary>
    /// Synthesizes mel-spectrogram using SpeedySpeech's lightweight convolutional pipeline.
    /// Per the paper (Vainer and Durnov, 2020):
    /// (1) Convolutional residual encoder with depthwise separable convolutions,
    /// (2) Teacher-student duration predictor (distilled from Tacotron 2 attention alignments),
    /// (3) Length regulation expands to mel-frame resolution,
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        // Duration prediction + length regulation (same as FastSpeech but simpler predictor)
        ...
        for (int i = 0; i < seqLen && frameIdx < expandedLen; i++)
            ...
            { expanded[frameIdx] = encoded[i % encoded.Length]; frameIdx++; }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "SpeedySpeech-Native" : "SpeedySpeech-ONNX", Description = "SpeedySpeech: Efficient Neural Speech Synthesis (Vainer and Durnov, 2020)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.HiddenDim, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers }; m.AdditionalInfo["Architecture"] = "SpeedySpeech"; return m; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.SampleRate); writer.Write(_options.MelChannels); writer.Write(_options.HiddenDim); writer.Write(_options.NumEncoderLayers); writer.Write(_options.NumDecoderLayers); writer.Write(_options.HopSize); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SpeedySpeech<T>(Architecture, mp, _options); return new SpeedySpeech<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SpeedySpeech<T>)); }
