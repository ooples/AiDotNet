...
public class SpeedySpeech<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "SpeedySpeech: Efficient Neural Speech Synthesis (Vainer and Durnov, 2020)",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new SpeedySpeech<T>(Architecture, mp, _options);
        return new SpeedySpeech<T>(Architecture, _options);
    }
