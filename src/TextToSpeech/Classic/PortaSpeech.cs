...
public class PortaSpeech<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int i = 0; i < seqLen; i++)
        {
            ...
            double prosody = Math.Sin(i * 0.5) * _options.ProsodyDim * 0.01;
            int dur = Math.Max(
                1,
                (int)Math.Round(1.0 + (val + Math.Abs(prosody)) * _options.DurationScale)
            );
            durations[i] = Math.Min(dur, _options.MaxDuration);
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "PortaSpeech: Portable and High-Quality Generative TTS (Ren et al., 2021)",
            FeatureCount = _options.HiddenDim,
            Complexity =
                _options.NumEncoderLayers + _options.NumDecoderLayers + _options.NumFlowLayers,
        };
