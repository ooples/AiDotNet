...
public class Tacotron<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Tacotron{T}"/> class in ONNX inference mode.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Tacotron{T}"/> class in native training/inference mode.
    /// </summary>
    /// <param name="architecture">The neural network architecture configuration.</param>
    /// <param name="options">Optional model configuration options.</param>
    ...
    /// <remarks>
    /// <para>Per the paper (Wang et al., 2017), the pipeline is:</para>
    /// <para>(1) Character embedding followed by CBHG encoder produces encoder hidden states.</para>
    /// <para>(2) Attention-based decoder autoregressively generates mel frames (r frames/step).</para>
    /// <para>(3) CBHG post-processing network refines mel to linear spectrogram.</para>
    /// <para><b>For Beginners:</b> This method takes text input, encodes the characters, then generates
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int step = 0; step < maxFrames / r && frameIdx < maxFrames; step++)
        {
            ...
            for (int e = 0; e < encLen; e++)
            {
                attn[e] = Math.Exp(energies[e] - maxEnergy);
                sumExp += attn[e];
            }
            ...
            {
                attn[e] /= sumExp;
                attnWeights[e] += attn[e];
                context += attn[e] * NumOps.ToDouble(encoded[e]);
            }
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.CbhgBankSize);
        writer.Write(_options.OutputsPerStep);
    }
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.CbhgBankSize = reader.ReadInt32();
        _options.OutputsPerStep = reader.ReadInt32();
        base.SampleRate = _options.SampleRate;
        base.MelChannels = _options.MelChannels;
        base.HopSize = _options.HopSize;
        base.HiddenDim = _options.HiddenDim;
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Tacotron<T>(Architecture, mp, _options);
        return new Tacotron<T>(Architecture, _options);
    }
