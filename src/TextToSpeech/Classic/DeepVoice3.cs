...
public class DeepVoice3<T> : TtsModelBase<T>, IAcousticModel<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        for (int step = 0; step < maxFrames / r && frameIdx < maxFrames; step++)
        {
            ...
            for (
                int e = Math.Max(0, attnPosInt - window);
                e < Math.Min(encoded.Length, attnPosInt + window + 1);
                e++
            )
            {
            ...
            attnPos = Math.Min(
                attnPos + 1.0 / (maxFrames / encoded.Length + 1),
                encoded.Length - 1
            );
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "Deep Voice 3: Scaling TTS with Convolutional Sequence Learning (Ping et al., 2018)",
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.NumSpeakers);
    }
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.NumSpeakers = reader.ReadInt32();
        base.SampleRate = _options.SampleRate;
        base.MelChannels = _options.MelChannels;
        base.HopSize = _options.HopSize;
        base.HiddenDim = _options.HiddenDim;
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new DeepVoice3<T>(Architecture, mp, new DeepVoice3Options(_options));
        ...
        var cloneOptimizer = _optimizer?.GetOptions() is AdamWOptimizerOptions<T, Tensor<T>, Tensor<T>> options
            ...
            : null;
        return new DeepVoice3<T>(Architecture, new DeepVoice3Options(_options), cloneOptimizer);
    }
