...
public class PlayHT<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "PlayHT-Native" : "PlayHT-ONNX", Description = "PlayHT 2.0 Turbo TTS", FeatureCount = _options.HiddenDim }; m.AdditionalInfo["Architecture"] = "PlayHT"; m.AdditionalInfo["Mode"] = _useNativeMode ? "Native" : "ONNX"; m.AdditionalInfo["HiddenDim"] = _options.HiddenDim; m.AdditionalInfo["SampleRate"] = base.SampleRate; m.AdditionalInfo["MelChannels"] = base.MelChannels; m.AdditionalInfo["HopSize"] = base.HopSize; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (IsOnnxMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new PlayHT<T>(Architecture, mp, _options); return new PlayHT<T>(Architecture, _options, _optimizer); }
