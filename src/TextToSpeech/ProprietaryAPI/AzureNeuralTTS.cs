...
public class AzureNeuralTTS<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    /// <summary>Synthesizes speech using Azure Neural TTS's API-compatible local inference pipeline.</summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "AzureNeuralTTS-Native" : "AzureNeuralTTS-ONNX", Description = "Microsoft Azure Cognitive Services Neural TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.HiddenDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new AzureNeuralTTS<T>(Architecture, mp, _options); return new AzureNeuralTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(AzureNeuralTTS<T>)); }
