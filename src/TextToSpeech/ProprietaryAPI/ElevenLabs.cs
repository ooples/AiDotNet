...
public class ElevenLabs<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    private readonly ElevenLabsOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    public ElevenLabs(NeuralNetworkArchitecture<T> architecture, string modelPath, ElevenLabsOptions? options = null) : base(architecture) { _options = options ?? new ElevenLabsOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public ElevenLabs(NeuralNetworkArchitecture<T> architecture, ElevenLabsOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new ElevenLabsOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; InitializeLayers(); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "ElevenLabs-Native" : "ElevenLabs-ONNX", Description = "ElevenLabs Neural Voice Synthesis", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.HiddenDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new ElevenLabs<T>(Architecture, mp, _options); return new ElevenLabs<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(ElevenLabs<T>)); }
