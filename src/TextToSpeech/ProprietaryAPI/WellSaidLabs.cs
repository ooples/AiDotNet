...
public class WellSaidLabs<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new WellSaidLabs<T>(Architecture, mp, _options); return new WellSaidLabs<T>(Architecture, _options); }
