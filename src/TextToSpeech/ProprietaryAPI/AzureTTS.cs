...
/// <summary>Azure Cognitive Services TTS: Microsoft's cloud-based neural TTS with 400+ voices in 140+ languages.</summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
public class AzureTTS<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    private readonly AzureTTSOptions _options; public override ModelOptions GetOptions() => _options;
    ...
    public AzureTTS(NeuralNetworkArchitecture<T> architecture, string modelPath, AzureTTSOptions? options = null) : base(architecture) { _options = options ?? new AzureTTSOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public AzureTTS(NeuralNetworkArchitecture<T> architecture, AzureTTSOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new AzureTTSOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.MelChannels = _options.MelChannels; base.HopSize = _options.HopSize; base.HiddenDim = _options.HiddenDim; InitializeLayers(); }
    ...
    /// <summary>Synthesizes speech using AzureTTS's API-compatible local inference pipeline.</summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() { return new ModelMetadata<T> { Name = _useNativeMode ? "AzureTTS-Native" : "AzureTTS-ONNX", Description = "Microsoft Azure Cognitive Services Neural TTS", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.HiddenDim }; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new AzureTTS<T>(Architecture, mp, _options); return new AzureTTS<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(AzureTTS<T>)); }
