...
public class Pheme<T> : TtsModelBase<T>, IEndToEndTts<T>
{
    ...
    public Tensor<T> Synthesize(string text)
    {
        ...
        double[] ctx = new double[mF]; for (int f = 0; f < mF; f++) { int t = Math.Min(f * textLen / mF, textLen - 1); double tt = (t == textLen - 1) ? -0.15 : 0.0; ctx[f] = Math.Tanh((text[t] % 128) / 128.0 * 0.6 + tt + Math.Sin(f * 0.04) * 0.1); }
        double[] mel = new double[mF]; double p = 0;
        for (int f = 0; f < mF; f++) { mel[f] = Math.Tanh(ctx[f] * 0.7 + p * 0.2 + Math.Sin(f * 0.06) * 0.08); p = mel[f]; }
