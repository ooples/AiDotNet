...
/// <summary>
/// LiteDVDNet optimized FastDVDNet variant achieving 3-5x faster inference.
/// </summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet">
/// <item>Paper: "LiteDVDNet: Lightweight Deep Video Denoising" (2025)</item>
/// </list></para>
/// <para>
/// LiteDVDNet optimizes the FastDVDNet architecture with lightweight modules for 3-5x faster inference with minimal quality loss.
/// </para>
/// </remarks>
public class LiteDVDNet<T> : VideoDenoisingBase<T>
{
    ...
    /// <summary>
    /// Creates a new LiteDVDNet model for native training and inference.
    /// </summary>
    ...
    {
        int numFrames = noisyFrames.Shape[0];
        int channels = noisyFrames.Shape[1];
        int height = noisyFrames.Shape[2];
        int width = noisyFrames.Shape[3];

        var output = new Tensor<T>(noisyFrames.Shape);

        for (int f = 0; f < numFrames; f++)
        {
            var frame = ExtractFrame(noisyFrames, f);
            var feat = _featureExtract!.Forward(frame);
            foreach (var block in _processingBlocks)
            {
                feat = block.Forward(feat);
            }
            var clean = _outputConv!.Forward(feat);

            // Residual learning: output = input + learned residual
            for (int i = 0; i < clean.Length; i++)
            {
                int srcIdx = f * clean.Length + i;
                output.Data.Span[srcIdx] = NumOps.Add(noisyFrames.Data.Span[srcIdx], clean.Data.Span[i]);
            }
        }
    ...
    {
        return new LiteDVDNet<T>(Architecture, _numFeatures, _numLayers);
    }
}
