...
public class SEALTrainer<T, TInput, TOutput> : MetaLearnerBase<T, TInput, TOutput>
{
        ...
        : base(metaModel, lossFunction, dataLoader, config ?? new SEALTrainerConfig<T>())
    {
        ...
        var sealConfig = (SEALTrainerConfig<T>)Configuration;
        if (sealConfig.UseAdaptiveInnerLR)
        {
            _adaptiveLearningRates = new Dictionary<string, T>();
        }
    }
    ...
    public override MetaTrainingStepResult<T> MetaTrainStep(int batchSize)
    {
        ...
        for (int taskIdx = 0; taskIdx < batchSize; taskIdx++)
        {
            ...
            {
                T temperature = NumOps.FromDouble(config.Temperature);
                queryLoss = NumOps.Divide(queryLoss, temperature);
            }

            // Add entropy regularization if configured
            if (config.EntropyCoefficient > 0.0)
            {
                TOutput queryPredictions = MetaModel.Predict(task.QuerySetX);
                T entropyTerm = ComputeEntropyRegularization(queryPredictions);
                T entropyCoef = NumOps.FromDouble(config.EntropyCoefficient);
                queryLoss = NumOps.Subtract(queryLoss, NumOps.Multiply(entropyCoef, entropyTerm));
            }
        ...
        {
            T decay = NumOps.FromDouble(config.WeightDecay);
            for (int i = 0; i < averageUpdate.Length; i++)
            {
                averageUpdate[i] = NumOps.Add(averageUpdate[i], NumOps.Multiply(decay, originalParameters[i]));
            }
        }

        // Apply gradient clipping if configured
        if (config.GradientClipThreshold.HasValue)
        {
            averageUpdate = ClipGradientByNorm(averageUpdate, NumOps.FromDouble(config.GradientClipThreshold.Value));
        }
    ...
    {
        var config = (SEALTrainerConfig<T>)Configuration;
        var currentParameters = MetaModel.GetParameters();
        ...
        for (int step = 0; step < steps; step++)
        {
            ...
            // Compute gradients on self-supervised task
            var gradients = ComputeGradients(MetaModel, augmentedX, augmentedY);

            // Adapt learning rate if enabled
            T effectiveLearningRate = config.InnerLearningRate;
            if (config.UseAdaptiveInnerLR && _adaptiveLearningRates != null)
            {
                // Simple adaptive learning rate based on gradient norm
                T gradientNorm = gradients.Norm();
                T baseLR = config.InnerLearningRate;

                // Scale learning rate inversely with gradient norm
                T avgGradNorm = NumOps.FromDouble(1.0);
                int paramCount = gradients.Length;
                if (paramCount > 0)
                {
                    avgGradNorm = NumOps.Divide(gradientNorm, NumOps.FromDouble(paramCount));
                }

                // Use heuristic: LR = baseLR * (1 / (1 + ||g||))
                T scale = NumOps.Divide(NumOps.One, NumOps.Add(NumOps.One, avgGradNorm));
                effectiveLearningRate = NumOps.Multiply(baseLR, scale);

                // Store adaptive learning rate for monitoring
                string key = $"step_{step}_selfsupervised";
                _adaptiveLearningRates[key] = effectiveLearningRate;
            }

            // Apply gradients with adaptive learning rate
            if (_innerOptimizer != null)
            {
                // Create a temporary optimizer with adapted learning rate
                var adaptedParameters = _innerOptimizer.UpdateParameters(currentParameters, gradients);
                MetaModel.UpdateParameters(adaptedParameters);
                currentParameters = adaptedParameters;
            }
            else
            {
                // Fallback to manual gradient application
                var scaledGradients = gradients.Multiply(effectiveLearningRate);
                currentParameters = currentParameters.Subtract(scaledGradients);
                MetaModel.UpdateParameters(currentParameters);
            }
        }
    }
        ...
        int steps)
    {
        ...
        for (int step = 0; step < steps; step++)
        {
            // Compute gradients on combined set
            var gradients = ComputeGradients(MetaModel, combinedX, combinedY);
            ...
            if (config.UseAdaptiveInnerLR && _adaptiveLearningRates != null)
            {
                ...
                // Store adaptive learning rate for monitoring
                string key = $"step_{step}_supervised";
                _adaptiveLearningRates[key] = effectiveLearningRate;
            }
    ...
    /// <summary>
    /// Computes entropy regularization for the predictions.
    /// </summary>
    /// <param name="predictions">The model predictions.</param>
    /// <returns>The entropy value.</returns>
    /// <remarks>
    /// <para>
    /// Entropy regularization encourages diverse predictions and prevents overconfidence.
    /// For classification with probability distributions, entropy = -sum(p * log(p)).
    /// Higher entropy means more uncertainty/uniformity in predictions.
    /// </para>
    /// </remarks>
        ...
        if (predictions is Tensor<T> tensor)
        {
            ...
            for (int i = 0; i < numExamples; i++)
            {
                ...
                for (int j = 0; j < numClasses; j++)
                {
                    ...
                    if (NumOps.LessThan(prob, NumOps.Zero))
                        prob = NumOps.Zero;
                    else if (NumOps.GreaterThan(prob, NumOps.One))
                        prob = NumOps.One;

                    // Compute -p * log(p)
                    if (NumOps.GreaterThan(prob, NumOps.Zero))
                    {
                        T logProb = NumOps.Log(prob);
                        T contribution = NumOps.Multiply(prob, logProb);
                        exampleEntropy = NumOps.Add(exampleEntropy, contribution);
                    }
                }
                totalEntropy = NumOps.Add(totalEntropy, NumOps.Negate(exampleEntropy));
            }

            // Return average entropy across examples
            T count = NumOps.FromDouble(numExamples);
            return NumOps.Divide(totalEntropy, count);
        }
        else if (predictions is Vector<T> vector)
        {
            // For regression or scalar outputs, entropy doesn't apply
            return NumOps.Zero;
        }
        else
        {
            throw new NotSupportedException($"Entropy calculation only supports Tensor<T> or Vector<T>, got {typeof(TOutput)}");
        }
    }

    /// <summary>
    /// Clips gradients by their L2 norm for training stability.
    /// </summary>
    ...
    /// that exceed a maximum norm threshold. This stabilizes training.
    /// </para>
    /// </remarks>
    private Vector<T> ClipGradientByNorm(Vector<T> gradient, T maxNorm)
    {
        ...
        if (NumOps.LessThanOrEqual(gradientNorm, maxNorm))
        {
            return gradient;
        }

        // Scale gradient: g_clipped = g * (max_norm / ||g||)
        T scale = NumOps.Divide(maxNorm, gradientNorm);
