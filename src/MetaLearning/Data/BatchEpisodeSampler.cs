...
public class BatchEpisodeSampler<T, TInput, TOutput>
{
        ...
        int prefetchCount = 0)
    {
        ...
        if (prefetchCount < 0)
            throw new ArgumentOutOfRangeException(nameof(prefetchCount), prefetchCount, "Prefetch count must be non-negative.");
    ...
    {
        Guard.NotNull(episodes);
        Guard.NotNull(losses);
        if (episodes.Count != losses.Count)
            throw new ArgumentException($"Episodes count ({episodes.Count}) must match losses count ({losses.Count}).");
