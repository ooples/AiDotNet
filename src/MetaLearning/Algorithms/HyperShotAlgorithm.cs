...
public class HyperShotAlgorithm<T, TInput, TOutput> : MetaLearnerBase<T, TInput, TOutput>
{
    ...
    public override T MetaTrain(TaskBatch<T, TInput, TOutput> taskBatch)
    {
        ...
        // Update backbone
        ApplyOuterUpdate(initParams, metaGradients, _hyperShotOptions.OuterLearningRate);
...
/// </para>
/// </remarks>
internal class HyperShotModel<T, TInput, TOutput> : MetaLearningModelBase<T, TInput, TOutput>, IAdaptedMetaModel<T>
{
    ...
    {
        return new HyperShotModel<T, TInput, TOutput>(BaseModel, parameters, _generatedKernel, _modulationFactors);
    }

    /// <inheritdoc/>
    public override IFullModel<T, TInput, TOutput> DeepCopy()
    {
        return new HyperShotModel<T, TInput, TOutput>(
            BaseModel.DeepCopy(), _backboneParams.Clone(), _generatedKernel?.Clone(),
