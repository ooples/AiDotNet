...
public class iMAMLAlgorithm<T, TInput, TOutput> : MetaLearnerBase<T, TInput, TOutput>
{
    ...
    public override T MetaTrain(TaskBatch<T, TInput, TOutput> taskBatch)
    {
        ...
        foreach (var task in taskBatch.Tasks)
        {
            ...
            var initialParams = ((IParameterizable<T, TInput, TOutput>)taskModel).GetParameters();
        ...
        IMetaLearningTask<T, TInput, TOutput> task)
    {
        ...
        ((IParameterizable<T, TInput, TOutput>)model).SetParameters(adaptedParams);
        ...
        Vector<T> v)
    {
        ...
        var theta = ((IParameterizable<T, TInput, TOutput>)model).GetParameters();

        // Vectorized Hessian-vector product via finite differences
        // theta + eps*v
        var eDeltaV = (Vector<T>)Engine.Multiply(v, eps);
        ((IParameterizable<T, TInput, TOutput>)model).SetParameters((Vector<T>)Engine.Add(theta, eDeltaV));
        var gradPlus = ComputeGradients(model, task.SupportInput, task.SupportOutput);

        // theta - eps*v
        ((IParameterizable<T, TInput, TOutput>)model).SetParameters((Vector<T>)Engine.Subtract(theta, eDeltaV));
        var gradMinus = ComputeGradients(model, task.SupportInput, task.SupportOutput);

        // Restore original parameters
        ((IParameterizable<T, TInput, TOutput>)model).SetParameters(theta);
