...
public class AdaptedMetaModel<T, TInput, TOutput> : MetaLearningModelBase<T, TInput, TOutput>, IAdaptedMetaModel<T>
{
    ...
    {
        var originalParams = ((IParameterizable<T, TInput, TOutput>)BaseModel).GetParameters();
        try
        {
            if (_modulationFactors is not null && _modulationFactors.Length > 0)
            {
                ...
                ((IParameterizable<T, TInput, TOutput>)BaseModel).SetParameters(modulated);
            }
            else
            {
                ((IParameterizable<T, TInput, TOutput>)BaseModel).SetParameters(_adaptedParams);
            }
            return BaseModel.Predict(input);
        }
        finally
        {
            ((IParameterizable<T, TInput, TOutput>)BaseModel).SetParameters(originalParams);
        }
    }
