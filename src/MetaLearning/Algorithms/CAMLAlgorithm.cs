...
public class CAMLAlgorithm<T, TInput, TOutput> : MetaLearnerBase<T, TInput, TOutput>
{
    ...
    public override T MetaTrain(TaskBatch<T, TInput, TOutput> taskBatch)
    {
        ...
        UpdateAuxiliaryParamsSPSA(taskBatch, ref _contextParams, _camlOptions.OuterLearningRate, ComputeAuxLoss);

        return ComputeMean(losses);
    }

    /// <summary>
    /// Computes the average loss over a task batch using the context module.
    /// Called by SPSA to measure how perturbed context params affect loss.
    /// </summary>
...
/// </para>
/// </remarks>
internal class CAMLModel<T, TInput, TOutput> : MetaLearningModelBase<T, TInput, TOutput>, IAdaptedMetaModel<T>
{
    ...
    {
        return new CAMLModel<T, TInput, TOutput>(BaseModel, parameters, _adaptedPrototypes, _modulationFactors);
    }

    /// <inheritdoc/>
    public override IFullModel<T, TInput, TOutput> DeepCopy()
    {
        return new CAMLModel<T, TInput, TOutput>(
            BaseModel.DeepCopy(), _backboneParams.Clone(), _adaptedPrototypes?.Clone(),
