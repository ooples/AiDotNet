...
public class CNPAlgorithm<T, TInput, TOutput> : NeuralProcessBase<T, TInput, TOutput>
{
    ...
    {
        if (features == null || labels == null || features.Length == 0 || labels.Length == 0)
            return new Vector<T>(RepresentationDim);

        int numExamples = labels.Length;
        if (features.Length % numExamples != 0)
            throw new InvalidOperationException(
                $"Support features length ({features.Length}) must be evenly divisible by labels length ({numExamples}).");
        int featureDim = features.Length / numExamples;

        var representations = new List<Vector<T>>();
        for (int i = 0; i < numExamples; i++)
        {
            int fStart = i * featureDim;
            var f = new Vector<T>(featureDim);
            for (int j = 0; j < featureDim; j++) f[j] = features[fStart + j];
