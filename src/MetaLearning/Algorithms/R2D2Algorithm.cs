...
/// </para>
/// </remarks>
internal class R2D2Model<T, TInput, TOutput> : MetaLearningModelBase<T, TInput, TOutput>, IAdaptedMetaModel<T>
{
    ...
    {
        return new R2D2Model<T, TInput, TOutput>(BaseModel, parameters, _ridgeWeights, _lambda, _modulationFactors);
    }

    /// <inheritdoc/>
    public override IFullModel<T, TInput, TOutput> DeepCopy()
    {
        return new R2D2Model<T, TInput, TOutput>(
            BaseModel.DeepCopy(), _backboneParams.Clone(), _ridgeWeights?.Clone(), _lambda,
