...
[ModelPaper("SPIRAL: Self-supervised Perturbation-Invariant Representation Learning for Speech Pre-Training", "https://arxiv.org/abs/2201.10207", Year = 2022, Authors = "Huang et al.")]
public class SPIRAL<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Huang et al. (2022): the encoder produces representations invariant to speech
    /// perturbations (speed, noise, masking). CTC head decodes the robust features.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SPIRAL does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SPIRAL-Native" : "SPIRAL-ONNX", Description = "SPIRAL: perturbation-invariant SSL + CTC (2022)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SPIRAL<T>(Architecture, mp, _options); return new SPIRAL<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SPIRAL<T>)); }
