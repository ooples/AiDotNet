...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "HuBERT: Self-Supervised Speech Representation Learning by Masked Prediction of Hidden Units" (Hsu et al., Meta, 2021)</item></list></para>
/// <para><b>For Beginners:</b> HuBERT uses an offline clustering step to provide pseudo-labels for masked prediction pre-training. Unlike wav2vec 2.0's contrastive loss, HuBERT predicts discrete cluster assignments of masked speech segments. The model iteratively refines its cl...</para>
/// <para>
...
public class HuBERTASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("HuBERTASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "HuBERTASR-Native" : "HuBERTASR-ONNX", Description = "HuBERT: hidden-unit BERT SSL + CTC (Meta, 2021)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new HuBERTASR<T>(Architecture, mp, _options); return new HuBERTASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(HuBERTASR<T>)); }
