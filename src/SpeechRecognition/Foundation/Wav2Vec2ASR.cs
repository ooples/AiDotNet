...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "wav2vec 2.0: A Framework for Self-Supervised Learning of Speech Representations" (Baevski et al., Meta, 2020)</item></list></para>
/// <para><b>For Beginners:</b> Wav2Vec 2.0 learns speech representations through self-supervised contrastive learning. A CNN feature encoder processes raw audio, then a Transformer encoder learns contextualized representations. During pre-training, latent speech representations...</para>
/// <para>
...
public class Wav2Vec2ASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Baevski et al. (2020): raw audio is processed by a CNN feature encoder, then
    /// a Transformer encodes contextualized representations. The CTC head produces
    /// per-frame token probabilities decoded by greedy search with blank collapsing.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("Wav2Vec2ASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Wav2Vec2ASR-Native" : "Wav2Vec2ASR-ONNX", Description = "Wav2Vec 2.0: SSL speech representations + CTC (Meta, 2020)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Wav2Vec2ASR<T>(Architecture, mp, _options); return new Wav2Vec2ASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Wav2Vec2ASR<T>)); }
