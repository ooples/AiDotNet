...
public partial class W2vBERT<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    /// Measured from this model's own output head, NOT from the contrastive codebook w2v-BERT uses for
    /// masked-language-modelling targets: <c>InitializeLayers</c> builds
    ...
    /// Per Chung et al. (2021): lower layers use contrastive learning for discrete tokens,
    /// upper layers do masked language modeling. CTC head decodes the fused representations.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("W2vBERT does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "W2vBERT-Native" : "W2vBERT-ONNX", Description = "w2v-BERT: contrastive + MLM SSL + CTC (Google, 2021)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new W2vBERT<T>(Architecture, mp, _options); return new W2vBERT<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(W2vBERT<T>)); }
