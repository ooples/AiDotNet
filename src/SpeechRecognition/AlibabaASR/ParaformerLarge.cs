...
public class ParaformerLarge<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override void Train(Tensor<T> input, Tensor<T> expected) { if (IsOnnxMode) throw new NotSupportedException("Training not supported in ONNX mode."); SetTrainingMode(true); TrainWithCifSupervision(input, expected, _optimizer); SetTrainingMode(false); }
