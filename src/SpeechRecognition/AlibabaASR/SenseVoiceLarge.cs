...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Model: "SenseVoice-Large" (Alibaba FunASR, 2024)</item></list></para>
/// <para><b>For Beginners:</b> SenseVoice-Large scales up the SenseVoice architecture with a larger encoder (1024-dim, 50 layers) for improved accuracy across all supported tasks. The model supports 50+ languages and includes enhanced emotion recognition and audio event detecti...</para>
/// <para>
...
public class SenseVoiceLarge<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SenseVoiceLarge(NeuralNetworkArchitecture<T> architecture, string modelPath, SenseVoiceLargeOptions? options = null) : base(architecture) { _options = options ?? new SenseVoiceLargeOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr", "it", "pt", "ru" }; InitializeLayers(); }
    public SenseVoiceLarge(NeuralNetworkArchitecture<T> architecture, SenseVoiceLargeOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SenseVoiceLargeOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this, new AdamWOptimizerOptions<T, Tensor<T>, Tensor<T>> { InitialLearningRate = _options.LearningRate, WeightDecay = _options.WeightDecay }); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr", "it", "pt", "ru" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using the scaled SenseVoice-Large multi-task pipeline.
    /// Per Alibaba (2024): the larger encoder captures finer speech details, with
    /// CIF-based parallel decoding for efficient multi-language transcription.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SenseVoiceLarge does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SenseVoiceLarge-Native" : "SenseVoiceLarge-ONNX", Description = "SenseVoice-Large: scaled multi-task speech (Alibaba, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader r) { _useNativeMode = r.ReadBoolean(); string mp = r.ReadString(); if (!string.IsNullOrEmpty(mp)) _options.ModelPath = mp; _options.SampleRate = r.ReadInt32(); _options.MaxAudioLengthSeconds = r.ReadInt32(); _options.EncoderDim = r.ReadInt32(); _options.NumEncoderLayers = r.ReadInt32(); _options.NumAttentionHeads = r.ReadInt32(); _options.NumMels = r.ReadInt32(); _options.VocabSize = r.ReadInt32(); _options.MaxTextLength = r.ReadInt32(); _options.DropoutRate = r.ReadDouble(); _options.Language = r.ReadString(); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels;
        ...
        if (!stream.CanSeek || stream.Position < stream.Length)
        {
            ...
            if (payloadVersion != NetworkSpecificPayloadVersion)
            {
                throw new InvalidOperationException(
                    $"SenseVoiceLarge was saved with network-payload version {payloadVersion}, but this build " +
        ...
        else
        {
            System.Diagnostics.Trace.TraceWarning(
                "AiDotNet.SenseVoiceLarge: this model was saved before the decoder and optimizer settings were " +
                "persisted, so they keep their defaults. Re-save the model to carry them forward.");
        }
 if (!_useNativeMode && _options.ModelPath is { } p && !string.IsNullOrEmpty(p)) OnnxEncoder = new OnnxModel<T>(p, _options.OnnxOptions); }
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { var options = new SenseVoiceLargeOptions(_options); if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SenseVoiceLarge<T>(Architecture, mp, options); return new SenseVoiceLarge<T>(Architecture, options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SenseVoiceLarge<T>)); }
