...
public class SenseVoiceLarge<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SenseVoiceLarge(NeuralNetworkArchitecture<T> architecture, SenseVoiceLargeOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SenseVoiceLargeOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr", "it", "pt", "ru" }; InitializeLayers(); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SenseVoiceLarge<T>(Architecture, mp, _options); return new SenseVoiceLarge<T>(Architecture, _options); }
