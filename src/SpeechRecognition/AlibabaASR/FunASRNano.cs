...
public class FunASRNano<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public FunASRNano(NeuralNetworkArchitecture<T> architecture, string modelPath, FunASRNanoOptions? options = null) : base(architecture) { _options = options ?? new FunASRNanoOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public FunASRNano(NeuralNetworkArchitecture<T> architecture, FunASRNanoOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new FunASRNanoOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using the ultra-compact FunASR-Nano encoder with CTC.
    /// Per Alibaba (2024): the lightweight Conformer encoder with aggressive subsampling
    /// processes mel features efficiently for on-device CTC decoding.
    /// </summary>
    public TranscriptionResult<T> Transcribe(Tensor<T> audio, string? language = null, bool includeTimestamps = false)
    {
        ...
        return new TranscriptionResult<T>
        {
            ...
            Confidence = NumOps.FromDouble(tokens.Count > 0 ? 0.82 : 0.0),
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("FunASRNano does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "FunASRNano-Native" : "FunASRNano-ONNX", Description = "FunASR-Nano: ultra-lightweight on-device ASR (Alibaba, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new FunASRNano<T>(Architecture, mp, _options); return new FunASRNano<T>(Architecture, _options); }
    ...
    private IReadOnlyList<TranscriptionSegment<T>> ExtractSegments(string text, double duration) { if (string.IsNullOrWhiteSpace(text)) return Array.Empty<TranscriptionSegment<T>>(); return new[] { new TranscriptionSegment<T> { Text = text, StartTime = 0.0, EndTime = duration, Confidence = NumOps.FromDouble(0.82) } }; }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(FunASRNano<T>)); }
