...
/// <example>
/// <code>
/// // Create a Qwen3-ASR model for LLM-integrated multilingual speech recognition
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 32000);
/// var model = new Qwen3ASR&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for instruction-following ASR
/// var onnxModel = new Qwen3ASR&lt;double&gt;(architecture, "qwen3asr.onnx");
/// </code>
/// </example>
...
public class Qwen3ASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public Qwen3ASR(NeuralNetworkArchitecture<T> architecture, string modelPath, Qwen3ASROptions? options = null) : base(architecture) { _options = options ?? new Qwen3ASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr", "it", "pt", "ru", "ar", "hi", "tr", "vi", "th" }; InitializeLayers(); }
    public Qwen3ASR(NeuralNetworkArchitecture<T> architecture, Qwen3ASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Qwen3ASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr", "it", "pt", "ru", "ar", "hi", "tr", "vi", "th" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Conformer encoder + Qwen3 LLM decoder.
    /// Per Alibaba (2025): audio features are encoded, projected via adapter into Qwen3's
    /// embedding space, then the LLM autoregressively generates text conditioned on task prompts.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("Qwen3ASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Qwen3ASR-Native" : "Qwen3ASR-ONNX", Description = "Qwen3-ASR: LLM-integrated ASR with Qwen3 (Alibaba, 2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Qwen3ASR<T>(Architecture, mp, _options); return new Qwen3ASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Qwen3ASR<T>)); }
