...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Model: "Qwen3-ASR-Small" (Alibaba Qwen, 2025)</item></list></para>
/// <para><b>For Beginners:</b> Qwen3-ASR-Small is a compact variant using a smaller encoder (512-dim, 12 layers) paired with Qwen3-0.5B for efficient deployment. Maintains multilingual support and instruction-following capabilities while targeting edge and mobile platforms with...</para>
/// <para>
...
public class Qwen3ASRSmall<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public Qwen3ASRSmall(NeuralNetworkArchitecture<T> architecture, string modelPath, Qwen3ASRSmallOptions? options = null) : base(architecture) { _options = options ?? new Qwen3ASRSmallOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr" }; InitializeLayers(); }
    public Qwen3ASRSmall(NeuralNetworkArchitecture<T> architecture, Qwen3ASRSmallOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Qwen3ASRSmallOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "zh", "en", "ja", "ko", "de", "es", "fr" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using compact Conformer encoder + Qwen3-0.5B decoder.
    /// Per Alibaba (2025): the lightweight encoder and smaller LLM enable efficient
    /// on-device ASR with instruction-following capabilities.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("Qwen3ASRSmall does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Qwen3ASRSmall-Native" : "Qwen3ASRSmall-ONNX", Description = "Qwen3-ASR-Small: lightweight LLM ASR (Alibaba, 2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Qwen3ASRSmall<T>(Architecture, mp, _options); return new Qwen3ASRSmall<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Qwen3ASRSmall<T>)); }
