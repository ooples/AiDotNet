...
/// <example>
/// <code>
/// // Create a Speechmatics model for real-time multilingual ASR
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 5000);
/// var model = new SpeechmaticsASR&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for Global English ASR
/// var onnxModel = new SpeechmaticsASR&lt;double&gt;(architecture, "speechmatics.onnx");
/// </code>
/// </example>
...
public class SpeechmaticsASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SpeechmaticsASR(NeuralNetworkArchitecture<T> architecture, string modelPath, SpeechmaticsASROptions? options = null) : base(architecture) { _options = options ?? new SpeechmaticsASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }
    public SpeechmaticsASR(NeuralNetworkArchitecture<T> architecture, SpeechmaticsASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SpeechmaticsASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Speechmatics' proprietary Conformer architecture.
    /// The model processes audio with Global English support for all accents and
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SpeechmaticsASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SpeechmaticsASR-Native" : "SpeechmaticsASR-ONNX", Description = "Speechmatics: real-time multilingual ASR (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SpeechmaticsASR<T>(Architecture, mp, _options); return new SpeechmaticsASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SpeechmaticsASR<T>)); }
