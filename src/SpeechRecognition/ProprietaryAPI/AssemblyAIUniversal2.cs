...
/// <summary>
/// AssemblyAI Universal-2: best-in-class transcription API
/// </summary>
...
///
/// // Or load a pre-trained ONNX model for audio intelligence ASR
/// var onnxModel = new AssemblyAIUniversal2&lt;double&gt;(architecture, "assemblyai.onnx");
/// </code>
/// </example>
...
public class AssemblyAIUniversal2<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("AssemblyAIUniversal2 does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "AssemblyAIUniversal2-Native" : "AssemblyAIUniversal2-ONNX", Description = "AssemblyAI Universal-2: best-in-class ASR API (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new AssemblyAIUniversal2<T>(Architecture, mp, _options); return new AssemblyAIUniversal2<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(AssemblyAIUniversal2<T>)); }
