...
/// <summary>
/// Google Cloud Speech-to-Text V2: production cloud ASR API
/// </summary>
...
///
/// // Or load a pre-trained ONNX model for Chirp-based cloud ASR
/// var onnxModel = new GoogleSpeechV2&lt;double&gt;(architecture, "googlespeechv2.onnx");
/// </code>
/// </example>
...
public class GoogleSpeechV2<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("GoogleSpeechV2 does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "GoogleSpeechV2-Native" : "GoogleSpeechV2-ONNX", Description = "Google Cloud STT V2: Chirp-based production API (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new GoogleSpeechV2<T>(Architecture, mp, _options); return new GoogleSpeechV2<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(GoogleSpeechV2<T>)); }
