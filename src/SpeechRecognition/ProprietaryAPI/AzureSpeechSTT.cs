...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>API: "Azure Cognitive Services Speech-to-Text" (Microsoft Azure, 2024)</item></list></para>
/// <para>
...
public class AzureSpeechSTT<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public AzureSpeechSTT(NeuralNetworkArchitecture<T> architecture, string modelPath, AzureSpeechSTTOptions? options = null) : base(architecture) { _options = options ?? new AzureSpeechSTTOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }
    public AzureSpeechSTT(NeuralNetworkArchitecture<T> architecture, AzureSpeechSTTOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new AzureSpeechSTTOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Azure Speech Service architecture.
    /// The model uses FastTransformer-based processing with support for custom domain
    /// adaptation and real-time streaming capabilities.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("AzureSpeechSTT does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "AzureSpeechSTT-Native" : "AzureSpeechSTT-ONNX", Description = "Azure Speech STT: enterprise ASR platform (Microsoft, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new AzureSpeechSTT<T>(Architecture, mp, _options); return new AzureSpeechSTT<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(AzureSpeechSTT<T>)); }
