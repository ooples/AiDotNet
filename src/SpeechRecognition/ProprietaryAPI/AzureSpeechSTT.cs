...
/// <para><b>For Beginners:</b> Azure Speech Service provides enterprise-grade speech-to-text through Microsoft's cloud platform. The service uses FastTransformer-based models trained on massive multilingual datasets. Features include custom speech models trained on domain data,...</para>
/// <para>
...
public class AzureSpeechSTT<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "AzureSpeechSTT-Native" : "AzureSpeechSTT-ONNX", Description = "Azure Speech STT: enterprise ASR platform (Microsoft, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
