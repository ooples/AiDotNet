...
/// <summary>
/// Deepgram Nova-2: fastest real-time ASR API
/// </summary>
...
public class DeepgramNova2<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public DeepgramNova2(NeuralNetworkArchitecture<T> architecture, string modelPath, DeepgramNova2Options? options = null) : base(architecture) { _options = options ?? new DeepgramNova2Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public DeepgramNova2(NeuralNetworkArchitecture<T> architecture, DeepgramNova2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new DeepgramNova2Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Deepgram Nova-2's low-latency architecture.
    /// The model is optimized for sub-300ms streaming latency with smart formatting,
    /// topic detection, and entity recognition across 36+ languages.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("DeepgramNova2 does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "DeepgramNova2-Native" : "DeepgramNova2-ONNX", Description = "Deepgram Nova-2: fastest real-time ASR API (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(DeepgramNova2<T>)); }
