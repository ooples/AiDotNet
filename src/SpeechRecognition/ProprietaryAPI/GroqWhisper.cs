...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>API: "Groq Whisper" (Groq, 2024)</item></list></para>
/// <para><b>For Beginners:</b> Groq Whisper provides Whisper model inference accelerated by Groq's LPU (Language Processing Unit) hardware. The LPU's deterministic execution model and high memory bandwidth enable real-time Whisper Large V3 inference at unprecedented speeds. Gro...</para>
/// <para>
...
/// <example>
/// <code>
/// // Create a Groq Whisper model for LPU-accelerated Whisper inference
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 5000);
/// var model = new GroqWhisper&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for hardware-accelerated ASR
/// var onnxModel = new GroqWhisper&lt;double&gt;(architecture, "groqwhisper.onnx");
/// </code>
/// </example>
...
public partial class GroqWhisper<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    /// Traced through the graph, not read off a name - and here the name actively misleads: despite
    /// "Whisper" this model does NOT build the Whisper encoder-decoder stack. InitializeLayers builds
    /// <c>LayerHelper.CreateDefaultProprietaryASRLayers(..., vocabSize: _options.VocabSize)</c>, whose
    /// last emitted layer is <c>DenseLayer&lt;T&gt;(vocabSize)</c> - the CTC head. Both factories
    /// happen to end at vocabSize, so the width agrees either way. PostprocessOutput is the identity.
    /// </remarks>
    ...
    public GroqWhisper(NeuralNetworkArchitecture<T> architecture, string modelPath, GroqWhisperOptions? options = null) : base(architecture) { _options = options ?? new GroqWhisperOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }
    public GroqWhisper(NeuralNetworkArchitecture<T> architecture, GroqWhisperOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new GroqWhisperOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Whisper architecture on Groq LPU hardware.
    /// Groq's deterministic LPU execution enables 189x real-time Whisper Large V3
    /// processing with support for all Whisper languages.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("GroqWhisper does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "GroqWhisper-Native" : "GroqWhisper-ONNX", Description = "Groq Whisper: LPU-accelerated Whisper (Groq, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new GroqWhisper<T>(Architecture, mp, _options); return new GroqWhisper<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(GroqWhisper<T>)); }
