...
public partial class CanaryQwen<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// branches (the Qwen2 GQA/RMSNorm path and the default one), so the width does not depend on
    /// which one runs. PredictCore folds Layers and PostprocessOutput is the identity.
    /// </remarks>
