...
public partial class CanaryQwen<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public CanaryQwen(NeuralNetworkArchitecture<T> architecture, CanaryQwenOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new CanaryQwenOptions(); _useNativeMode = true; _optimizer = optimizer
