...
public class CanaryQwen<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public CanaryQwen(NeuralNetworkArchitecture<T> architecture, string modelPath, CanaryQwenOptions? options = null) : base(architecture) { _options = options ?? new CanaryQwenOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = _options.SupportedLanguagesList; InitializeLayers(); }
    public CanaryQwen(NeuralNetworkArchitecture<T> architecture, CanaryQwenOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new CanaryQwenOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = _options.SupportedLanguagesList; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Fast Conformer encoder + Qwen-2.5 LLM decoder.
    /// Per NVIDIA (2025): the encoder produces audio representations which are downsampled
    /// by a perceiver adapter, then fed as prefix tokens to the Qwen-2.5 decoder. Task tokens
    /// (language ID, transcribe/translate mode) condition the generation. The LLM decoder
    /// autoregressively produces text leveraging its pre-trained multilingual capabilities.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("CanaryQwen does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "CanaryQwen-Native" : "CanaryQwen-ONNX", Description = "Canary-Qwen: multilingual ASR + translation with Qwen-2.5 LLM (NVIDIA, 2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers };
    protected override void SerializeNetworkSpecificData(BinaryWriter w) { w.Write(_useNativeMode); w.Write(_options.ModelPath ?? string.Empty); w.Write(_options.SampleRate); w.Write(_options.EncoderDim); w.Write(_options.DecoderDim); w.Write(_options.NumEncoderLayers); w.Write(_options.NumDecoderLayers); w.Write(_options.NumAttentionHeads); w.Write(_options.NumMels); w.Write(_options.VocabSize); w.Write(_options.MaxTextLength); w.Write(_options.DropoutRate); w.Write(_options.Language); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new CanaryQwen<T>(Architecture, mp, _options); return new CanaryQwen<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(CanaryQwen<T>)); }
