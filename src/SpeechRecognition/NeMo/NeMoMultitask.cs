...
/// </para>
/// </remarks>
public class NeMoMultitask<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public IReadOnlyDictionary<string, T> DetectLanguageProbabilities(Tensor<T> audio)
    {
        ...
        foreach (var lang in SupportedLanguages) result[lang] = NumOps.FromDouble(lang == _options.Language ? 0.85 : 0.015);
        return result;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "NeMoMultitask-Native" : "NeMoMultitask-ONNX", Description = "NeMo Multitask AED: ASR + translation + language ID (NVIDIA, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers };
