...
public partial class NeMoMultitask<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public NeMoMultitask(NeuralNetworkArchitecture<T> architecture, NeMoMultitaskOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new NeMoMultitaskOptions(); _useNativeMode = true; _optimizer = optimizer
        ?? PaperOptimizerFactory.CreateFor<T, Tensor<T>, Tensor<T>>(this)
        ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "de", "es", "fr", "it", "pt", "nl", "ja", "ko", "zh" }; InitializeLayers(); }
