...
public partial class NeMoMultitask<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    /// The multitask heads are decoded from those same vocabulary logits, so they do not widen the
    /// tensor. PredictCore folds Layers and PostprocessOutput is the identity.
    /// </remarks>
    protected override int OutputFeatureWidth => _options.VocabSize;

    private readonly NeMoMultitaskOptions _options; public override ModelOptions GetOptions() => _options;
    private IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? _optimizer; private bool _useNativeMode; private bool _disposed;
    public IReadOnlyList<string> SupportedLanguages { get; }
    public bool SupportsStreaming => false;
    public bool SupportsWordTimestamps => false;

    public NeMoMultitask(NeuralNetworkArchitecture<T> architecture, string modelPath, NeMoMultitaskOptions? options = null) : base(architecture) { _options = options ?? new NeMoMultitaskOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "de", "es", "fr", "it", "pt", "nl", "ja", "ko", "zh" }; InitializeLayers(); }
    public NeMoMultitask(NeuralNetworkArchitecture<T> architecture, NeMoMultitaskOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new NeMoMultitaskOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "de", "es", "fr", "it", "pt", "nl", "ja", "ko", "zh" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using the multitask AED pipeline.
    /// Per NVIDIA (2024): the encoder produces frame-level representations, and the decoder
    /// generates text conditioned on task tokens (language, transcribe/translate). The shared
    /// encoder architecture enables efficient multi-task inference by changing only the prompt.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("NeMoMultitask does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "NeMoMultitask-Native" : "NeMoMultitask-ONNX", Description = "NeMo Multitask AED: ASR + translation + language ID (NVIDIA, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new NeMoMultitask<T>(Architecture, mp, _options); return new NeMoMultitask<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(NeMoMultitask<T>)); }
