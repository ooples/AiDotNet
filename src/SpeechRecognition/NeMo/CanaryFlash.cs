...
public class CanaryFlash<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per NVIDIA (2025): the encoder produces both CTC logits (auxiliary) and hidden states.
    /// The attention-based decoder cross-attends to encoder states and autoregressively generates
    /// tokens. At inference, the decoder optionally uses CTC prefix scores for rescoring.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("CanaryFlash does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "CanaryFlash-Native" : "CanaryFlash-ONNX", Description = "Canary-Flash: lightweight multilingual ASR with hybrid CTC/attention (NVIDIA, 2025)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new CanaryFlash<T>(Architecture, mp, _options); return new CanaryFlash<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(CanaryFlash<T>)); }
