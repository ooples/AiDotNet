...
public class EBranchformer<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Kim et al. (2023): parallel attention and convolution branches are merged via
    /// point-wise FFN with depthwise convolution. CTC head produces the output.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("EBranchformer does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "EBranchformer-Native" : "EBranchformer-ONNX", Description = "E-Branchformer: enhanced branch merging (2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
