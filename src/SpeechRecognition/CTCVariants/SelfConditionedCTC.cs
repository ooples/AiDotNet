...
[ResearchPaper("Self-Conditioned CTC: CTC Alignment Conditioning on Intermediate Predictions", "https://arxiv.org/abs/2104.02724", Year = 2021, Authors = "Nozaki and Komatsu")]
public class SelfConditionedCTC<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SelfConditionedCTC(NeuralNetworkArchitecture<T> architecture, string modelPath, SelfConditionedCTCOptions? options = null) : base(architecture) { _options = options ?? new SelfConditionedCTCOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public SelfConditionedCTC(NeuralNetworkArchitecture<T> architecture, SelfConditionedCTCOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SelfConditionedCTCOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using self-conditioned CTC iterative refinement.
    /// Per Nozaki and Komatsu (2021): each encoder block receives CTC posteriors from the
    /// previous block, enabling progressive alignment refinement through the stack.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SelfConditionedCTC does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SelfConditionedCTC-Native" : "SelfConditionedCTC-ONNX", Description = "Self-Conditioned CTC: iterative CTC refinement (2021)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SelfConditionedCTC<T>(Architecture, mp, _options); return new SelfConditionedCTC<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SelfConditionedCTC<T>)); }
