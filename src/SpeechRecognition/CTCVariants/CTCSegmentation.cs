...
public class CTCSegmentation<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public CTCSegmentation(NeuralNetworkArchitecture<T> architecture, string modelPath, CTCSegmentationOptions? options = null) : base(architecture) { _options = options ?? new CTCSegmentationOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public CTCSegmentation(NeuralNetworkArchitecture<T> architecture, CTCSegmentationOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new CTCSegmentationOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using CTC posteriors with forced alignment.
    /// Per Kurzinger et al. (2020): CTC posteriors from a Conformer encoder are used with
    /// dynamic programming to find optimal frame-to-character alignment.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("CTCSegmentation does not support streaming.");
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(CTCSegmentation<T>)); }
