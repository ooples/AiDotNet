...
public class SeedASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SeedASR(NeuralNetworkArchitecture<T> architecture, string modelPath, SeedASROptions? options = null) : base(architecture) { _options = options ?? new SeedASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "zh", "en" }; InitializeLayers(); }
    public SeedASR(NeuralNetworkArchitecture<T> architecture, SeedASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SeedASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this, new AiDotNet.Models.Options.AdamWOptimizerOptions<T, Tensor<T>, Tensor<T>> { InitialLearningRate = 1e-4 }); SetBaseTrainOptimizer(_optimizer); /* Default optimizer, overridable via the `optimizer` param. LR 1e-4 (not the framework default 1e-3) because this deep LLM-integrated ASR overshoots at 1e-3; gradient clipping is already on by default but is a near-no-op under AdamW. */ base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "zh", "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Seed-ASR's encoder + LLM decoder architecture.
    /// Per ByteDance (2024): Conformer encoder processes audio, an adapter aligns features
    /// with the LLM, and the decoder generates context-aware transcriptions.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SeedASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SeedASR-Native" : "SeedASR-ONNX", Description = "Seed-ASR: Conformer + LLM decoder (ByteDance, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SeedASR<T>(Architecture, mp, _options); return new SeedASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SeedASR<T>)); }
