...
public class GraniteSpeech<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per IBM (2025): a Conformer speech encoder produces features, a lightweight adapter
    /// maps to the Granite LLM embedding space, and the decoder generates transcriptions.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("GraniteSpeech does not support streaming.");
    ...
    protected override IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>> GetOrCreateBaseOptimizer() =>
        _optimizer ?? throw new InvalidOperationException("A native GraniteSpeech optimizer is not available in ONNX mode.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "GraniteSpeech-Native" : "GraniteSpeech-ONNX", Description = "Granite Speech: enterprise speech-LLM (IBM, 2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new GraniteSpeech<T>(Architecture, mp, new GraniteSpeechOptions(_options));
        ...
        var cloneOptimizer = _optimizer?.GetOptions() is AdamWOptimizerOptions<T, Tensor<T>, Tensor<T>> optimizerOptions
            ...
            : null;
        return new GraniteSpeech<T>(Architecture, new GraniteSpeechOptions(_options), cloneOptimizer);
    }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(GraniteSpeech<T>)); }
