...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "Phi-4-mini Technical Report: Compact yet Powerful Multimodal Language Models via Mixture-of-LoRAs" (Microsoft, 2025)</item></list></para>
/// <para><b>For Beginners:</b> Phi-4 Audio extends the Phi-4 language model with speech understanding through a Mixture-of-LoRA adapter approach. A lightweight speech encoder processes audio and produces features that are injected into the Phi-4 model via specialized LoRA adapt...</para>
/// <para>
...
public partial class Phi4Audio<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public Phi4Audio(NeuralNetworkArchitecture<T> architecture, string modelPath, Phi4AudioOptions? options = null) : base(architecture) { _options = options ?? new Phi4AudioOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public Phi4Audio(NeuralNetworkArchitecture<T> architecture, Phi4AudioOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Phi4AudioOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Phi-4's Mixture-of-LoRA speech adapter.
    /// Per Microsoft (2025): speech features are processed by specialized LoRA adapter experts
    /// that route prosodic, phonetic, and semantic features to the Phi-4 decoder.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("Phi4Audio does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Phi4Audio-Native" : "Phi4Audio-ONNX", Description = "Phi-4 Audio: Mixture-of-LoRA speech adapter (Microsoft, 2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    {
        var options = new Phi4AudioOptions(_options);
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Phi4Audio<T>(Architecture, mp, options);
        return new Phi4Audio<T>(Architecture, options);
    }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Phi4Audio<T>)); }
