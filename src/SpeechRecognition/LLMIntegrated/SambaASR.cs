...
public class SambaASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Miyazaki et al. (2024): audio features are processed by selective state-space
    /// layers that dynamically filter based on content. CTC head decodes the output.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SambaASR-Native" : "SambaASR-ONNX", Description = "Samba-ASR: Mamba SSM encoder + CTC (2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SambaASR<T>(Architecture, mp, _options); return new SambaASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SambaASR<T>)); }
