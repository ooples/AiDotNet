...
/// <example>
/// <code>
/// // Create a SpeechGPT model for multi-modal speech-text conversation
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 32000);
/// var model = new SpeechGPTASR&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for discrete speech token ASR
/// var onnxModel = new SpeechGPTASR&lt;double&gt;(architecture, "speechgptasr.onnx");
/// </code>
/// </example>
...
public class SpeechGPTASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public SpeechGPTASR(NeuralNetworkArchitecture<T> architecture, string modelPath, SpeechGPTASROptions? options = null) : base(architecture) { _options = options ?? new SpeechGPTASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public SpeechGPTASR(NeuralNetworkArchitecture<T> architecture, SpeechGPTASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SpeechGPTASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using SpeechGPT's discrete speech token approach.
    /// Per Zhang et al. (2023): audio is discretized via HuBERT clustering into speech tokens,
    /// which are added to the LLM vocabulary. The model translates speech tokens to text.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("SpeechGPTASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "SpeechGPTASR-Native" : "SpeechGPTASR-ONNX", Description = "SpeechGPT: discrete speech tokens + LLM (Fudan, 2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new SpeechGPTASR<T>(Architecture, mp, _options); return new SpeechGPTASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(SpeechGPTASR<T>)); }
