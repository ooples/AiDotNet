...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "Adapting Large Language Models for Speech Recognition" (OLMo Team, AI2, 2024)</item></list></para>
/// <para><b>For Beginners:</b> OLMo-ASR adapts the open-source OLMo language model for speech recognition by adding a speech encoder adapter. A Conformer encoder processes audio, then a learned adapter projects encoder representations into the LLM's embedding space. The LLM dec...</para>
/// <para>
...
[ModelPaper("OLMo: Accelerating the Science of Language Models", "https://arxiv.org/abs/2402.00838", Year = 2024, Authors = "Groeneveld et al.")]
public class OLMoASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public OLMoASR(NeuralNetworkArchitecture<T> architecture, OLMoASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new OLMoASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Conformer encoder + OLMo LLM decoder.
    /// Per AI2 (2024): a Conformer encoder processes audio, an adapter projects to LLM
    /// embedding space, and the OLMo decoder generates text autoregressively.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "OLMoASR-Native" : "OLMoASR-ONNX", Description = "OLMo-ASR: OLMo LLM + speech adapter (AI2, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new OLMoASR<T>(Architecture, mp, _options); return new OLMoASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(OLMoASR<T>)); }
