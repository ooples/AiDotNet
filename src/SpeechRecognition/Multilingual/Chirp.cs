...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Model: "Chirp" (Google Cloud Speech-to-Text V2, 2023)</item></list></para>
/// <para><b>For Beginners:</b> Chirp is Google's production multilingual ASR model based on the USM architecture, deployed in Google Cloud Speech-to-Text V2. It supports 100+ languages and dialects with a single model. The model uses USM's universal encoder with optimized servi...</para>
/// <para>
...
/// </para>
/// </remarks>
public class Chirp<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    private readonly ChirpOptions _options; public override ModelOptions GetOptions() => _options;
    private IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? _optimizer; private bool _useNativeMode; private bool _disposed;
    public IReadOnlyList<string> SupportedLanguages { get; }
    public bool SupportsStreaming => false;
    public bool SupportsWordTimestamps => false;

    public Chirp(NeuralNetworkArchitecture<T> architecture, string modelPath, ChirpOptions? options = null) : base(architecture) { _options = options ?? new ChirpOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl" }; InitializeLayers(); }
    public Chirp(NeuralNetworkArchitecture<T> architecture, ChirpOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new ChirpOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Chirp's production-optimized USM encoder.
    /// Per Google (2023): the USM-based encoder processes speech for 100+ languages,
    /// optimized for low-latency production serving with CTC decoding.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Chirp-Native" : "Chirp-ONNX", Description = "Chirp: Google Cloud production multilingual ASR (2023)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Chirp<T>(Architecture, mp, _options); return new Chirp<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Chirp<T>)); }
