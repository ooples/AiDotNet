...
public partial class Chirp2<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Chirp2-Native" : "Chirp2-ONNX", Description = "Chirp 2: enhanced Google Cloud multilingual ASR (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Chirp2<T>(Architecture, mp, new Chirp2Options(_options)); return new Chirp2<T>(Architecture, new Chirp2Options(_options)); }
