...
public partial class USM<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// last emitted layer is the CTC fine-tuning head <c>DenseLayer&lt;T&gt;(vocabSize)</c>. This
    /// model's PredictCore additionally pads or truncates its INPUT to <c>_options.NumMels</c> before
    /// folding Layers - that reshapes what enters the stack, not what leaves it; the trailing head
    /// still sets the last axis. PostprocessOutput is the identity.
    /// </remarks>
    ...
    public USM(NeuralNetworkArchitecture<T> architecture, string modelPath, USMOptions? options = null) : base(architecture) { _options = options ?? new USMOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl", "pl", "sv" }; InitializeLayers(); }
    public USM(NeuralNetworkArchitecture<T> architecture, USMOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new USMOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl", "pl", "sv" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using USM's universal speech-text encoder with CTC.
    /// Per Zhang et al. (2023): BEST-RQ pre-trained encoder with speech-text alignment
    /// produces multilingual representations decoded by CTC for 100+ languages.
    /// </summary>
    ...
    public override void Train(Tensor<T> input, Tensor<T> expected) { ThrowIfDisposed(); if (IsOnnxMode) throw new NotSupportedException("Training not supported in ONNX mode."); SetTrainingMode(true); try { TrainWithTape(input, expected, _optimizer); } finally { SetTrainingMode(false); } }
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "USM-Native" : "USM-ONNX", Description = "USM: 2B universal speech model for 100+ languages (Google, 2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new USM<T>(Architecture, mp, _options); return new USM<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(USM<T>)); }
