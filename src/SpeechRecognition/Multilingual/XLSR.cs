...
///
/// // Or load a pre-trained ONNX model for cross-lingual ASR
/// var onnxModel = new XLSR&lt;double&gt;(architecture, "xlsr.onnx");
/// </code>
/// </example>
...
public class XLSR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Babu et al. (2022): the encoder pre-trained on 128 languages via contrastive
    /// learning produces cross-lingual representations decoded by CTC.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "XLSR-Native" : "XLSR-ONNX", Description = "XLS-R: cross-lingual SSL at scale + CTC (Meta, 2022)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new XLSR<T>(Architecture, mp, _options); return new XLSR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(XLSR<T>)); }
