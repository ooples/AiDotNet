...
/// <summary>
/// Chirp 3: latest-generation multilingual ASR
/// </summary>
...
public partial class Chirp3<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    private readonly Chirp3Options _options; public override ModelOptions GetOptions() => _options;
    private IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? _optimizer; private bool _useNativeMode; private bool _disposed;
    public IReadOnlyList<string> SupportedLanguages { get; }
    public bool SupportsStreaming => false;
    public bool SupportsWordTimestamps => false;

    public Chirp3(NeuralNetworkArchitecture<T> architecture, string modelPath, Chirp3Options? options = null) : base(architecture) { _options = options ?? new Chirp3Options(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl" }; InitializeLayers(); }
    public Chirp3(NeuralNetworkArchitecture<T> architecture, Chirp3Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Chirp3Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "fr", "ja", "ko", "pt", "ru", "ar", "hi", "tr", "it", "nl" }; InitializeLayers(); }
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("Chirp3 does not support streaming.");

    // Chirp 3 is Google's USM-based ASR stack, and USM is a CONFORMER encoder (Zhang et al. 2023,
    // arXiv:2303.01037 §3.1, built on Gulati et al. 2020) — residual blocks with per-sublayer
    // LayerNorm, closed by a LayerNorm + vocabulary projection. It is NOT the generic
    // CreateDefaultProprietaryASRLayers stack it used to build, whose bare
    // MHA→LN→FFN→FFN→LN blocks have NO residual/skip path: with no gradient highway the encoder
    // cannot reduce its loss (Training_ShouldReduceLoss regressed 0.3818 → 0.6206) — the same
    // residual-free degeneracy fixed across the other ASR factories in #1208/#1221. That generic
    // builder is left untouched for the 12 undisclosed proprietary-API models that share it;
    // Chirp 3 has a published architecture, so it uses the paper-faithful Conformer factory.
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Chirp3-Native" : "Chirp3-ONNX", Description = "Chirp 3: latest Google Cloud multilingual ASR (2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Chirp3<T>(Architecture, mp, _options); return new Chirp3<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Chirp3<T>)); }
