...
public class OmnilangualASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Meta (2024): language-agnostic encoder processes speech, then language-adaptive
    /// projection heads and CTC decode multilingual speech including code-switching.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("OmnilangualASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "OmnilangualASR-Native" : "OmnilangualASR-ONNX", Description = "Omnilingual ASR: universal multilingual recognition (Meta, 2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new OmnilangualASR<T>(Architecture, mp, _options); return new OmnilangualASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(OmnilangualASR<T>)); }
