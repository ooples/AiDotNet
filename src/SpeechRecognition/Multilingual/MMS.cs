...
public class MMS<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Pratap et al. (2023): shared Transformer encoder with language-specific adapters
    /// processes speech, CTC head decodes per-frame tokens for 1100+ languages.
    /// </summary>
    public TranscriptionResult<T> Transcribe(Tensor<T> audio, string? language = null, bool includeTimestamps = false)
    {
        ...
        double duration = audio.Shape.Length > 0 ? (double)audio.Shape[^1] / SampleRate : 0;
    ...
    {
        // MMS uses language-specific adapter weights rather than a separate LID head.
        // Without the model's language identification logits, we return the detected
        // language with full confidence. A proper implementation requires exposing the
        // adapter activation scores from the ONNX model.
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("MMS does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "MMS-Native" : "MMS-ONNX", Description = "MMS: 1100+ language SSL + CTC (Meta, 2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new MMS<T>(Architecture, mp, _options); return new MMS<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(MMS<T>)); }
