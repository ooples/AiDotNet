...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "OWSM: Open Whisper-style Speech Models" (Peng et al., CMU, 2024)</item></list></para>
/// <para><b>For Beginners:</b> OWSM is an open-source reproduction of Whisper's training approach using publicly available data. It uses ESPnet's encoder-decoder architecture with a Conformer encoder and Transformer decoder, trained on a curated collection of open speech datase...</para>
/// <para>
...
public class OWSM<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Peng et al. (2024): Conformer encoder + Transformer decoder trained on open
    /// data reproduces Whisper's multilingual capabilities with full reproducibility.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "OWSM-Native" : "OWSM-ONNX", Description = "OWSM: open Whisper-style multilingual ASR (CMU, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new OWSM<T>(Architecture, mp, _options); return new OWSM<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(OWSM<T>)); }
