...
/// <example>
/// <code>
/// // Create a Code-Switching ASR model for mixed-language speech
...
///
/// // Or load a pre-trained ONNX model for code-mixed transcription
/// var onnxModel = new CodeSwitchingASR&lt;double&gt;(architecture, "codeswitching.onnx");
/// </code>
/// </example>
...
[ResearchPaper("End-to-End Speech Recognition for Code-Switching", "https://arxiv.org/abs/2011.13573")]
public class CodeSwitchingASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public CodeSwitchingASR(NeuralNetworkArchitecture<T> architecture, string modelPath, CodeSwitchingASROptions? options = null) : base(architecture) { _options = options ?? new CodeSwitchingASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "es", "hi" }; InitializeLayers(); }
    public CodeSwitchingASR(NeuralNetworkArchitecture<T> architecture, CodeSwitchingASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new CodeSwitchingASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "es", "hi" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes code-switched speech using a multilingual Conformer encoder.
    /// The shared encoder handles mixed-language utterances with a unified multilingual
    /// vocabulary. CTC decoding produces mixed-language transcriptions.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("CodeSwitchingASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "CodeSwitchingASR-Native" : "CodeSwitchingASR-ONNX", Description = "Code-Switching ASR: multilingual mixed-speech (2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new CodeSwitchingASR<T>(Architecture, mp, _options); return new CodeSwitchingASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(CodeSwitchingASR<T>)); }
