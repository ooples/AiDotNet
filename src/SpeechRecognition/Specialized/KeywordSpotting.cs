...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "Streaming Keyword Spotting on Mobile Devices" (2023)</item></list></para>
/// <para><b>For Beginners:</b> Keyword Spotting provides lightweight, always-on detection of specific keywords and voice commands. The model uses a compact encoder (4-layer Conformer with reduced dimensions) optimized for low-power continuous inference. Instead of full CTC voca...</para>
/// <para>
...
public partial class KeywordSpotting<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// <c>PostprocessOutput</c> is the identity. Here that vocabulary is the KEYWORD inventory rather
    /// than a full transcription alphabet (the option defaults to 100), but it is the same axis: the
    /// width is what the head projects to, not what the task is called.
    /// </remarks>
    ...
    public KeywordSpotting(NeuralNetworkArchitecture<T> architecture, KeywordSpottingOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new KeywordSpottingOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Detects keywords using a compact streaming Conformer encoder.
    /// The model processes audio in a sliding window and outputs confidence scores for
    /// predefined keywords using a lightweight CTC-like classification head.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "KeywordSpotting-Native" : "KeywordSpotting-ONNX", Description = "Keyword Spotting: lightweight wake-word detection (2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
