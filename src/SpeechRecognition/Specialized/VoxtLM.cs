...
public class VoxtLM<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "VoxtLM-Native" : "VoxtLM-ONNX", Description = "VoxtLM: unified decoder-only ASR/TTS (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
