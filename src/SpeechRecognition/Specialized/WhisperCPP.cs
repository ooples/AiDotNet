...
/// <example>
/// <code>
/// // Create a Whisper.cpp model for optimized CPU-based ASR
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 5000);
/// var model = new WhisperCPP&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for quantized Whisper inference
/// var onnxModel = new WhisperCPP&lt;double&gt;(architecture, "whispercpp.onnx");
/// </code>
/// </example>
...
public partial class WhisperCPP<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    public WhisperCPP(NeuralNetworkArchitecture<T> architecture, string modelPath, WhisperCPPOptions? options = null) : base(architecture) { _options = options ?? new WhisperCPPOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }
    public WhisperCPP(NeuralNetworkArchitecture<T> architecture, WhisperCPPOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new WhisperCPPOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using Whisper architecture with optimized inference.
    /// Whisper.cpp uses quantized encoder-decoder processing with hardware-specific
    /// acceleration for real-time CPU-based transcription.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("WhisperCPP does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "WhisperCPP-Native" : "WhisperCPP-ONNX", Description = "Whisper.cpp: optimized C++ Whisper inference (2022-2025)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new WhisperCPP<T>(Architecture, mp, _options); return new WhisperCPP<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(WhisperCPP<T>)); }
