...
public class ConformerTransducer<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "ConformerTransducer-Native" : "ConformerTransducer-ONNX", Description = "Conformer-Transducer: Streaming ASR (Gulati+Graves)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
