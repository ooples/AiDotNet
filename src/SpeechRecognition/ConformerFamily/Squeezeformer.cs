...
public class Squeezeformer<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per the paper: the encoder uses pre-norm blocks with progressive time downsampling
    /// in middle layers and upsampling at the end, enabling efficient computation.
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Squeezeformer-Native" : "Squeezeformer-ONNX", Description = "Squeezeformer: Efficient Transformer for ASR (Kim et al., 2022)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Squeezeformer<T>)); }
