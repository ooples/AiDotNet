...
/// <example>
/// <code>
/// // Create a FastEmit model for low-latency RNN-Transducer ASR
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 5000);
/// var model = new FastEmit&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for emission-regularized ASR
/// var onnxModel = new FastEmit&lt;double&gt;(architecture, "fastemit.onnx");
/// </code>
/// </example>
...
public class FastEmit<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Yu et al. (2021): the Conformer encoder processes audio, and the RNN-T decoder
    /// with emission regularization produces tokens with minimal emission latency.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => new FastEmitStreamingSession(this, language ?? _options.Language);
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "FastEmit-Native" : "FastEmit-ONNX", Description = "FastEmit: emission-regularized RNN-T (Google, 2021)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new FastEmit<T>(Architecture, mp, _options); return new FastEmit<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(FastEmit<T>)); }
    protected override void Dispose(bool disposing) { if (_disposed) return; if (disposing) OnnxEncoder?.Dispose(); _disposed = true; base.Dispose(disposing); }

    private sealed class FastEmitStreamingSession : IStreamingTranscriptionSession<T>
    {
        private readonly FastEmit<T> _model; private readonly string _language; private readonly List<Tensor<T>> _chunks = new(); private bool _disposed;
        public FastEmitStreamingSession(FastEmit<T> model, string language) { _model = model; _language = language; }
        public void FeedAudio(Tensor<T> audioChunk) { if (_disposed) throw new ObjectDisposedException(nameof(FastEmitStreamingSession)); _chunks.Add(audioChunk); }
        public TranscriptionResult<T> GetPartialResult() { if (_disposed) throw new ObjectDisposedException(nameof(FastEmitStreamingSession)); if (_chunks.Count == 0) return new TranscriptionResult<T> { Language = _language }; int totalLen = 0; foreach (var c in _chunks) totalLen += c.Length; var combined = new Tensor<T>(new[] { totalLen }); int offset = 0; foreach (var c in _chunks) { for (int i = 0; i < c.Length; i++) combined[offset + i] = c[i]; offset += c.Length; } return _model.Transcribe(combined, _language); }
        public TranscriptionResult<T> Finalize() { if (_disposed) throw new ObjectDisposedException(nameof(FastEmitStreamingSession)); var result = GetPartialResult(); _disposed = true; return result; }
