...
public class TDTDecoder<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "TDTDecoder-Native" : "TDTDecoder-ONNX", Description = "TDT: token-and-duration transducer (NVIDIA, 2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
