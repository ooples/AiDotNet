...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "Moonshine: Speech Recognition for Live Transcription and Voice Commands" (Useful Sensors, 2024)</item></list></para>
/// <para><b>For Beginners:</b> Moonshine is designed for real-time on-device speech recognition with minimal latency. It uses a compact encoder-decoder architecture with rotary position embeddings (RoPE) instead of absolute positional encodings, enabling efficient processing of...</para>
/// <para>
...
public class Moonshine<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    /// Per Useful Sensors (2024): audio chunks are processed by the RoPE-based encoder,
    /// and the decoder generates text with cross-attention at 5x Whisper speed.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => new MoonshineStreamingSession(this, language ?? _options.Language);
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "Moonshine-Native" : "Moonshine-ONNX", Description = "Moonshine: efficient real-time ASR (Useful Sensors, 2024)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Moonshine<T>(Architecture, mp, _options); return new Moonshine<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Moonshine<T>)); }
    protected override void Dispose(bool disposing) { if (_disposed) return; if (disposing) OnnxEncoder?.Dispose(); _disposed = true; base.Dispose(disposing); }

    private sealed class MoonshineStreamingSession : IStreamingTranscriptionSession<T>
    {
        private readonly Moonshine<T> _model; private readonly string _language; private readonly List<Tensor<T>> _chunks = new(); private bool _disposed;
        public MoonshineStreamingSession(Moonshine<T> model, string language) { _model = model; _language = language; }
        public void FeedAudio(Tensor<T> audioChunk) { if (_disposed) throw new ObjectDisposedException(nameof(MoonshineStreamingSession)); _chunks.Add(audioChunk); }
        ...
        public TranscriptionResult<T> Finalize() { if (_disposed) throw new ObjectDisposedException(nameof(MoonshineStreamingSession)); var result = GetPartialResult(); _disposed = true; return result; }
