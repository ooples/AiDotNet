...
/// <remarks>
/// <para><b>References:</b>
/// <list type="bullet"><item>Paper: "Learning Audio-Visual Speech Representation by Masked Multimodal Cluster Prediction" (Shi et al., Meta, 2022)</item></list></para>
/// <para><b>For Beginners:</b> AV-HuBERT extends HuBERT to jointly learn from audio and visual (lip) modalities. The model uses separate audio (CNN) and visual (ResNet) feature encoders followed by a shared Transformer that learns multimodal representations. During pre-training...</para>
/// <para>
...
public partial class AVHuBERT<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public AVHuBERT(NeuralNetworkArchitecture<T> architecture, string modelPath, AVHuBERTOptions? options = null) : base(architecture) { _options = options ?? new AVHuBERTOptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayersCore(); }
    ...
    /// Per Shi et al. (2022): although trained on audio-visual data, the encoder processes
    /// audio-only input using representations enriched by cross-modal pre-training.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("AVHuBERT does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "AVHuBERT-Native" : "AVHuBERT-ONNX", Description = "AV-HuBERT: audio-visual pre-trained + CTC (Meta, 2022)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(AVHuBERT<T>)); }
