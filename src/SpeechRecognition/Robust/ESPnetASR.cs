...
/// <example>
/// <code>
/// // Create an ESPnet ASR model for end-to-end speech processing
/// var architecture = new NeuralNetworkArchitecture&lt;double&gt;(
///     inputType: InputType.OneDimensional,
///     taskType: NeuralNetworkTaskType.Classification,
///     inputHeight: 16000, inputWidth: 1, inputDepth: 1, outputSize: 5000);
/// var model = new ESPnetASR&lt;double&gt;(architecture);
///
/// // Or load a pre-trained ONNX model for Conformer/Branchformer ASR
/// var onnxModel = new ESPnetASR&lt;double&gt;(architecture, "espnetasr.onnx");
/// </code>
/// </example>
...
public partial class ESPnetASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    public ESPnetASR(NeuralNetworkArchitecture<T> architecture, string modelPath, ESPnetASROptions? options = null) : base(architecture) { _options = options ?? new ESPnetASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public ESPnetASR(NeuralNetworkArchitecture<T> architecture, ESPnetASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new ESPnetASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using ESPnet's Conformer encoder with joint CTC/attention.
    /// Per Watanabe et al. (2024): the model uses joint CTC/attention training for robust
    /// decoding with CTC greedy search as the primary output.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("ESPnetASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "ESPnetASR-Native" : "ESPnetASR-ONNX", Description = "ESPnet: Conformer + joint CTC/attention (2024)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new ESPnetASR<T>(Architecture, mp, _options); return new ESPnetASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(ESPnetASR<T>)); }
