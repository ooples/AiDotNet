...
///
/// // Or load a pre-trained ONNX model for noisy environment ASR
/// var onnxModel = new NoiseRobustASR&lt;double&gt;(architecture, "noiserobust.onnx");
/// </code>
/// </example>
...
public class NoiseRobustASR<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public NoiseRobustASR(NeuralNetworkArchitecture<T> architecture, string modelPath, NoiseRobustASROptions? options = null) : base(architecture) { _options = options ?? new NoiseRobustASROptions(); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en" }; InitializeLayers(); }
    public NoiseRobustASR(NeuralNetworkArchitecture<T> architecture, NoiseRobustASROptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new NoiseRobustASROptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en" }; InitializeLayers(); }

    /// <summary>
    /// Transcribes audio using noise-robust Conformer encoder with CTC decoding.
    /// The model uses multi-condition training and noise-aware preprocessing to handle
    /// babble, traffic, music, and reverberation at low SNR levels.
    /// </summary>
    ...
    public IStreamingTranscriptionSession<T> StartStreamingSession(string? language = null) => throw new NotSupportedException("NoiseRobustASR does not support streaming.");
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "NoiseRobustASR-Native" : "NoiseRobustASR-ONNX", Description = "Noise-Robust ASR: multi-condition Conformer (2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new NoiseRobustASR<T>(Architecture, mp, _options); return new NoiseRobustASR<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(NoiseRobustASR<T>)); }
