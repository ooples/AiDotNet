...
public class WavLMRobust<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "WavLMRobust-Native" : "WavLMRobust-ONNX", Description = "WavLM-Robust: denoising pre-trained + CTC (Microsoft, 2022)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers, AdditionalInfo = BaseAudioMetadataInfo() };
