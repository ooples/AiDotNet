...
public partial class WhisperX<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public TranscriptionResult<T> Transcribe(Tensor<T> audio, string? language = null, bool? includeTimestamps = null)
    {
        ...
        return new TranscriptionResult<T>
        {
            ...
            Segments = ResolveReturnTimestamps(includeTimestamps) ? ExtractAlignedSegments(text, duration, confidence) : Array.Empty<TranscriptionSegment<T>>()
        };
    }
    ...
    private IReadOnlyList<TranscriptionSegment<T>> ExtractAlignedSegments(string text, double duration, double confidence) { if (string.IsNullOrWhiteSpace(text)) return Array.Empty<TranscriptionSegment<T>>(); var words = text.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries); if (words.Length == 0) return Array.Empty<TranscriptionSegment<T>>(); var segments = new List<TranscriptionSegment<T>>(); double timePerWord = duration / words.Length; for (int i = 0; i < words.Length; i++) { segments.Add(new TranscriptionSegment<T> { Text = words[i], StartTime = i * timePerWord, EndTime = (i + 1) * timePerWord, Confidence = NumOps.FromDouble(confidence) }); } return segments; }
