...
public partial class FasterWhisper<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    /// <inheritdoc />
    /// <remarks>
    ...
    public FasterWhisper(NeuralNetworkArchitecture<T> architecture, string modelPath, FasterWhisperOptions? options = null) : base(architecture) { _options = options is null ? new FasterWhisperOptions() : new FasterWhisperOptions(options); _useNativeMode = false; base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path required.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxEncoder = new OnnxModel<T>(modelPath, _options.OnnxOptions); SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr", "pl", "ca", "nl", "ar", "sv", "it", "id", "hi", "fi", "vi", "he", "uk", "el", "ms", "cs", "ro", "da", "hu", "ta", "no", "th", "ur", "hr", "bg", "lt", "la" }; InitializeLayers(); }
    public FasterWhisper(NeuralNetworkArchitecture<T> architecture, FasterWhisperOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options is null ? new FasterWhisperOptions() : new FasterWhisperOptions(options); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.SampleRate = _options.SampleRate; base.NumMels = _options.NumMels; SupportedLanguages = new[] { "en", "zh", "de", "es", "ru", "ko", "fr", "ja", "pt", "tr", "pl", "ca", "nl", "ar", "sv", "it", "id", "hi", "fi", "vi", "he", "uk", "el", "ms", "cs", "ro", "da", "hu", "ta", "no", "th", "ur", "hr", "bg", "lt", "la" }; InitializeLayers(); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { var options = new FasterWhisperOptions(_options); if (!_useNativeMode && options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new FasterWhisper<T>(Architecture, mp, options); return new FasterWhisper<T>(Architecture, options); }
