...
public partial class FasterWhisper<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public FasterWhisper(NeuralNetworkArchitecture<T> architecture, FasterWhisperOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options is null ? new FasterWhisperOptions() : new FasterWhisperOptions(options); _useNativeMode = true; _optimizer = optimizer
