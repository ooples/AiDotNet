...
public class WhisperTimestamped<T> : AudioNeuralNetworkBase<T>, ISpeechRecognizer<T>
{
    ...
    public TranscriptionResult<T> Transcribe(Tensor<T> audio, string? language = null, bool includeTimestamps = false)
    {
        ...
        return new TranscriptionResult<T>
        {
            ...
            Segments = includeTimestamps ? ExtractCrossAttentionTimestamps(tokens, text, duration, confidence) : Array.Empty<TranscriptionSegment<T>>()
        };
    }
    ...
    public override ModelMetadata<T> GetModelMetadata() => new() { Name = _useNativeMode ? "WhisperTimestamped-Native" : "WhisperTimestamped-ONNX", Description = "WhisperTimestamped: cross-attention word timestamps (Louradour, 2023)", FeatureCount = _options.NumMels, Complexity = _options.NumEncoderLayers + _options.NumDecoderLayers, AdditionalInfo = new Dictionary<string, object> { { "ModelName", "WhisperTimestamped" }, { "Mode", _useNativeMode ? "Native" : "ONNX" }, { "SampleRate", _options.SampleRate }, { "NumMels", _options.NumMels }, { "EncoderDim", _options.EncoderDim }, { "DecoderDim", _options.DecoderDim }, { "NumEncoderLayers", _options.NumEncoderLayers }, { "NumDecoderLayers", _options.NumDecoderLayers }, { "NumAttentionHeads", _options.NumAttentionHeads }, { "VocabSize", _options.VocabSize }, { "Language", _options.Language }, { "MinWordConfidence", _options.MinWordConfidence } }, ModelData = SerializeForMetadata() };
    ...
    private IReadOnlyList<TranscriptionSegment<T>> ExtractCrossAttentionTimestamps(List<int> tokens, string text, double duration, double confidence) { if (string.IsNullOrWhiteSpace(text) || tokens.Count == 0) return Array.Empty<TranscriptionSegment<T>>(); var words = text.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries); if (words.Length == 0) return Array.Empty<TranscriptionSegment<T>>(); var segments = new List<TranscriptionSegment<T>>(); double timePerWord = duration / words.Length; for (int i = 0; i < words.Length; i++) { if (confidence >= _options.MinWordConfidence) { segments.Add(new TranscriptionSegment<T> { Text = words[i], StartTime = i * timePerWord, EndTime = (i + 1) * timePerWord, Confidence = NumOps.FromDouble(confidence) }); } } return segments; }
