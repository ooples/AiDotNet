...
public static class OnnxTensorConverter
{
    ...
    public static OnnxTensors.DenseTensor<float> ToOnnxFloat<T>(Tensor<T> tensor)
    {
        ...
        var shape = tensor._shape.Select(d => (long)d).ToArray();
        var onnxTensor = new OnnxTensors.DenseTensor<float>(tensor._shape);
    ...
    {
        var numOps = MathHelper.GetNumericOperations<T>();
        var onnxTensor = new OnnxTensors.DenseTensor<double>(tensor._shape);
    ...
    {
        var numOps = MathHelper.GetNumericOperations<T>();
        var onnxTensor = new OnnxTensors.DenseTensor<long>(tensor._shape);
    ...
    {
        var numOps = MathHelper.GetNumericOperations<T>();
        var onnxTensor = new OnnxTensors.DenseTensor<int>(tensor._shape);
    ...
    public static OnnxTensors.DenseTensor<float> ToOnnxFloatWithBatch<T>(Tensor<T> tensor)
    {
        ...
        batchShape[0] = 1;
        Array.Copy(tensor._shape, 0, batchShape, 1, tensor.Shape.Length);
