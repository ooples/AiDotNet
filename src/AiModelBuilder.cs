        ...
        for (int epoch = 0; epoch < epochs; epoch++)
        {
            ...
            await foreach (var (inputs, outputs) in streamingLoader.GetBatchesAsync(shuffle: true))
            {
                ...
                for (int i = 0; i < inputs.Length; i++)
                {
                    ...
                    var gradients = ((IGradientComputable<T, TInput, TOutput>)_model).ComputeGradients(processedInput, target, lossFunction);

                    // Apply gradients with current learning rate
                    ((IGradientComputable<T, TInput, TOutput>)_model).ApplyGradients(gradients, learningRate);
...
#pragma warning restore CS8604
        ...
        else if (!((IParameterizable<T, TInput, TOutput>)model).SupportsParameterInitialization)
        {
...
#pragma warning restore CS1998
        ...
        TInput x, TOutput y)
    {
        ...
        var problemType = _autoMLOptions?.TaskFamilyOverride switch
        {
                ...
                AutoMLTaskFamily.MultiLabelClassification => "classification",
                AutoMLTaskFamily.TextClassification => "classification",
                AutoMLTaskFamily.ImageClassification => "classification",
                AutoMLTaskFamily.GraphNodeClassification => "classification",
                AutoMLTaskFamily.GraphClassification => "classification",
                AutoMLTaskFamily.PointCloudClassification => "classification",
                AutoMLTaskFamily.VolumetricClassification => "classification",
                AutoMLTaskFamily.MeshClassification => "classification",
                ...
                AutoMLTaskFamily.Recommendation => "ranking",
                AutoMLTaskFamily.ObjectDetection => "detection",
                AutoMLTaskFamily.ThreeDObjectDetection => "detection",
                AutoMLTaskFamily.ImageSegmentation => "segmentation",
                AutoMLTaskFamily.PointCloudSegmentation => "segmentation",
                AutoMLTaskFamily.VolumetricSegmentation => "segmentation",
                AutoMLTaskFamily.MeshSegmentation => "segmentation",
                AutoMLTaskFamily.SequenceTagging => "sequence_labeling",
                AutoMLTaskFamily.Translation => "sequence_to_sequence",
                AutoMLTaskFamily.TextGeneration => "generation",
                AutoMLTaskFamily.SpeechRecognition => "speech",
                AutoMLTaskFamily.ReinforcementLearning => "reinforcement_learning",
                AutoMLTaskFamily.GraphLinkPrediction => "link_prediction",
                AutoMLTaskFamily.GraphGeneration => "generation",
                AutoMLTaskFamily.PointCloudCompletion => "generation",
                AutoMLTaskFamily.RadianceFieldReconstruction => "reconstruction",
                AutoMLTaskFamily.DepthEstimation => "regression",
                AutoMLTaskFamily.Regression => "regression",
                _ => "regression"
            };
        ...
        IEnumerable<TInput>? calibrationData = null)
    {
        ...
        if (internalConfig.UseQuantizationAwareTraining)
        {
            try
            {
                ...
                model = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(qatConditionedParams);
        ...
        var quantizedParameters = ((IParameterizable<T, TInput, TOutput>)quantizedModel).GetParameters();
