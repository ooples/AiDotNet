...
/// <summary>
/// Interface for models that are quantized.
/// </summary>
/// <remarks>
/// <para>
/// This interface should be implemented by models that have been quantized.
/// </para>
/// <para><b>For Beginners:</b> This marks a model as being quantized.
/// 
/// A quantized model:
/// - Uses lower precision for its internal parameters
/// - Knows how to dequantize during inference
/// - May have special methods for serialization and deserialization
/// </para>
/// </remarks>
    ...
    where T : unmanaged
{
    ...
    /// <remarks>
    /// <para>
    /// The bit width determines the precision of the quantized parameters.
    /// Common values are 8, 4, or 16 bits.
    /// </para>
    /// <para><b>For Beginners:</b> This tells how many bits each number uses.
    /// 
    /// For example:
    /// - 8 bits means each number uses only 8 bits instead of 32
    ...
    /// <summary>
    /// Gets a value indicating whether mixed precision was used.
    /// </summary>
    /// <remarks>
    /// <para>
    /// When true, different parameters in the model may use different bit widths.
    /// </para>
    /// <para><b>For Beginners:</b> This tells if different parts use different precision.
    /// 
    /// Mixed precision means:
    /// - Some parts of the model might use 8 bits
    /// - Other parts might use 4 or 16 bits
    /// - This optimizes the accuracy-size tradeoff
    /// </para>
    /// </remarks>
    bool UseMixedPrecision { get; }

    /// <summary>
    /// Gets statistics about the weight distribution before and after quantization.
    /// </summary>
    /// <remarks>
    /// <para>
    /// These statistics provide insight into how the quantization affected the
    /// distribution of weight values.
    /// </para>
    /// <para><b>For Beginners:</b> This gives information about the quantization effects.
    /// 
    /// These statistics show:
    /// - The range of values before and after quantization
    /// - How many outliers were clipped
    /// - How the distribution of values changed
    /// </para>
    /// </remarks>
    WeightDistributionStatistics? WeightDistributionStatistics { get; }

    /// <summary>
    /// Serializes the quantized model to a binary writer.
    /// </summary>
    /// <param name="writer">The binary writer to write to.</param>
    /// <remarks>
    /// <para>
    /// This method writes the quantized model's data to a binary stream in a format
    /// that preserves the quantization information.
    /// </para>
    /// <para><b>For Beginners:</b> This saves the quantized model to a stream.
    /// 
    /// The method efficiently writes:
    /// - The quantized values themselves
    /// - The quantization parameters needed to interpret them
    /// - Any model-specific configuration
    /// </para>
    /// </remarks>
    void SerializeQuantized(BinaryWriter writer);
}
