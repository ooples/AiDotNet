...
///     .ConfigureModel(new NeuralNetworkARIMAModel&lt;double&gt;(options))
...
public partial class NeuralNetworkARIMAModel<T> : TimeSeriesModelBase<T>
{
    ...
    private NeuralNetwork<T> CreateDefaultNeuralNetwork()
    {
        ...
        var hiddenLayer = new DenseLayer<T>(hiddenSize, activationFunction: reluActivation);
        var outputLayer = new DenseLayer<T>(outputSize, activationFunction: linearActivation);
