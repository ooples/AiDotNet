...
[ModelPaper("SDXL: Improving Latent Diffusion Models for High-Resolution Image Synthesis", "https://arxiv.org/abs/2307.01952", Year = 2023, Authors = "Podell et al.")]
public class SDXLModel<T> : LatentDiffusionModelBase<T>
{
    ...
    private Tensor<T> ApplyMicroCondition(Tensor<T> embedding, Tensor<T> microCond)
    {
        ...
        var shape = embedding.Shape._dims;
        var batch = shape[0];
        var seqLen = shape[1];
...
public class SDXLRefiner<T>
{
        ...
        int? seed = null)
    {
        ...
        var noise = SampleNoise(latent.Shape._dims, rng);
        ...
        foreach (var timestep in _scheduler.Timesteps.Skip(startStep))
        {
            ...
            latent = new Tensor<T>(latent.Shape._dims, latentVector);
        }
