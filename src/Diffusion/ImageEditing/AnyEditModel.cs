...
/// var model = new AnyEditModel&lt;float&gt;(options);
...
public class AnyEditModel<T> : LatentDiffusionModelBase<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        m.SetProperty("architecture", "instructpix2pix-sd15-editing");
        m.SetProperty("base_model", "Stable Diffusion 1.5");
        m.SetProperty("text_encoder", "CLIP ViT-L/14");
        m.SetProperty("context_dim", SD15_CONTEXT_DIM);
        m.SetProperty("latent_channels", LATENT_CHANNELS);
        m.SetProperty("default_guidance_scale", DEFAULT_GUIDANCE);
        m.SetProperty("edit_types", "local, global, style, action, visual-concept");
        return m;
    }
}
