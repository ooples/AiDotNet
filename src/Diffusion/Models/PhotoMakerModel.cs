...
public class PhotoMakerModel<T> : LatentDiffusionModelBase<T>
{
    ...
    {
        var m = new ModelMetadata<T> { Name = "PhotoMaker", Version = "2.0", ModelType = ModelType.NeuralNetwork,
            Description = "PhotoMaker identity-customized photo generation", FeatureCount = ParameterCount, Complexity = ParameterCount };
        m.SetProperty("architecture", "sdxl-stacked-id-embedding");
        m.SetProperty("identity_encoder", "CLIP-ViT-L");
