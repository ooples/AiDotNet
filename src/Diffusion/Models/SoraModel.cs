...
/// <summary>
/// Sora-architecture model — DiT-based video generation with spatiotemporal patches.
/// </summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks>
/// <para>
/// Sora uses a Diffusion Transformer (DiT) operating on spatiotemporal patches of video,
/// enabling variable-duration, resolution, and aspect-ratio video generation.
/// </para>
/// <para>
/// <b>For Beginners:</b> Sora creates videos from text using a transformer architecture:
///
/// Key characteristics:
/// - DiT backbone (no U-Net) with 3D spatiotemporal patches
/// - Native variable duration and resolution support
...
public class SoraModel<T> : VideoDiffusionModelBase<T>
{
    ...
    {
        var m = new ModelMetadata<T> { Name = "Sora", Version = "1.0", ModelType = ModelType.NeuralNetwork,
            Description = "Sora-architecture DiT video generation model", FeatureCount = ParameterCount, Complexity = ParameterCount };
        m.SetProperty("architecture", "dit-spatiotemporal");
        m.SetProperty("hidden_dim", HIDDEN_DIM);
        m.SetProperty("num_layers", NUM_LAYERS);
        m.SetProperty("training_objective", "flow-matching");
        return m;
    }
}
