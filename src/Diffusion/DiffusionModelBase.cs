...
public abstract class DiffusionModelBase<T> : IDiffusionModel<T>, IConfigurableModel<T>, IModelShape
{
    ...
    public virtual T ComputeLoss(Tensor<T> cleanSamples, Tensor<T> noise, int[] timesteps)
    {
        ...
        var noisySampleTensor = new Tensor<T>(cleanSamples.Shape._dims, noisySample);
    ...
    public virtual Tensor<T> Predict(Tensor<T> input)
    {
        ...
        return Generate(input.Shape._dims, _options.DefaultInferenceSteps, seed);
    }
    ...
    public virtual Vector<T> ComputeGradients(Tensor<T> input, Tensor<T> target, ILossFunction<T>? lossFunction = null)
    {
        ...
        var noisySampleTensor = new Tensor<T>(input.Shape._dims, noisySample);
