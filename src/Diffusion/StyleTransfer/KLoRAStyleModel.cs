...
/// var model = new KLoRAStyleModel&lt;float&gt;(options);
/// var input = Tensor&lt;float&gt;.Random(new[] { 1, 4, 64, 64 });
/// var blended = model.Predict(input);
/// </code>
/// </example>
...
public class KLoRAStyleModel<T> : LatentDiffusionModelBase<T>
{
    ...
    {
        var clone = new KLoRAStyleModel<T>(conditioner: _conditioner, seed: RandomGenerator.Next());
