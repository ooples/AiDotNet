...
/// character window (<c>x</c>) and the next character (<c>y</c>).
/// </para>
...
public sealed class LeafShakespeareFederatedDatasetLoader
{
        ...
        bool validateDeclaredSamples)
    {
        ...
        var windows = new string[sampleCount];
        var nextChars = new string[sampleCount];

        for (int i = 0; i < sampleCount; i++)
        {
            windows[i] = ReadRequiredString(userId, "x", xArray[i]);

            var label = ReadRequiredString(userId, "y", yArray[i]);
            if (label.Length != 1)
            {
                throw new InvalidDataException($"LEAF Shakespeare user '{userId}' labels must be single-character strings.");
            }

            nextChars[i] = label;
        }

        return new FederatedClientDataset<string[], string[]>(windows, nextChars, sampleCount);
    }
    ...
    private static string ReadRequiredString(string userId, string fieldName, JToken token)
    {
        ...
        {
            throw new InvalidDataException($"LEAF Shakespeare user '{userId}' {fieldName} values must be strings.");
        }
