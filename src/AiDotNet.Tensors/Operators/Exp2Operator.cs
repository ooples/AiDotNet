...
/// <summary>
/// Implements the exponential function base 2 (2^x) using hardware-accelerated SIMD instructions.
/// </summary>
/// <remarks>
/// <para>
/// This operator provides optimized implementations of exp2(x) for:
/// - Scalar float/double (using Math.Pow(2.0, x) for accuracy)
...
/// </para>
/// </remarks>
public readonly struct Exp2OperatorDouble : IUnaryOperator<double, double>
{
    /// <summary>
    /// Computes 2^x for a single double value.
    /// </summary>
    public double Invoke(double x) => Math.Pow(2.0, x);

#if NET5_0_OR_GREATER
    /// <summary>
    /// Computes 2^x for a Vector128 of doubles (2 values).
    /// </summary>
    /// <remarks>
    ...
    public Vector128<double> Invoke(Vector128<double> x)
    {
        ...
        {
            values[i] = Math.Pow(2.0, values[i]);
        }
    ...
    /// <summary>
    /// Computes 2^x for a Vector256 of doubles (4 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes 2^x for a Vector512 of doubles (8 values).
    /// </summary>
...
/// <summary>
/// Implements the exponential function base 2 (2^x) for single-precision floats using hardware-accelerated SIMD instructions.
/// </summary>
public readonly struct Exp2OperatorFloat : IUnaryOperator<float, float>
{
    /// <summary>
    /// Computes 2^x for a single float value.
    /// </summary>
    public float Invoke(float x) => MathF.Pow(2.0f, x);

#if NET5_0_OR_GREATER
    /// <summary>
    /// Computes 2^x for a Vector128 of floats (4 values).
    /// </summary>
    public Vector128<float> Invoke(Vector128<float> x)
    {
        ...
        for (int i = 0; i < values.Length; i++)
        {
            values[i] = MathF.Pow(2.0f, values[i]);
        }
    ...
    /// <summary>
    /// Computes 2^x for a Vector256 of floats (8 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes 2^x for a Vector512 of floats (16 values).
    /// </summary>
