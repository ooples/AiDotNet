...
/// <summary>
/// Implements the inverse sine function (arcsin(x)) using hardware-accelerated SIMD instructions.
/// </summary>
/// <remarks>
/// <para>
/// This operator provides optimized implementations of asin(x) for:
/// - Scalar float/double (using Math.Asin for accuracy)
...
/// <para>
/// <b>Algorithm:</b>
/// Uses scalar Math.Asin for accurate computation. SIMD implementations use
/// scalar fallback with per-element Math.Asin for reliability and numerical stability.
/// </para>
/// </remarks>
public readonly struct AsinOperatorDouble : IUnaryOperator<double, double>
{
    /// <summary>
    /// Computes asin(x) for a single double value.
    /// </summary>
    public double Invoke(double x) => Math.Asin(x);

#if NET5_0_OR_GREATER
    /// <summary>
    /// Computes asin(x) for a Vector128 of doubles (2 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes asin(x) for a Vector256 of doubles (4 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes asin(x) for a Vector512 of doubles (8 values).
    /// </summary>
...
/// <summary>
/// Implements the inverse sine function (arcsin(x)) for single-precision floats using hardware-accelerated SIMD instructions.
/// </summary>
/// <remarks>
/// <para>
/// Provides the same functionality as <see cref="AsinOperatorDouble"/> but optimized for single-precision (float) values.
...
/// </para>
/// </remarks>
public readonly struct AsinOperatorFloat : IUnaryOperator<float, float>
{
    /// <summary>
    /// Computes asin(x) for a single float value.
    /// </summary>
    public float Invoke(float x) => MathF.Asin(x);

#if NET5_0_OR_GREATER
    /// <summary>
    /// Computes asin(x) for a Vector128 of floats (4 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes asin(x) for a Vector256 of floats (8 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes asin(x) for a Vector512 of floats (16 values).
    /// </summary>
