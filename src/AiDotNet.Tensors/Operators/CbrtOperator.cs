...
/// <summary>
/// Implements the cube root function (cbrt(x)) using hardware-accelerated SIMD instructions.
/// </summary>
public readonly struct CbrtOperatorDouble : IUnaryOperator<double, double>
{
    /// <summary>
    ...
    /// <summary>
    /// Computes cbrt(x) for a Vector128 of doubles (2 values).
    /// </summary>
    public Vector128<double> Invoke(Vector128<double> x)
    {
        // Scalar fallback - can be optimized with Newton-Raphson iterations
        Span<double> values = stackalloc double[Vector128<double>.Count];
        x.CopyTo(values);

        for (int i = 0; i < values.Length; i++)
        {
            values[i] = Math.Cbrt(values[i]);
        }
    ...
    /// <summary>
    /// Computes cbrt(x) for a Vector256 of doubles (4 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes cbrt(x) for a Vector512 of doubles (8 values).
    /// </summary>
...
/// <summary>
/// Implements the cube root function (cbrt(x)) for single-precision floats using hardware-accelerated SIMD instructions.
/// </summary>
public readonly struct CbrtOperatorFloat : IUnaryOperator<float, float>
{
    /// <summary>
...
#if NET5_0_OR_GREATER
    /// <summary>
    /// Computes cbrt(x) for a Vector128 of floats (4 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes cbrt(x) for a Vector256 of floats (8 values).
    /// </summary>
    ...
    /// <summary>
    /// Computes cbrt(x) for a Vector512 of floats (16 values).
    /// </summary>
