...
// OpenCL kernels for octonion neural network operations.
// Implements 8-component non-associative algebra with full forward and backward support.

namespace AiDotNet.Tensors.Engines.DirectGpu.OpenCL.Kernels;

/// <summary>
/// OpenCL kernels for octonion algebra operations used in octonion neural networks.
/// Implements the Cayley-Dickson construction for octonion multiplication.
/// </summary>
internal static class OctonionKernels
{
...
inline void octonion_multiply(float* a, float* b, float* r) {
    // Real part: e0
...
inline void octonion_conjugate(float* a, float* r) {
    r[0] = a[0];
    for (int i = 1; i < 8; i++) {
        r[i] = -a[i];
    }
...
inline float octonion_norm_sq(float* a) {
    float sum = 0.0f;
    for (int i = 0; i < 8; i++) {
        sum += a[i] * a[i];
    }
...
// Octonion addition
inline void octonion_add(float* a, float* b, float* r) {
    for (int i = 0; i < 8; i++) {
        r[i] = a[i] + b[i];
    }
}
...
inline void octonion_scale(float* a, float s, float* r) {
    for (int i = 0; i < 8; i++) {
        r[i] = a[i] * s;
    }
}
...
inline void octonion_multiply_jacobian_a(float* b, float* jac) {
...
// This is an 8x8 matrix stored row-major
inline void octonion_multiply_jacobian_b(float* a, float* jac) {
...
__kernel void octonion_linear_forward(
    ...
    const int batch, const int inputFeatures, const int outputFeatures)
{
...
__kernel void octonion_linear_backward_input(
...
__kernel void octonion_linear_backward_weights(
    __global const float* gradOutput,
    __global const float* input,
...
__kernel void octonion_linear_backward_biases(
    __global const float* gradOutput,
    __global float* gradBiases,
    const int batch, const int outputFeatures)
{
    int gid = get_global_id(0);
...
__kernel void octonion_multiply_backward(
    __global const float* gradOutput,
...
__kernel void octonion_conjugate_backward(
    __global const float* gradOutput,
    __global float* gradInput,
    const int count)
{
...
__kernel void octonion_norm_backward(
    __global const float* gradOutput,
    __global const float* input,
    __global float* gradInput,
    const int count)
...
__kernel void octonion_split_relu_forward(
    __global const float* input,
    __global float* output,
    const int count)
{
    ...
    float val = input[gid];
    output[gid] = fmax(0.0f, val);
}

__kernel void octonion_split_relu_backward(
    __global const float* gradOutput,
    __global const float* input,
    __global float* gradInput,
    const int count)
...
__kernel void octonion_modulus_relu_forward(
    __global const float* input,
    __global float* output,
    const int count)
{
    ...
    // Apply ReLU to norm
    float activatedNorm = fmax(0.0f, norm);
...
__kernel void octonion_modulus_relu_backward(
    __global const float* gradOutput,
    __global const float* input,
    __global float* gradInput,
    const int count)
