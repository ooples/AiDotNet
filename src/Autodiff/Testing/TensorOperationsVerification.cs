...
public class TensorOperationsVerification<T>
{
        ...
        string operationName)
    {
        ...
        using (var tape = new GradientTape<T>())
        {
            ...
            var outputGradient = CreateOnes(outputNode.Value.Shape._dims);
            outputNode.Gradient = outputGradient;

            // Run backward pass
            RunBackward(outputNode);

            autodiffGradient = inputNode.Gradient ?? new Tensor<T>(input.Shape._dims);
        }

        // Compute numerical gradient
        var outputGrad = CreateOnes(input.Shape._dims);
            ...
            string operationName)
    {
        ...
        using (var tape = new GradientTape<T>())
        {
            ...
            autodiffGrad1 = node1.Gradient ?? new Tensor<T>(input1.Shape._dims);
            autodiffGrad2 = node2.Gradient ?? new Tensor<T>(input2.Shape._dims);
        }

        // Compute numerical gradients
        var outputGrad = CreateOnes(input1.Shape._dims);
