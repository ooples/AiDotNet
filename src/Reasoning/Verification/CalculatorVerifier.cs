...
public class CalculatorVerifier<T> : IExternalToolVerifier<T>
{
    ...
    public bool CanVerify(ReasoningStep<T> step)
    {
        ...
        return content.Contains("calculate") ||
               ...
               content.Contains("×") ||
               content.Contains("÷") ||
               Regex.IsMatch(content, @"\d+\s*[\+\-\*/]\s*\d+") || // arithmetic expressions
               Regex.IsMatch(content, @"\d+%") || // percentages
               Regex.IsMatch(content, @"(?:sum|product|quotient|difference)");
    }
    ...
    private List<(string expression, string result)> ExtractMathematicalExpressions(string text)
    {
        ...
        // Pattern 1: "X = Y" style
        var equalsMatches = Regex.Matches(text, @"([0-9\.\+\-\*/\(\)\^%\s×÷]+)\s*=\s*([0-9\.]+)");
        ...
        // Pattern 2: "X% of Y"
        var percentMatches = Regex.Matches(text, @"([0-9\.]+)%\s+of\s+([0-9\.]+)");
        foreach (Match match in percentMatches)
        {
            ...
            var resultMatch = Regex.Match(text, $@"{Regex.Escape(match.Value)}[^\d]*([0-9\.]+)");
    ...
    private string EvaluatePowerOperations(string expression)
    {
        ...
        {
            var match = Regex.Match(expression, @"([0-9\.]+)\s*\*\*\s*([0-9\.]+)");
