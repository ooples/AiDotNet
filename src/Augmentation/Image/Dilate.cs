...
/// <summary>
/// Morphological dilation - expands bright regions using a structuring element.
/// </summary>
/// <typeparam name="T">The numeric type for calculations.</typeparam>
public class Dilate<T> : ImageAugmenterBase<T>
{
    public int KernelSize { get; }
    public int Iterations { get; }

    public Dilate(int kernelSize = 3, int iterations = 1,
            ...
            for (int c = 0; c < data.Channels; c++)
                for (int y = 0; y < data.Height; y++)
                    for (int x = 0; x < data.Width; x++)
                    {
                        ...
                        result.SetPixel(y, x, c, NumOps.FromDouble(maxVal));
                    }
        }
