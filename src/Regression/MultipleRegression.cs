...
public class MultipleRegression<T> : RegressionModelBase<T>
{
	/// <summary>
	...
	public override void Train(Matrix<T> x, Vector<T> y)
	{
		// Validate inputs
		if (x == null || y == null)
			throw new ArgumentNullException(x == null ? nameof(x) : nameof(y), "Input data cannot be null");

		if (x.Rows != y.Length)
			throw new ArgumentException($"Number of rows in X ({x.Rows}) must match length of y ({y.Length})");

		if (x.Rows == 0 || x.Columns == 0)
			throw new ArgumentException("Input data matrix cannot be empty");

		// Store original feature count for reference during prediction
		int originalFeatureCount = x.Columns;

		// Create a working copy of the design matrix
		Matrix<T> designMatrix = x;

		// Add intercept column if needed
		if (Options.UseIntercept)
			designMatrix = designMatrix.AddConstantColumn(NumOps.One);

		// Perform normal equation calculations    
		var xTx = designMatrix.Transpose().Multiply(designMatrix);
		var regularizedXTx = xTx.Add(Regularization.Regularize(xTx));
		var xTy = designMatrix.Transpose().Multiply(y);

		// Solve the system
		var solution = SolveSystem(regularizedXTx, xTy);

		// Verify solution dimensions
		int expectedSolutionLength = Options.UseIntercept ? originalFeatureCount + 1 : originalFeatureCount;
		if (solution.Length != expectedSolutionLength)
		{
			throw new InvalidOperationException(
				$"Solver returned solution of length {solution.Length}, expected {expectedSolutionLength}");
		}
		...
		if (Options.UseIntercept)
		{
			...
			// Handle the case where we only have an intercept (no features)
			if (solution.Length > 1)
			{
				Coefficients = new Vector<T>(solution.GetRange(1, solution.Length - 1));
			}
			else
			{
				Coefficients = new Vector<T>(0); // Empty coefficients vector
			}
		}
		else
		{
		...
		// Store the feature count for validation during prediction
		FeatureCount = originalFeatureCount;
	}
	...
	protected override IFullModel<T, Matrix<T>, Vector<T>> CreateNewInstance()
    {
		...
		if (FeatureCount > 0)
		{
			newModel.FeatureCount = FeatureCount;
		}
