...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> State-of-the-art real-time segmentation. Production deployment on GPU and edge.
///
/// Common use cases:
...
/// var model = new YOLO26Seg&lt;double&gt;(architecture);
/// // Every tunable is now set through the options object; these are the defaults:
/// var custom = new YOLO26Seg&lt;double&gt;(architecture,
///     options: new YOLO26SegOptions { NumClasses = 80, DropRate = 0, ModelSize = YOLO26SegModelSize.N });
///
/// // Or load a pre-trained ONNX model optimized for TensorRT
/// var onnxModel = new YOLO26Seg&lt;double&gt;(architecture, "yolo26n-seg.onnx");
/// </code>
/// </example>
...
public partial class YOLO26Seg<T> : Common.InstanceSegmentationBase<T>
{
    ...
    #region Constructors
    /// <summary>
    /// Initializes YOLO26Seg in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable YOLO26Seg model.
    /// </para>
    /// </remarks>
    public YOLO26Seg(NeuralNetworkArchitecture<T> architecture,
        ...
        : base(architecture, optimizer, lossFunction, (options ??= new YOLO26SegOptions()).NumClasses)
    {
    ...
    /// <summary>
    /// Initializes YOLO26Seg in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained YOLO26Seg from ONNX for inference.
    /// </para>
    /// </remarks>
    ...
    public YOLO26Seg(NeuralNetworkArchitecture<T> architecture,
        ...
        : base(architecture, onnxModelPath, (options ??= new YOLO26SegOptions()).NumClasses)
    {
        _options = options; Options = _options;
        _height = architecture.InputHeight > 0 ? architecture.InputHeight : 640;
        _width = architecture.InputWidth > 0 ? architecture.InputWidth : 640;
    ...
    private static (int[] ChannelDims, int[] Depths, int DecoderDim) GetModelConfig(YOLO26SegModelSize modelSize) => modelSize switch
    {
        YOLO26SegModelSize.N => ([16, 32, 64, 128], [1, 2, 2, 1], 64),
        YOLO26SegModelSize.S => ([32, 64, 128, 256], [1, 2, 2, 1], 128),
        YOLO26SegModelSize.M => ([48, 96, 192, 384], [2, 4, 4, 2], 192),
        YOLO26SegModelSize.L => ([64, 128, 256, 512], [2, 4, 4, 2], 256),
        YOLO26SegModelSize.X => ([80, 160, 320, 640], [2, 4, 4, 2], 320),
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateYOLO26SegEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateYOLO26SegDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
