...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Unified visual perception and understanding. Object-level and pixel-level reasoning.
///
/// Common use cases:
...
public class OMGLLaVA<T> : NeuralNetworkBase<T>, IReferringSegmentation<T>
{
    ...
    #region Properties
    /// <summary>
    /// Gets whether this OMGLLaVA instance supports training.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    #region Constructors
    /// <summary>
    /// Initializes OMGLLaVA in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable OMGLLaVA model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes OMGLLaVA in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained OMGLLaVA from ONNX for inference.
    /// </para>
    /// </remarks>
        ...
        : base(architecture, new CrossEntropyLoss<T>())
    {
        ...
        if (!File.Exists(onnxModelPath))
            throw new FileNotFoundException($"OMGLLaVA ONNX model not found: {onnxModelPath}");
        _height = architecture.InputHeight > 0 ? architecture.InputHeight : 1024;
        _width = architecture.InputWidth > 0 ? architecture.InputWidth : 1024;
        _channels = architecture.InputDepth > 0 ? architecture.InputDepth : 3;
        ...
        catch (Exception ex) { throw new InvalidOperationException($"Failed to load OMGLLaVA ONNX model: {ex.Message}", ex); }
        InitializeLayers();
    }
    #endregion
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateOMGLLaVAEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateOMGLLaVADecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => _useNativeMode
        ...
        : new OMGLLaVA<T>(Architecture, _onnxModelPath!, _numClasses, _options);
