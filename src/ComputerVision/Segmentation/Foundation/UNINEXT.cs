...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> UNINEXT reformulates over 10 different instance perception tasks into a
...
/// </para>
/// </remarks>
public class UNINEXT<T> : NeuralNetworkBase<T>
{
    private readonly UNINEXTOptions _options;

    /// <summary>
    /// Gets the configuration options for this UNINEXT model.
    /// </summary>
    /// <returns>The <see cref="UNINEXTOptions"/> for this model instance.</returns>
    /// <remarks>
    /// <para>
    ...
    private readonly UNINEXTModelSize _modelSize;
    ...
    /// <summary>
    /// Gets whether this UNINEXT instance supports training.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    internal UNINEXTModelSize ModelSize => _modelSize;
    ...
    /// <summary>
    /// Initializes UNINEXT in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable UNINEXT model. Task-specific prompts are embedded
    /// with the queries so the model knows which of 10+ tasks to perform during inference.
    /// </para>
    /// </remarks>
    public UNINEXT(
    ...
    /// <summary>
    /// Initializes UNINEXT in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained UNINEXT from ONNX for fast multi-task inference.
    /// </para>
    /// </remarks>
        ...
        : base(architecture, new CrossEntropyLoss<T>())
    {
        ...
        if (!File.Exists(onnxModelPath))
            throw new FileNotFoundException($"UNINEXT ONNX model not found: {onnxModelPath}");
        ...
        catch (Exception ex) { throw new InvalidOperationException($"Failed to load UNINEXT ONNX model: {ex.Message}", ex); }
    ...
    /// <summary>
    /// Runs a forward pass through UNINEXT for unified instance perception.
    /// </summary>
    /// <param name="input">The input image tensor [C, H, W] or [B, C, H, W].</param>
    /// <returns>Per-pixel segmentation logits tensor.</returns>
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Pass an image to get unified instance perception predictions.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Trains UNINEXT on multi-task instance perception. Only native mode.
    /// </para>
    /// </remarks>
    ...
    private static (int[] ChannelDims, int[] Depths, int DecoderDim) GetModelConfig(UNINEXTModelSize modelSize)
    {
        return modelSize switch
        {
            UNINEXTModelSize.R50 => ([256, 512, 1024, 2048], [3, 4, 6, 3], 256),
            UNINEXTModelSize.SwinLarge => ([192, 384, 768, 1536], [2, 2, 18, 2], 256),
            UNINEXTModelSize.ViTHuge => ([1280, 1280, 1280, 1280], [32, 0, 0, 0], 256),
    ...
    /// <summary>
    /// Initializes the encoder and decoder layers for UNINEXT.
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> In native mode, builds the backbone encoder and query-based decoder.
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateUNINEXTEncoderLayers(
            ...
            var decoderLayers = LayerHelper<T>.CreateUNINEXTDecoderLayers(
    ...
    /// <summary>
    /// Collects metadata describing this UNINEXT model's configuration.
    /// </summary>
