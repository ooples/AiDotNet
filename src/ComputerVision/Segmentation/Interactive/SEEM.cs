...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Multi-modal interactive segmentation. Text-guided, click-guided, and box-guided segmentation.
///
/// Common use cases:
...
/// <example>
/// <code>
/// // Create a SEEM model for multi-modal interactive segmentation
...
public class SEEM<T> : NeuralNetworkBase<T>, IPromptableSegmentation<T>
{
    ...
    #region Constructors
    /// <summary>
    /// Initializes SEEM in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable SEEM model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes SEEM in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained SEEM from ONNX for inference.
    /// </para>
    /// </remarks>
    ...
    protected override void InitializeLayers()
    {
        ...
        else
        {
            ...
            var decoderLayers = LayerHelper<T>.CreateSEEMDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    PromptedSegmentationResult<T> IPromptableSegmentation<T>.SegmentFromPoints(Tensor<T> points, Tensor<T> labels)
    {
        ...
        for (int i = 0; i < numPts; i++)
        {
            ...
            double sign = NumOps.GreaterThanOrEquals(labels[i], NumOps.FromDouble(0.5)) ? 1.0 : -1.0;
    ...
    {
        var modulated = new Tensor<T>(features.Shape._dims);
