...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Panoptic segmentation (stuff + things). Scene understanding for autonomous driving.
///
/// Common use cases:
...
public class KMaXDeepLab<T> : NeuralNetworkBase<T>, IPanopticSegmentation<T>
{
    ...
    private readonly KMaXDeepLabModelSize _modelSize;
    ...
    #region Properties
    /// <summary>
    /// Gets whether this KMaXDeepLab instance supports training.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    internal KMaXDeepLabModelSize ModelSize => _modelSize;
    internal int NumClasses => _numClasses;
    #endregion

    #region Constructors
    /// <summary>
    /// Initializes KMaXDeepLab in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable KMaXDeepLab model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes KMaXDeepLab in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained KMaXDeepLab from ONNX for inference.
    /// </para>
    /// </remarks>
        ...
        : base(architecture, new CrossEntropyLoss<T>())
    {
        ...
        if (!File.Exists(onnxModelPath))
            throw new FileNotFoundException($"KMaXDeepLab ONNX model not found: {onnxModelPath}");
        _height = architecture.InputHeight > 0 ? architecture.InputHeight : 640;
        _width = architecture.InputWidth > 0 ? architecture.InputWidth : 640;
        _channels = architecture.InputDepth > 0 ? architecture.InputDepth : 3;
        ...
        catch (Exception ex) { throw new InvalidOperationException($"Failed to load KMaXDeepLab ONNX model: {ex.Message}", ex); }
        InitializeLayers();
    }
    #endregion
    ...
    private static (int[] ChannelDims, int[] Depths, int DecoderDim) GetModelConfig(KMaXDeepLabModelSize modelSize) => modelSize switch
    {
        KMaXDeepLabModelSize.R50 => ([256, 512, 1024, 2048], [3, 4, 6, 3], 256),
        KMaXDeepLabModelSize.ConvNeXtLarge => ([192, 384, 768, 1536], [3, 3, 27, 3], 256),
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateKMaXDeepLabEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateKMaXDeepLabDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => _useNativeMode
        ? new KMaXDeepLab<T>(Architecture, _optimizer, LossFunction, _numClasses, _modelSize, _dropRate, _options)
        : new KMaXDeepLab<T>(Architecture, _onnxModelPath ?? throw new InvalidOperationException("ONNX model path not initialized."), _numClasses, _modelSize, _options);
