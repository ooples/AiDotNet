...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Mobile segment anything. On-device interactive segmentation.
///
/// Common use cases:
...
/// <example>
/// <code>
/// // Create a MobileSAM model for lightweight mobile segmentation
...
public class MobileSAM<T> : NeuralNetworkBase<T>, IPromptableSegmentation<T>
{
    ...
    #region Constructors
    /// <summary>
    /// Initializes MobileSAM in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable MobileSAM model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes MobileSAM in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained MobileSAM from ONNX for inference.
    /// </para>
    /// </remarks>
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateMobileSAMEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateMobileSAMDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => _useNativeMode
        ? new MobileSAM<T>(Architecture, _optimizer, LossFunction, _numClasses, _dropRate, _options)
        : new MobileSAM<T>(Architecture, _onnxModelPath ?? throw new InvalidOperationException("ONNX model path not initialized."), _numClasses, _options);
    ...
    {
        // Run decoder layers on prompt-modulated features
        var output = features;
        if (_useNativeMode)
        {
    ...
    PromptedSegmentationResult<T> IPromptableSegmentation<T>.SegmentFromPoints(Tensor<T> points, Tensor<T> labels)
    {
        ...
        // Build point attention map using Gaussian kernels
        ...
        // Modulate encoder features with attention and decode
        var modulated = ModulateFeatures(encoderFeatures, sigAtt, numC, h, w);
        var decoded = DecodeFromFeatures(modulated);
        int dh = decoded.Shape[^2], dw = decoded.Shape[^1];
        var scoreMap = ReduceChannelsToScoreMap(decoded);
