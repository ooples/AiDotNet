...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Edge-device segment anything. On-device interactive segmentation.
///
/// Common use cases:
...
public class EdgeSAM<T> : NeuralNetworkBase<T>, IPromptableSegmentation<T>
{
    ...
    #region Properties
    /// <summary>
    /// Gets whether this EdgeSAM instance supports training.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    #region Constructors
    /// <summary>
    /// Initializes EdgeSAM in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable EdgeSAM model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes EdgeSAM in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained EdgeSAM from ONNX for inference.
    /// </para>
    /// </remarks>
        ...
        : base(architecture, new CrossEntropyLoss<T>())
    {
        ...
        if (!File.Exists(onnxModelPath))
            throw new FileNotFoundException($"EdgeSAM ONNX model not found: {onnxModelPath}");
        _height = architecture.InputHeight > 0 ? architecture.InputHeight : 1024;
        _width = architecture.InputWidth > 0 ? architecture.InputWidth : 1024;
        _channels = architecture.InputDepth > 0 ? architecture.InputDepth : 3;
        ...
        catch (Exception ex) { throw new InvalidOperationException($"Failed to load EdgeSAM ONNX model: {ex.Message}", ex); }
        InitializeLayers();
    }
    #endregion
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateEdgeSAMEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateEdgeSAMDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => _useNativeMode
        ? new EdgeSAM<T>(Architecture, _optimizer, LossFunction, _numClasses, _dropRate, _options)
        : new EdgeSAM<T>(Architecture, _onnxModelPath ?? throw new InvalidOperationException("ONNX model path not initialized."), _numClasses, _options);
