...
/// var model = new SlimSAM&lt;double&gt;(architecture);
/// // Every tunable is now set through the options object; these are the defaults:
/// var custom = new SlimSAM&lt;double&gt;(architecture,
///     options: new SlimSAMOptions { NumClasses = 1, DropRate = 0 });
///
/// // Or load a pre-trained ONNX model for pruned SAM inference
/// var onnxModel = new SlimSAM&lt;double&gt;(architecture, "slimsam.onnx");
/// </code>
/// </example>
...
public partial class SlimSAM<T> : Common.PromptableSegmentationBase<T>
{
    ...
    public SlimSAM(NeuralNetworkArchitecture<T> architecture,
        ...
        : base(architecture, optimizer, lossFunction, (options ??= new SlimSAMOptions()).NumClasses)
    {
    ...
    public SlimSAM(NeuralNetworkArchitecture<T> architecture,
        ...
        SlimSAMOptions? options = null)
        : base(architecture, onnxModelPath, (options ??= new SlimSAMOptions()).NumClasses)
    {
        _options = options; Options = _options;
        ApplySamDefaultGeometry(architecture);
        _dropRate = _options.DropRate;
