...
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Medical image segmentation across organs and modalities. CT/MRI organ segmentation.
///
/// Common use cases:
...
[ModelPaper("nnU-Net: a self-configuring method for deep learning-based biomedical image segmentation", "https://doi.org/10.1038/s41592-020-01008-z", Year = 2021, Authors = "Fabian Isensee, Paul F. Jaeger, Simon A. A. Kohl, Jens Petersen, Klaus H. Maier-Hein")]
public class NnUNet<T> : NeuralNetworkBase<T>, IMedicalSegmentation<T>
{
    ...
    private readonly NnUNetModelSize _modelSize;
    ...
    #region Properties
    /// <summary>
    /// Gets whether this NnUNet instance supports training.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    internal NnUNetModelSize ModelSize => _modelSize;
    internal int NumClasses => _numClasses;
    #endregion

    #region Constructors
    /// <summary>
    /// Initializes NnUNet in native (trainable) mode.
    /// </summary>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Creates a trainable NnUNet model.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Initializes NnUNet in ONNX (inference-only) mode.
    /// </summary>
    /// <param name="architecture">Neural network architecture defining input dimensions.</param>
    ...
    /// <remarks>
    /// <para>
    /// <b>For Beginners:</b> Loads a pre-trained NnUNet from ONNX for inference.
    /// </para>
    /// </remarks>
        ...
        : base(architecture, new CrossEntropyLoss<T>())
    {
        ...
        if (!File.Exists(onnxModelPath))
            throw new FileNotFoundException($"NnUNet ONNX model not found: {onnxModelPath}");
        _height = architecture.InputHeight > 0 ? architecture.InputHeight : 256;
        _width = architecture.InputWidth > 0 ? architecture.InputWidth : 256;
        _channels = architecture.InputDepth > 0 ? architecture.InputDepth : 3;
        ...
        catch (Exception ex) { throw new InvalidOperationException($"Failed to load NnUNet ONNX model: {ex.Message}", ex); }
        InitializeLayers();
    }
    #endregion
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            var encoderLayers = LayerHelper<T>.CreateNnUNetEncoderLayers(_channels, _height, _width, _channelDims, _depths, _dropRate).ToList();
            _encoderLayerEnd = encoderLayers.Count; Layers.AddRange(encoderLayers);
            int fH = _height / 32, fW = _width / 32;
            var decoderLayers = LayerHelper<T>.CreateNnUNetDecoderLayers(_channelDims[^1], _decoderDim, _numClasses, fH, fW);
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() => _useNativeMode
        ? new NnUNet<T>(Architecture, _optimizer, LossFunction, _numClasses, _modelSize, _dropRate, _options)
        : new NnUNet<T>(Architecture, _onnxModelPath ?? throw new InvalidOperationException("ONNX model path not initialized."), _numClasses, _modelSize, _options);
    ...
    IReadOnlyList<string> IMedicalSegmentation<T>.SupportedModalities => ["CT", "MRI_T1", "MRI_T2", "Xray", "Ultrasound"];
