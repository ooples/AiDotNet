...
public partial class RTDETR<T> : ObjectDetectorBase<T>, IDetectionTrainingModel<T>
{
    ...
    /// <summary>Trains the final RT-DETR heads with exact assignment, varifocal loss, L1 and GIoU.</summary>
    /// <remarks>
    /// <para>
    /// Uses RT-DETR's published recipe by default (Zhao et al. 2023, Table A): varifocal loss with alpha
    /// 0.75 and gamma 2, matching costs 2/5/2 and loss weights 1/5/2 for class/L1/GIoU. Override it
    /// with <see cref="ObjectDetectionOptions{T}.SetPredictionLoss"/>.
    /// </para>
    /// <para>
    ...
    /// exposes only its final decoder heads, so per-layer auxiliary and encoder query-selection
    ...
    {
        if (heads.Count != 2)
            throw new InvalidOperationException("RT-DETR training requires the actual final class and box heads.");
...
internal class RTDETRDecoder<T> : CvParameterModule<T>
{
        ...
        int imageWidth)
    {
        ...
        for (int b = 0; b < batch; b++)
        {
            ...
            {
                // RT-DETR scores each class with an independent sigmoid trained toward the box IoU
                // (varifocal loss, Zhao et al. 2023); there is no no-object column.
                double maxScore = 0;
                int maxClassId = 0;
