...
public class RadFM<T> : VisionLanguageModelBase<T>, IMedicalVLM<T>
{
    ...
    public RadFM(
        ...
        string modelPath,
        RadFMOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new RadFMOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "RadFM-Native" : "RadFM-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new RadFM<T>(Architecture, mp, _options);
        return new RadFM<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(RadFM<T>));
    }
