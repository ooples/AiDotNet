...
public class DragonflyMed<T> : VisionLanguageModelBase<T>, IMedicalVLM<T>
{
    ...
    public DragonflyMed(
        ...
        string modelPath,
        DragonflyMedOptions? options = null
    )
        : base(architecture)
    {
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Dragonfly-Med-Native" : "Dragonfly-Med-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new DragonflyMed<T>(Architecture, mp, _options);
        return new DragonflyMed<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(DragonflyMed<T>));
    }
