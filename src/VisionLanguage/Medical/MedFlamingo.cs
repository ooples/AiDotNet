...
public class MedFlamingo<T> : VisionLanguageModelBase<T>, IMedicalVLM<T>
{
    ...
    public MedFlamingo(
        ...
        string modelPath,
        MedFlamingoOptions? options = null
    )
        : base(architecture)
    {
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            Layers.AddRange(
                LayerHelper<T>.CreateDefaultPerceiverResamplerLayers(
                    _options.VisionDim,
                    _options.VisionDim,
                    _options.DecoderDim,
    ...
    private void ComputeEncoderDecoderBoundary()
    {
        ...
        _encoderLayerEnd = 1 + _options.NumVisionLayers * lpb + 6 * lpb;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Med-Flamingo-Native" : "Med-Flamingo-ONNX",
            Description =
                "Med-Flamingo: few-shot medical visual question answering via Flamingo architecture.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MedFlamingo<T>(Architecture, mp, _options);
        return new MedFlamingo<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MedFlamingo<T>));
    }
