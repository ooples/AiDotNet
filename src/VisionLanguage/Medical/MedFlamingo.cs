...
[ResearchPaper("Med-Flamingo: A Multimodal Medical Few-shot Learner", "https://arxiv.org/abs/2307.15189", Year = 2023, Authors = "Moor et al.")]
public class MedFlamingo<T> : VisionLanguageModelBase<T>, IMedicalVLM<T>
{
    ...
    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) { Layers.AddRange(Architecture.Layers); _encoderLayerEnd = Layers.Count / 2; } else { Layers.AddRange(LayerHelper<T>.CreateDefaultPerceiverResamplerLayers(_options.VisionDim, _options.VisionDim, _options.DecoderDim, _options.NumVisionLayers, 6, _options.NumDecoderLayers, 64, _options.NumHeads, 16, _options.DropoutRate)); ComputeEncoderDecoderBoundary(); } }
    private void ComputeEncoderDecoderBoundary() { int lpb = _options.DropoutRate > 0 ? 6 : 5; _encoderLayerEnd = 1 + _options.NumVisionLayers * lpb + 6 * lpb; }
