...
public class LLaVAMed<T> : VisionLanguageModelBase<T>, IMedicalVLM<T>
{
    ...
    public LLaVAMed(
        ...
        string modelPath,
        LLaVAMedOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new LLaVAMedOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "LLaVA-Med-Native" : "LLaVA-Med-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new LLaVAMed<T>(Architecture, mp, _options);
        return new LLaVAMed<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(LLaVAMed<T>));
    }
