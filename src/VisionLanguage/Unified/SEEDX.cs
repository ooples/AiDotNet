...
public class SEEDX<T> : VisionLanguageModelBase<T>, IUnifiedVisionModel<T>
{
    ...
    public SEEDX(
        ...
        string modelPath,
        SEEDXOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new SEEDXOptions();
    ...
    public Tensor<T> GenerateImage(string textDescription)
    {
        ...
        for (int s = 0; s < numSemantic; s++)
        {
            ...
            for (int h = 0; h < Math.Min(hiddenDim, 64); h++)
                val +=
                    NumOps.ToDouble(textHidden[h % hiddenDim]) * Math.Cos((s + 1) * (h + 1) * 0.01);
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SEED-X-Native" : "SEED-X-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new SEEDX<T>(Architecture, mp, _options);
        return new SEEDX<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SEEDX<T>));
    }
