...
public class PLLaVA<T> : VisionLanguageModelBase<T>, IVideoLanguageModel<T>
{
    ...
    public PLLaVA(NeuralNetworkArchitecture<T> architecture, PLLaVAOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new PLLaVAOptions(); if (architecture.InputType == InputType.FourDimensional && architecture.InputHeight > 0) { _options = new PLLaVAOptions(_options) { ImageSize = architecture.InputHeight }; } _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
