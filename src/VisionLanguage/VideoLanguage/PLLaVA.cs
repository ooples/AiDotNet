...
public class PLLaVA<T> : VisionLanguageModelBase<T>, IVideoLanguageModel<T>
{
        ...
        if (architecture.InputType == InputType.FourDimensional && architecture.InputHeight > 0)
        {
            if (architecture.InputWidth > 0 && architecture.InputWidth != architecture.InputHeight)
                throw new ArgumentException(
                    "PLLaVA native mode requires square frames (InputWidth must equal InputHeight).",
                    nameof(architecture)
                );
            if (architecture.InputDepth > 0 && architecture.InputDepth != 3)
                throw new ArgumentException(
                    "PLLaVA native mode requires 3-channel RGB frames (InputDepth must equal 3).",
                    nameof(architecture)
                );
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "PLLaVA-Native" : "PLLaVA-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new PLLaVA<T>(Architecture, mp, _options);
        return new PLLaVA<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(PLLaVA<T>));
    }
