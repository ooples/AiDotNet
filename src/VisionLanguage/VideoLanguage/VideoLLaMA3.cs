...
public class VideoLLaMA3<T> : VisionLanguageModelBase<T>, IVideoLanguageModel<T>
{
    ...
    public VideoLLaMA3(
        ...
        string modelPath,
        VideoLLaMA3Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new VideoLLaMA3Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "VideoLLaMA3-Native" : "VideoLLaMA3-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new VideoLLaMA3<T>(Architecture, mp, _options);
        return new VideoLLaMA3<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(VideoLLaMA3<T>));
    }
