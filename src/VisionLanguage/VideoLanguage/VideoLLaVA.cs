...
/// <para><b>For Beginners:</b> Video-LLaVA is a video-language model that aligns image and
...
public class VideoLLaVA<T> : VisionLanguageModelBase<T>, IVideoLanguageModel<T>
{
    ...
    public VideoLLaVA(NeuralNetworkArchitecture<T> architecture, VideoLLaVAOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new VideoLLaVAOptions(); if (architecture.InputType == InputType.FourDimensional && architecture.InputHeight > 0) { _options = new VideoLLaVAOptions(_options) { ImageSize = architecture.InputHeight }; } _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    public override ModelMetadata<T> GetModelMetadata() {
        var m = new ModelMetadata<T> { Name = _useNativeMode ? "Video-LLaVA-Native" : "Video-LLaVA-ONNX", Description = "Video-LLaVA: united visual representation for video understanding.", FeatureCount = _options.DecoderDim, Complexity = _options.NumVisionLayers + _options.NumDecoderLayers };
