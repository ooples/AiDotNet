...
public class ViT<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public ViT(
        ...
        string modelPath,
        ViTOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new ViTOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "ViT-Native" : "ViT-ONNX",
        ...
        m.AdditionalInfo["Architecture"] = "ViT";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new ViT<T>(Architecture, mp, _options);
        return new ViT<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(ViT<T>));
    }
