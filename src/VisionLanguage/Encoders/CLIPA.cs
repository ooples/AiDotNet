...
/// <para><b>Architecture layout:</b> Vision encoder (patch embedding + transformer + projection)
/// lives in <see cref="NeuralNetworkBase{T}.Layers"/>; text encoder lives in
/// <see cref="VisionLanguageModelBase{T}.TextEncoderLayers"/>. Default
/// <see cref="Predict"/> + <see cref="Train(Tensor{T},Tensor{T})"/> is vision-only;
/// <see cref="EncodeText"/> walks the text stack independently.</para>
/// </remarks>
/// <example>
/// <code>
...
public class CLIPA<T> : VisionLanguageModelBase<T>, IContrastiveVisionLanguageModel<T>
{
    ...
    /// default (224) is preserved when the architecture leaves H/W unset (= 0);
    /// a CI-fast architecture at smaller resolution drives imageSize down so
    /// the patch grid matches the actual input.
    /// </summary>
