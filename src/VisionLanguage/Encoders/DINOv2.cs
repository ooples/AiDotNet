...
public class DINOv2<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public DINOv2(NeuralNetworkArchitecture<T> architecture, string modelPath, DINOv2Options? options = null) : base(architecture) { _options = options ?? new DINOv2Options(); _useNativeMode = false; base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.EmbeddingDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path cannot be null or empty.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); InitializeLayers(); }
    public DINOv2(NeuralNetworkArchitecture<T> architecture, DINOv2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new DINOv2Options();
        if (architecture.InputType == InputType.ThreeDimensional && architecture.InputHeight > 0)
        {
            if (architecture.InputWidth > 0 && architecture.InputWidth != architecture.InputHeight)
                throw new ArgumentException("DINOv2 native mode requires square images (InputWidth must equal InputHeight).", nameof(architecture));
            if (architecture.InputDepth > 0 && architecture.InputDepth != 3)
                throw new ArgumentException("DINOv2 native mode requires 3-channel RGB input (InputDepth must equal 3).", nameof(architecture));
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "DINOv2-Native" : "DINOv2-ONNX", Description = "DINOv2: Learning Robust Visual Features without Supervision (Oquab et al., 2024)", FeatureCount = _options.EmbeddingDim, Complexity = _options.NumLayers }; m.AdditionalInfo["Architecture"] = "DINOv2"; m.AdditionalInfo["RegisterTokens"] = _options.NumRegisterTokens.ToString(); return m; }
    protected override void SerializeNetworkSpecificData(BinaryWriter writer) { writer.Write(_useNativeMode); writer.Write(_options.ModelPath ?? string.Empty); writer.Write(_options.ImageSize); writer.Write(_options.EmbeddingDim); writer.Write(_options.NumLayers); writer.Write(_options.NumHeads); writer.Write(_options.NumRegisterTokens); }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new DINOv2<T>(Architecture, mp, _options); return new DINOv2<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(DINOv2<T>)); }
