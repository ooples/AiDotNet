...
public class SigLIPSO<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public SigLIPSO(
        ...
        string modelPath,
        SigLIPSOOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new SigLIPSOOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SigLIP-SO-Native" : "SigLIP-SO-ONNX",
            Description = "SigLIP Shape-Optimized 400M Vision Encoder (Zhai et al., ICCV 2023)",
            FeatureCount = _options.EmbeddingDim,
            Complexity = _options.NumLayers,
        };
        m.AdditionalInfo["Architecture"] = "SigLIP-SO";
        m.AdditionalInfo["NumOutputTokens"] = _options.NumOutputTokens.ToString();
        return m;
    }
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SigLIPSO<T>));
    }
