...
public class SigLIPSO<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public SigLIPSO(NeuralNetworkArchitecture<T> architecture, SigLIPSOOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new SigLIPSOOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.EmbeddingDim; InitializeLayers(); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "SigLIP-SO-Native" : "SigLIP-SO-ONNX", Description = "SigLIP Shape-Optimized 400M Vision Encoder (Zhai et al., ICCV 2023)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.EmbeddingDim, Complexity = _options.NumLayers }; m.AdditionalInfo["Architecture"] = "SigLIP-SO"; m.AdditionalInfo["NumOutputTokens"] = _options.NumOutputTokens.ToString(); return m; }
