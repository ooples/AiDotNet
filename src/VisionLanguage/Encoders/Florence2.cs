...
public class Florence2<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public Tensor<T> EncodeImage(Tensor<T> image) { ThrowIfDisposed(); var p = PreprocessImage(image); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(p); var c = p; int encoderEnd = Layers.Count / 2; for (int i = 0; i < encoderEnd; i++) c = Layers[i].Forward(c); return c; }

    protected override void InitializeLayers() { if (!_useNativeMode) return; if (Architecture.Layers is not null && Architecture.Layers.Count > 0) Layers.AddRange(Architecture.Layers); else Layers.AddRange(LayerHelper<T>.CreateDefaultFlorence2Layers(_options.EmbeddingDim, _options.DecoderEmbeddingDim, _options.NumLayers, _options.NumDecoderLayers, _options.NumHeads, _options.NumDecoderHeads, _options.DropoutRate)); }
