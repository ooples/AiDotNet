...
public class PerceptionEncoder<T> : VisionLanguageModelBase<T>, IVisualEncoder<T>
{
    ...
    public PerceptionEncoder(
        ...
        string modelPath,
        PerceptionEncoderOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new PerceptionEncoderOptions();
        ...
        : base(architecture)
    {
        ...
        {
            _options = new PerceptionEncoderOptions(_options)
            {
                ImageSize = architecture.InputHeight,
            };
        }
        _useNativeMode = true;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "PerceptionEncoder-Native" : "PerceptionEncoder-ONNX",
            Description =
                "Perception Encoder: Vision Foundation for Multimodal Alignment (Meta, 2025)",
            FeatureCount = _options.EmbeddingDim,
            Complexity = _options.NumLayers,
        };
        m.AdditionalInfo["Architecture"] = "PerceptionEncoder";
        m.AdditionalInfo["UseDenseFeatures"] = _options.UseDenseFeatures.ToString();
        m.AdditionalInfo["GlobalPooling"] = _options.GlobalPooling.ToString();
        return m;
    }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new PerceptionEncoder<T>(Architecture, mp, _options);
        return new PerceptionEncoder<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(PerceptionEncoder<T>));
    }
