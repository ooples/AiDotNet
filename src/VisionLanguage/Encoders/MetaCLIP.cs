...
/// <summary>
/// MetaCLIP model using metadata-driven data curation for balanced CLIP training.
/// </summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks>
/// <para>
/// MetaCLIP (Xu et al., 2023) improves CLIP by curating training data using metadata from WordNet
/// and Wikipedia to ensure balanced concept coverage. This achieves 70.8% zero-shot on ImageNet
/// with ViT-B/16 on 400M pairs, surpassing CLIP's original results.
/// </para>
/// <para><b>References:</b>
...
public class MetaCLIP<T> : VisionLanguageModelBase<T>, IContrastiveVisionLanguageModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "MetaCLIP-Native" : "MetaCLIP-ONNX", Description = "MetaCLIP: Demystifying CLIP Data (Xu et al., 2023)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.ProjectionDim, Complexity = _options.NumVisionLayers + _options.NumTextLayers }; m.AdditionalInfo["Architecture"] = "MetaCLIP"; m.AdditionalInfo["Dataset"] = _options.Dataset.ToString(); return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ImageEncoderModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new MetaCLIP<T>(Architecture, mp, _options); return new MetaCLIP<T>(Architecture, _options); }
    ...
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(MetaCLIP<T>)); }
