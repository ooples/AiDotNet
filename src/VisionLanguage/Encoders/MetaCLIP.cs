...
/// <summary>
/// MetaCLIP (Demystifying CLIP Data) reproduces and improves CLIP via a fully transparent data curation pipeline.
/// </summary>
/// <typeparam name="T">The numeric type used for calculations.</typeparam>
/// <remarks>
/// <para>
/// MetaCLIP (Xu et al., 2023) provides a fully transparent CLIP data curation algorithm
/// reproducing CLIP's results from scratch and outperforming OpenAI CLIP at scale.
/// </para>
/// <para><b>References:</b>
...
public class MetaCLIP<T> : VisionLanguageModelBase<T>, IContrastiveVisionLanguageModel<T>
{
    ...
    public MetaCLIP(NeuralNetworkArchitecture<T> architecture, string imageEncoderModelPath, MetaCLIPOptions? options = null) : base(architecture)
    {
    ...
    public MetaCLIP(NeuralNetworkArchitecture<T> architecture, MetaCLIPOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new MetaCLIPOptions();
        SyncImageSizeWithArchitecture();
        _useNativeMode = true;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MetaCLIP-Native" : "MetaCLIP-ONNX",
            Description = "MetaCLIP: Demystifying CLIP Data (Xu et al., 2023)",
            FeatureCount = _options.ProjectionDim,
            Complexity = _options.NumVisionLayers + _options.NumTextLayers
        };
        m.AdditionalInfo["Architecture"] = "MetaCLIP";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ImageEncoderModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MetaCLIP<T>(Architecture, mp, _options);
        return new MetaCLIP<T>(Architecture, _options);
    }
    ...
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(MetaCLIP<T>));
    }
