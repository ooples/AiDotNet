...
public class DeCLIP<T> : VisionLanguageModelBase<T>, IContrastiveVisionLanguageModel<T>
{
    ...
    public DeCLIP(NeuralNetworkArchitecture<T> architecture, string imageEncoderModelPath, DeCLIPOptions? options = null) : base(architecture)
    {
    ...
    public DeCLIP(NeuralNetworkArchitecture<T> architecture, DeCLIPOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new DeCLIPOptions();
        SyncImageSizeWithArchitecture();
        _useNativeMode = true;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "DeCLIP-Native" : "DeCLIP-ONNX",
            Description = "DeCLIP: Data-Efficient Contrastive Language-Image Pre-training (Li et al., ICLR 2022)",
            FeatureCount = _options.ProjectionDim,
            Complexity = _options.NumVisionLayers + _options.NumTextLayers
        };
        m.AdditionalInfo["Architecture"] = "DeCLIP";
        m.AdditionalInfo["ImageSelfSupervisedWeight"] = _options.ImageSelfSupervisedWeight.ToString();
        m.AdditionalInfo["TextMLMWeight"] = _options.TextMLMWeight.ToString();
        return m;
    }
    ...
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(DeCLIP<T>));
    }
