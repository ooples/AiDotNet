...
public class DeCLIP<T> : VisionLanguageModelBase<T>, IContrastiveVisionLanguageModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "DeCLIP: Data-Efficient Contrastive Language-Image Pre-training (Li et al., ICLR 2022)",
        ...
        m.AdditionalInfo["ImageSelfSupervisedWeight"] =
            _options.ImageSelfSupervisedWeight.ToString();
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(DeCLIP<T>));
    }
