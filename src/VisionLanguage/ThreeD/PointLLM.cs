...
public class PointLLM<T> : VisionLanguageModelBase<T>, IThreeDVisionLanguageModel<T>
{
    ...
    /// <summary>
    /// Generates from 2D image using PointLLM's point cloud tokenization approach.
    /// For 2D images, visual tokens from the encoder are fused with text instruction
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "PointLLM-Native" : "PointLLM-ONNX",
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(PointLLM<T>));
    }
