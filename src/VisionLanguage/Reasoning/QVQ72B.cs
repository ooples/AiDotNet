...
public class QVQ72B<T> : VisionLanguageModelBase<T>, IReasoningVLM<T>
{
    ...
    public QVQ72B(
        ...
        string modelPath,
        QVQ72BOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new QVQ72BOptions();
        ...
        for (int step = 0; step < numReasoningSteps; step++)
        {
            ...
            for (int d = 0; d < dim; d++)
            {
                ...
                for (int v = 0; v < numPatches; v++)
                {
                    ...
                    double qBias =
                        NumOps.ToDouble(questionTokens[v % qLen]) / _options.VocabSize * 0.2;
                ...
                reasoningState[d] =
                    prevReasoning * (1.0 - stepWeight * 0.3) + (visAttn + qEmb * 0.2) * stepWeight;
            }
        }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "QVQ-72B-Native" : "QVQ-72B-ONNX",
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(QVQ72B<T>));
    }
