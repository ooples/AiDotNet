...
public class Emu3<T> : VisionLanguageModelBase<T>, IGenerativeVisionLanguageModel<T>
{
    ...
    public Emu3(NeuralNetworkArchitecture<T> architecture, string modelPath, Emu3Options? options = null) : base(architecture)
    {
    ...
    public Emu3(NeuralNetworkArchitecture<T> architecture, Emu3Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new Emu3Options();
        SyncImageSizeWithArchitecture();
        _useNativeMode = true;
    ...
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Emu3<T>));
    }
