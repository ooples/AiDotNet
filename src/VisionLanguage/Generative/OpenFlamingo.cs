...
public class OpenFlamingo<T> : VisionLanguageModelBase<T>, IGenerativeVisionLanguageModel<T>
{
    ...
    public OpenFlamingo(NeuralNetworkArchitecture<T> architecture, string modelPath, OpenFlamingoOptions? options = null) : base(architecture)
    {
    ...
    public OpenFlamingo(NeuralNetworkArchitecture<T> architecture, OpenFlamingoOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new OpenFlamingoOptions();
        SyncImageSizeWithArchitecture();
        _useNativeMode = true;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description = "OpenFlamingo: An Open-Source Framework for Training Large Autoregressive Vision-Language Models (Awadalla et al., 2023)",
    ...
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(OpenFlamingo<T>));
    }
