...
/// <para><b>For Beginners:</b> Emu2 scales the original Emu architecture to 37 billion parameters,
...
public class Emu2<T> : VisionLanguageModelBase<T>, IGenerativeVisionLanguageModel<T>
{
    ...
    public Emu2(NeuralNetworkArchitecture<T> architecture, string modelPath, Emu2Options? options = null) : base(architecture)
    {
    ...
    public Emu2(NeuralNetworkArchitecture<T> architecture, Emu2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture)
    {
        _options = options ?? new Emu2Options();
        SyncImageSizeWithArchitecture();
        _useNativeMode = true;
    ...
    /// (5) Visual regression head enables image generation through diffusion.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Emu2-Native" : "Emu2-ONNX",
            Description = "Emu2: Generative Multimodal Models are In-Context Learners (Sun et al., 2024)",
        ...
        m.AdditionalInfo["Architecture"] = "Emu2";
    ...
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Emu2<T>));
    }
