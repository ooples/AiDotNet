...
public class Emu2<T> : VisionLanguageModelBase<T>, IGenerativeVisionLanguageModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        var m = new ModelMetadata<T>
        {
            ...
            Description =
                "Emu2: Generative Multimodal Models are In-Context Learners (Sun et al., 2024)",
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Emu2<T>));
    }
