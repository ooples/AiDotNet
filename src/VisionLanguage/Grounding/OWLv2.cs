...
public class OWLv2<T> : VisionLanguageModelBase<T>, IVisualGroundingModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "OWLv2-Native" : "OWLv2-ONNX",
    ...
    {
        var options = new OWLv2Options(_options);
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new OWLv2<T>(Architecture, mp, options);
        return new OWLv2<T>(Architecture, options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(OWLv2<T>));
    }
