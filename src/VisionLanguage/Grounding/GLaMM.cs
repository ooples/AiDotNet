...
public class GLaMM<T> : VisionLanguageModelBase<T>, IVisualGroundingModel<T>
{
    ...
    public Tensor<T> GroundText(Tensor<T> image, string textQuery)
    {
        ...
        if (maskGridSize < 4)
            maskGridSize = (int)Math.Sqrt(visDim / 4);
        if (maskGridSize < 4)
            maskGridSize = 4;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "GLaMM-Native" : "GLaMM-ONNX",
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(GLaMM<T>));
    }
