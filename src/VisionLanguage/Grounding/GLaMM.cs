...
public class GLaMM<T> : VisionLanguageModelBase<T>, IVisualGroundingModel<T>
{
    ...
    public GLaMM(NeuralNetworkArchitecture<T> architecture, GLaMMOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) {
        ...
        base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim;
    ...
    public override void Train(Tensor<T> input, Tensor<T> expected) {
        ...
        try {
            // Pass the model's own non-AMSGrad AdamW so the fused-Adam
            // fast path engages; otherwise GetOrCreateBaseOptimizer's
            // AMSGrad-Adam blocks the fused kernel and per-iter cost
            // balloons under eager-tape execution.
    ...
    public override ModelMetadata<T> GetModelMetadata() {
        var m = new ModelMetadata<T> { Name = _useNativeMode ? "GLaMM-Native" : "GLaMM-ONNX", Description = "GLaMM: pixel-level grounded LMM generating text and segmentation masks.", FeatureCount = _options.DecoderDim, Complexity = _options.NumVisionLayers + _options.NumDecoderLayers };
