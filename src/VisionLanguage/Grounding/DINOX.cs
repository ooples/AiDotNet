...
{
    private readonly DINOXOptions _options;
    ...
    public DINOX(
        ...
        string modelPath,
        DINOXOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new DINOXOptions();
    ...
    public Tensor<T> GroundText(Tensor<T> image, string textQuery)
    {
        ...
        for (int q = 0; q < maxDet; q++)
        {
            ...
            for (int d = start; d < end; d++)
            {
                ...
                if (localIdx < span / 4)
                    cx += val;
                else if (localIdx < span / 2)
                    cy += val;
                else if (localIdx < 3 * span / 4)
                    w += val;
                else
                    h += val;
                conf += val * val;
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "DINO-X-Native" : "DINO-X-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new DINOX<T>(Architecture, mp, _options);
        return new DINOX<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(DINOX<T>));
    }
