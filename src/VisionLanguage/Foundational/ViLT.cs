...
/// <example>
/// <code>
/// // Create a ViLT model for lightweight vision-language processing
/// // with direct patch embeddings and no CNN/object detector overhead
...
/// var model = new ViLT&lt;double&gt;(architecture, "vilt.onnx");
///
/// // Training mode with native layers
/// var trainModel = new ViLT&lt;double&gt;(architecture, new ViLTOptions());
/// </code>
/// </example>
...
public class ViLT<T> : VisionLanguageModelBase<T>, IVisionLanguageFusionModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "ViLT-Native" : "ViLT-ONNX",
            Description =
                "ViLT: Vision-and-Language Transformer Without Convolution or Region Supervision (Kim et al., ICML 2021)",
            FeatureCount = _options.FusionDim,
            Complexity = _options.NumFusionLayers,
        };
        m.AdditionalInfo["Architecture"] = "ViLT";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(ViLT<T>));
    }
