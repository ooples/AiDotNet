...
public class DeepSeekVL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public DeepSeekVL(NeuralNetworkArchitecture<T> architecture, DeepSeekVLOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new DeepSeekVLOptions(); if (architecture.Layers == null) _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            // Validate visual-patch options only when we're building the default VLM
            // stack — that's the branch that consumes _options.ImageSize and
            // _options.MaxVisualTokens via ComputePatchSize(). When the caller
            // supplies Architecture.Layers, those options are irrelevant and the
            // validation would spuriously reject otherwise valid custom-layer setups.
    ...
    // DeepSeek-VL (Lu 2024) uses SigLIP-L/16 + SAM-B/16 — 16x16 patches.
