...
public class Eagle25<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Eagle25(
        ...
        string modelPath,
        Eagle25Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new Eagle25Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Eagle-2.5-Native" : "Eagle-2.5-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Eagle25<T>(Architecture, mp, _options);
        return new Eagle25<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Eagle25<T>));
    }
