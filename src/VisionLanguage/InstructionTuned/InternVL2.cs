...
[ResearchPaper("InternVL2: Better than the Best - Expanding Performance Boundaries of Open-Source Multimodal Models", "https://arxiv.org/abs/2404.16821", Year = 2024, Authors = "Chen et al.")]
public class InternVL2<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public InternVL2(NeuralNetworkArchitecture<T> architecture, InternVL2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new InternVL2Options(); _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
