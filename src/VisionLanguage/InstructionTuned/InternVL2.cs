...
public class InternVL2<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public InternVL2(
        ...
        string modelPath,
        InternVL2Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new InternVL2Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "InternVL2-Native" : "InternVL2-ONNX",
            Description =
        ...
        m.AdditionalInfo["Architecture"] = "InternVL2";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new InternVL2<T>(Architecture, mp, _options);
        return new InternVL2<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(InternVL2<T>));
    }
