...
{
    private readonly OvisOptions _options;
    ...
    public Ovis(
        ...
        string modelPath,
        OvisOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new OvisOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Ovis-Native" : "Ovis-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Ovis<T>(Architecture, mp, _options);
        return new Ovis<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Ovis<T>));
    }
