...
public class Aria<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Aria(
        ...
        string modelPath,
        AriaOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new AriaOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Aria-Native" : "Aria-ONNX",
            Description =
                "Aria: mixture-of-experts multimodal model with efficient expert routing.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Aria<T>(Architecture, mp, _options);
        return new Aria<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Aria<T>));
    }
