...
public class Fuyu<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Fuyu(
        ...
        string modelPath,
        FuyuOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new FuyuOptions();
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            Layers.AddRange(
                LayerHelper<T>.CreateDefaultDecoderOnlyVisionLayers(
                    3072,
                    _options.DecoderDim,
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Fuyu-Native" : "Fuyu-ONNX",
            Description = "Fuyu: Direct Patch-to-Transformer Multimodal Model (Adept, 2023)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "Fuyu";
        m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString();
        m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName;
        m.AdditionalInfo["PatchSize"] = _options.PatchSize.ToString();
        return m;
    }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Fuyu<T>(Architecture, mp, _options);
        return new Fuyu<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Fuyu<T>));
    }
