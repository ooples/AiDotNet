...
public class Moondream<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Moondream(
        ...
        string modelPath,
        MoondreamOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new MoondreamOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Moondream-Native" : "Moondream-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Moondream<T>(Architecture, mp, _options);
        return new Moondream<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Moondream<T>));
    }
