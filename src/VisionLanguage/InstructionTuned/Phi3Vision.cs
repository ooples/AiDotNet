...
public class Phi3Vision<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Phi3Vision(NeuralNetworkArchitecture<T> architecture, Phi3VisionOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Phi3VisionOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    public Tensor<T> GenerateFromImage(Tensor<T> image, string? prompt = null)
    {
        ...
        // Step 2: Sub-image decomposition for high resolution
        // Split into overlapping sub-images and encode each
        int subImageSize = Math.Max(1, visLen / 4);
        var subImageFeatures = new double[visLen];
        for (int v = 0; v < visLen; v++)
        {
            double val = NumOps.ToDouble(visualFeatures[v]);
            int subIdx = v / Math.Max(1, subImageSize);
            // Sub-image position encoding
            double subPos = Math.Sin((v % Math.Max(1, subImageSize)) * 0.05) * 0.03;
            subImageFeatures[v] = val + subPos;
        }

        // Step 3: 2-layer MLP connector with GELU
        var projected = new double[visLen];
        for (int v = 0; v < visLen; v++)
        {
            double x = subImageFeatures[v];
            double h = x * 0.8;
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Phi3Vision<T>(Architecture, mp, _options); return new Phi3Vision<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Phi3Vision<T>)); }
