...
public class Dragonfly<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Dragonfly-Native" : "Dragonfly-ONNX",
            Description =
                "Dragonfly: multi-resolution visual encoding VLM for fine-grained understanding.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Dragonfly<T>(Architecture, mp, _options);
        return new Dragonfly<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Dragonfly<T>));
    }
