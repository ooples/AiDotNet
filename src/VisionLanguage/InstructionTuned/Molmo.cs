...
public class Molmo<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Molmo(NeuralNetworkArchitecture<T> architecture, string modelPath, MolmoOptions? options = null) : base(architecture) { _options = options ?? new MolmoOptions(); _options.ValidateVisualSizing(); _useNativeMode = false; base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path cannot be null or empty.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    public Molmo(NeuralNetworkArchitecture<T> architecture, MolmoOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new MolmoOptions(); _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    /// <summary>
    /// Generates text using Molmo's pointing-capable architecture with diverse data mixture.
    /// Molmo (AI2, 2024) uses:
    /// (1) PixMo data: diverse data mixture including pointing annotations where the model
    ///     can output normalized (x,y) coordinates for grounding,
    /// (2) Coordinate-aware visual processing: spatial position encoding that enables the
    ///     model to map visual features to specific image coordinates,
    /// (3) Multi-crop visual encoding: image processed at multiple crops with a connector
    ///     MLP that preserves spatial correspondence,
    /// (4) OLMo decoder backbone.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() {
        ...
        m.AdditionalInfo["Pointing"] = _options.EnablePointing.ToString();
        return m;
    }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Molmo<T>(Architecture, mp, _options); return new Molmo<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Molmo<T>)); }
