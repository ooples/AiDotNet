...
public class NVLM<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public NVLM(
        ...
        string modelPath,
        NVLMOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new NVLMOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "NVLM-Native" : "NVLM-ONNX",
            Description = "NVLM 1.0: Cross-Attention + Decoder-Only Hybrid VLM (NVIDIA, 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "NVLM";
        m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString();
        m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName;
        m.AdditionalInfo["CrossAttention"] = _options.EnableCrossAttention.ToString();
        return m;
    }
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(NVLM<T>));
    }
