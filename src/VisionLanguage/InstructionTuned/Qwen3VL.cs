...
public partial class Qwen3VL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    protected override void InitializeLayers()
    {
        ...
        if (Architecture is TripleStreamArchitecture<T> triple)
        {
            ...
            _resamplerLayers.AddRange(triple.AuxiliaryLayers);
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Qwen3VL<T>));
    }
