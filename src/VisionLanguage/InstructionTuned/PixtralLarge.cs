...
public partial class PixtralLarge<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    private readonly PixtralLargeOptions _options;
    ...
    public PixtralLarge(
        ...
        string modelPath,
        PixtralLargeOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new PixtralLargeOptions();
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            // ComputePatchSize() validates patch options itself; gating the
            // default-path else-branch keeps custom Architecture.Layers
            // stacks (which never call ComputePatchSize) from being rejected
            // on unused options.
            Layers.AddRange(
    ...
    // Pixtral-Large (Mistral 2024): 1024 / sqrt(1024) = 32 — same Pixtral ViT.
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Pixtral-Large-Native" : "Pixtral-Large-ONNX",
            Description = "Pixtral Large: 124B Decoder + 1B Vision Encoder (Mistral, 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "Pixtral-Large";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(PixtralLarge<T>));
    }
