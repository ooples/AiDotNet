...
public partial class LLaVAOneVision<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    private readonly LLaVAOneVisionOptions _options;
    ...
    public LLaVAOneVision(
        NeuralNetworkArchitecture<T> architecture,
        string modelPath,
        LLaVAOneVisionOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new LLaVAOneVisionOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "LLaVA-OneVision-Native" : "LLaVA-OneVision-ONNX",
            Description = "LLaVA-OneVision: Easy Visual Task Transfer (Li et al., 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "LLaVA-OneVision";
    ...
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(LLaVAOneVision<T>));
    }
