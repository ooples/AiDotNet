...
[ResearchPaper("Qwen2-VL: Enhancing Vision-Language Model's Perception of the World at Any Resolution", "https://arxiv.org/abs/2409.12191", Year = 2024, Authors = "Wang et al.")]
public class Qwen2VL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    private readonly Qwen2VLOptions _options;
    ...
    public Qwen2VL(NeuralNetworkArchitecture<T> architecture, string modelPath, Qwen2VLOptions? options = null) : base(architecture)
    {
    ...
    /// <summary>
    /// Generates text using Qwen2-VL's dynamic-resolution + M-RoPE architecture.
    /// Qwen2-VL (Wang et al., 2024) features:
    /// (1) Naive Dynamic Resolution — process images at any aspect ratio,
    /// (2) M-RoPE — multimodal positional encoding across text/space/time,
    /// (3) Cross-attention resampler with learnable query tokens,
    /// (4) Native video understanding via temporal RoPE.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Qwen2-VL-Native" : "Qwen2-VL-ONNX",
            Description = "Qwen2-VL: Enhancing Vision-Language Model's Perception of the World at Any Resolution (Wang et al., 2024)",
        ...
        m.AdditionalInfo["Architecture"] = "Qwen2-VL";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Qwen2VL<T>(Architecture, mp, _options);
        return new Qwen2VL<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Qwen2VL<T>));
    }
