...
[ModelPaper("Qwen2.5-VL Technical Report", "https://arxiv.org/abs/2502.13923", Year = 2025, Authors = "Bai et al.")]
public class Qwen25VL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    private readonly Qwen25VLOptions _options; public override ModelOptions GetOptions() => _options;
    private readonly IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? _optimizer;
    private readonly ITokenizer? _tokenizer; private bool _useNativeMode; private bool _disposed;
    private int _visionLayerEnd; private int _resamplerLayerEnd;

    public Qwen25VL(NeuralNetworkArchitecture<T> architecture, string modelPath, Qwen25VLOptions? options = null) : base(architecture) { _options = options ?? new Qwen25VLOptions(); _useNativeMode = false; base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path cannot be null or empty.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    public Qwen25VL(NeuralNetworkArchitecture<T> architecture, Qwen25VLOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new Qwen25VLOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    /// <summary>
    /// Generates text using Qwen2.5-VL's visual agent architecture with localization.
    /// Qwen2.5-VL (2025) extends Qwen2-VL with:
    /// (1) 1hr+ video understanding: M-RoPE temporal encoding supports very long
    ///     video sequences with efficient temporal attention,
    /// (2) Bounding box and point localization: spatial position encoding enables
    ///     precise object localization output as coordinate tokens,
    /// (3) Enhanced visual agent: can follow complex multi-step visual instructions
    ///     with tool-use and structured output capabilities,
    /// (4) Qwen2.5 decoder backbone with improved instruction following.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "Qwen2.5-VL-Native" : "Qwen2.5-VL-ONNX", Description = "Qwen2.5-VL: Visual Agent with Long Video and Localization Capabilities (2025)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.DecoderDim, Complexity = _options.NumVisionLayers + _options.NumResamplerLayers + _options.NumDecoderLayers }; m.AdditionalInfo["Architecture"] = "Qwen2.5-VL"; m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString(); m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName; m.AdditionalInfo["MRoPE"] = _options.EnableMRoPE.ToString(); return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new Qwen25VL<T>(Architecture, mp, _options); return new Qwen25VL<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(Qwen25VL<T>)); }
