...
{
    private readonly Llama32VisionOptions _options;
    ...
    public Llama32Vision(
        NeuralNetworkArchitecture<T> architecture,
        string modelPath,
        Llama32VisionOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new Llama32VisionOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Llama-3.2-Vision-Native" : "Llama-3.2-Vision-ONNX",
            Description = "Llama 3.2 Vision: Meta's Vision Models for Edge/Mobile (2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "Llama-3.2-Vision";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Llama32Vision<T>(Architecture, mp, _options);
        return new Llama32Vision<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Llama32Vision<T>));
    }
