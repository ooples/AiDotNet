...
public class LLaVA15<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "LLaVA-1.5-Native" : "LLaVA-1.5-ONNX",
            Description =
                "LLaVA-1.5: Improved Baselines with Visual Instruction Tuning (Liu et al., 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "LLaVA-1.5";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new LLaVA15<T>(Architecture, mp, _options);
        return new LLaVA15<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(LLaVA15<T>));
    }
