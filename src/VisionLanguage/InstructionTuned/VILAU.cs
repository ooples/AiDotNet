...
public class VILAU<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public VILAU(
        ...
        string modelPath,
        VILAUOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new VILAUOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "VILA-U-Native" : "VILA-U-ONNX",
            Description =
                "VILA-U: unified VLM with autoregressive visual generation and understanding.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new VILAU<T>(Architecture, mp, _options);
        return new VILAU<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(VILAU<T>));
    }
