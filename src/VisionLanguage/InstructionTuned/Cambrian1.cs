...
public class Cambrian1<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Cambrian1(
        ...
        string modelPath,
        Cambrian1Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new Cambrian1Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Cambrian-1-Native" : "Cambrian-1-ONNX",
            Description = "Cambrian-1: Spatial Vision Aggregator VLM (NYU, 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "Cambrian-1";
        m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString();
        m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName;
        m.AdditionalInfo["NumVisionEncoders"] = _options.NumVisionEncoders.ToString();
        return m;
    }
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.NumVisionEncoders);
        writer.Write(_options.EnableSpatialVisionAggregator);
    }
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.NumVisionEncoders = reader.ReadInt32();
        _options.EnableSpatialVisionAggregator = reader.ReadBoolean();
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Cambrian1<T>(Architecture, mp, _options);
        return new Cambrian1<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Cambrian1<T>));
    }
