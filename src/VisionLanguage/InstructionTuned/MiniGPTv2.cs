...
public class MiniGPTv2<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    /// <summary>
    /// Generates text using MiniGPT-v2's unified multi-task Q-Former architecture.
    /// MiniGPT-v2 (Chen et al., 2023) extends MiniGPT-4 with:
    /// (1) Task-specific identifier tokens: [vqa], [caption], [grounding], [identify],
    ///     [refer] prefixed to instructions to route to task-specific behavior,
    /// (2) Concatenated visual tokens: Q-Former outputs are concatenated rather than
    ///     averaged, preserving spatial information for grounding tasks,
    /// (3) Three-stage training: (a) pretrain on broad datasets, (b) multi-task
    ///     training with task identifiers, (c) instruction fine-tuning,
    /// (4) LLaMA-2 decoder backbone with multi-task instruction support.
    /// </summary>
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "MiniGPT-v2-Native" : "MiniGPT-v2-ONNX", Description = "MiniGPT-v2: Large Language Model as a Unified Interface for Vision-Language Multi-task Learning (Chen et al., 2023)", ModelType = ModelType.NeuralNetwork, FeatureCount = _options.DecoderDim, Complexity = _options.NumVisionLayers + _options.NumQFormerLayers + _options.NumDecoderLayers }; m.AdditionalInfo["Architecture"] = "MiniGPT-v2"; m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString(); m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName; return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new MiniGPTv2<T>(Architecture, mp, _options); return new MiniGPTv2<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(MiniGPTv2<T>)); }
