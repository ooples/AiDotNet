...
public class Pixtral<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public Pixtral(
        ...
        string modelPath,
        PixtralOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new PixtralOptions();
    ...
    private void ComputeEncoderDecoderBoundary()
    {
        ...
        _encoderLayerEnd = 2 + _options.NumVisionLayers * lpb + 6;
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Pixtral-Native" : "Pixtral-ONNX",
            Description = "Pixtral: 12B Decoder + 400M Vision Encoder (Mistral, 2024)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "Pixtral";
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Pixtral<T>(Architecture, mp, _options);
        return new Pixtral<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Pixtral<T>));
    }
