...
[ResearchPaper("VILA: On Pre-training for Visual Language Models", "https://arxiv.org/abs/2312.07533", Year = 2024, Authors = "Lin et al.")]
public class VILA<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public VILA(NeuralNetworkArchitecture<T> architecture, VILAOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new VILAOptions(); _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
