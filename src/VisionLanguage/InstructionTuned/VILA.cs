...
public class VILA<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public VILA(
        ...
        string modelPath,
        VILAOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new VILAOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "VILA-Native" : "VILA-ONNX",
            Description =
                "VILA: VLM pre-trained with interleaved image-text data for enhanced in-context learning.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new VILA<T>(Architecture, mp, _options);
        return new VILA<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(VILA<T>));
    }
