...
[ResearchPaper(
    "AquilaVL: Advanced Vision-Language Model",
    "https://huggingface.co/BAAI/Aquila-VL-2B",
    Year = 2024,
    Authors = "BAAI"
)]
public class AquilaVL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public AquilaVL(
        ...
        string modelPath,
        AquilaVLOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new AquilaVLOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "AquilaVL-Native" : "AquilaVL-ONNX",
            Description =
                "AquilaVL: bilingual VLM aligned with Chinese and English visual-language understanding.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new AquilaVL<T>(Architecture, mp, _options);
        return new AquilaVL<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(AquilaVL<T>));
    }
