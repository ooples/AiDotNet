...
/// <para><b>For Beginners:</b> AquilaVL is a bilingual (Chinese/English) model that can look
...
/// details. Default values follow the original paper settings.</para>
/// </remarks>
/// <example>
/// <code>
/// // Create an AquilaVL model for bilingual Chinese/English visual QA
/// // using SigLIP encoder + MLP projector + Qwen2.5 decoder
...
/// var model = new AquilaVL&lt;double&gt;(architecture, "aquilavl.onnx");
///
/// // Training mode with native layers
/// var trainModel = new AquilaVL&lt;double&gt;(architecture, new AquilaVLOptions());
/// </code>
/// </example>
...
public class AquilaVL<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public AquilaVL(NeuralNetworkArchitecture<T> architecture, AquilaVLOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new AquilaVLOptions(); _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
