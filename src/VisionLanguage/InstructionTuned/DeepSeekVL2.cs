...
[ResearchPaper("DeepSeek-VL2: Mixture-of-Experts Vision-Language Models for Advanced Multimodal Understanding", "https://arxiv.org/abs/2412.10302", Year = 2024, Authors = "Wu et al.")]
public class DeepSeekVL2<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public DeepSeekVL2(NeuralNetworkArchitecture<T> architecture, DeepSeekVL2Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new DeepSeekVL2Options(); if (architecture.Layers == null) _options.ValidateVisualSizing(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    protected override void InitializeLayers()
    {
        ...
        else
        {
            ...
            Layers.AddRange(LayerHelper<T>.CreateDefaultTokenReductionVLMLayers(_options.VisionDim, _options.VisionDim * 2, _options.DecoderDim, _options.NumVisionLayers, _options.NumDecoderLayers, _options.NumHeads, _options.DropoutRate, patchSize: ComputePatchSize()));
