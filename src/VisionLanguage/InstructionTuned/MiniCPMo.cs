...
[ResearchPaper(
    "MiniCPM-o: A GPT-4o Level MLLM for Vision, Speech and Multimodal Live Streaming",
...
public class MiniCPMo<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public MiniCPMo(
        ...
        string modelPath,
        MiniCPMoOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new MiniCPMoOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "MiniCPM-o-Native" : "MiniCPM-o-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MiniCPMo<T>(Architecture, mp, _options);
        return new MiniCPMo<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MiniCPMo<T>));
    }
