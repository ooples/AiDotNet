...
/// <para><b>For Beginners:</b> InternVL 2.5 refines InternVL2 with better training data and
...
/// and visual question answering. Default values follow the original paper settings.</para>
/// </remarks>
/// <example>
/// <code>
/// // Create an InternVL 2.5 model with improved training recipes
/// // using InternViT-6B encoder with InternLM2.5 backbone
...
/// var model = new InternVL25&lt;double&gt;(architecture, "internvl25.onnx");
///
/// // Training mode with native layers
/// var trainModel = new InternVL25&lt;double&gt;(architecture, new InternVL25Options());
/// </code>
/// </example>
...
public class InternVL25<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public InternVL25(NeuralNetworkArchitecture<T> architecture, string modelPath, InternVL25Options? options = null) : base(architecture) { _options = options ?? new InternVL25Options(); _useNativeMode = false; base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path cannot be null or empty.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    public InternVL25(NeuralNetworkArchitecture<T> architecture, InternVL25Options? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new InternVL25Options(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    /// <summary>
    /// Generates text using InternVL2.5's improved pixel shuffle + dynamic resolution.
    /// InternVL2.5 (Chen et al., 2024) refines InternVL2 with:
    /// (1) Improved training data curation: higher quality instruction data with
    ///     better coverage of OCR, charts, and multi-language scenarios,
    /// (2) Enhanced pixel shuffle: same 2x2→1 compression with improved feature
    ///     aggregation via weighted channel mixing instead of simple averaging,
    /// (3) Better MLP alignment from refined training strategy,
    /// (4) InternLM2.5 decoder backbone with improved reasoning.
    /// </summary>
    ...
    protected override void InitializeLayers()
    {
        ...
        else { Layers.AddRange(LayerHelper<T>.CreateDefaultPixelShuffleProjectorLayers(_options.VisionDim, _options.VisionDim * 2, _options.DecoderDim, _options.NumVisionLayers, _options.NumDecoderLayers, _options.NumHeads, _options.DropoutRate, patchSize: ComputePatchSize())); ComputeEncoderDecoderBoundary(); }
    ...
    private void ComputeEncoderDecoderBoundary() { _encoderLayerEnd = ComputeVisionLanguageBoundary(leadingLayerCount: 2, visionLayerCount: _options.NumVisionLayers, visionBlockLayerCount: TransformerBlockLayerCount(_options.DropoutRate), trailingLayerCount: 5); }
    ...
    public override ModelMetadata<T> GetModelMetadata() { var m = new ModelMetadata<T> { Name = _useNativeMode ? "InternVL2.5-Native" : "InternVL2.5-ONNX", Description = "InternVL 2.5: Expanding Performance Boundaries of Open-Source Multimodal Models (Chen et al., 2024)", FeatureCount = _options.DecoderDim, Complexity = _options.NumVisionLayers + _options.NumDecoderLayers }; m.AdditionalInfo["Architecture"] = "InternVL2.5"; m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString(); m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName; m.AdditionalInfo["DynamicResolution"] = _options.EnableDynamicResolution.ToString(); return m; }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new InternVL25<T>(Architecture, mp, _options); return new InternVL25<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(InternVL25<T>)); }
