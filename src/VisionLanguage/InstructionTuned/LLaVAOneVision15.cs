...
public class LLaVAOneVision15<T> : VisionLanguageModelBase<T>, IInstructionTunedVLM<T>
{
    ...
    public LLaVAOneVision15(
        NeuralNetworkArchitecture<T> architecture,
        string modelPath,
        LLaVAOneVision15Options? options = null
    )
        : base(architecture)
    {
        _options = options ?? new LLaVAOneVision15Options();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "LLaVA-OV-1.5-Native" : "LLaVA-OV-1.5-ONNX",
            Description = "LLaVA-OneVision 1.5: Improved and Fully Open (Li et al., 2025)",
            FeatureCount = _options.DecoderDim,
            Complexity = _options.NumVisionLayers + _options.NumDecoderLayers,
        };
        m.AdditionalInfo["Architecture"] = "LLaVA-OneVision-1.5";
        m.AdditionalInfo["InstructionType"] = _options.InstructionArchitectureType.ToString();
        m.AdditionalInfo["LanguageModel"] = _options.LanguageModelName;
        m.AdditionalInfo["VideoEnabled"] = _options.EnableVideo.ToString();
        return m;
    }
    ...
    protected override void SerializeNetworkSpecificData(BinaryWriter writer)
    {
        ...
        writer.Write(_options.EnableVideo);
    ...
    protected override void DeserializeNetworkSpecificData(BinaryReader reader)
    {
        ...
        _options.EnableVideo = reader.ReadBoolean();
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new LLaVAOneVision15<T>(Architecture, mp, _options);
        return new LLaVAOneVision15<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(LLaVAOneVision15<T>));
    }
