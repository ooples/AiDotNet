...
[ResearchPaper(
    ...
    "https://arxiv.org/abs/2310.05126",
    Year = 2024,
    Authors = "Ye et al."
)]
public class UReader<T> : VisionLanguageModelBase<T>, IDocumentUnderstandingModel<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "UReader-Native" : "UReader-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new UReader<T>(Architecture, mp, _options);
        return new UReader<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(UReader<T>));
    }
