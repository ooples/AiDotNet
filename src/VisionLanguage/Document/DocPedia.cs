...
[ResearchPaper(
    "DocPedia: Unleashing the Power of Large Multimodal Model in the Frequency Domain",
    "https://arxiv.org/abs/2311.11810",
    Year = 2024,
    Authors = "Feng et al."
)]
public class DocPedia<T> : VisionLanguageModelBase<T>, IDocumentUnderstandingModel<T>
{
    ...
    {
        ThrowIfDisposed();
        return GenerateFromImage(documentImage, "Extract all text from this document.");
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "DocPedia-Native" : "DocPedia-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new DocPedia<T>(Architecture, mp, _options);
        return new DocPedia<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(DocPedia<T>));
    }
