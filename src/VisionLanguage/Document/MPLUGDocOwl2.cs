...
public class MPLUGDocOwl2<T> : VisionLanguageModelBase<T>, IDocumentUnderstandingModel<T>
{
    ...
    public MPLUGDocOwl2(
        ...
        string modelPath,
        MPLUGDocOwl2Options? options = null
    )
        : base(architecture)
    {
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "mPLUG-DocOwl-2-Native" : "mPLUG-DocOwl-2-ONNX",
            Description =
                "mPLUG-DocOwl 2: high-res compressing for multi-page document understanding.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MPLUGDocOwl2<T>(Architecture, mp, _options);
        return new MPLUGDocOwl2<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MPLUGDocOwl2<T>));
    }
