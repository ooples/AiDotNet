...
[ResearchPaper(
    "Surya: Multi-language OCR Toolkit",
...
public class Surya<T> : VisionLanguageModelBase<T>, IDocumentUnderstandingModel<T>
{
    ...
    public Surya(
        ...
        string modelPath,
        SuryaOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new SuryaOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Surya-Native" : "Surya-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new Surya<T>(Architecture, mp, _options);
        return new Surya<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(Surya<T>));
    }
