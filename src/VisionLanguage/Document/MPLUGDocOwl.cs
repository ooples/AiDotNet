...
public class MPLUGDocOwl<T> : VisionLanguageModelBase<T>, IDocumentUnderstandingModel<T>
{
    ...
    public MPLUGDocOwl(
        ...
        string modelPath,
        MPLUGDocOwlOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new MPLUGDocOwlOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "mPLUG-DocOwl-Native" : "mPLUG-DocOwl-ONNX",
            Description =
                "mPLUG-DocOwl: modular MLLM for document understanding with visual abstractor.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new MPLUGDocOwl<T>(Architecture, mp, _options);
        return new MPLUGDocOwl<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(MPLUGDocOwl<T>));
    }
