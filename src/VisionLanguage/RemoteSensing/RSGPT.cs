...
[ResearchPaper("RSGPT: A Remote Sensing Vision Language Model and Benchmark", "https://arxiv.org/abs/2307.15266", Year = 2023, Authors = "Hu et al.")]
public class RSGPT<T> : VisionLanguageModelBase<T>, IRemoteSensingVLM<T>
{
    ...
    public RSGPT(NeuralNetworkArchitecture<T> architecture, string modelPath, RSGPTOptions? options = null) : base(architecture) { _options = options ?? new RSGPTOptions(); _useNativeMode = false; base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; if (string.IsNullOrWhiteSpace(modelPath)) throw new ArgumentException("Model path cannot be null or empty.", nameof(modelPath)); if (!File.Exists(modelPath)) throw new FileNotFoundException($"ONNX model not found: {modelPath}", modelPath); _options.ModelPath = modelPath; OnnxModel = new OnnxModel<T>(modelPath, _options.OnnxOptions); _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    public RSGPT(NeuralNetworkArchitecture<T> architecture, RSGPTOptions? options = null, IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null) : base(architecture) { _options = options ?? new RSGPTOptions(); _useNativeMode = true; _optimizer = optimizer ?? new AdamWOptimizer<T, Tensor<T>, Tensor<T>>(this); base.ImageSize = _options.ImageSize; base.ImageChannels = 3; base.EmbeddingDim = _options.DecoderDim; _tokenizer = ClipTokenizerFactory.CreateSimple(vocabSize: _options.VocabSize); InitializeLayers(); }
    ...
    /// <summary>
    /// Generates text from a remote sensing image using RSGPT's InstructBLIP pipeline.
    /// Per the paper (Various, 2024), RSGPT uses InstructBLIP architecture for RS via:
    /// (1) EVA-G ViT encoder (VisionDim=1408, 39 layers) for high-resolution RS features,
    /// (2) Q-Former: a lightweight transformer with learned query tokens that cross-attend
    ///     to the frozen ViT features, compressing spatial information into a fixed number
    ///     of query embeddings while being conditioned on the instruction text,
    /// (3) Instruction-aware visual feature selection: the Q-Former queries are modulated
    ///     by the instruction text, so different queries activate for different tasks
    ///     (captioning, VQA, scene classification),
    /// (4) Linear projection to Vicuna LLM embedding space.
    /// </summary>
    public Tensor<T> GenerateFromImage(Tensor<T> image, string? prompt = null)
    {
        ...
        // Step 1: EVA-G ViT encoding (39 layers, 1408-dim)
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance() { if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp)) return new RSGPT<T>(Architecture, mp, _options); return new RSGPT<T>(Architecture, _options); }
    private void ThrowIfDisposed() { if (_disposed) throw new ObjectDisposedException(GetType().FullName ?? nameof(RSGPT<T>)); }
