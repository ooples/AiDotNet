...
public class RSGPT<T> : VisionLanguageModelBase<T>, IRemoteSensingVLM<T>
{
    ...
    public RSGPT(
        ...
        string modelPath,
        RSGPTOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new RSGPTOptions();
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "RSGPT-Native" : "RSGPT-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new RSGPT<T>(Architecture, mp, _options);
        return new RSGPT<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(RSGPT<T>));
    }
