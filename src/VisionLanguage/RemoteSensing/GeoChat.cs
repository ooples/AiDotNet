...
public class GeoChat<T> : VisionLanguageModelBase<T>, IRemoteSensingVLM<T>
{
    ...
    public string SupportedBands => _options.SupportedBands;
    ...
    public Tensor<T> AnswerRemoteSensingQuestion(Tensor<T> image, string question) =>
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "GeoChat-Native" : "GeoChat-ONNX",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new GeoChat<T>(Architecture, mp, _options);
        return new GeoChat<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(GeoChat<T>));
    }
