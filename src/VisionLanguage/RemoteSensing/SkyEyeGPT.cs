...
[ResearchPaper(
    ...
    "https://arxiv.org/abs/2401.09712",
    Year = 2024,
    Authors = "Zhan et al."
)]
public class SkyEyeGPT<T> : VisionLanguageModelBase<T>, IRemoteSensingVLM<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "SkyEyeGPT-Native" : "SkyEyeGPT-ONNX",
            Description =
                "SkyEyeGPT: unified remote sensing vision-language tasks with 968K instruction samples.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new SkyEyeGPT<T>(Architecture, mp, _options);
        return new SkyEyeGPT<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(SkyEyeGPT<T>));
    }
