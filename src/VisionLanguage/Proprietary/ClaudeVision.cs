...
public class ClaudeVision<T> : VisionLanguageModelBase<T>, IProprietaryVLM<T>
{
    ...
    public ClaudeVision(
        ...
        string modelPath,
        ClaudeVisionOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new ClaudeVisionOptions();
    ...
    protected override void InitializeLayers()
    {
        ...
        {
            Layers.AddRange(
                LayerHelper<T>.CreateDefaultProprietaryAPILayers(
                    _options.VisionDim,
                    _options.DecoderDim,
    ...
    {
        // Single source of truth shared with CreateDefaultProprietaryAPILayers (and GeminiVision /
        // GrokVision) so the encoder/decoder boundary can't drift from the actual layer layout.
        _encoderLayerEnd = LayerHelper<T>.ComputeProprietaryAPIEncoderBoundary(
            _options.NumVisionLayers, _options.DropoutRate);
    }
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Claude-Vision-Native" : "Claude-Vision-ONNX",
            Description =
                "Claude Vision: reference implementation of Anthropic's multimodal reasoning model.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new ClaudeVision<T>(Architecture, mp, _options);
        return new ClaudeVision<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(ClaudeVision<T>));
    }
