...
public class GrokVision<T> : VisionLanguageModelBase<T>, IProprietaryVLM<T>
{
    ...
    public override Tensor<T> Predict(Tensor<T> input) { ThrowIfDisposed(); if (IsOnnxMode && OnnxModel is not null) return OnnxModel.Run(input); return Accelerate(input, () => { var c = input; foreach (var l in Layers) c = l.Forward(c); return c; }); }
