...
[ResearchPaper("Grok", "https://x.ai/grok")]
public class GrokVision<T> : VisionLanguageModelBase<T>, IProprietaryVLM<T>
{
    private readonly GrokVisionOptions _options;
    ...
    public GrokVision(
        ...
        string modelPath,
        GrokVisionOptions? options = null
    )
        : base(architecture)
    {
        _options = options ?? new GrokVisionOptions();
    ...
    private void ComputeEncoderDecoderBoundary()
    {
        ...
        // GeminiVision) so the encoder/decoder boundary can't drift from the actual layer layout.
    ...
    public override ModelMetadata<T> GetModelMetadata()
    {
        ...
        {
            Name = _useNativeMode ? "Grok-Vision-Native" : "Grok-Vision-ONNX",
            Description =
                "Grok Vision: reference implementation of xAI's real-time multimodal model.",
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        if (!_useNativeMode && _options.ModelPath is { } mp && !string.IsNullOrEmpty(mp))
            return new GrokVision<T>(Architecture, mp, _options);
        return new GrokVision<T>(Architecture, _options);
    }

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(GetType().FullName ?? nameof(GrokVision<T>));
    }
