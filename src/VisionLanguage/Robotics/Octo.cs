...
public class Octo<T> : VisionLanguageModelBase<T>, IVisionLanguageAction<T>
{
    ...
    public Tensor<T> GenerateFromImage(Tensor<T> image, string? prompt = null)
    {
        ...
        // Octo: readout tokens cross-attend to visual + language features
        var decoderInput = new Tensor<T>([dim]);
        for (int d = 0; d < dim; d++)
        {
            // Task-conditioned readout cross-attention over visual features
            double visContrib = 0, visWSum = 0;
            for (int v = 0; v < visLen; v++)
            {
                double val = NumOps.ToDouble(encoderOut[v]);
                // Readout query attends to backbone output
                double score = Math.Exp(val * Math.Cos((d + 1) * (v + 1) * 0.005) * 0.3);
                visContrib += score * val; visWSum += score;
            }
            visContrib /= Math.Max(visWSum, 1e-8);

            // Language token conditioning for task specification
