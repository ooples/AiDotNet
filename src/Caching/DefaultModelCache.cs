...
public class DefaultModelCache<T, TInput, TOutput> : IModelCache<T, TInput, TOutput>
{
    ...
    ///
    /// The key is generated by combining hash codes from both the model's parameters and the input data.
    ...
    public string GenerateCacheKey(IFullModel<T, TInput, TOutput> solution, OptimizationInputData<T, TInput, TOutput> inputData)
    {
        ...
        // Generate hash from solution parameters
        int solutionHash = ComputeParameterHash(solution.ParameterCount);
    ...
    /// <summary>
    /// Computes a hash code from model parameters.
    /// </summary>
    /// <param name="parameterCount">The number of parameters in the model.</param>
    /// <returns>A hash code representing the parameters.</returns>
    private int ComputeParameterHash(int parameterCount)
    {
        unchecked
        {
            int hash = 17;
