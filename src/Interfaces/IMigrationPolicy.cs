...
/// (non-null transfers, distinct in-range island indices, at most <c>migrantsPerIsland</c> transfers for any
/// one ordered source-to-destination pair, a round no larger than the ordered island pairs times that bound,
/// and an entry that is genuinely present in its source island), orders the transfers deterministically by
/// source, destination, and genome identifier, and then offers a marked copy of each migrant to its
/// destination archive through the normal insertion rules, so a migrant that does not beat the destination's
/// incumbent is simply not copied. The bound is per destination rather than per source precisely so that a
/// broadcasting topology such as a star or a fully connected graph, where one island legitimately feeds every
/// other, is expressible. Any randomness must come from the supplied <see cref="StableRandom"/> stream so
/// that resumed runs reproduce the same transfers. <see cref="Id"/> and <see cref="VersionHash"/> are folded
/// into the checkpoint compatibility hash, so swapping or revising a policy prevents an older checkpoint from
/// being resumed under different migration semantics.
/// </para>
...
/// built-in <see cref="TopologyMigrationPolicy{TGenome}"/>, which the engine uses unless you supply your own,
/// copies each island's top elites along whichever topology you configured, and
/// <see cref="RingMigrationPolicy{TGenome}"/> is the fixed one-directional ring. Implement this interface
/// yourself only when you need something those cannot express, such as migrating random rather than best
/// elites or routing by a similarity measure. Because the engine hands you a seeded random stream, any
/// randomness you use stays reproducible and checkpoint-safe.</para>
/// <para>
...
public interface IMigrationPolicy<TGenome>
{
    ...
    /// <param name="migrantsPerIsland">
    /// The maximum number of elites one source island may send to any single destination island. A policy that
    /// feeds several destinations emits this many transfers per destination.
    /// </param>
