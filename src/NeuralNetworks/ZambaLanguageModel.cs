...
/// </remarks>
public class ZambaLanguageModel<T> : LayerBase<T>
{
    ...
    /// <summary>
    /// Creates a new Zamba language model with Mamba backbone + shared attention.
    /// </summary>
    /// <param name="attentionInterval">Every Nth block uses the shared attention layer (default 6).</param>
    public ZambaLanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 12,
        int stateDimension = 16, int attentionInterval = 6, int maxSeqLength = 512, IActivationFunction<T>? activationFunction = null)
        : base([maxSeqLength, vocabSize], [maxSeqLength, vocabSize], activationFunction ?? new ActivationFunctions.IdentityActivation<T>())
    {
        ...
        // Build hybrid block array: Mamba backbone with shared attention at regular intervals
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "zamba_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "zamba_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "zamba_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "zamba_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "zamba_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["Architecture"] = "Zamba"; return m; }
}
