...
/// <example>
/// <code>
/// var options = new LSTMNeuralNetworkOptions { InputSize = 10, HiddenSize = 128, NumLayers = 2 };
...
public class LSTMNeuralNetwork<T> : NeuralNetworkBase<T>
{
        ...
        Tensor<T> input, Tensor<T> hiddenState, Tensor<T> cellState, int layerIndex)
    {
        ...
        var newCellState = TensorAllocator.Rent<T>(cellState.Shape._dims);
        var newHiddenState = TensorAllocator.Rent<T>(hiddenState.Shape._dims);
    ...
    public override void Train(Tensor<T> input, Tensor<T> expectedOutput)
    {
        ...
        var outputGradients = new Tensor<T>(predictions.Shape._dims, gradientVector);
    ...
    private void BackpropagateOverTime(Tensor<T> outputGradients, Tensor<T> input)
    {
        ...
        if (Architecture.ShouldReturnFullSequence)
        {
            ...
            for (int t = 0; t < sequenceLength; t++)
            {
                ...
                if (shape.Length > 1)
                {
                    Array.Copy(outputGradients.Shape._dims, 2, shape, 1, shape.Length - 1);
                }
        ...
        else
        {
            ...
            for (int t = 0; t < sequenceLength - 1; t++)
            {
                ...
                gradientPerTimeStep[t] = new Tensor<T>(outputGradients.Shape._dims);
            }
    ...
    private Tensor<T> TransformHiddenGradientToOutputGradient(Tensor<T> hiddenGradient, int[] outputShape)
    {
        ...
        if (hiddenGradient.Shape._dims.SequenceEqual(outputShape))
        {
