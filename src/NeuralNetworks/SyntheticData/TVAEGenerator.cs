...
[PaperOptimizer(OptimizerKind.Adam, LearningRate = 2e-4,
                Source = "Xu et al. 2019, Sec. 4: the Adam optimizer with a learning rate of 2e-4.")]
public partial class TVAEGenerator<T> : NeuralSyntheticTabularGeneratorBase<T>, ISyntheticTabularGenerator<T>
{
    ...
    public TVAEGenerator(NeuralNetworkArchitecture<T> architecture,
        TVAEOptions<T>? options = null,
        IGradientBasedOptimizer<T, Tensor<T>, Tensor<T>>? optimizer = null,
        ILossFunction<T>? lossFunction = null)
        : base(architecture, lossFunction ?? NeuralNetworkHelper<T>.GetDefaultLossFunction(architecture.TaskType), (options ??= new TVAEOptions<T>()).MaxGradNorm)
    {
        _options = options;
