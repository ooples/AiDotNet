    ...
    IUpstreamBiasRedundancy, IDisposable
{
    ...
    /// overrides Forward keeps working and is simply invisible to tracing - a silent coverage hole, so
    /// ADNSHAPE004 reports each one at build time and the count only goes down. Sealing it is the last
    /// step, once that diagnostic reads zero.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// The trainable-only sibling supports gradients and copy-on-write. This state view also includes
    /// buffers because a checkpoint that drops running state cannot reproduce the original model.
    /// Legacy storage is carried by <see cref="Parameters"/> itself and therefore has no component slot.
    /// </remarks>
