...
public class AveragePoolingLayer<T> : LayerBase<T>
{
    ...
    /// </remarks>
    public AveragePoolingLayer(int poolSize, int strides)
        : base(new[] { -1, -1, -1 }, new[] { -1, -1, -1 })
    {
        if (poolSize <= 0) throw new ArgumentOutOfRangeException(nameof(poolSize));
        if (strides <= 0) throw new ArgumentOutOfRangeException(nameof(strides));
        PoolSize = poolSize;
        Strides = strides;
    }

    /// <summary>
    /// Resolves input shape on first forward (PyTorch AvgPool2d-style).
    /// </summary>
    protected override void OnFirstForward(Tensor<T> input)
    {
        ...
        if (rank == 4) { c = input.Shape[1]; h = input.Shape[2]; w = input.Shape[3]; }
            ...
            $"AveragePoolingLayer requires rank-3 [C,H,W] or rank-4 [B,C,H,W] input; got rank {rank}.",
            nameof(input));

        int outH = (h - PoolSize) / Strides + 1;
        int outW = (w - PoolSize) / Strides + 1;
