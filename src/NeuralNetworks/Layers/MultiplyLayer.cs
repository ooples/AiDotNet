    ...
    public override Tensor<T> Forward(IReadOnlyDictionary<string, Tensor<T>> inputs)
    {
        ...
        for (int i = 0; ; i++)
        {
            if (!inputs.TryGetValue($"input_{i}", out var tensor))
                break;
            if (tensor == null)
                throw new ArgumentException($"MultiplyLayer input 'input_{i}' must not be null.", nameof(inputs));
            tensors.Add(tensor);
        }
        if (tensors.Count < 2)
            throw new ArgumentException($"MultiplyLayer requires at least 'input_0' and 'input_1'. Got {tensors.Count} sequential inputs.", nameof(inputs));
