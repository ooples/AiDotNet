...
public class PrimaryCapsuleLayer<T> : LayerBase<T>
{
    ...
    public override IGpuTensor<T> ForwardGpu(params IGpuTensor<T>[] inputs)
    {
        ...
        // Parse input dimensions - detect NCHW vs NHWC format
        int batchSize, inputHeight, inputWidth;
        bool inputIsNCHW = false;

        if (rank == 3)
        {
            if (inputShape[0] == _inputChannels)
            {
                inputIsNCHW = true;
                batchSize = 1;
        ...
        {
            if (inputShape[1] == _inputChannels && inputShape[3] != _inputChannels)
            {
                inputIsNCHW = true;
            }
            batchSize = inputShape[0];
            inputHeight = inputIsNCHW ? inputShape[2] : inputShape[1];
            inputWidth = inputIsNCHW ? inputShape[3] : inputShape[2];
        }
        else
        {
            batchSize = 1;
            inputHeight = inputShape.Length > 2 ? inputShape[rank - 3] : 1;
            inputWidth = inputShape.Length > 1 ? inputShape[rank - 2] : 1;
        }
        ...
        // Upload convolution weights: [outputChannels, inputChannels * kernelSize * kernelSize]
        using var weightsBuffer = backend.AllocateBuffer(DirectGpuEngine.ToFloatArray<T>(_convWeights.Data));
        using var biasBuffer = backend.AllocateBuffer(DirectGpuEngine.ToFloatArray<T>(_convBias.Data));

        // Im2Col transformation for efficient convolution
        int patchSize = _inputChannels * _kernelSize * _kernelSize;
        int numPatches = outputHeight * outputWidth;
        var patchMatrix = new float[batchSize * numPatches * patchSize];
        ...
        for (int b = 0; b < batchSize; b++)
        {
            ...
            for (int oh = 0; oh < outputHeight; oh++)
            {
                ...
                {
                    int patchOffset = (b * numPatches + patchIdx) * patchSize;
                    int colIdx = 0;
                    ...
                    for (int c = 0; c < _inputChannels; c++)
                    {
                        for (int kh = 0; kh < _kernelSize; kh++)
                        {
                            ...
                            {
                                int ih = oh * _stride + kh;
                                int iw = ow * _stride + kw;

                                float val;
                                if (inputIsNCHW)
                                {
                                    val = inputData[((b * _inputChannels + c) * inputHeight + ih) * inputWidth + iw];
                                }
                                else
                                {
                                    val = inputData[((b * inputHeight + ih) * inputWidth + iw) * _inputChannels + c];
                                }
                                patchMatrix[patchOffset + colIdx] = val;
                                colIdx++;
                            }
                        }
                    }
        ...
        // GEMM: [batch*numPatches, patchSize] @ [patchSize, outputChannels]^T = [batch*numPatches, outputChannels]
        // Weights are [outputChannels, patchSize], so we need to transpose
        var weightsData = backend.DownloadBuffer(weightsBuffer);
        var weightsTransposed = new float[patchSize * outputChannels];
        for (int i = 0; i < outputChannels; i++)
        {
            for (int j = 0; j < patchSize; j++)
            {
                weightsTransposed[j * outputChannels + i] = weightsData[i * patchSize + j];
            }
        }

        using var patchBuffer = backend.AllocateBuffer(patchMatrix);
        using var weightsTransBuffer = backend.AllocateBuffer(weightsTransposed);
        using var convOutputBuffer = backend.AllocateBuffer(new float[batchSize * numPatches * outputChannels]);

        backend.Gemm(patchBuffer, weightsTransBuffer, convOutputBuffer,
            batchSize * numPatches, outputChannels, patchSize);

        // Add bias
        var convOutput = backend.DownloadBuffer(convOutputBuffer);
        var biasData = backend.DownloadBuffer(biasBuffer);

        for (int i = 0; i < batchSize * numPatches; i++)
        {
            for (int c = 0; c < outputChannels; c++)
            {
                convOutput[i * outputChannels + c] += biasData[c];
            }
        }

        // Reshape to capsules: [batch, outH, outW, capsuleChannels, capsuleDimension]
        // Then apply Squash to each capsule vector
        int totalCapsules = batchSize * outputHeight * outputWidth * _capsuleChannels;
        var outputData = new float[totalCapsules * _capsuleDimension];

        // Reshape and apply Squash
        for (int b = 0; b < batchSize; b++)
        {
            for (int oh = 0; oh < outputHeight; oh++)
            {
                for (int ow = 0; ow < outputWidth; ow++)
                {
                    int patchIdx = b * numPatches + oh * outputWidth + ow;

                    for (int cc = 0; cc < _capsuleChannels; cc++)
                    {
                        int capsuleIdx = ((b * outputHeight + oh) * outputWidth + ow) * _capsuleChannels + cc;
                        int capsuleOffset = capsuleIdx * _capsuleDimension;

                        // Extract capsule vector and compute squared norm
                        float normSq = 0f;
                        for (int d = 0; d < _capsuleDimension; d++)
                        {
                            int convIdx = patchIdx * outputChannels + cc * _capsuleDimension + d;
                            float val = convOutput[convIdx];
                            outputData[capsuleOffset + d] = val;
                            normSq += val * val;
                        }

                        // Apply Squash: v = ||s||^2 / (1 + ||s||^2) * s / ||s||
                        ...
                        for (int d = 0; d < _capsuleDimension; d++)
                        {
                            outputData[capsuleOffset + d] *= scale * invNorm;
                        }
                    }
                }
            }
        ...
        return new GpuTensor<T>(backend, outputBuffer, new[] { batchSize, outputHeight, outputWidth, _capsuleChannels, _capsuleDimension }, GpuTensorRole.Activation, ownsBuffer: true);
    }
