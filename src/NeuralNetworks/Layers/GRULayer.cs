...
public class GRULayer<T> : LayerBase<T>
{
    ...
    public override Tensor<T> Forward(Tensor<T> input)
    {
        ...
        for (int t = 0; t < sequenceLength; t++)
        {
            ...
            if (_cachedOnesForGate == null || !_cachedOnesForGate.Shape._dims.SequenceEqual(z.Shape._dims))
            {
                _cachedOnesForGate = Tensor<T>.CreateDefault(z.Shape._dims, NumOps.One);
            }
    ...
    private Tensor<T> BackwardManual(Tensor<T> outputGradient)
    {
        ...
        if (!_returnSequences || _allHiddenStates == null || _allHiddenStates.Count == 0)
        {
            ...
            if (dh.Length != _lastZ.Length)
            {
                ...
                dh = new Tensor<T>(_lastZ.Shape._dims, new Vector<T>(dhData));
            }
            else if (!dh.Shape._dims.SequenceEqual(_lastZ.Shape._dims))
            {
                dh = dh.Reshape(_lastZ.Shape._dims);
            }

            // Vectorized: compute (1 - _lastZ) using Tensor operations
            var ones1 = new Tensor<T>(_lastZ.Shape._dims);
            ...
            var h_prev_for_dz = (_allHiddenStates != null && _allHiddenStates.Count >= 2)
                ...
                : new Tensor<T>(_lastH.Shape._dims); // zeros for first timestep
            ...
            var onesR = new Tensor<T>(_lastR.Shape._dims);
        ...
        else
        {
            ...
            for (int t = 0; t < sequenceLength; t++)
            {
                ...
                var ones2 = new Tensor<T>(z.Shape._dims);
            ...
            for (int t = sequenceLength - 1; t >= 0; t--)
            {
                ...
                // Vectorized: compute (1 - z) using Tensor operations
                var ones3 = new Tensor<T>(z.Shape._dims);
                ...
                var onesR = new Tensor<T>(r.Shape._dims);
                onesR.Fill(NumOps.One);
    ...
    public override void UpdateParameters(T learningRate)
    {
        ...
        if (Engine is DirectGpuTensorEngine gpuEngine)
        {
            ...
            if (_WzVelocity == null)
            {
                _WzVelocity = new Tensor<T>(_Wz.Shape._dims); _WzVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_WzVelocity, PersistentTensorRole.OptimizerState);
                _WrVelocity = new Tensor<T>(_Wr.Shape._dims); _WrVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_WrVelocity, PersistentTensorRole.OptimizerState);
                _WhVelocity = new Tensor<T>(_Wh.Shape._dims); _WhVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_WhVelocity, PersistentTensorRole.OptimizerState);
                _UzVelocity = new Tensor<T>(_Uz.Shape._dims); _UzVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_UzVelocity, PersistentTensorRole.OptimizerState);
                _UrVelocity = new Tensor<T>(_Ur.Shape._dims); _UrVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_UrVelocity, PersistentTensorRole.OptimizerState);
                _UhVelocity = new Tensor<T>(_Uh.Shape._dims); _UhVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_UhVelocity, PersistentTensorRole.OptimizerState);
                _bzVelocity = new Tensor<T>(_bz.Shape._dims); _bzVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_bzVelocity, PersistentTensorRole.OptimizerState);
                _brVelocity = new Tensor<T>(_br.Shape._dims); _brVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_brVelocity, PersistentTensorRole.OptimizerState);
                _bhVelocity = new Tensor<T>(_bh.Shape._dims); _bhVelocity.Fill(NumOps.Zero); gpuEngine.RegisterPersistentTensor(_bhVelocity, PersistentTensorRole.OptimizerState);
            }
    ...
    {
        var ones = new Tensor<T>(tensor.Shape._dims);
    ...
    public override IGpuTensor<T> BackwardGpu(IGpuTensor<T> outputGradient)
    {
        ...
        var shape = _gpuLastInput.Shape._dims;
        int rank = shape.Length;
        int timeSteps;
        int batchSize;
