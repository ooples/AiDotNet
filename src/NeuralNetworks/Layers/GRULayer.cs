...
public partial class GRULayer<T> : LayerBase<T>
{
    ...
    public override void UpdateParametersGpu(IGpuOptimizerConfig config)
    {
        ...
        _gpuWz ??= Tensor<T>.FromGpuBuffer(backend, _Wz, GpuTensorRole.Weight);
        _gpuWr ??= Tensor<T>.FromGpuBuffer(backend, _Wr, GpuTensorRole.Weight);
        _gpuWh ??= Tensor<T>.FromGpuBuffer(backend, _Wh, GpuTensorRole.Weight);
        _gpuUz ??= Tensor<T>.FromGpuBuffer(backend, _Uz, GpuTensorRole.Weight);
        _gpuUr ??= Tensor<T>.FromGpuBuffer(backend, _Ur, GpuTensorRole.Weight);
        _gpuUh ??= Tensor<T>.FromGpuBuffer(backend, _Uh, GpuTensorRole.Weight);
        _gpuBz ??= Tensor<T>.FromGpuBuffer(backend, _bz, GpuTensorRole.Bias);
        _gpuBr ??= Tensor<T>.FromGpuBuffer(backend, _br, GpuTensorRole.Bias);
        _gpuBh ??= Tensor<T>.FromGpuBuffer(backend, _bh, GpuTensorRole.Bias);
    ...
    private void EnsureGruOptimizerState(IGpuOptimizerConfig config, IDirectGpuBackend backend)
    {
        ...
        if (optimizerType == GpuOptimizerType.Sgd || optimizerType == GpuOptimizerType.Nag || optimizerType == GpuOptimizerType.Lars)
        {
            ...
            _gpuWzVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWrVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWhVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUzVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUrVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUhVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBzVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBrVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBhVelocity ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
        }
        else if (optimizerType == GpuOptimizerType.Adam || optimizerType == GpuOptimizerType.AdamW)
        {
            ...
            _gpuWzM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWrM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWhM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUzM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUrM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUhM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBzM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBrM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBhM ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);

            // Second moment (V) tensors
            _gpuWzV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWrV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuWhV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([inputWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUzV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUrV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuUhV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([hiddenWeightSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBzV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBrV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
            _gpuBhV ??= Tensor<T>.FromGpuBuffer(backend, Tensor<T>.CreateDefault([biasSize], NumOps.Zero), GpuTensorRole.OptimizerState);
        }
    }
