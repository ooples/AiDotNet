...
/// <typeparam name="T">The numeric type used for tensor values and trainable parameters.</typeparam>
/// <remarks>
/// <para><b>For Beginners:</b> Local mixing lets each visual token attend to nearby text strokes,
/// while global mixing lets later blocks connect the full word. The residual MLP then refines each
/// token without changing the image-token grid.</para>
...
public partial class SVTRMixingBlockLayer<T> : LayerBase<T>
{
    ...
    private Tensor<bool>? _localMaskPlane;
    private readonly object _localMaskLock = new();
    ...
    private Tensor<bool> GetLocalMask(int batch)
    {
        ...
        var plane = System.Threading.Volatile.Read(ref _localMaskPlane);
        if (plane is null)
        {
            lock (_localMaskLock)
            {
                plane = _localMaskPlane;
                if (plane is null)
                {
                    plane = new Tensor<bool>([sequence, sequence]);
                    ...
                    for (int query = 0; query < sequence; query++)
                    {
                        ...
                        for (int key = 0; key < sequence; key++)
                        {
                            int ky = key / _width;
                            int kx = key % _width;
                            plane[query, key] = Math.Abs(qy - ky) <= radiusY &&
                                Math.Abs(qx - kx) <= radiusX;
                        }
                    }
                    System.Threading.Volatile.Write(ref _localMaskPlane, plane);
                }
            }
        }
