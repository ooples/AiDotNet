...
[LayerProperty(IsTrainable = true, IsStateful = true, TestInputShape = "1, 4", TestConstructorArgs = "4")]
public class AnomalyDetectorLayer<T> : LayerBase<T>
{
    ...
    {
        _lastInputShape = input.Shape._dims;
        ...
        int[] lengthActual = (int[])input.Shape._dims.Clone();
        lengthActual[rank - 1] = halfSize;

        int[] startPred = new int[rank];
        startPred[rank - 1] = halfSize;
        int[] lengthPred = (int[])input.Shape._dims.Clone();
    ...
    private Tensor<T> CalculateAnomalyScores(Tensor<T> actual, Tensor<T> predicted)
    {
        ...
        var scores = new Tensor<T>(totalActiveSum.Shape._dims);
