...
public class HighwayLayer<T> : LayerBase<T>, IAuxiliaryLossLayer<T>
{
    ...
    public override IGpuTensor<T> ForwardGpu(params IGpuTensor<T>[] inputs)
    {
        ...
        else
        {
            ...
            transformOutput = new GpuTensor<T>(backend, tanhBuffer, input.Shape._dims, GpuTensorRole.Activation, ownsBuffer: true);
        }
        ...
        else
        {
            ...
            gateOutput = new GpuTensor<T>(backend, sigmoidBuffer, input.Shape._dims, GpuTensorRole.Activation, ownsBuffer: true);
        }
    ...
    private Tensor<T> BackwardManual(Tensor<T> outputGradient)
    {
        ...
        // (1 - gate) contribution
        var ones = Tensor<T>.CreateDefault(_lastGateOutput.Shape._dims, NumOps.One);
    ...
    public override void SetParameters(Vector<T> parameters)
    {
        ...
        _transformWeights = new Tensor<T>(_transformWeights.Shape._dims, parameters.Slice(index, transformWeightsSize));
        index += transformWeightsSize;

        _transformBias = new Tensor<T>(_transformBias.Shape._dims, parameters.Slice(index, _transformBias.Length));
        index += _transformBias.Length;

        _gateWeights = new Tensor<T>(_gateWeights.Shape._dims, parameters.Slice(index, gateWeightsSize));
        index += gateWeightsSize;

        _gateBias = new Tensor<T>(_gateBias.Shape._dims, parameters.Slice(index, _gateBias.Length));
