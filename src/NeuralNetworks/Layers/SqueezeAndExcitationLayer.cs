...
[LayerProperty(IsTrainable = true, TestInputShape = "4, 4, 4", TestConstructorArgs = "4, 4")]
public class SqueezeAndExcitationLayer<T> : LayerBase<T>, IAuxiliaryLossLayer<T>, IChainableComputationGraph<T>
{
    ...
    private Tensor<T> ApplyTensorActivationDerivative(Tensor<T> input, bool isFirstActivation)
    {
        ...
        var result = TensorAllocator.Rent<T>(input.Shape._dims);
    ...
    private Tensor<T> BackwardManual(Tensor<T> outputGradient)
    {
        ...
        Tensor<T> squeezeBackprop = spatialAxes.Length > 0
            ? Engine.ReduceMeanBackward(squeezedGradient, _lastInput.Shape._dims, spatialAxes)
            : (rank == 1
    ...
    private Tensor<T> BroadcastElementwiseMultiply(Tensor<T> a, Tensor<T> b)
    {
        ...
        var result = new Tensor<T>(large.Shape._dims);
