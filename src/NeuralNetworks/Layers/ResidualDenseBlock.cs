...
public class ResidualDenseBlock<T> : LayerBase<T>, IChainableComputationGraph<T>
{
    ...
    public override Tensor<T> ForwardGpu(params Tensor<T>[] inputs)
    {
        ...
        var concat1Tensor = Tensor<T>.FromGpuBuffer(backend, concat1Buffer,
        ...
        var concat2Tensor = Tensor<T>.FromGpuBuffer(backend, concat2Buffer,
        ...
        var concat3Tensor = Tensor<T>.FromGpuBuffer(backend, concat3Buffer,
        ...
        var concat4Tensor = Tensor<T>.FromGpuBuffer(backend, concat4Buffer,
        ...
        if (IsTrainingMode)
        {
            ...
            _gpuX1Activated = Tensor<T>.FromGpuBuffer(backend, x1ActivatedBuffer, activationShape, GpuTensorRole.Intermediate, ownsBuffer: true);
            _gpuConcat1 = concat1Tensor;
            _gpuConv2Out = conv2Out;
            _gpuX2Activated = Tensor<T>.FromGpuBuffer(backend, x2ActivatedBuffer, activationShape, GpuTensorRole.Intermediate, ownsBuffer: true);
            _gpuConcat2 = concat2Tensor;
            _gpuConv3Out = conv3Out;
            _gpuX3Activated = Tensor<T>.FromGpuBuffer(backend, x3ActivatedBuffer, activationShape, GpuTensorRole.Intermediate, ownsBuffer: true);
            _gpuConcat3 = concat3Tensor;
            _gpuConv4Out = conv4Out;
            _gpuX4Activated = Tensor<T>.FromGpuBuffer(backend, x4ActivatedBuffer, activationShape, GpuTensorRole.Intermediate, ownsBuffer: true);
        ...
        var result = Tensor<T>.FromGpuBuffer(backend, outputBuffer, shape, GpuTensorRole.Activation, ownsBuffer: true);
