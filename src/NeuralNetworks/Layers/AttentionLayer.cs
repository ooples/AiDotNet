...
public class AttentionLayer<T> : LayerBase<T>, IAuxiliaryLossLayer<T>
{
    ...
    public override Tensor<T> Forward(Tensor<T> input)
    {
        ...
        else if (rank == 3)
        {
            ...
            if (input.Shape[2] != _inputSize)
            {
                throw new ArgumentException(
                    $"AttentionLayer input size mismatch. Expected InputSize={_inputSize}, " +
                    $"but got {input.Shape[2]} in shape [{string.Join(", ", input.Shape._dims)}].",
                    nameof(input));
            }
            input3D = input;
        }
        else
        {
            ...
            if (input.Shape[rank - 1] != _inputSize)
            {
                throw new ArgumentException(
                    $"AttentionLayer input size mismatch. Expected InputSize={_inputSize}, " +
                    $"but got {input.Shape[rank - 1]} in shape [{string.Join(", ", input.Shape._dims)}].",
                    nameof(input));
            }
            int flatBatch = 1;
    ...
    public override Tensor<T> Forward(params Tensor<T>[] inputs)
    {
        ...
        else if (inputs.Length == 2)
        {
            ...
            {
                throw new ArgumentException(
                    $"Second input tensor has ambiguous shape {string.Join("x", secondInput.Shape._dims)}. " +
    ...
    private Tensor<T> BackwardManual(Tensor<T> outputGradient)
    {
        ...
        var dAttnOutput = dAttnOutputFlat.Reshape(_lastAttentionOutput.Shape._dims);
        ...
        if (VectorActivation is SoftmaxActivation<T>)
        {
            ...
            var sumExpanded = new Tensor<T>(_lastAttentionWeights.Shape._dims);
            var totalBatches = 1;
        ...
        var dQ = new Tensor<T>(Q.Shape._dims);
        var dK = new Tensor<T>(K.Shape._dims);
        for (int b = 0; b < batchSize; b++)
        {
        ...
        var dinput = dinputFlat.Reshape(_lastInput.Shape._dims);
