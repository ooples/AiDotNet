...
public class SpyNetLayer<T> : LayerBase<T>, IChainableComputationGraph<T>
{
        ...
        Tensor<T> image, Tensor<T> flow, DirectGpuTensorEngine engine, IDirectGpuBackend backend)
    {
        ...
        if (!_identityGridCache.TryGetValue(gridKey, out var identityGrid))
        {
            ...
            identityGrid = Tensor<T>.FromGpuBuffer(backend, buffer, [batch, height, width, 2], GpuTensorRole.Constant, ownsBuffer: true);
        ...
        var scaleTensor = Tensor<T>.FromGpuBuffer(backend, scaleBuffer, [1, 1, 1, 2], GpuTensorRole.Constant, ownsBuffer: false);
