...
public class FalconMambaLanguageModel<T> : LayerBase<T>
{
    ...
    public FalconMambaLanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 4,
        int stateDimension = 16, int expandFactor = 2, int maxSeqLength = 512, IActivationFunction<T>? activationFunction = null)
        : base([maxSeqLength, vocabSize], [maxSeqLength, vocabSize], activationFunction ?? new ActivationFunctions.IdentityActivation<T>())
    {
        ...
        _vocabSize = vocabSize; _modelDimension = modelDimension; _numLayers = numLayers; _stateDimension = stateDimension; _expandFactor = expandFactor;
        _embeddingWeights = new Tensor<T>(new[] { vocabSize, modelDimension }); InitT(_embeddingWeights);
        _blocks = new MambaBlock<T>[numLayers];
        for (int i = 0; i < numLayers; i++) _blocks[i] = new MambaBlock<T>(maxSeqLength, modelDimension, stateDimension, expandFactor);
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "fm_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "fm_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "fm_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "fm_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "fm_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["StateDimension"] = _stateDimension.ToString(); m["Architecture"] = "FalconMamba"; return m; }
}
