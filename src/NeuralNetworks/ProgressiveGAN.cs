...
public class ProgressiveGAN<T> : NeuralNetworkBase<T>
{
    ...
    private Tensor<T> ApplyPixelNormalization(Tensor<T> features)
    {
        ...
        if (features.Shape.Length == 4)
        {
            ...
            var meanPlusEps = mean.Add(new Tensor<T>(mean.Shape._dims).Tap(t => t.Fill(epsilon)));
            ...
            var norm = new Tensor<T>(meanPlusEps.Shape._dims, normData);

            // Broadcast norm back and divide
            // norm shape: [batch, height, width], need to broadcast to [batch, channels, height, width]
            var result = TensorAllocator.Rent<T>(features.Shape._dims);
    ...
    public (T discriminatorLoss, T generatorLoss) TrainStep(Tensor<T> realImages, int batchSize)
    {
        ...
        var realGradient = CreateFilledTensor(realOutput.Shape._dims, negativeScale);
        ...
        var fakeGradient = CreateFilledTensor(fakeOutput.Shape._dims, positiveScale);
        ...
        var genGradient = CreateFilledTensor(generatorOutput.Shape._dims, negativeScale);
    ...
    private T ComputeGradientPenalty(Tensor<T> realImages, Tensor<T> fakeImages, int batchSize)
    {
        ...
        var interpolated = new Tensor<T>(realImages.Shape._dims);
        ...
        var ones = CreateFilledTensor(interpolatedOutput.Shape._dims, NumOps.One);
