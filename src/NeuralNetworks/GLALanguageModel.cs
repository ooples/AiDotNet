...
/// </remarks>
public class GLALanguageModel<T> : LayerBase<T>
{
    private readonly int _vocabSize, _modelDimension, _numLayers, _numHeads;
    private Tensor<T> _embeddingWeights;
    private readonly GatedLinearAttentionLayer<T>[] _blocks;
    private Tensor<T> _finalNormGamma, _lmHeadWeights, _lmHeadBias;
    private Tensor<T>? _lastInput, _lastOutput, _lastEmbedded, _lastNormedOutput, _lastPostBlocksOutput;
    ...
    public int VocabSize => _vocabSize; public int ModelDimension => _modelDimension;
    public int NumLayers => _numLayers; public int NumHeads => _numHeads;
    ...
    public GLALanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 4, int numHeads = 8,
        ...
        : base([maxSeqLength, vocabSize], [maxSeqLength, vocabSize], activationFunction ?? new ActivationFunctions.IdentityActivation<T>())
    {
        ...
        _blocks = new GatedLinearAttentionLayer<T>[numLayers];
        for (int i = 0; i < numLayers; i++) _blocks[i] = new GatedLinearAttentionLayer<T>(maxSeqLength, modelDimension, numHeads);
    ...
    {
        _originalInputShape = input.Shape; int rank = input.Shape.Length;
        ...
        var emb = Engine.TensorMatMul(in3D.Reshape(bs * sl, id), _embeddingWeights).Reshape(bs, sl, _modelDimension); _lastEmbedded = emb;
    ...
    private Tensor<T> RMSNorm(Tensor<T> input, Tensor<T> gamma, int bs, int sl)
    {
    ...
    private Tensor<T> BackRMSNorm(Tensor<T> dO, Tensor<T> inp, Tensor<T> gamma, int bs, int sl, out Tensor<T> dG)
    {
    ...
    public override void ResetState() { _lastInput = null; _lastOutput = null; _lastEmbedded = null; _lastNormedOutput = null; _lastPostBlocksOutput = null; _lastBlockInputs = null;
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "gla_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "gla_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "gla_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "gla_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "gla_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["NumHeads"] = _numHeads.ToString(); m["Architecture"] = "GLA"; return m; }
}
