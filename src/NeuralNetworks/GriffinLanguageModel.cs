...
/// <summary>
/// Implements a Griffin language model: embedding + N RGLR blocks with local attention + RMS norm + LM head.
/// </summary>
/// <remarks>
/// <para>
...
/// </remarks>
public class GriffinLanguageModel<T> : LayerBase<T>
{
    ...
    public GriffinLanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 4,
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "griffin_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "griffin_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "griffin_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "griffin_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "griffin_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["Architecture"] = "Griffin"; return m; }
}
