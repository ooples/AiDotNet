...
///     .ConfigureModel(new RestrictedBoltzmannMachine&lt;float&gt;())
///     .Build(trainX, trainY);
...
public partial class RestrictedBoltzmannMachine<T> : VectorModelLayoutBase<T>, ITrainingObjectiveProvider<T>
{
    ...
    public Tensor<T> GetVisibleLayerActivation(Tensor<T> hiddenLayer)
    {
        ...
        else if (hiddenLayer.Rank == 2)
        {
            ...
            {
                throw new ArgumentException($"Hidden layer shape {string.Join(",", hiddenLayer._shape)} is incompatible with hidden size {hiddenSize}");
            }
        }
        else
        {
