...
/// <summary>
/// Implements a RecurrentGemma language model: embedding + N RGLR blocks + RMS norm + LM head.
/// </summary>
/// <remarks>
/// <para>
...
public class RecurrentGemmaLanguageModel<T> : LayerBase<T>
{
    ...
    public RecurrentGemmaLanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 4, int maxSeqLength = 512, IActivationFunction<T>? activationFunction = null)
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "rg_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "rg_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "rg_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "rg_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "rg_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["Architecture"] = "RecurrentGemma"; return m; }
}
