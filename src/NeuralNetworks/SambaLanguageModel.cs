...
public class SambaLanguageModel<T> : LayerBase<T>
{
    ...
    /// <summary>
    /// Creates a new Samba language model with alternating Mamba + sliding window attention blocks.
    /// </summary>
    /// <param name="attentionInterval">Every Nth block is attention (default 2 = alternating Mamba/Attention).</param>
    public SambaLanguageModel(int vocabSize, int modelDimension = 256, int numLayers = 8,
        int stateDimension = 16, int attentionInterval = 2, int maxSeqLength = 512, IActivationFunction<T>? activationFunction = null)
        : base([maxSeqLength, vocabSize], [maxSeqLength, vocabSize], activationFunction ?? new ActivationFunctions.IdentityActivation<T>())
    {
        ...
        // Build hybrid block array: alternating Mamba and sliding window attention
        ...
        _hybridScheduler = new HybridBlockScheduler<T>(maxSeqLength, blocks, isAttn, HybridSchedulePattern.SambaStyle, modelDimension);
    ...
    public override ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes) { if (inputNodes == null) throw new ArgumentNullException(nameof(inputNodes));
        var inN = TensorOperations<T>.Variable(new Tensor<T>(new int[] { 1, _vocabSize }), "samba_in"); inputNodes.Add(inN);
        var eN = TensorOperations<T>.Variable(_embeddingWeights, "samba_emb"); inputNodes.Add(eN); var cur = TensorOperations<T>.MatrixMultiply(inN, eN);
        ...
        var nN = TensorOperations<T>.Variable(_finalNormGamma, "samba_norm"); inputNodes.Add(nN); cur = TensorOperations<T>.ElementwiseMultiply(cur, nN);
        var wN = TensorOperations<T>.Variable(_lmHeadWeights, "samba_w"); var bN = TensorOperations<T>.Variable(_lmHeadBias, "samba_b"); inputNodes.Add(wN); inputNodes.Add(bN);
    ...
    internal override Dictionary<string, string> GetMetadata() { var m = base.GetMetadata(); m["VocabSize"] = _vocabSize.ToString(); m["ModelDimension"] = _modelDimension.ToString();
        m["NumLayers"] = _numLayers.ToString(); m["Architecture"] = "Samba"; return m; }
}
