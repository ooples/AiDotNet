    ...
    where TGene : class
{
    ...
    {
        return ((IParameterizable<T, TInput, TOutput>)_innerModel).GetParameters();
    }
    ...
    {
        _innerModel = ((IParameterizable<T, TInput, TOutput>)_innerModel).WithParameters(parameters);
    }
    ...
    {
        var newModel = ((IParameterizable<T, TInput, TOutput>)_innerModel).WithParameters(parameters);
    ...
    {
        return ((IFeatureAware)_innerModel).GetActiveFeatureIndices();
    }
    ...
    {
        ((IFeatureAware)_innerModel).SetActiveFeatureIndices(featureIndices);
    }

    public bool IsFeatureUsed(int featureIndex)
    {
        return ((IFeatureAware)_innerModel).IsFeatureUsed(featureIndex);
    }
    ...
    public virtual int ParameterCount
        => _parameterCountCache ??= ((IParameterizable<T, TInput, TOutput>)_innerModel).GetParameters()?.Length ?? 0;
    ...
    {
        return ((IGradientComputable<T, TInput, TOutput>)_innerModel).ComputeGradients(input, target, lossFunction);
    }
    ...
    {
        ((IGradientComputable<T, TInput, TOutput>)_innerModel).ApplyGradients(gradients, learningRate);
    }
    ...
    public virtual bool SupportsJitCompilation
    {
        get
        {
            ...
            return ((IJitCompilable<T>)_innerModel).SupportsJitCompilation;
        }
    }
    ...
    public virtual ComputationNode<T> ExportComputationGraph(List<ComputationNode<T>> inputNodes)
    {
        ...
        if (!((IJitCompilable<T>)_innerModel).SupportsJitCompilation)
        ...
        return ((IJitCompilable<T>)_innerModel).ExportComputationGraph(inputNodes);
    }
