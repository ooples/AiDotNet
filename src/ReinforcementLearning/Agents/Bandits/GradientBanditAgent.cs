...
public class GradientBanditAgent<T> : ReinforcementLearningAgentBase<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new ModelMetadata<T> { ModelType = "GradientBandit", InputSize = 1, OutputSize = _options.NumArms, ParameterCount = ParameterCount };
    ...
    public override Matrix<T> GetParameters() => new Matrix<T>(new[] { _preferences });
    public override void SetParameters(Matrix<T> parameters) { for (int i = 0; i < _options.NumArms && i < parameters.Columns; i++) _preferences[i] = parameters[0, i]; }
