...
public class WatkinsQLambdaAgent<T> : ReinforcementLearningAgentBase<T>
{
    ...
    public override ModelMetadata<T> GetModelMetadata() => new ModelMetadata<T> { ModelType = "WatkinsQLambda", InputSize = _options.StateSize, OutputSize = _options.ActionSize, ParameterCount = ParameterCount };
    ...
    public override IFullModel<T, Vector<T>, Vector<T>> Clone() => new WatkinsQLambdaAgent<T>(_options);
