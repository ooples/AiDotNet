...
public class QLambdaAgent<T> : ReinforcementLearningAgentBase<T>
{
    ...
    private T GetMaxQValue(string stateKey) { T max = _qTable[stateKey][0]; for (int a = 1; a < _options.ActionSize; a++) if (NumOps.Compare(_qTable[stateKey][a], max) > 0) max = _qTable[stateKey][a]; return max; }
    ...
    public override ModelMetadata<T> GetModelMetadata() => new ModelMetadata<T> { ModelType = "QLambda", InputSize = _options.StateSize, OutputSize = _options.ActionSize, ParameterCount = ParameterCount };
    ...
    public override IFullModel<T, Vector<T>, Vector<T>> Clone() => new QLambdaAgent<T>(_options);
