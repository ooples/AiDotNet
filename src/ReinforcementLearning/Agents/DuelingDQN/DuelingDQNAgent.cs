...
public class DuelingDQNAgent<T> : DeepReinforcementLearningAgentBase<T>
{
    ...
    {
        return _qNetwork.GetFlattenedParameters();
    }
...
internal class DuelingNetwork<T>
{
        ...
        INumericOperations<T> numOps)
    {
        ...
        {
            _sharedLayers.Add(new DenseLayer<T>(prevSize, size, new ReLUActivation<T>()));
            prevSize = size;
        }
        ...
        {
            _valueLayers.Add(new DenseLayer<T>(prevSize, size, new ReLUActivation<T>()));
            prevSize = size;
        }
        _valueLayers.Add(new DenseLayer<T>(prevSize, 1, new LinearActivation<T>())); // Output single value

        // Build advantage stream
        prevSize = sharedOutputSize;
        foreach (var size in advantageLayerSizes)
        {
            _advantageLayers.Add(new DenseLayer<T>(prevSize, size, new ReLUActivation<T>()));
            prevSize = size;
        }
        _advantageLayers.Add(new DenseLayer<T>(prevSize, actionSize, new LinearActivation<T>())); // Output per-action advantages
    }
