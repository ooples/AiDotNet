...
public abstract class GradientBasedOptimizerBase<T, TInput, TOutput> : OptimizerBase<T, TInput, TOutput>, IGradientBasedOptimizer<T, TInput, TOutput>
{
    ...
    protected virtual IFullModel<T, TInput, TOutput> ApplyGradientsCore(Vector<T> originalParameters, Vector<T> gradients, IFullModel<T, TInput, TOutput> model)
    {
        ...
        return ((IParameterizable<T, TInput, TOutput>)model).WithParameters(updatedParameters);
    }
        ...
        OptimizationInputData<T, TInput, TOutput> inputData)
    {
        ...
        if (model is IGradientComputable<T, TInput, TOutput> gradientComputable)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                var perturbedModel = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(perturbedParams);
        ...
        OptimizationInputData<T, TInput, TOutput> inputData)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = i; j < n; j++)  // Symmetric matrix, only compute upper triangle
            {
                ...
                var model_pp = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(params_pp);
                ...
                var model_pm = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(params_pm);
                ...
                var model_mp = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(params_mp);
                ...
                var model_mm = ((IParameterizable<T, TInput, TOutput>)model).WithParameters(params_mm);
    ...
    protected T LineSearch(IFullModel<T, TInput, TOutput> currentSolution, Vector<T> direction, Vector<T> gradient, OptimizationInputData<T, TInput, TOutput> inputData)
    {
        ...
        while (true)
        {
            var newCoefficients = ((IParameterizable<T, TInput, TOutput>)currentSolution).GetParameters().Add(direction.Multiply(alpha));
            var newSolution = ((IParameterizable<T, TInput, TOutput>)currentSolution).WithParameters(newCoefficients);
        ...
        Vector<T> gradient)
    {
        ...
        return ((IParameterizable<T, TInput, TOutput>)currentSolution).WithParameters(newParameters);
    }
