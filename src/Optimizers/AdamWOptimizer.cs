...
public partial class AdamWOptimizer<T, TInput, TOutput> : GradientBasedOptimizerBase<T, TInput, TOutput>, Fused.IFusedOptimizerSpec
{
    ...
    public AdamWOptimizer(
        ...
        : base(model, PaperOptimizerDefaults.Resolve(model, options, OptimizerKind.AdamW))
    {
        _m = Vector<T>.Empty();
        _v = Vector<T>.Empty();
        _t = 0;
    ...
    /// <summary>
    /// Creates an AdamW optimizer for minimizing a plain function, with no model attached.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public static AdamWOptimizer<T, TInput, TOutput> CreateForFunction(
        AdamWOptimizerOptions<T, TInput, TOutput>? options = null)
        => new(null, options);
