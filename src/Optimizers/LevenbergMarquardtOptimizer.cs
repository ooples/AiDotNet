...
public partial class LevenbergMarquardtOptimizer<T, TInput, TOutput> : GradientBasedOptimizerBase<T, TInput, TOutput>, Fused.IFusedOptimizerSpec
{
    /// <summary>
    ...
    /// <summary>
    /// Creates a Levenberg-Marquardt optimizer for minimizing a plain function, with no model attached.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public static LevenbergMarquardtOptimizer<T, TInput, TOutput> CreateForFunction(
        LevenbergMarquardtOptimizerOptions<T, TInput, TOutput>? options = null)
        => new(options);
    ...
    private T CalculatePartialDerivative(IFullModel<T, TInput, TOutput> model, TInput x, int paramIndex)
    {
        ...
        var modifiedModel = InterfaceGuard.Parameterizable(model).WithParameters(parameters);
        ...
        modifiedModel = InterfaceGuard.Parameterizable(model).WithParameters(parameters);
        ...
        InterfaceGuard.Parameterizable(model).WithParameters(parameters);
    ...
    private IFullModel<T, TInput, TOutput> UpdateSolution(IFullModel<T, TInput, TOutput> currentSolution, Matrix<T> jacobian, Vector<T> residuals)
    {
        ...
        var newCoefficients = (Vector<T>)Engine.Add(InterfaceGuard.Parameterizable(currentSolution).GetParameters(), delta);
