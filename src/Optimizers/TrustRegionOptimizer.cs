...
public class TrustRegionOptimizer<T, TInput, TOutput> : GradientBasedOptimizerBase<T, TInput, TOutput>
{
    ...
    {
        var coefficients = ((IParameterizable<T, TInput, TOutput>)currentSolution).GetParameters();
        ...
        for (int i = 0; i < coefficients.Length; i++)
        {
            for (int j = i; j < coefficients.Length; j++)
            {
                ...
                perturbed1 = ((IParameterizable<T, TInput, TOutput>)perturbed1).WithParameters(coeffs1);
                perturbed2 = ((IParameterizable<T, TInput, TOutput>)perturbed2).WithParameters(coeffs2);
                perturbed3 = ((IParameterizable<T, TInput, TOutput>)perturbed3).WithParameters(coeffs3);
                perturbed4 = ((IParameterizable<T, TInput, TOutput>)perturbed4).WithParameters(coeffs4);
    ...
    private IFullModel<T, TInput, TOutput> MoveInDirection(IFullModel<T, TInput, TOutput> currentSolution, Vector<T> direction, T stepSize)
    {
        ...
        var currentCoefficients = ((IParameterizable<T, TInput, TOutput>)newModel).GetParameters();
        ...
        return ((IParameterizable<T, TInput, TOutput>)newModel).WithParameters(newCoefficients);
    }
