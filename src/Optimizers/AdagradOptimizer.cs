...
public partial class AdagradOptimizer<T, TInput, TOutput> : GradientBasedOptimizerBase<T, TInput, TOutput>, Fused.IFusedOptimizerSpec
{
    /// <summary>
    ...
    /// <summary>
    /// Creates an Adagrad optimizer for minimizing a plain function, with no model attached.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public static AdagradOptimizer<T, TInput, TOutput> CreateForFunction(
        AdagradOptimizerOptions<T, TInput, TOutput>? options = null)
        => new(options);
    ...
    /// <summary>
    /// GPU buffer for accumulated squared gradients.
    /// </summary>
    ...
    {
        if (_gpuStateInitialized && _gpuAccumulatedGrad != null)
            return;
    ...
    public override void UpdateParametersGpu(IGpuBuffer parameters, IGpuBuffer gradients, int parameterCount, IDirectGpuBackend backend)
    {
        ...
        backend.AdagradUpdate(
            parameters,
            gradients,
            _gpuAccumulatedGrad!,
            (float)NumOps.ToDouble(CurrentLearningRate),
            (float)_options.Epsilon,
            0.0f, // Adagrad doesn't have weight decay in these options
            parameterCount
        );
    }
