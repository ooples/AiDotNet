...
[ModelPaper("EAST: An Efficient and Accurate Scene Text Detector", "https://doi.org/10.48550/arXiv.1704.03155", Year = 2017, Authors = "Xinyu Zhou, Cong Yao, He Wen, Yuzhi Wang, Shuchang Zhou, Weiran He, Jiajun Liang")]
    ...
    private static Tensor<T> ConcatenateTensors(Tensor<T> a, Tensor<T> b)
    {
        ...
        if (a.Shape[0] != b.Shape[0] || a.Shape[2] != b.Shape[2] || a.Shape[3] != b.Shape[3])
            throw new ArgumentException(
                $"Tensor shapes must match on batch/height/width: a=[{string.Join(",", a.Shape._dims)}], b=[{string.Join(",", b.Shape._dims)}].");
