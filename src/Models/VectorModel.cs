...
public class VectorModel<T> : IFullModel<T, Matrix<T>, Vector<T>> 
{
    ...
    // NOTE: IInterpretableModel implementation temporarily removed due to signature mismatch.
    // VectorModel uses Matrix<T>/Vector<T> but IInterpretableModel requires Tensor<T>.
    // This needs to be redesigned to properly support interpretability.
}