...
public class CSDIOptions<T> : TimeSeriesRegressionOptions<T>
{
    ...
    public CSDIOptions()
    {
        ...
        // NumSamples paths still differ from one another, so the median stays a real estimate
        // and the 5% / 95% intervals the paper reports remain meaningful. It fixes only that
        // Predict called twice on the same input returns the same answer - the sampler
        // equivalent of seeding a generator before inference, not of switching sampling off.
        // Set it to another value for a different sample set, or vary it per call for
        // independent draws.
        Seed = 1;
    }
