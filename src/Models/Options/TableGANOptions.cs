...
public class TableGANOptions<T> : RiskModelOptions<T>
{
    ...
    /// <summary>
    /// Gets or sets whether the generator's loss includes the classification auxiliary term.
    /// </summary>
    /// <value>
    /// <c>true</c> to include the weighted classification loss in the generator objective;
    /// <c>false</c> (default) to suppress it.
    /// </value>
    /// <remarks>
    /// <para>
    /// Defaults to <c>false</c>: the classifier head (<c>_classLayers</c> / <c>_classOutput</c>)
    /// currently has no dedicated training path that updates its parameters against real-row
    /// labels, so its logits are effectively random and the generator gradient driven by
    /// <see cref="ClassificationWeight"/> is noise. Enable only after wiring a real
    /// classifier-training routine (e.g., per-step optimizer update against real
    /// (row, label) pairs) — until then the toggle stays false to keep the generator
    /// objective honest.
    /// </para>
    /// <para><b>For Beginners:</b> Some flavors of TableGAN add a "label classifier"
    /// loss to the generator so the synthetic rows preserve the original label
    /// distribution. That extra loss only helps if the classifier itself has
    /// actually learned what each label looks like; right now it hasn't, so we
    /// keep the term turned off so it doesn't poison the generator. Once the
    /// project wires a classifier training pass, set this to <c>true</c> to
    /// re-enable the label-preservation signal.</para>
    /// </remarks>
