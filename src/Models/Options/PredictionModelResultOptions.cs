...
public class PredictionModelResultOptions<T, TInput, TOutput>
{
    ...
    /// <summary>
    /// Gets or sets the active learning configuration for sample-efficient training.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Active learning enables the model to strategically select which samples to learn from,
    /// dramatically reducing the amount of labeled data required. This configuration controls
    /// query strategies like BALD (Bayesian Active Learning by Disagreement), BatchBALD,
    /// Query-by-Committee, CoreSet selection, and uncertainty-based sampling.
    /// </para>
    /// <para><b>For Beginners:</b> Active learning helps your model learn smarter, not harder.
    /// Instead of training on all data, it picks the most informative samples to learn from.
    /// This is especially useful when:
    /// - Labeling data is expensive (medical images, legal documents)
    /// - You have limited annotation budget
    /// - You want to reach good accuracy with fewer training examples
    ///
    /// Example: A medical imaging model might need 1000 expert-labeled images normally,
    /// but with active learning, it can achieve similar accuracy with only 200 carefully-selected images.
    /// </para>
    /// </remarks>
    public ActiveLearnerConfig<T>? ActiveLearningConfig { get; set; }

    /// <summary>
    /// Gets or sets the continual learning configuration for lifelong learning capabilities.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Continual learning enables models to learn new tasks sequentially without forgetting
    /// previously learned knowledge. This configuration controls strategies like:
    /// - EWC (Elastic Weight Consolidation): Protects important weights for old tasks
    /// - LwF (Learning without Forgetting): Uses knowledge distillation
    /// - GEM (Gradient Episodic Memory): Ensures gradients don't conflict with old tasks
    /// - SI (Synaptic Intelligence): Online importance estimation
    /// - MAS (Memory Aware Synapses): Uses sensitivity-based importance
    /// - PackNet: Network pruning for task-specific subnetworks
    /// </para>
    /// <para><b>For Beginners:</b> Continual learning solves the "catastrophic forgetting" problem.
    /// Normally, when you train a model on new data, it forgets what it learned before.
    /// Continual learning lets your model:
    /// - Learn new skills without losing old ones
    /// - Adapt to changing data over time
    /// - Handle sequential tasks (like learning multiple languages one after another)
    ///
    /// Example: A sentiment analysis model trained on product reviews can later learn
    /// to analyze movie reviews without forgetting how to analyze products.
    /// </para>
    /// </remarks>
    public ContinualLearnerConfig<T>? ContinualLearningConfig { get; set; }

    /// <summary>
    /// Gets or sets the curriculum learning configuration for structured training progression.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Curriculum learning organizes training samples from easy to hard, mimicking how humans
    /// learn. This configuration controls the learning schedule, difficulty metrics, phases,
    /// and progression strategies. Research shows curriculum learning can improve both
    /// convergence speed and final model performance.
    /// </para>
    /// <para><b>For Beginners:</b> Curriculum learning teaches your model like a teacher
    /// teaches a student - starting with easy examples and gradually increasing difficulty.
    /// This approach can:
    /// - Help models converge faster during training
    /// - Achieve better final accuracy
    /// - Prevent the model from getting "confused" by hard examples early on
    ///
    /// Example: When training a language model, curriculum learning might start with
    /// simple sentences like "The cat sat" before progressing to complex paragraphs
