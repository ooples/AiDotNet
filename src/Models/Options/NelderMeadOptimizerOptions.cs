...
public class NelderMeadOptimizerOptions<T, TInput, TOutput> : OptimizationAlgorithmOptions<T, TInput, TOutput>
{
    ...
    /// starting point.
    /// </summary>
    /// <value>The relative simplex step, defaulting to 0.05 (five percent).</value>
    /// <remarks>
    /// <para>
    /// Nelder-Mead needs <c>n + 1</c> starting points in <c>n</c> dimensions, but callers normally
    /// supply just one. The remaining vertices are generated by perturbing one coordinate at a
    /// time: coordinate <c>i</c> of vertex <c>i</c> becomes <c>x[i] + InitialSimplexStep * x[i]</c>.
    /// The default of 0.05 is the value used by MATLAB's <c>fminsearch</c> and is scale-invariant,
    /// so it behaves the same whether your parameters are measured in millimetres or kilometres.
    /// </para>
    /// <para>
    /// Coordinates that are exactly zero cannot be perturbed relatively, so those use
    /// <see cref="ZeroCoordinateSimplexStep"/> instead.
    /// </para>
    /// <para><b>For Beginners:</b> This algorithm searches by moving a small cluster of trial
    /// points around, so it needs a cluster to start with — but you only gave it one point. This
    /// setting says how far apart to spread the rest of the cluster: 0.05 means each extra point
    /// is placed five percent away from your starting guess along one axis.
    ///
    /// Increase it if your starting guess is rough and you want the search to look further afield
    /// before settling. Decrease it if you already know roughly where the answer is and want the
    ...
    /// </summary>
    /// <value>The absolute simplex step for zero coordinates, defaulting to 0.00025.</value>
    /// <remarks>
    /// <para>
    /// A relative perturbation of a zero coordinate produces zero, which would create a degenerate
    /// simplex that collapses to a lower-dimensional subspace and can never search along that axis.
    /// Those coordinates are therefore set to this absolute value instead. The default of 0.00025
    /// matches MATLAB's <c>fminsearch</c>.
    /// </para>
    /// <para><b>For Beginners:</b> If one of your starting values is exactly zero, "move it five
    /// percent" means "don't move it at all" — and the search would then be blind along that
    /// direction forever. This setting is the small fixed nudge used instead, just for those
    /// coordinates.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <value>The zero-coordinate threshold, defaulting to 0.0 (exact zero only).</value>
    /// <remarks>
    /// <para>
    /// Coordinates whose absolute value is at or below this threshold are offset by
    /// <see cref="ZeroCoordinateSimplexStep"/> rather than by the relative
    /// <see cref="InitialSimplexStep"/>. The default of 0.0 matches MATLAB's <c>fminsearch</c>,
    /// which switches to the absolute step only for coordinates that are exactly zero.
    /// </para>
    /// <para>
    /// Raise it when a starting coordinate can be legitimately tiny but nonzero — a relative step
    /// off 1e-9 produces a vertex 1e-10 away, which is numerically indistinguishable from the
    /// starting point and yields a near-degenerate simplex that cannot search that axis.
    /// </para>
    /// <para><b>For Beginners:</b> "Move this value by five percent" stops being a useful nudge
    /// once the value is extremely small — five percent of a millionth is still a millionth.
    /// This setting says how small counts as "too small to nudge proportionally", at which point
