...
public class OneVsOneClassifier<T> : MetaClassifierBase<T>
{
    ...
    public override byte[] Serialize()
    {
        ...
        var classPairsFirst = _classPairs?.Select(p => p.Item1).ToArray();
        var classPairsSecond = _classPairs?.Select(p => p.Item2).ToArray();
        ...
        var modelDict = new Dictionary<string, object?>
        {
            ...
            { "ClassPairsFirst", classPairsFirst },
            { "ClassPairsSecond", classPairsSecond },
