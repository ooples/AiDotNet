...
///
/// // Train one independent classifier per target label
...
public class MultiOutputClassifier<T> : MetaClassifierBase<T>
{
    ...
    public override void Deserialize(byte[] modelData)
    {
        ...
        var metadata = JsonConvert.DeserializeObject<ModelMetadata<T>>(jsonString)
            ?? throw new InvalidOperationException("Failed to deserialize MultiOutputClassifier: invalid metadata.");
        if (metadata.ModelData is null)
            throw new InvalidOperationException("Failed to deserialize MultiOutputClassifier: missing model data.");

        var dataString = Encoding.UTF8.GetString(metadata.ModelData);
        var jObj = JsonConvert.DeserializeObject<JObject>(dataString)
            ?? throw new InvalidOperationException("Failed to deserialize MultiOutputClassifier: invalid model payload.");
        ...
        if (types is null || data is null || types.Length != data.Length)
            throw new InvalidOperationException(
                "Failed to deserialize MultiOutputClassifier: estimator types/data arrays are missing or mismatched.");
