    ...
    private T TrainEpoch(IDataset<T, TInput, TOutput> data, int epoch)
    {
        ...
        for (int i = 0; i < data.Count; i += _config.BatchSize)
        {
            ...
            for (int j = i; j < batchEnd; j++)
            {
                ...
                // Compute gradients
                var gradients = ((IGradientComputable<T, TInput, TOutput>)_baseModel).ComputeGradients(input, output);

                // Apply gradients with learning rate
                ((IGradientComputable<T, TInput, TOutput>)_baseModel).ApplyGradients(gradients, _config.LearningRate);
