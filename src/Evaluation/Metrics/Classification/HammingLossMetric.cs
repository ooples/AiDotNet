...
/// </summary>
/// <remarks>
/// <para>Hamming Loss = (1/N) * Σ(prediction ≠ actual)</para>
/// <para><b>For Beginners:</b> Hamming loss is the complement of accuracy (1 - accuracy).
/// It tells you what fraction of your predictions are wrong.
/// <list type="bullet">
/// <item>Hamming Loss = 0: Perfect predictions (all correct)</item>
/// <item>Hamming Loss = 0.3: 30% of predictions are wrong</item>
/// <item>Hamming Loss = 1: All predictions are wrong</item>
/// </list>
/// Particularly useful for multi-label classification.</para>
/// </remarks>
public class HammingLossMetric<T> : IClassificationMetric<T>
{
    private static readonly INumericOperations<T> NumOps = MathHelper.GetNumericOperations<T>();

    public string Name => "HammingLoss";
    public string Category => "Classification";
    public string Description => "Fraction of labels incorrectly predicted.";
    ...
    public T Compute(ReadOnlySpan<T> predictions, ReadOnlySpan<T> actuals)
    {
        ...
        {
            // For binary: compare rounded values
            int pred = (int)Math.Round(NumOps.ToDouble(predictions[i]));
            int actual = (int)Math.Round(NumOps.ToDouble(actuals[i]));
            if (pred != actual) incorrect++;
        }

        return NumOps.FromDouble((double)incorrect / predictions.Length);
    }
