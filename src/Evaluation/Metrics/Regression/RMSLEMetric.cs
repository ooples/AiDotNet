...
/// </summary>
/// <remarks>
/// <para>RMSLE = √[(1/N) * Σ(log(1 + y_i) - log(1 + ŷ_i))²]</para>
/// <para><b>For Beginners:</b> RMSLE is useful when:
/// <list type="bullet">
/// <item>You care about percentage/ratio errors rather than absolute differences</item>
/// <item>Under-predictions should be penalized more than over-predictions</item>
/// <item>The target spans several orders of magnitude (like prices or counts)</item>
/// </list>
/// Note: Requires all values to be non-negative (≥ 0) since we take logarithms.</para>
/// </remarks>
public class RMSLEMetric<T> : IRegressionMetric<T>
{
    private static readonly INumericOperations<T> NumOps = MathHelper.GetNumericOperations<T>();

    public string Name => "RMSLE";
    public string Category => "Regression";
    public string Description => "Root Mean Squared Logarithmic Error for ratio-based evaluation.";
    ...
    public T Compute(ReadOnlySpan<T> predictions, ReadOnlySpan<T> actuals)
    {
        ...
        {
            double actual = Math.Max(0, NumOps.ToDouble(actuals[i])); // Clamp to non-negative
            double pred = Math.Max(0, NumOps.ToDouble(predictions[i])); // Clamp to non-negative
        ...
        return NumOps.FromDouble(Math.Sqrt(sumSquaredLogError / predictions.Length));
    }
