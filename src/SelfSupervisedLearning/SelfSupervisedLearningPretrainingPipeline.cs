...
/// var pipeline = new SelfSupervisedLearningPretrainingPipeline&lt;double&gt;(encoder, encoderOutputDim)
...
public class SelfSupervisedLearningPretrainingPipeline<T>
{
    private readonly INeuralNetwork<T> _encoder;
    private readonly int _encoderOutputDim;
    private SelfSupervisedLearningConfig<T> _config;
    private ISelfSupervisedLearningMethod<T>? _method;
    ...
    public SelfSupervisedLearningPretrainingPipeline(INeuralNetwork<T> encoder, int encoderOutputDim)
    {
        Guard.NotNull(encoder);
        _encoder = encoder;
        _encoderOutputDim = encoderOutputDim;
        _config = new SelfSupervisedLearningConfig<T>();
    }
    ...
    /// encoder; any other <see cref="ISelfSupervisedLearningMethod{T}"/> works too. Leave this unset to get
    /// <see cref="SimCLR{T}"/>.
    /// </param>
    public SelfSupervisedLearningPretrainingPipeline<T> WithMethod(ISelfSupervisedLearningMethod<T> method)
    {
    ...
    /// </summary>
    public SelfSupervisedLearningPretrainingPipeline<T> WithConfig(Action<SelfSupervisedLearningConfig<T>> configure)
    {
        configure(_config);
        return this;
    }
        ...
        int[]? validationLabels = null)
    {
        ...
        // Create session
        var session = new SelfSupervisedLearningSession<T>(_method, _config);
    ...
    private ISelfSupervisedLearningMethod<T> CreateMethod() => _config.Method ?? CreateSimCLR();

    private ISelfSupervisedLearningMethod<T> CreateSimCLR()
    {
    ...
    private SelfSupervisedLearningResult<T> RunFinalEvaluation(
        SelfSupervisedLearningResult<T> result,
