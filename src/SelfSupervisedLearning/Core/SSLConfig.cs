...
public class SSLConfig
{
    ...
    /// <summary>
    /// Creates an SSL configuration for SimCLR with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for SimCLR training.</returns>
    public static SSLConfig ForSimCLR()
    {
        return new SSLConfig
        {
            Method = SSLMethodType.SimCLR,
            Temperature = 0.1,
            BatchSize = 256,  // SimCLR works better with larger batches
            ProjectorOutputDimension = 128
        };
    }

    /// <summary>
    /// Creates an SSL configuration for MoCo v2 with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for MoCo v2 training.</returns>
    public static SSLConfig ForMoCo()
    {
        return new SSLConfig
        {
    ...
    /// <summary>
    /// Creates an SSL configuration for BYOL with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for BYOL training.</returns>
    public static SSLConfig ForBYOL()
    {
        return new SSLConfig
        {
    ...
    /// <summary>
    /// Creates an SSL configuration for DINO with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for DINO training.</returns>
    public static SSLConfig ForDINO()
    {
        return new SSLConfig
        {
            Method = SSLMethodType.DINO,
            DINO = new DINOConfig()
        };
    }

    /// <summary>
    /// Creates an SSL configuration for MAE with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for MAE training.</returns>
    public static SSLConfig ForMAE()
    {
        return new SSLConfig
        {
            Method = SSLMethodType.MAE,
            MAE = new MAEConfig()
        };
    }

    /// <summary>
    /// Creates an SSL configuration for Barlow Twins with standard defaults.
    /// </summary>
    /// <returns>A configuration optimized for Barlow Twins training.</returns>
    public static SSLConfig ForBarlowTwins()
    {
        return new SSLConfig
        {
            Method = SSLMethodType.BarlowTwins,
            ProjectorOutputDimension = 8192,  // Barlow Twins uses larger projection
            BarlowTwins = new BarlowTwinsConfig()
        };
    }
