        ...
        CancellationToken cancellationToken)
    {
        ...
        if (batch.HasPairwisePreferenceData)
        {
            ...
            for (int i = 0; i < batch.Count; i++)
            {
                ...
                var gradients = ((IGradientComputable<T, TInput, TOutput>)policyModel).ComputeGradients(input, chosen);
            ...
            for (int i = 0; i < batch.Count; i++)
            {
                ...
                var gradients = ((IGradientComputable<T, TInput, TOutput>)policyModel).ComputeGradients(input, rejected);
                ((IGradientComputable<T, TInput, TOutput>)policyModel).ApplyGradients(gradients, NumOps.FromDouble(learningRate));
        ...
        else if (batch.HasUnpairedPreferenceData)
        {
            ...
            for (int i = 0; i < batch.DesirabilityLabels.Length; i++)
            {
                ...
                if (isDesirable)
                {
                    ...
                    var gradients = ((IGradientComputable<T, TInput, TOutput>)policyModel).ComputeGradients(input, output);
                    ((IGradientComputable<T, TInput, TOutput>)policyModel).ApplyGradients(gradients, NumOps.FromDouble(learningRate));
                }
                else
                {
