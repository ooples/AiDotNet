...
public class ReinforcementLearningHumanFeedback<T, TInput, TOutput> : FineTuningBase<T, TInput, TOutput>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var exp in experience)
        {
            ...
            var gradients = ((IGradientComputable<T, TInput, TOutput>)policyModel).ComputeGradients(input, exp.Output);
            ...
            if (_valueModel != null)
            {
                ...
                var valueGradients = ((IGradientComputable<T, TInput, TOutput>)_valueModel).ComputeGradients(input, exp.Output);
                ((IGradientComputable<T, TInput, TOutput>)_valueModel).ApplyGradients(valueGradients, NumOps.FromDouble(Options.LearningRate * valueCoeff * cappedValueError));
            }
