    ...
    IFederatedClientDataLoader<T, Matrix<T>, Vector<T>>
{
        ...
        IReadOnlyDictionary<int, FederatedClientDataset<Matrix<T>, Vector<T>>> clientData)
    {
        ...
        foreach (var dataset in clientData.Values)
        {
            ...
            int datasetFeatureCount = dataset.Features.Columns;

            if (dataset.SampleCount > 0 && datasetFeatureCount == 0)
            {
                throw new InvalidOperationException("LEAF client dataset has samples but zero features.");
            }

            if (datasetFeatureCount > 0 && featureCount < 0)
            {
                featureCount = datasetFeatureCount;
            }
            else if (datasetFeatureCount > 0 && datasetFeatureCount != featureCount)
            {
                throw new InvalidOperationException(
                    $"LEAF client datasets must have consistent feature counts. Expected {featureCount} but found {datasetFeatureCount}.");
            }
        }

        if (featureCount < 0)
        {
            throw new InvalidOperationException("Client data must contain at least one dataset with features.");
        }

        if (totalSamples == 0)
        {
            throw new InvalidOperationException("Client data must contain at least one sample.");
        }
