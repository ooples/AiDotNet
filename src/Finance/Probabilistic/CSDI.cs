...
[ModelPaper("CSDI: Conditional Score-based Diffusion Models for Probabilistic Time Series Imputation", "https://arxiv.org/abs/2107.03502", Year = 2021, Authors = "Yusuke Tashiro, Jiaming Song, Yang Song, Stefano Ermon")]
public class CSDI<T> : ForecastingModelBase<T>
{
    ...
    public override void Train(Tensor<T> input, Tensor<T> target)
    {
        ...
        var mask = GenerateRandomMask(input.Shape._dims);
        ...
        Backward(Tensor<T>.FromVector(gradient, maskedOutput.Shape._dims));
    ...
    private Tensor<T> ImputeOnnx(Tensor<T> input)
    {
        ...
        var onnxInput = new OnnxTensors.DenseTensor<float>(inputArray, input.Shape._dims);
        ...
        return new Tensor<T>(input.Shape._dims, new Vector<T>(outputData));
    }
    ...
    private Tensor<T> InitializeWithNoise(Tensor<T> data, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(data.Shape._dims, new Vector<T>(resultVec));
    }
    ...
    private Tensor<T> PreserveObservedValues(Tensor<T> imputed, Tensor<T> original, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(imputed.Shape._dims, new Vector<T>(resultVec));
    }
    ...
    private Tensor<T> ApplyMask(Tensor<T> tensor, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(tensor.Shape._dims, new Vector<T>(resultVec));
    }
