...
public class CSDI<T> : ForecastingModelBase<T>
{
    ...
    private Tensor<T> ImputeOnnx(Tensor<T> input)
    {
        ...
        var onnxInput = new OnnxTensors.DenseTensor<float>(inputArray, input._shape);
        ...
        return new Tensor<T>(input._shape, new Vector<T>(outputData));
    }
    ...
    private Tensor<T> InitializeWithNoise(Tensor<T> data, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(data._shape, new Vector<T>(resultVec));
    }
    ...
    private Tensor<T> PreserveObservedValues(Tensor<T> imputed, Tensor<T> original, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(imputed._shape, new Vector<T>(resultVec));
    }
    ...
    private Tensor<T> ApplyMask(Tensor<T> tensor, Tensor<T> mask)
    {
        ...
        return new Tensor<T>(tensor._shape, new Vector<T>(resultVec));
    }
