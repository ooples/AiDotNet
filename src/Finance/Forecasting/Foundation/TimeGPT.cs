...
public class TimeGPT<T> : ForecastingModelBase<T>
{
    ...
    /// TimeGPT normalizes each input series before the embedding and restores the
    ...
    public override Tensor<T> ApplyInstanceNormalization(Tensor<T> input)
    {
        ...
        // series. Stats are taken over every non-batch element of each row.
    ...
    private Tensor<T> Forward(Tensor<T> input)
    {
        ...
        // TimeGPT embeds per timestep ([numFeatures] → hiddenDim) and pools over
        // the sequence, so it needs a rank-3 [batch, seqLen, numFeatures] input. A
        // bare rank-1 [contextLength] context would hit the embedding
        // FeedForwardLayer as a rank-1 tensor (which its matmul rejects).
