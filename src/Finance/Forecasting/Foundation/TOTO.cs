...
public class TOTO<T> : TimeSeriesFoundationModelBase<T>
{
    ...
    private readonly TOTOOptions<T> _options;
    ...
    /// <summary>
    /// Creates a TOTO model using a pretrained ONNX model.
    /// </summary>
    public TOTO(
        NeuralNetworkArchitecture<T> architecture,
        string onnxModelPath,
        TOTOOptions<T>? options = null,
    ...
    /// <summary>
    /// Creates a TOTO model in native mode for training or fine-tuning.
    /// </summary>
    public TOTO(
        NeuralNetworkArchitecture<T> architecture,
        TOTOOptions<T>? options = null,
        ...
        : base(architecture, lossFunction ?? new MeanSquaredErrorLoss<T>(), 1.0)
    {
        ...
        if (architecture.InputSize > 0)
            _contextLength = architecture.InputSize;
        InitializeLayers();
    }
    ...
    protected override IFullModel<T, Tensor<T>, Tensor<T>> CreateNewInstance()
    {
        ...
        if (!_useNativeMode && OnnxModelPath is not null)
            return new TOTO<T>(Architecture, OnnxModelPath, opts);

        return new TOTO<T>(Architecture, opts);
    }
    ...
    public override Tensor<T> Forecast(Tensor<T> historicalData, double[]? quantiles = null)
    {
        if (quantiles is not null && quantiles.Length > 0)
            throw new NotSupportedException("TOTO does not support quantile forecasting. Pass null for point forecasts.");
    ...
    /// the forecast so it is on the input's original scale. Tape-connected.
    /// </summary>
    ...
    private Tensor<T> ForwardNative(Tensor<T> input)
    {
        ...
        // Flatten the whole series to a single batch row [1, length] so the patch
        // ReshapeLayer reshapes the entire sequence rather than mis-reading the
        // leading dimension as a batch axis.
