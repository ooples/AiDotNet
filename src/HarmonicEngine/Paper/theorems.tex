...
\label{thm:sparsity}
Let $f^*: \mathbb{R}^N \to \mathbb{R}$ be a target function that is
$K$-sparse in the spectral domain (i.e., its Fourier representation has
at most $K$ non-zero coefficients). An HRE model with top-$K$ spectral
selection can recover $f^*$ to within $\epsilon$ accuracy using
$m = O(K \log(N/K) \cdot \text{poly}(1/\epsilon))$ training samples,
compared to $O(N^2/\epsilon^2)$ for a dense two-layer network with
$N$ hidden units.
\end{theorem}

\begin{proof}[Proof Sketch]
The proof applies compressed sensing theory to the spectral learning problem.

\textbf{Step 1: Measurement matrix.}
Each training sample $(x^{(m)}, y^{(m)})$ provides a linear measurement
of the target function's spectral coefficients. The FFT of the input
$x^{(m)}$ acts as a row of the measurement matrix $\mathbf{\Phi}$.

\textbf{Step 2: Restricted Isometry Property (RIP).}
By the Cand\`es-Tao RIP theorem \cite{candes2006}, random Fourier measurements
satisfy the RIP of order $K$ with probability exceeding $1 - \delta$ when
$m \geq C \cdot K \log(N/K) \cdot \log(1/\delta)$
for a universal constant $C$.

\textbf{Step 3: Recovery guarantee.}
Under RIP, the top-$K$ spectral selection (equivalent to $\ell_0$-constrained
minimization relaxed to $\ell_1$) recovers the true $K$-sparse spectral
coefficients with error:
\begin{equation}
\|\hat{h} - h^*\|_2 \leq C' \cdot \frac{\sigma}{\sqrt{m}}
\end{equation}
where $\sigma$ is the noise level and $\hat{h}$ is the recovered coefficient vector.

\textbf{Step 4: Comparison to dense networks.}
A dense two-layer network with $N$ hidden units has $O(N^2)$ parameters.
By standard VC-dimension bounds, it requires $O(N^2/\epsilon^2)$ samples
for $\epsilon$-accurate generalization. When $K \ll N$, the spectral approach
achieves exponential sample efficiency.
\end{proof}
...
\end{equation}
This balances signal fidelity against model complexity, automatically adapting
to the true sparsity level of the data.
\end{remark}
...
\begin{equation}
H_{\text{opt}}(k) = \frac{S_{yx}(k)}{(1 + \alpha) \cdot S_{xx}(k)}
= \frac{1}{1+\alpha} \cdot \frac{S_{yx}(k)}{S_{xx}(k)}
\end{equation}
which is a scaled version of the Wiener optimal filter
$H_W(k) = S_{yx}(k) / S_{xx}(k)$.
For $\alpha \to 0$, this converges to the exact Wiener filter.

The convergence rate is geometric with rate $r = 1 - \eta(1 + \alpha)$,
requiring $O(\frac{1}{\eta(1+\alpha)} \log(1/\epsilon))$ iterations
for $\epsilon$-accuracy.
\end{theorem}

\begin{proof}
At equilibrium, $\Delta H(k) = 0$, so:
\begin{equation}
Y(k) \cdot X^*(k) = \alpha \cdot H_{\text{eq}}(k) \cdot |X(k)|^2
\end{equation}

Taking expectations over the stationary process:
\begin{align}
\mathbb{E}[Y(k) \cdot X^*(k)] &= \alpha \cdot H_{\text{eq}}(k) \cdot \mathbb{E}[|X(k)|^2] \\
S_{yx}(k) &= \alpha \cdot H_{\text{eq}}(k) \cdot S_{xx}(k)
\end{align}

Wait --- with the power-normalized update, the actual equilibrium condition
from the full update rule is:
\begin{equation}
...
Define the error $e_n(k) = H_n(k) - H_{\text{eq}}(k)$.
Substituting into the update rule (for a single stationary realization):
\begin{align}
...
This converges geometrically provided $|1 - \eta\alpha| < 1$,
i.e., $0 < \eta < 2/\alpha$. The optimal rate is achieved at
$\eta = 1/\alpha$, giving single-step convergence.

For stochastic (non-stationary) inputs, the update becomes a
Robbins-Monro stochastic approximation, and convergence follows from
standard results with diminishing step sizes $\eta_n = \eta_0 / n$.
\end{proof}
...
operations, the spectral Hebbian rule achieves comparable filter quality
in $O(N \log N)$ operations (one FFT + pointwise update per sample).
For a single training pass over $M$ samples, the total cost is
$O(M \cdot N \log N)$ vs. $O(\text{epochs} \cdot M \cdot N^2)$
