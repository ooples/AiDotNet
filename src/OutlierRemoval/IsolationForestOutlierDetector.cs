...
/// <summary>
/// Implements the Isolation Forest algorithm for anomaly/outlier detection.
/// </summary>
/// <typeparam name="T">The numeric type used for calculations (e.g., float, double).</typeparam>
/// <remarks>
/// <para>
/// <b>For Beginners:</b> Isolation Forest is an efficient algorithm for detecting outliers.
/// The key insight is that outliers are "few and different" - they are easier to isolate
...
/// 3. Outliers tend to be isolated in fewer steps (closer to the root of the tree)
...
/// </para>
/// </remarks>
public class IsolationForestOutlierDetector<T> : OutlierDetectorBase<T>
{
    ...
    /// <summary>
    /// Creates a new Isolation Forest outlier detector.
    /// </summary>
    /// <param name="numTrees">
    /// The number of isolation trees in the forest. Default is 100.
    ...
    /// The number of samples to draw from X to train each tree. Default is 256.
    ...
    private Vector<T> DecisionFunctionInternal(Matrix<T> X)
    {
        ...
        for (int i = 0; i < X.Rows; i++)
        {
            ...
            // Higher score = more anomalous, but we negate for consistency
            // (lower values = outliers in our Predict method)
            double anomalyScore = Math.Pow(2, -avgPathLength / _averagePathLength);

            // Convert to a score where lower = more anomalous (matches our Predict logic)
            // We use negative anomaly score so outliers have lower values
