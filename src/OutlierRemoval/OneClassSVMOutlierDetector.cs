...
/// </para>
/// </remarks>
public class OneClassSVMOutlierDetector<T> : OutlierDetectorBase<T>
{
    ...
    /// <summary>
    /// Creates a new One-Class SVM outlier detector.
    /// </summary>
    /// <param name="nu">
    ...
    /// </param>
    /// <param name="gamma">
    /// Kernel coefficient for RBF. Default is "auto" which uses 1/(n_features).
    /// Larger values mean each point has a smaller "influence radius".
    /// Use 0 or negative for auto-detection.
    /// </param>
    ...
    /// Thrown when nu is not in (0, 1].
    /// </exception>
    public OneClassSVMOutlierDetector(
        double nu = 0.1,
        double gamma = 0,
    ...
    public override void Fit(Matrix<T> X)
    {
        ...
        var trainingScores = DecisionFunctionInternal(X, effectiveGamma);
    ...
    public override Vector<T> DecisionFunction(Matrix<T> X)
    {
        ...
        double effectiveGamma = _gamma > 0 ? _gamma : 1.0 / _supportVectors!.Columns;
        return DecisionFunctionInternal(X, effectiveGamma);
    }

    private Vector<T> DecisionFunctionInternal(Matrix<T> X, double gamma)
    {
        ValidateInput(X);
        ...
        for (int i = 0; i < X.Rows; i++)
        {
            ...
            for (int j = 0; j < _supportVectors!.Rows; j++)
            {
                T kernelVal = RbfKernel(point, _supportVectors.GetRow(j), gamma);
                score = NumOps.Add(score, NumOps.Multiply(_alphas![j], kernelVal));
            }

            // Decision function: sign(sum(alpha_i * K(x, x_i)) - rho)
            // We return the signed distance, not just the sign
            scores[i] = NumOps.Subtract(score, _rho);
        }
    ...
    {
        T squaredDist = NumOps.Zero;

        for (int i = 0; i < x1.Length; i++)
        {
            T diff = NumOps.Subtract(x1[i], x2[i]);
            squaredDist = NumOps.Add(squaredDist, NumOps.Multiply(diff, diff));
        }

        double dist = NumOps.ToDouble(squaredDist);
        return NumOps.FromDouble(Math.Exp(-gamma * dist));
    }
