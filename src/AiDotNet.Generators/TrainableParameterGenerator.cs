...
public class TrainableParameterGenerator : IIncrementalGenerator
{
    ...
    private static bool ExtendsLayerBase(INamedTypeSymbol type)
    {
        ...
        {
            if (current.OriginalDefinition.ToDisplayString().StartsWith(LayerBaseTypeName))
                return true;
    ...
    private static bool IsTensorType(ITypeSymbol type)
    {
        ...
        return original.ToDisplayString().StartsWith(TensorTypeName);
    }
    ...
    private static bool IsLayerType(ITypeSymbol type)
    {
        ...
        if (type is INamedTypeSymbol named)
        {
            ...
            {
                if (iface.OriginalDefinition.ToDisplayString().StartsWith(ILayerTypeName))
                    return true;
            }
            // Also check the type itself
            if (named.OriginalDefinition.ToDisplayString().StartsWith(LayerBaseTypeName))
                return true;
        }
        return false;
    }
