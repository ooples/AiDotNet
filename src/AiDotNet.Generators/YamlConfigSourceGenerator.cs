...
{
    /// <summary>
    /// The section name for the full RAG pipeline (produced by
    /// <c>AiModelBuilder.ConfigureRetrievalAugmentedGeneration(...)</c>). This section is treated as a
    /// COMPOSITE: instead of surfacing only the first parameter (the retriever), it exposes every RAG
    /// sub-component (reranker, generator, query processors, document/graph stores, embedding model,
    /// similarity metric, knowledge-graph options) and the applier wires each one to the matching facade
    /// method. See <see cref="EmitRagCompositeApplier"/> and <see cref="EmitYamlRagSection"/>.
    /// </summary>
    private const string RagCompositeSectionName = "RetrievalAugmentedGeneration";

    /// <summary>
    /// The standalone advanced knowledge-graph section (produced by
    /// <c>AiModelBuilder.ConfigureKnowledgeGraph(Action&lt;KnowledgeGraphOptions&gt;)</c>). Unlike the
    /// pipeline-style <c>Action&lt;Pipeline&gt;</c> sections, this one is an options bag: the builder
    /// creates the options object and invokes the supplied action, so the applier just applies the YAML
    /// <c>params</c> onto it (no factory or ctor needed). This replaces the old Action-builder TODO.
    /// </summary>
    private const string KnowledgeGraphSectionName = "KnowledgeGraph";
...
";
    ...
    private static void EmitYamlRagSection(SourceProductionContext context)
    {
        const string source = @"// <auto-generated/>
#nullable enable
...
/// <summary>
/// Composite YAML section for the full retrieval-augmented generation (RAG) pipeline.
/// Each sub-component names a concrete implementation (via <see cref=""YamlTypeSection""/>) that the
/// applier resolves from the type registry and passes to the matching builder facade method.
/// </summary>
/// <remarks>
/// <para><b>For Beginners:</b> A RAG pipeline is made of several parts working together. This section
/// lets you configure them all at once from YAML:
/// <code>
...
/// </para>
/// </remarks>
public class YamlRagSection
{
    /// <summary>The retriever that finds relevant documents (resolved from the ""Retriever"" registry).</summary>
    public YamlTypeSection? Retriever { get; set; }

    /// <summary>The reranker that reorders retrieved documents (resolved from the ""Reranker"" registry).</summary>
    public YamlTypeSection? Reranker { get; set; }

    /// <summary>The generator that produces grounded answers (resolved from the ""Generator"" registry).</summary>
    public YamlTypeSection? Generator { get; set; }

    /// <summary>Ordered query processors applied before retrieval (resolved from the ""QueryProcessor"" registry).</summary>
    public List<YamlTypeSection> QueryProcessors { get; set; } = new List<YamlTypeSection>();

    /// <summary>The document store for vector/hybrid retrieval (resolved from the ""DocumentStore"" registry).</summary>
    public YamlTypeSection? DocumentStore { get; set; }

    /// <summary>The graph storage backend for Graph RAG (resolved from the ""GraphStore"" registry).</summary>
    public YamlTypeSection? GraphStore { get; set; }

    /// <summary>The embedding model used to vectorize text (resolved from the ""EmbeddingModel"" registry).</summary>
    public YamlTypeSection? EmbeddingModel { get; set; }

    /// <summary>The similarity metric used for vector search (resolved from the ""SimilarityMetric"" registry).</summary>
    public YamlTypeSection? SimilarityMetric { get; set; }

    /// <summary>
    /// Advanced knowledge-graph options. When present, the applier calls
    /// <c>ConfigureKnowledgeGraph(...)</c> and applies <c>params</c> onto the KnowledgeGraphOptions.
    /// </summary>
    public YamlTypeSection? KnowledgeGraph { get; set; }
}
";
        context.AddSource("YamlRagSection.g.cs", source);
    }
...
";
    ...
    private static void EmitYamlConfigApplier(SourceProductionContext context, List<SectionInfo> sections)
    {
        ...
        foreach (var section in sections)
        {
            ...
            // Composite RAG pipeline: wire EVERY sub-component (retriever, reranker, generator, query
            // processors, document/graph stores, embedding model, similarity metric, knowledge graph)
            // rather than dropping all but the retriever. See EmitRagCompositeApplier.
            if (section.Method.SectionName == RagCompositeSectionName)
            {
                EmitRagCompositeApplier(sb, propName);
                continue;
            }

            // Standalone advanced knowledge-graph options: apply YAML params onto the options object the
            // builder creates and passes to the Action<KnowledgeGraphOptions>. Removes the old KG TODO.
            if (section.Method.SectionName == KnowledgeGraphSectionName &&
                section.Method.Category == SectionCategory.ActionBuilder)
            {
                sb.AppendLine($"        if (config.{propName} is not null)");
                sb.AppendLine($"        {{");
                sb.AppendLine($"            var __kgParams = config.{propName}.Params;");
                sb.AppendLine($"            builder.{section.Method.MethodName}(__kgOpts => YamlParamsHelper.ApplyParams(__kgOpts!, __kgParams));");
                sb.AppendLine($"        }}");
                sb.AppendLine();
                continue;
            }
    ...
    /// <summary>
    /// Emits the applier body for the composite RAG section. Every sub-component named in YAML is
    /// resolved from its own type-registry section and passed to the matching builder facade —
    /// <c>ConfigureRetrievalAugmentedGeneration</c> (retriever, reranker, generator, query processors,
    /// graph store, document store), <c>ConfigureEmbeddingModel</c>, <c>ConfigureSimilarityMetric</c>,
    /// and <c>ConfigureKnowledgeGraph</c> — so nothing is dropped.
    /// </summary>
    private static void EmitRagCompositeApplier(StringBuilder sb, string propName)
    {
        const string retrieverType = "global::AiDotNet.Interfaces.IRetriever<T>";
        const string rerankerType = "global::AiDotNet.Interfaces.IReranker<T>";
        const string generatorType = "global::AiDotNet.Interfaces.IGenerator<T>";
        const string queryProcessorType = "global::AiDotNet.Interfaces.IQueryProcessor";
        const string graphStoreType = "global::AiDotNet.Interfaces.IGraphStore<T>";
        const string documentStoreType = "global::AiDotNet.Interfaces.IDocumentStore<T>";
        const string embeddingType = "global::AiDotNet.Interfaces.IEmbeddingModel<T>";
        const string similarityType = "global::AiDotNet.RetrievalAugmentedGeneration.VectorSearch.ISimilarityMetric<T>";
        const string registry = "YamlTypeRegistry<T, TInput, TOutput>";

        sb.AppendLine($"        if (config.{propName} is not null)");
        sb.AppendLine($"        {{");
        sb.AppendLine($"            var __rag = config.{propName};");
        sb.AppendLine();

        // Single-instance interface sub-components resolved from their registry sections.
        EmitRagOptionalComponent(sb, retrieverType, "__retriever", "Retriever", "Retriever");
        EmitRagOptionalComponent(sb, rerankerType, "__reranker", "Reranker", "Reranker");
        EmitRagOptionalComponent(sb, generatorType, "__generator", "Generator", "Generator");
        EmitRagOptionalComponent(sb, graphStoreType, "__graphStore", "GraphStore", "GraphStore");
        EmitRagOptionalComponent(sb, documentStoreType, "__documentStore", "DocumentStore", "DocumentStore");

        // Ordered list of query processors.
        sb.AppendLine($"            System.Collections.Generic.List<{queryProcessorType}>? __queryProcessors = null;");
        sb.AppendLine($"            if (__rag.QueryProcessors is not null && __rag.QueryProcessors.Count > 0)");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                __queryProcessors = new System.Collections.Generic.List<{queryProcessorType}>();");
        sb.AppendLine($"                foreach (var __qp in __rag.QueryProcessors)");
        sb.AppendLine($"                {{");
        sb.AppendLine($"                    if (__qp is not null && !string.IsNullOrWhiteSpace(__qp.Type))");
        sb.AppendLine($"                    {{");
        sb.AppendLine($"                        __queryProcessors.Add({registry}.CreateInstance<{queryProcessorType}>(");
        sb.AppendLine($"                            \"QueryProcessor\", __qp.Type, __qp.Params));");
        sb.AppendLine($"                    }}");
        sb.AppendLine($"                }}");
        sb.AppendLine($"            }}");
        sb.AppendLine();

        // Only call the RAG facade when at least one component is present, so an empty section does not
        // clear previously-configured RAG state.
        sb.AppendLine($"            if (__retriever is not null || __reranker is not null || __generator is not null");
        sb.AppendLine($"                || __queryProcessors is not null || __graphStore is not null || __documentStore is not null)");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                builder.ConfigureRetrievalAugmentedGeneration(");
        sb.AppendLine($"                    retriever: __retriever,");
        sb.AppendLine($"                    reranker: __reranker,");
        sb.AppendLine($"                    generator: __generator,");
        sb.AppendLine($"                    queryProcessors: __queryProcessors,");
        sb.AppendLine($"                    graphStore: __graphStore,");
        sb.AppendLine($"                    documentStore: __documentStore);");
        sb.AppendLine($"            }}");
        sb.AppendLine();

        // Embedding model and similarity metric have their own facade methods.
        sb.AppendLine($"            if (__rag.EmbeddingModel is not null && !string.IsNullOrWhiteSpace(__rag.EmbeddingModel.Type))");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                builder.ConfigureEmbeddingModel({registry}.CreateInstance<{embeddingType}>(");
        sb.AppendLine($"                    \"EmbeddingModel\", __rag.EmbeddingModel.Type, __rag.EmbeddingModel.Params));");
        sb.AppendLine($"            }}");
        sb.AppendLine();
        sb.AppendLine($"            if (__rag.SimilarityMetric is not null && !string.IsNullOrWhiteSpace(__rag.SimilarityMetric.Type))");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                builder.ConfigureSimilarityMetric({registry}.CreateInstance<{similarityType}>(");
        sb.AppendLine($"                    \"SimilarityMetric\", __rag.SimilarityMetric.Type, __rag.SimilarityMetric.Params));");
        sb.AppendLine($"            }}");
        sb.AppendLine();

        // Advanced knowledge-graph options: the builder creates the options object and invokes our action.
        sb.AppendLine($"            if (__rag.KnowledgeGraph is not null)");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                var __kgParams = __rag.KnowledgeGraph.Params;");
        sb.AppendLine($"                builder.ConfigureKnowledgeGraph(__kgOpts => YamlParamsHelper.ApplyParams(__kgOpts!, __kgParams));");
        sb.AppendLine($"            }}");
        sb.AppendLine($"        }}");
        sb.AppendLine();
    }

    private static void EmitRagOptionalComponent(
        StringBuilder sb, string interfaceType, string localName, string yamlProperty, string registrySection)
    {
        const string registry = "YamlTypeRegistry<T, TInput, TOutput>";
        sb.AppendLine($"            {interfaceType}? {localName} = null;");
        sb.AppendLine($"            if (__rag.{yamlProperty} is not null && !string.IsNullOrWhiteSpace(__rag.{yamlProperty}.Type))");
        sb.AppendLine($"            {{");
        sb.AppendLine($"                {localName} = {registry}.CreateInstance<{interfaceType}>(");
        sb.AppendLine($"                    \"{registrySection}\", __rag.{yamlProperty}.Type, __rag.{yamlProperty}.Params);");
        sb.AppendLine($"            }}");
        sb.AppendLine();
    }
    ...
    private static void EmitYamlSchemaMetadata(SourceProductionContext context, List<SectionInfo> sections, Compilation compilation)
    {
        ...
        foreach (var section in sections)
        {
            ...
            var category =
                section.Method.SectionName == RagCompositeSectionName ? "RagComposite" :
                section.Method.SectionName == KnowledgeGraphSectionName ? "ActionParams" :
                section.Method.Category switch
                {
    ...
    private static string GetYamlPropertyType(SectionInfo section)
    {
        ...
        // The composite RAG pipeline surfaces every sub-component through YamlRagSection.
        if (section.Method.SectionName == RagCompositeSectionName)
        {
            return "YamlRagSection";
        }

        // The standalone knowledge-graph section is an options bag applied via ConfigureKnowledgeGraph.
        if (section.Method.SectionName == KnowledgeGraphSectionName)
        {
            return "YamlTypeSection";
        }
