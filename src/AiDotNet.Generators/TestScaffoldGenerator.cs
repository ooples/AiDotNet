...
public class TestScaffoldGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // ModelPerfProbe baseline manifest (artifacts/perf-baseline.json) is fed in via
        // AdditionalFiles so we can read which models have been measured as slow / heavy-
        // allocating and emit [Trait("Speed","Slow")] on their generated scaffolds. The
        // .csproj's AdditionalFiles glob picks it up; we filter by file name here so other
        // additional files (analyzer-release notes, etc.) are ignored.
        var perfBaseline = context.AdditionalTextsProvider
            .Where(static at => at.Path.Replace('\\', '/').EndsWith("perf-baseline.json", System.StringComparison.OrdinalIgnoreCase))
        ...
        var combined = modelClasses.Collect()
            ...
            .Combine(perfBaseline);

        context.RegisterSourceOutput(combined, static (spc, source) =>
        {
            var (((((((models, tests), activations), losses), layers), algorithms), compilation), perfTexts) = source;
            var slowModels = new System.Collections.Generic.HashSet<string>(System.StringComparer.Ordinal);
            foreach (var text in perfTexts)
            {
                foreach (var m in PerfBaselineParser.ParseSlowModels(text))
                    slowModels.Add(m);
            }
            Execute(spc, models, tests, compilation, slowModels);
    ...
    private static void Execute(
        ...
        System.Collections.Generic.HashSet<string> slowModels)
    {
        ...
        if (!modelsFoundFromSource)
        {
            ...
            foreach (var model in untestedModels)
            {
                ...
                EmitGeneratedTestClass(context, model, family.Value, testClassName, slowModels);
                autoGenerated.Add(model);
                testNames.Add(testClassName);
            }
        ...
        System.Collections.Generic.HashSet<string> slowModels)
    {
        ...
        sb.AppendLine();
        // ModelPerfProbe-driven slow-model tag: the artifacts/perf-baseline.json AdditionalFile
        // lists every model whose measured train-step latency or alloc-per-step exceeds the
        // probe's slow thresholds. Emit [Trait("Speed", "Slow")] on the scaffold so the test
        // runner can filter slow models out of fast CI lanes (dotnet test --filter "Speed!=Slow"),
        // while still running them in the nightly perf sweep. The lookup keys are both the
        // backtick-suffixed reflection name (LayoutXLM`1) and the stripped C# name (LayoutXLM)
        // — PerfBaselineParser populates both forms.
        string modelClassName = model.ClassName;
        if (slowModels.Contains(modelClassName) || slowModels.Contains(modelClassName + "`1"))
        {
            sb.AppendLine("[Xunit.Trait(\"Speed\", \"Slow\")]");
        }
