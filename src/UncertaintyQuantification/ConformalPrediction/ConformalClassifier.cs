...
public class ConformalClassifier<T>
{
    ...
    public HashSet<int> PredictSet(Tensor<T> input, double confidenceLevel = 0.9)
    {
        ...
        for (int c = 0; c < _numClasses && c < probabilities.Length; c++)
        {
            if (_numOps.Compare(probabilities[c], threshold) >= 0)
            {
                predictionSet.Add(c);
            }
        }
        ...
        if (predictionSet.Count == 0)
        {
            ...
            var maxClass = 0;
            for (int c = 1; c < _numClasses && c < probabilities.Length; c++)
            {
                if (_numOps.Compare(probabilities[c], maxProb) > 0)
                {
    ...
    public double EvaluateCoverage(Matrix<T> testInputs, Vector<int> testLabels, double confidenceLevel = 0.9)
    {
        ...
        {
            var input = new Tensor<T>([testInputs.Cols], testInputs.GetRow(i).ToArray());
