...
public static class GradientOps
{
    ...
    {
        // grad_x = grad_y * y * (1 - y)
        var ones = CreateOnes<T>(forwardOutput.Shape._dims);
    ...
    public static Tensor<T> GradHardSigmoid<T>(Tensor<T> gradOutput, Tensor<T> forwardInput)
    {
        ...
        return new Tensor<T>(gradOutput.Shape._dims, new Vector<T>(resultData));
    }
    ...
    private static Tensor<T> CreateMask<T>(Tensor<T> input)
    {
        ...
        return new Tensor<T>(input.Shape._dims, new Vector<T>(resultData));
    }
    ...
    private static Tensor<T> NegateHelper<T>(Tensor<T> input)
    {
        ...
        return new Tensor<T>(input.Shape._dims, new Vector<T>(data));
    }
    ...
    {
        if (!numerator.Shape._dims.SequenceEqual(denominator.Shape._dims))
        ...
        return new Tensor<T>(numerator.Shape._dims, new Vector<T>(resultData));
    }
    ...
    private static Tensor<T> SumWithKeepdims<T>(Tensor<T> input, int[] axes)
    {
        ...
        var newShape = new List<int>(input.Shape._dims);
    ...
    public static Tensor<T> GradConv2D<T>(Tensor<T> gradOutput, Tensor<T> savedTensor, int inputIndex, int[] stride, int[] padding)
    {
        ...
        if (inputIndex == 0)
        {
            ...
            var filterShape = filters.Shape._dims;
            var gradShape = gradOutput.Shape._dims;
        ...
        else
        {
            ...
            return new Tensor<T>(result.Shape._dims, new Vector<T>(resultData));
        }
    }
    ...
    {
        var numOps = MathHelper.GetNumericOperations<T>();
        var inputShape = forwardInput.Shape._dims;
    ...
    public static Tensor<T> GradPower<T>(Tensor<T> gradOutput, Tensor<T> forwardInput, double exponent)
    {
        ...
        return new Tensor<T>(forwardInput.Shape._dims, new Vector<T>(resultData));
    }
    ...
    public static Tensor<T> GradPad<T>(Tensor<T> gradOutput, int[] padding)
    {
        ...
        var shape = gradOutput.Shape._dims;
    ...
    public static Tensor<T> GradBroadcast<T>(Tensor<T> gradOutput, int[] originalShape, int[] broadcastedAxes)
    {
        ...
        if (!result.Shape._dims.SequenceEqual(originalShape))
        {
        ...
        int hiddenSize)
    {
        ...
        switch (inputIndex)
        {
            ...
            case 1: // h_prev gradient
            {
                ...
                return new Tensor<T>(hPrev.Shape._dims, new Vector<T>(result));
            }
            case 2: // c_prev gradient
            {
                ...
                return new Tensor<T>(cPrev.Shape._dims, new Vector<T>(result));
            }
