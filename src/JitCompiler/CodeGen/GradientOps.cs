...
public static class GradientOps
{
    ...
    {
        // grad_x = grad_y * y * (1 - y)
        var ones = CreateOnes<T>(forwardOutput._shape);
    ...
    public static Tensor<T> GradHardSigmoid<T>(Tensor<T> gradOutput, Tensor<T> forwardInput)
    {
        ...
        return new Tensor<T>(gradOutput._shape, new Vector<T>(resultData));
    }
    ...
    private static Tensor<T> CreateMask<T>(Tensor<T> input)
    {
        ...
        return new Tensor<T>(input._shape, new Vector<T>(resultData));
    }
    ...
    private static Tensor<T> NegateHelper<T>(Tensor<T> input)
    {
        ...
        return new Tensor<T>(input._shape, new Vector<T>(data));
    }
    ...
    {
        if (!numerator._shape.SequenceEqual(denominator._shape))
        ...
        return new Tensor<T>(numerator._shape, new Vector<T>(resultData));
    }
    ...
    public static Tensor<T> GradConv2D<T>(Tensor<T> gradOutput, Tensor<T> savedTensor, int inputIndex, int[] stride, int[] padding)
    {
        ...
        if (inputIndex == 0)
        {
            ...
            var filterShape = filters._shape;
        ...
        else
        {
            ...
            return new Tensor<T>(result._shape, new Vector<T>(resultData));
        }
    }
    ...
    {
        var numOps = MathHelper.GetNumericOperations<T>();
        var inputShape = forwardInput._shape;
    ...
    public static Tensor<T> GradPower<T>(Tensor<T> gradOutput, Tensor<T> forwardInput, double exponent)
    {
        ...
        return new Tensor<T>(forwardInput._shape, new Vector<T>(resultData));
    }
    ...
    public static Tensor<T> GradBroadcast<T>(Tensor<T> gradOutput, int[] originalShape, int[] broadcastedAxes)
    {
        ...
        if (!result._shape.SequenceEqual(originalShape))
        {
        ...
        int hiddenSize)
    {
        ...
        switch (inputIndex)
        {
            ...
            case 1: // h_prev gradient
            {
                ...
                return new Tensor<T>(hPrev._shape, new Vector<T>(result));
            }
            case 2: // c_prev gradient
            {
                ...
                return new Tensor<T>(cPrev._shape, new Vector<T>(result));
            }
        ...
        int hiddenSize)
    {
        ...
        if (savedTensors.Length < 3)
        {
            ...
            var resultShape = inputIndex switch
            {
                0 => savedTensors[0]._shape,
                1 => savedTensors.Length > 1 ? savedTensors[1]._shape : new int[] { hiddenSize },
