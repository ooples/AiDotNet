...
public class IRBuilder
{
    ...
    public IRGraph Build<T>(ComputationNode<T> outputNode, List<ComputationNode<T>> inputs)
    {
        ...
        foreach (var input in inputs)
        {
            ...
            graph.TensorShapes[tensorId] = input.Value._shape;
        }
    ...
    private IROp? ConvertNodeToOp<T>(ComputationNode<T> node)
    {
        ...
        // Get output shape
        var outputShape = node.Value._shape;
    ...
    public IRGraph BuildBackward<T>(ComputationNode<T> outputNode, List<ComputationNode<T>> inputs)
    {
        ...
        graph.TensorShapes[outputGradId] = outputNode.Value._shape;
            ...
            for (int i = 0; i < node.Parents.Count && i < backwardOps.Count; i++)
            {
                ...
                if (gradientMap.TryGetValue(parent, out var existingGradId))
                {
                    ...
                    var accumOp = new Operations.GradAccumulateOp
                    {
                        ...
                        OutputShape = parent.Value._shape
                    };
        ...
        switch (node.OperationType.Value)
        {
            case OperationType.Add:
                ...
                for (int i = 0; i < 2; i++)
                {
                    ops.Add(new Operations.GradAddOp
                    {
                        ...
                        InputIndex = i,
                        OutputType = irType,
                        OutputShape = node.Parents[i].Value._shape
                    });
                }
                break;
            ...
            case OperationType.MatMul:
                ...
                ops.Add(new Operations.GradMatMulLeftOp
                {
                    ...
                    OutputType = irType,
                    OutputShape = node.Parents[0].Value._shape
                });
                // grad_B = A^T @ grad_C
                ops.Add(new Operations.GradMatMulRightOp
                {
                    ...
                    OutputType = irType,
                    OutputShape = node.Parents[1].Value._shape
                });
                break;
            ...
            case OperationType.Reshape:
                ...
                ops.Add(new Operations.GradReshapeOp
                {
                    OutputId = _nextTensorId++,
                    InputIds = new[] { outputGradId },
                    OriginalShape = node.Parents[0].Value._shape,
                    OutputType = irType,
                    OutputShape = node.Parents[0].Value._shape
                });
                break;
            ...
            case OperationType.Concat:
                ...
                {
                    var parentShape = node.Parents[i].Value._shape;
            ...
            case OperationType.ReduceMean:
                ...
                var originalShape = node.Parents[0].Value._shape;
            ...
            case OperationType.Embedding:
                ...
                var embeddingShape = node.Parents[0].Value._shape;
            ...
            case OperationType.Gather:
                ...
                var gatherInputShape = node.Parents[0].Value._shape;
            ...
            case OperationType.Slice:
                ...
                var sliceOriginalShape = node.Parents[0].Value._shape;
            ...
            case OperationType.Broadcast:
                ...
                var broadcastOriginalShape = GetParam<int[]>(node, "OriginalShape", node.Parents[0].Value._shape);
