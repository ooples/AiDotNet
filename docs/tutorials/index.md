# Tutorials

Step-by-step guides to help you master AiDotNet.

---

## Tutorial Categories

| Category | Description |
|:---------|:------------|
| [Classification](./classification/) | Binary and multi-class classification |
| [Regression](./regression/) | Predicting continuous values |
| [Clustering](./clustering/) | Grouping similar data points |
| [Computer Vision](./computer-vision/) | Object detection, image classification, OCR |
| [NLP & RAG](./nlp/) | Text processing and retrieval-augmented generation |
| [Audio Processing](./audio/) | Speech recognition, text-to-speech |
| [Reinforcement Learning](./reinforcement-learning/) | Training agents through rewards |
| [Time Series](./time-series/) | Forecasting and anomaly detection |
| [LLM Fine-tuning](./llm-fine-tuning/) | LoRA, QLoRA, and efficient fine-tuning |
| [Distributed Training](./distributed-training/) | Multi-GPU and multi-node training |
| [Deployment](./deployment/) | Production model serving |

## Learning Path

### Beginner
1. [Getting Started](../getting-started/index.md) - Installation and first model
2. [Classification](classification/index.md) - Learn the fundamentals
3. [Regression](regression/index.md) - Predict continuous values

### Intermediate
4. [Computer Vision](computer-vision/index.md) - Image understanding
5. [NLP & RAG](nlp/index.md) - Text processing
6. [Time Series](time-series/) - Temporal data

### Advanced
7. [LLM Fine-tuning](llm-fine-tuning/index.md) - Customize language models
8. [Distributed Training](distributed-training/index.md) - Scale to multiple GPUs
9. [Deployment](deployment/index.md) - Production deployment

## Quick Links

- [Samples Repository](https://github.com/ooples/AiDotNet/tree/master/samples) - Complete runnable examples
- [API Reference](../../api/index.md) - Full API documentation
- [GitHub Issues](https://github.com/ooples/AiDotNet/issues) - Get help
